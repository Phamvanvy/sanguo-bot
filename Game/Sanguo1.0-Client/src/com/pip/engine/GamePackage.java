...
public class GamePackage extends PackageFile{
    ...
    public ImageSet loadTileImage() throws Exception{
    	ImageSet ret = new ImageSet(getFile("/tile.pip"));
    	ret.fileName = "tile.pip";
//#if opengl == true
        //# if (Canvas.openglMode) {
           //# ret.bindTexture(Canvas.GL_POOL_MAP, "tile.pip");
   		//# }
//#endif
        return ret;
    }
...
//#if opengl == true
        	 //# if (Canvas.openglMode) {
        		 //# result[i].bindTexture(Canvas.GL_POOL_MAP, "l" + i + ".ldf");
    		 //# }
//#endif
        }
    ...
    public byte[] loadLandformTileInfo(int lid) throws Exception{
        return parseTileInfo(getFile("/l" + lid + ".ts"))[1];
    }
...
//#if opengl == true
	        //# if (Canvas.openglMode) {
	        	//# imgs[i - 1].bindTexture(Canvas.GL_POOL_MAP, pkg.fileNames[i]);
			//# }
//#endif
        }
    ...
    /**
     * 载入地图NPC的阻挡区域设置。
     * @return 返回地图NPC动画数量相同的2维int数组，第2维是npc身上碰撞区域的数量*2，碰撞区域的x，y拼成到第1个int，w，h拼成第2个int
     * @throws Exception
     */
...
//#if opengl == true
		 ...
		 //# }
//#endif
        ByteArrayInputStream bis = new ByteArrayInputStream(Tool.inflate(getFile("/" + mapID + ".m")));
        //#if NewUI2
        DataInputStream dis = new DataInputStream(new BufferedInputStream(bis));
        //#else
        //# DataInputStream dis = new DataInputStream(bis);
