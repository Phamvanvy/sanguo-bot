...
public class UASocketConnection implements Runnable{
    ...
    //#if SupportEncryptNetwork == true
    //# private boolean encrypt;				// 是否加密，只有代理连接才支持加密
    //# private static byte[] keyEncryptKey;	// 发送加密KEY时，用来加密加密KEY的KEY
	//# static {
		//# try {
			//# keyEncryptKey = ":xj-vuiM;kvqMbqn".getBytes("UTF-8");
		//# } catch (Exception e) {
		//# }
	//# }
	//# protected byte[] clientEncryptKey;		// 客户端发送包的加密key，如果encrypt为true，则不能为null
	//# protected byte[] serverEncryptKey;		// 客户端接收包的解密key，null表示暂时未设定
	//# private final static int SET_ENCRYPT_KEY = 30003; 
    //#endif
    ...
    */
    ...
    public UASocketConnection(String url) throws IOException{
        ...
        //#else
        	//#if NewUI2
        _in = new DataInputStream(new BufferedInputStream(_connection.openInputStream()));
        _out = new DataOutputStream(new BufferedOutputStream(_connection.openOutputStream()));
        	//#else
      ...
      //# if (serverID != 0) {
      //# 	// 代理连接可以使用加密协议
      //# 	clientEncryptKey = new byte[16];
      //# 	Random rand = new Random();
      //# 	for (int i = 0; i < clientEncryptKey.length; i++) {
      //# 		clientEncryptKey[i] = (byte)rand.nextInt();
      //# 	}
      //# 	byte[] arr = new byte[clientEncryptKey.length];
      //# 	System.arraycopy(clientEncryptKey, 0, arr, 0, clientEncryptKey.length);
      //# 	xorBytes(arr, 0, arr.length, keyEncryptKey);
      //# 	UASegment seg = new UASegment(SET_ENCRYPT_KEY);
      //# 	seg.writeBytes(arr);
      //# 	write(seg);
      //# 	encrypt = true;
      //#  }
    ...
    //# /*
	 ...
	 //# */
	//# protected void xorBytes(byte[] data, int start, int len, byte[] key) {
		//# int end = start + len;
...
//#     	
    ...
    protected void write(UASegment segment) throws IOException{
        ...
        //#if SupportEncryptNetwork == true
        //# if (encrypt) {
        //# 	xorBytes(segment.data, 2, segment.data.length - 2, clientEncryptKey);
        //# }
        //#endif
    ...
    public UASegment readSegment() throws IOException{
        ...
        //#if SupportEncryptNetwork == true
        //# if (encrypt) {
        	//# // 如果支持加密，则服务器可能下发SET_ENCRYPT_KEY包，在这里处理
        //# 	if (data.type == SET_ENCRYPT_KEY) {
        //# 		serverEncryptKey = data.readBytes();
        //# 		xorBytes(serverEncryptKey, 0, serverEncryptKey.length, keyEncryptKey);
        //# 		return null;
        //# 	}
//#         	
        	//# // SET_ENCRYPT_KEY之后收到的包都做加密处理
        //# 	if (serverEncryptKey != null) {
        //# 		xorBytes(buf, 2, buf.length - 2, serverEncryptKey);
        //# 	}
        //# }
        //#endif
    ...
    public void runReader(){
        ...
        while(parent._isConnected){
            try{
                ...
                parent.lastReadTime = Tool.getSystemTime();
              //#ifdef buildtest
            ...
            try{
                ...
                while(UASocketConnection._segments.size() != 0){
                  ...
                  //#endif
                    ...
                    parent.lastWriteTime = Tool.getSystemTime();
                }
            }catch(Exception ex){
            	//#ifdef buildtest
                ex.printStackTrace();
