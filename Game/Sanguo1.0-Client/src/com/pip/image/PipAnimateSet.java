...
public class PipAnimateSet{
    ...
    public PipAnimateSet(ImageSet[] images, byte[] def){
        ...
        try{
            ...
            //#if NewUI2
            load(new DataInputStream(new BufferedInputStream(new ByteArrayInputStream(def))));
            //#else
...
//#endif
    ...
    public int[] getAnimateBox(int[] box, int index){
        System.arraycopy(getAnimateSize(index), 0, box, 0, 4);
        return box;
    }
    ...
    public int[] getAnimateSize(int index){
        if (animateBox[index] != null) {
            return animateBox[index];
        }else{
            animateBox[index] = new int[4];
            ...
            for (int i = start; i < start + len; i++){
                ...
                Tool.mergeBox(animateBox[index], frameBox);
            }
     ...
     * 返回制定动画帧的显示大小
     * @param frame 动画帧
     * @return int[] [0]左上角x [1]左上角y [2]宽 [3]高
     */
