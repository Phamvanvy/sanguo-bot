...
//#endif
    ...
    public PipImage(InputStream is) throws IOException {
        ...
        //#endif
        //#if NewUI2
    	load(new DataInputStream(new BufferedInputStream(is)));
    	//#else
...
//#if opengl == true
    	 //# if (Canvas.openglMode) {
             ...
             //# } else {
                 ...
                 //# }
             //# }
    		 //# gg.drawTexture(texture, frame, trans, x, y);
    		 //# return;
    	 //# }
//#endif
        ...
        //#if polish.api.nokia-ui && (ModelID == Nokia5310XM)
        //# com.nokia.mid.ui.DirectGraphics dg = com.nokia.mid.ui.DirectUtils.getDirectGraphics(g);
        //# dg.drawPixels((short[])drawData, true, 0, w, x, y, w, h, TRANS_MAP[trans], DirectGraphics.TYPE_USHORT_4444_ARGB);
        //#else
...
//# 
     ...
     */
//#if opengl == true
//#    public void draw(Graphics g, int frame, int x, int y, int trans, int anchor,int destw,int desth){
//#		 if (Canvas.openglMode) {
//#			 GLGraphics gg = (GLGraphics)g;
//#			 // 处理对齐方式
//#			 if (trans < 4) {
...
//#				gg.drawTexture(texture, frame, trans, x, y,desth,destw);
//#		 	} else {
//#		 		gg.drawTexture(texture, frame, trans, x, y,destw,desth);
//#	        }
//#			 
//#		 }
//#	    	
...
//# //    		 if (isTrueColor) {
//# //    			 GLBitmap bmp = new GLBitmap(frameData32[f], w, h);
//# //    			 bmp.convertARGB2ABGR();
//# //    			 return bmp;
//# //    		 }
             //# int[] rgb = new int[w * h];
             //# int[] usePal = palette[p];
             //# byte[] useData = frameData[f];
             //# if (canChangeColor && nextColorOp != COLOR_OP_NONE) {
                 //# usePal = performColorOp(usePal);
             //# }
             //# // 调色板中的颜色有ARGB转换为ABGR
             //# int[] arr = new int[usePal.length];
             //# for (int i = usePal.length - 1; i >= 0; i--) {
            	 //# int pixel = usePal[i];
            	 //# arr[i] = pixel & 0xFF00FF00 | (pixel & 0x000000FF) << 16 | (pixel & 0x00FF0000) >> 16;
             //# }
             //# usePal = arr;
             //# int id = 0;
             //# for(int y = 0; y < h; y++){
...
//#endif
        ...
        for(int y = 0; y < h; y++){
            for(int x = 0; x < w; x++){
                ...
                if (bytesPerPixel == 1) {
                	//#if polish.api.nokia-ui && (ModelID == Nokia5310XM)
                	//# int icolor = usePal[useData[id] & 0xff];
                	//# short scolor = (short)(((icolor & 0xf0000000) >> 16) | ((icolor & 0xf00000) >> 12) | ((icolor & 0xf000) >> 8)| ((icolor & 0xf0) >> 4));
                	//# rgb[id] = scolor;
                	//# id++;
                	//#else
                	rgb[id] = usePal[useData[id] & 0xff];
            		...
            		//#if polish.api.nokia-ui && (ModelID == Nokia5310XM)
            		//# int icolor = ((useData[id * 2] & 0xFF) << 8) | (useData[id * 2 + 1] & 0xFF);
                	//# short scolor = (short)(((icolor & 0xf0000000) >> 16) | ((icolor & 0xf00000) >> 12) | ((icolor & 0xf000) >> 8)| ((icolor & 0xf0) >> 4));
                	//# rgb[id] = scolor;
                	//# id++;
                	//#else
...
//#             
    ...
    private void load(DataInputStream dis) throws IOException{
        ...
        if(head[2] == HEAD_M[2]){
            ...
            //#if NewUI2
            DataInputStream fdis = new DataInputStream(new BufferedInputStream(fbis));
            //#else
...
//#if opengl == true
            //# if (Canvas.openglMode) {
            	//# mergeImageData = new byte[imgCount][];
            //# }
//#endif
    	...
    	//# if (Canvas.openglMode) {
	   		 //# if(texture == null){
	   			 //# if(fileName.equals("")){
	   				 ...
	   				 //# fileName = name;
	   			 //# } 
	   			 //# bindTexture(Canvas.GL_POOL_MISC, fileName);
	   		 //# }
	   	 //# }
    //# }
//#endif
...
//#if opengl == true
    	 //# if (Canvas.openglMode) {
    		 //# if(texture == null){
    			 //# System.out.println(fileName+" is unbinded!!!!!!!");
    		 //# }
			 //# return texture.getAreaWidth(frame);
    	 //# }
//#endif
...
//#if opengl == true
   	 //# if (Canvas.openglMode) {
   		 //# //checkRegister();
   		 //# return texture.getAreaHeight(frame);
   	 //# }
//#endif
        int ff = frame % frameInfo.length;
...
//#if opengl == true
     //# public void bindTexture(String pool, String fileName) {
    	 //# if (texture != null) {
    		 ...
    		 //# return;
    	 //# }
    	 //# // 给所有材质加上一像素的边，防止放缩时的黑边现象
    	 //# if (mergeMode) {
    		 //# buffer = new Object[frameInfo.length / 2];
    		 //# // 合并模式，创建每一个小图并加边
    		 //# for (int i = 0; i < frameInfo.length / 2; i++) {
        		 //# int tileX = (frameInfo[i << 1] >> 16) & 0xFFFF;
        		 //# int tileY = frameInfo[i << 1] & 0xFFFF;
        		 //# int tileWidth = (frameInfo[(i << 1) + 1] >> 16) & 0xFFFF;
        		 //# int tileHeight = frameInfo[(i << 1) + 1] & 0xFFFF;
        		 //# int iid = (tileX >> 14) & 0x03;
        		 //# tileX &= 0x3FFF;
        		 //# int[] rgb = new int[tileWidth * tileHeight];
        		 //# mergeImage[iid].getRGB(rgb, 0, tileWidth, tileX, tileY, tileWidth, tileHeight);
        		 //# buffer[i] = new GLBitmap(rgb, tileWidth, tileHeight);
        		 //# ((GLBitmap)buffer[i]).convertARGB2ABGR();
        		 //# ((GLBitmap)buffer[i]).addBorder();
        	 //# }
        	 //# // 释放内存
        	 //# mergeImage = null;
        	 //# mergeImageData = null;
    	 ...
    	 //# }
    	 //# // 注册所有帧图片
    	 //# int[][] imagePos = new int[buffer.length][];
    	 //# GLBitmap[] bmps = new GLBitmap[buffer.length];
    	 //# for (int i = 0; i < buffer.length; i++) {
    		 //# bmps[i] = (GLBitmap)buffer[i];
    	 //# }
		 //# texture = new GLTextureWrapper(GLTextureManager.registerDynamicImage(pool, fileName, bmps, imagePos), buffer.length);
    	 //# for (int i = 0; i < buffer.length; i++) {
    		 //# int[] pos = imagePos[i];
			 //# texture.defineArea(pos[0] + 1, pos[1] + 1, pos[2] - 2, pos[3] - 2);
         //# }
    	 //# // 释放内存
    	 //# buffer = null;
     //# }
