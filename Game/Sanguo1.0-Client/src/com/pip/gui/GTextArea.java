...
public class GTextArea extends GContainer implements IGPaint{
	String text;    //原始文本
	String[] lines = new String[0]; //按宽度分割好的文本
	...
	private void setLines() {
		...
		curPage = 0;
		//#if NewUI2
		//# lines = Tool.formatText(text, vmData[GW_VM_W] - vmData[GW_VM_BORDERLEFT] - vmData[GW_VM_BORDERRIGHT], font,true);
	    //#else
	    ...
	    if(gsb != null) {
	    	gsb.reset();
	    	//#if NewUI2
	    	//# gsb.maxScrollDis = testHeight(vmData[GW_VM_W] - vmData[GW_VM_BORDERLEFT] - vmData[GW_VM_BORDERRIGHT]);
	    	//#else
...
//		Utilities.graphics.setClip(rect[0], rect[1], rect[2], rect[3]);
	    ...
	    //# 	int currLineHeight = (Integer.parseInt(lines[i].substring(lines[i].lastIndexOf("{$}")+3,lines[i].length())) + lineSpace);
	    //#     Tool.drawMixedText( 
	    //#     		Utilities.graphics, 
	    //#     		lines[i].substring(0, lines[i].lastIndexOf("{$}")), 
	    //#     		this.vmData[GW_VM_OFFSET_X] + vmData[GW_VM_XX] + vmData[GW_VM_BORDERLEFT],

	    //#     		_y + ((currLineHeight - font.getHeight())>>1), 
		//#
	    //#     		color, 
	     //#    		0x0,
	     //#    		is3d, 
...
//	    Utilities.graphics.setClip(0, 0, GameMain.viewWidth, GameMain.viewHeight);
	...
	public int testHeight(int _width){
	    ...
	    //#if NewUI2
	   //#  int _realHeight = (lineSpace) * (_totalLines - 1) + vmData[GW_VM_BORDERTOP] + vmData[GW_VM_BORDERBOTTOM];
	  //#   
	  //#   for (int i = 0; i < lines.length; i++) {
	  //#   	int token = lines[i].lastIndexOf("{$}");
	  //#   	if(token != -1){
	  //#   		int currLineHeight = (Integer.parseInt(lines[i].substring(token+3,lines[i].length())));
	//# 	    	_realHeight += currLineHeight;
	   //#  	} else {
	   //#  		_realHeight += font.getHeight();
	  //#   	}
	   //#  	
	   //#  }
