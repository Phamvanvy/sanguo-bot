...
public class GLabel extends GWidget implements IGPaint, IGCycle{
	...
	public void setText(String text) {
		...
		if(isChangeShow){
			if(strWidth > vmData[GW_VM_W]){
				int defStrWidth = font.stringWidth("...");
				String[] showStrs = Tool.formatText(label, vmData[GW_VM_W] - defStrWidth, Utilities.font);
	...
	public void paint() {	
		...
		int clipw = Utilities.graphics.getClipWidth();
		int cliph = Utilities.graphics.getClipHeight();
		//# getIntersect(rect);	
		//#else
		if(parentNeedScroll()) {
			getIntersect(rect);		
		    ...
		    rect[0] = 0;
		    rect[1] = 0;
		    rect[2] = GameMain.viewWidth;
		    rect[3] = GameMain.viewHeight;
		}
		//#endif
		//绘制背景
		if(colors != null) {
		...
		if(label != null) {
			...
			Tool.rectGetIntersection(rect[0], rect[1], rect[2], rect[3], this.vmData[GW_VM_OFFSET_X] + vmData[GW_VM_XX] + vmData[GW_VM_BORDERLEFT], this.vmData[GW_VM_OFFSET_Y] + vmData[GW_VM_YY] + vmData[GW_VM_BORDERTOP], _w, _h, rect);
			...
			//#if NewUI2
			if(needExecuteCycle && needScroll){
				Utilities.graphics.setClip(clipx, clipy, clipw, cliph);
			}
			//#else
