...
public class GamePanel implements IVMGameProcessor{
   ...
   //# public Font landMarkFont;
    ...
    //#if NewUI2
    //# public static final byte HMSG_STEP = 2;       //横向滚动的速度
    //# public static final byte VMSG_STEP = 1;       //纵向滚动的速度
    //#else
    ...
    public void init(){
    	//#if NewUI2
    	//# VMGame.loadVMGame("game_panel", VMGame.VM_TYPE_UI, true);
    ...
    public int game_panel_add_item_hmessage_bar(int x, int y, int w, int h, int layer, int edge, int alpha,byte type){
        //为了提高性能
    	hMessageItem = new GamePanelItem(idKey.nextKey(), type);
    ...
    public int game_panel_add_item_hmessage_bar(int x, int y, int w, int h, int layer, int edge, int alpha){
    	return game_panel_add_item_hmessage_bar(x, y, w, h, layer, edge, alpha, GAME_PANEL_HMSG_BAR);
    }
    ...
    public void game_panel_post_hmessage(int id, String msg, int color, int maxRecord){
        ...
        if(_count < maxRecord){
            ...
            if(hMessageItem.type == GAME_PANEL_HMSG_BAR){
...
//#     
    ...
    public static int[] getPointItem(int pointX0, int pointY0){
    	//#if NewUI2
    	//10.22
    	//# int pointX = ((int)(pointX0 * GameMain.getScale() * GameMain.getScaleFixMap2()));
		//# int pointY = ((int)(pointY0 * GameMain.getScale() * GameMain.getScaleFixMap2()));
		//#else
		int pointX = pointX0;
		int pointY = pointY0;
		//#endif
        ...
        if(GameWorld.gameView != null){
            ...
            for(int i = 0; i < spriteCount; i++){
                ...
                if(GameWorld.gameView.checkTarget(gameSprite) != null){
                    ...
                    //还要判断是否有任务标记（!,?）
                    int questMarkH = 0;
                    if(gameSprite.getType() == Tool.SPRITE_TYPE_NPC && gameSprite.faction == GameWorld.player.faction && gameSprite.canSelect == true && gameSprite.canAttack == false){
                    	Object hintKey = gameSprite.readGameData("AnimateKey_Hint");
                    	if(hintKey != null){
                    		questMarkH = 47 + Utilities.CHAR_HEIGHT;//问号高度
                    	}
                    }
                    //#if NewUI2
                    //10.22
                    //# if(Tool.rectIn(box[0], box[1] - questMarkH, box[2], box[3] + questMarkH, realX, realY)){
                    //#		binaryAdd(candidates, gameSprite, realX, realY);
                    //# }
                    //#else
                    if(Tool.rectIn(box[0], box[1] - questMarkH, box[2], box[3] + questMarkH, realX, realY)){
                    	binaryAdd(candidates, gameSprite, realX, realY);
            ...
            if(spriteCount > 1){//从多个候选者中找
            	//距离x,y最近的Sprite
        		GameSprite top = (GameSprite)candidates.elementAt(0);
        		Integer disTopObj = (Integer)sprite2dis.get(top);
        		int disTop = disTopObj.intValue();
        		
        		int maxIndex = 0;
        		for(int i = 1; i < spriteCount; i++){
                    GameSprite gameSprite = (GameSprite)candidates.elementAt(i);
                    Integer disObj = (Integer)sprite2dis.get(gameSprite);
            		int dis = disObj.intValue();
            		if(Math.abs(dis - disTop) > SELECT_SPRITE_FRESHHOLD){
            			maxIndex = i - 1;
            			break;
            		}
                }
        		
        		if(maxIndex == 0){//只有top满足条件
        			return new int[]{
            			top.getInstanceId(), 0, 0, 0, 0, 0, 0
            		};
        		} else {//根据:敌对玩家>npc>怪>队友>自己的顺序再行选择
        			int enemyIndex = -1;
        			int npcIndex = -1;
        			int creatureIndex = -1;
        			int teammateIndex = -1;
        			for(int i=0;i<=maxIndex;i++){
        				GameSprite gameSprite = (GameSprite)candidates.elementAt(i);
        				if(gameSprite.getType() == Tool.SPRITE_TYPE_PLAYER){
        					if(GameWorld.player.faction != gameSprite.faction){
        						...
        						//判断是否是队友
        						if(GameWorld.player.isTeamState() && gameSprite.isTeamState()){//组对状态
        							int[] roleInfo = (int[])GameWorld.instance.readGameData("game_role_infor");
            						SortHashtable teamlist = (SortHashtable)VM.globalVMData.get(new Integer(roleInfo[59]));
            						if(teamlist != null){
            							int teamSize = teamlist.size();
            							for (int j = 0; j < teamSize; j++) {
											int[] memberInfo = (int[])teamlist.getValue(j);
											if(memberInfo[0] == gameSprite.getInstanceId()){
												if(teammateIndex == -1){
													teammateIndex = i;
												}
												break;
        				...
        				} else if(gameSprite.getType() == Tool.SPRITE_TYPE_NPC){
        					int[] npcData = (int[])gameSprite.readGameData("npc_data");
        					if(npcData[1] == VM.TRUE){//功能NPC
        						if(!gameSprite.canAttack){
        							if(npcIndex == -1){
            							npcIndex = i;
            						}
        						}
        					} else if(gameSprite.canAttack){//怪
        						if(creatureIndex == -1){
        							creatureIndex = i;
        						}
        					}
        			...
        			if(candidate != null){
        				return new int[]{
    	        			candidate.getInstanceId(), 0, 0, 0, 0, 0, 0
    	        		};
        			}
        		}
            } else if(spriteCount == 1){//有唯一的选项
...
//#endif               
        ...
        switch(item.type){
            ...
            case GAME_PANEL_ICON:{
                ...
                if(item.intData1 == GAME_PANEL_ACTION_STATE_NON_ACTIVE){
                	//#if ModelID == AndroidAuto
                	//# if((item.frame >= 0 && item.frame <= 11) || sequences[1] == -1){
                	//# } else {
            ...
            case GAME_PANEL_STRING:{
            	...
            	Font f;
        		if(item.objData3 != null){
        			f = (Font)item.objData3;
        		} else {
        			f = Utilities.font;
        		}
        		Vector v = Tool.formatString((String)item.objData, GameMain.viewWidth, f, true);
            	if(strData[0] == 1){//draw 3d string
            		Tool.drawMixedText(g, v, (int)item.x, my, strData[2], strData[3], true, item.anchor, f);
            	} else {//draw 2d string
            		Tool.drawMixedText(g, v, (int)item.x, my, strData[2], strData[3], false, item.anchor, f);
            	}
            	v = null;
    ...
    /**
     * 获得聊天数量
     * @param type 0:vmessage 1:hmessage
     * @return
     */
    public int getMessageCount(int type){
    	if(type == 0){
    		Vector vmsgs = (Vector) vMessageItem.objData;
