...
public class DebugVM extends VM implements Runnable{
        ...
        while(connection != null){
            try{
                ...
                if(cmdType == COMMAND_INTERRUPT){
                    ...
                    writeState(dos);
                }else if (cmdType == COMMAND_MODIFY) {
                	int op = dis.readInt();
                	switch (op) {
                	case 0:
                		modifyInt(dis.readInt(), dis.readInt());
                		break;
                	case 1:
                		modifyIntMember(dis.readInt(), dis.readInt(), dis.readInt());
                		break;
                	case 2:
                		modifyString(dis.readInt(), dis.readUTF());
                		break;
                	}
                }
            }catch(IOException e){
        ...
        while(eip < eipmax){
            ...
            switch(inst){
                ...
                case SYSCALLSAVEVS:
                {
                    ...
                    esp -= parCount;
                    int ret = syscall(callFunc, syscallParams);
                    if(hasRet){
    ...
    public synchronized void execute(int funcID, int[] params){
        ...
        try{
            ...
            //System.out.println("exec func " + funcID + ": " + (procCounter - oldCounter) + "/" + (syscallCounter - oldCounter2) + "/" + (Tool.getSystemTime() - startTime));
        }catch(Exception e){
            e.printStackTrace();
            try{
                generateInterrupt(7);
    ...
    public static String getExecCount(){
        ...
        }else if(Tool.getSystemTime() - execFuncMaxCountUpdateTime > statMillis){
            execFuncMaxCountSecond = result;
            execFuncMaxCountUpdateTime = Tool.getSystemTime();
        }
    ...
    private void modifyString(int pointer, String value) {
        ...
        if ((pointer & 0x80000000) != 0) {
            ...
            if (libID == 0) {
                // 引用执行文件的字符串表
                stringTable[pointer & 0xFFFF] = value;
            } else {
                // 引用库文件的字符串表
                for (int i = 1; i < libraries.length; i++) {
                    if (libID == libraries[i].libraryID) {
                        libraries[i].stringTable[pointer & 0xFFFF] = value;
                    }
                }
            }
        } else {
