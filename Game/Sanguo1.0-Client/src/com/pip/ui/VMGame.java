...
public class VMGame{
    ...
    public static void cycle(){
        if(state == STATE_REQUESTING_VMUI){
            ...
            while(enu.hasMoreElements()){
                ...
                if(Tool.getSystemTime() - startTime.longValue() < UASocketConnection.TIME_SEND_TIMEOUT){
    ...
    public static int addUI(String vmId, byte[] etfData, byte vmType) throws Exception{
        ...
        if(vmType == VM_TYPE_UI){
            ...
            //#if ChannelCode == K_PAY_NEW_CHANNEL_ANDROID 
            //# if((vmId.equals("ui_mainmenu") || vmId.equals("game_init_once"))&& PipActivity.DEFAULT_ACTIVITY.isFristShow){
            //#	 PipActivity.DEFAULT_ACTIVITY.showFlowAlert();
            //# }
	    	//#endif
        }
    ...
    public static void closeAllUI(VMGame refUI, int type){
        ...
        if(vgs != null){
        	 for(int i = vgs.length -1 ; i >= 0; i--){
                ...
                //#if NewUI2
                if(vg.vmId.equals("game_panel")||vg.vmId.equals("ui_gamemenu")){
                	continue;
                }
                //#endif
                if(vg.equals(refUI)){
                	if(vg.getVMType() == VM_TYPE_UI && type == 0){
                		vg.close();
                	}
                	return;
                }
