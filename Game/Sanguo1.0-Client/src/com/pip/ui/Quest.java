...
//    public static Hashtable npcRelateQuest = new Hashtable();
    ...
    public static Quest addQuest(int id, int type, int startNpcId, int endNpcId, byte[] etf) throws IOException{
        ...
        //#if NewUI2
        synchronized (questStates) {
        	QuestState qs = new QuestState();
			qs.state = quest.state;
			questStates.put(new Integer(id),qs);
		}
        //#endif
        return quest;
    }
    ...
    public static void removeQuest(int id, int startNpcId, int endNpcId){
        ...
        //#if NewUI2
        synchronized (questStates) {
			questStates.remove(new Integer(id));
		}
        //#endif
    }
...
//#if NewUI2
    public static final int QUEST_DATA_ID = 0;
    public static final int QUEST_DATA_STATE = 1;
    public static final int QUEST_DATA_NAME = 2;
    public static final int QUEST_DATA_DESC = 3;
    public static final int QUEST_DATA_TARGETS = 4;
    public static final int QUEST_DATA_TYPE = 5;
    public static final int QUEST_DATA_START_NPC_ID = 6;
    public static final int QUEST_DATA_END_NPC_ID = 7;
    public static final int QUEST_DATA_QUEST_LEVEL = 8;
    public static final int QUEST_DATA_REWARD_GROUP = 9;
    public static final int QUEST_DATA_REWARDS = 10;
    public static final int QUEST_DATA_PATH_POINT_DESCS = 11;
    public static final int QUEST_DATA_PATH_POINT_MAP_IDS = 12;
    public static final int QUEST_DATA_PATH_POINT_XYS = 13;
    public static final int QUEST_DATA_SYNC_DESC_TYPE = 14;
    public static final int QUEST_DATA_PATHS = 15;
    
    
    public static final int PATHPOINT_DESC = 0;
    public static final int PATHPOINT_MAP_ID = 1;
    public static final int PATHPOINT_NPC_ID = 2;
    public static final int PATHPOINT_X = 3;
    public static final int PATHPOINT_Y = 4;
    
    private static final Vector _npcs = new Vector();
    /**
     * 从任务信息中解析寻路信息
     * @param _qi
     * @param _str
     ...
     * @param vm
     */
    public static void parsePathPoint(int[] _qi,String _str, Vector _pathPoints,VM vm) {
    	int startNpcId = _qi[QUEST_DATA_START_NPC_ID];
    	int endNpcId = _qi[QUEST_DATA_END_NPC_ID];
    	int questState = _qi[QUEST_DATA_STATE];

    	int _count = _str.length();
        int _state = 0;
        StringBuffer _buf = new StringBuffer();
        String _exprStr;
        int _firstComma;
        int[] pathPoint;
        _npcs.removeAllElements();
        
        for(int i = 0; i < _count; i++){
            char _ch = _str.charAt(i);
            char _nextCh;
            if(i < _count - 1) {
            	_nextCh = _str.charAt(i+1);
            } else {
            	_nextCh = (char)-1;
            }
            switch(_state) {
            case 0:
                if(_ch == '<' && _nextCh == 'n') {
                	i++;
                    _state = 100; //解析npc tag
                    _buf.setLength(0);
                } else if(_ch == '<' && _nextCh == 'l') {
                	i++;
                    _state = 101; //解析坐标 tag
                    _buf.setLength(0);
                }
            ...
            case 100:
                if(_ch == '>'){
                	_exprStr = _buf.toString();
                	if(_exprStr.endsWith("</n")) {
                		int startPos = _exprStr.indexOf('>') + 1;
                		int endPos = startPos + _exprStr.length() - 4;
                		String _contentText = _exprStr.substring(startPos,endPos);

                		//#if NewUI2
                		String[] _splitInfo = _contentText.split(",");
                		//#else
                		//# String[] _splitInfo = Tool.splitString(_contentText, ',');
                		//#endif
                		
                		//NpcId
                		int _npcId = Integer.parseInt(_splitInfo[0]);
                		//寻路描述
                		pathPoint = new int[5];
                		pathPoint[PATHPOINT_NPC_ID] = _npcId;
                		pathPoint[PATHPOINT_MAP_ID] = _npcId >> 12;
            
            			
			          	//#if NewUI2
            			String desc = _splitInfo[1].concat(",").concat(_splitInfo[2]);
			    		//#else
			    		//# String desc = _splitInfo[1] + "," + _splitInfo[2];
			    		//#endif
            			pathPoint[PATHPOINT_DESC] = vm.makeTempObject(desc);
            			
                		//Npc坐标
                		_firstComma = desc.indexOf(':');
                		startPos = _firstComma + 1;
                		endPos = startPos + desc.length() - _firstComma - 2;
                		_exprStr = desc.substring(startPos,endPos);
                		_firstComma = _exprStr.indexOf(',');
                		
                		String _npcIdStr = _exprStr.substring(0,_firstComma);
                		pathPoint[PATHPOINT_X] = Integer.parseInt(_npcIdStr);
                		startPos = _firstComma + 1;
                		endPos = startPos + _exprStr.length() - _firstComma - 1;
                		_npcIdStr = _exprStr.substring(startPos, endPos);
                		pathPoint[PATHPOINT_Y] = Integer.parseInt(_npcIdStr);
                		//
                		_npcs.addElement(new Integer(_npcId));
                		_npcs.addElement(pathPoint);
                		_state = 0;
                	} else {
                		_buf.append(_ch);
                	}
            ...
            case 101:
                if(_ch ==  '>'){
                	_exprStr = _buf.toString();
                	if(_exprStr.endsWith("</l")) {
                		int startPos = 1;
                		int endPos = startPos + _exprStr.length() - 4;
                		_exprStr = _exprStr.substring(startPos,endPos);
                		_firstComma = _exprStr.indexOf(',');
                		//场景Id
                		String _sceneIdStr = _exprStr.substring(0, _firstComma);
                		int _sceneId = Integer.parseInt(_sceneIdStr);
                		
                		pathPoint = new int[5];
                		pathPoint[PATHPOINT_NPC_ID] = -1;
                		pathPoint[PATHPOINT_MAP_ID] = _sceneId;
                		startPos = _firstComma + 1;
                		endPos = startPos + _exprStr.length() - _firstComma - 1;
                		String desc = _exprStr.substring(startPos,endPos );
            			pathPoint[PATHPOINT_DESC] = vm.makeTempObject(desc);
                		
                		_firstComma = desc.indexOf(':');
                		startPos = _firstComma + 1;
                		endPos = startPos + desc.length() - _firstComma - 1;
                		_exprStr = desc.substring(startPos,endPos);
                		_firstComma = _exprStr.indexOf(',');
                		...
                		_npcIdStr = _exprStr.substring(startPos,endPos );
                		pathPoint[PATHPOINT_Y] = Integer.parseInt(_npcIdStr);
                		if(questState != QUEST_STATE_CAN_FINISH){	
                			_pathPoints.addElement(pathPoint);
                		}
                		_state = 0;            		
                	} else {
                		_buf.append(_ch);
        ...
        int len = _npcs.size();
        int _npcId = 0;
        if(questState == QUEST_STATE_DOING){
        	//正在做的任务照寻怪
    	    for(int i=0;i<len;i+=2){
    	    	_npcId = ((Integer)_npcs.elementAt(i)).intValue();
    			if(_npcId != endNpcId && _npcId != startNpcId){
    				_pathPoints.addElement(_npcs.elementAt(i + 1));
    			}
    	    }
        } else if(questState == QUEST_STATE_CAN_FINISH){
        	//可以交的任务寻结束NPC
    	    for(int i=0;i<len;i+=2){
    	    	_npcId = ((Integer)_npcs.elementAt(i)).intValue();
    			if(endNpcId == _npcId){
        ...
        } else if(questState == QUEST_STSATE_CAN_ACCEPT){
        	//未接的任务寻开始NPC
    	    for(int i=0;i<len;i+=2){
    	    	_npcId = ((Integer)_npcs.elementAt(i)).intValue();
    			if(startNpcId == _npcId){
    				_pathPoints.addElement(_npcs.elementAt(i + 1));
    				break;
    			}
    	    }
    ...
    public static final int questid = 0;
    public static final int queststate = 1;
    public static final int name = 2;    //String
    public static final int desc = 3;    //String
    public static final int targets = 4; //String[]
    public static final int questtype = 5;
    public static final int startNpcId = 6;
    public static final int endNpcId = 7;
    public static final int questLevel = 8;
    public static final int rewardGroup = 9;
    public static final int rewards = 10;     //Vector<QuestReward>
    public static final int pathPointDescs = 11; //String[]
    public static final int pathPointMapIds = 12; //int[]
    public static final int pathPointXYs = 13; //int[], 高16位是x坐标，底16位是y坐标
    public static final int syncDescType = 14;	//是否需要更新描述
    public static final int pathes = 15;		//String[] 寻路信息
    
    /**
     * 保存任务的上个cycle的各种状态
     */
    private static final Hashtable questStates = new Hashtable();
    
    private static final Vector questTmp = new Vector();
    /**
     * check任务状态
     * @param quest
     */
    public static void checkQuestStates(Vector quests){
    	try {
    		questTmp.removeAllElements();
        	synchronized (quests) {
    			int size = quests.size();
    			for (int i = 0; i < size; i++) {
    				int[] questInfo = (int[])quests.elementAt(i);
    				int id = questInfo[questid];
    				QuestState qs = (QuestState)questStates.get(new Integer(id));
    				Quest q = findQuest(id, true);
    				if(qs == null){//新加了任务
    					qs = new QuestState();
    					qs.state = questInfo[queststate];
    					synchronized(VM.globalVMData){
    	                    String[] targetstrs = (String[])VM.globalVMData.get(new Integer(questInfo[targets]));
    	                    if(targetstrs != null){
    	                    	int targetLen = targetstrs.length;
    	                    	qs.targets = new String[targetLen];
    	                    	qs.targetStates = new int[targetLen];
    	                    	for (int j = 0; j < targetstrs.length; j++) {
    								qs.targets[j] = new String(translateText(id, targetstrs[j]));
    								qs.targetStates[j] = q.getTargetStatus(j)?VM.TRUE:VM.FALSE;
    	                    	}
    	                    }
    					}
    					questStates.put(new Integer(id), qs);
    				} else {
    					if(qs.state != questInfo[queststate]){//任务状态改变
    						qs.state = questInfo[queststate];
    						sendQuestEvent(id, QUEST_EVENT_STATE_CHANGED, -1);
    					} else {
    						String[] targetstrs = null;
    						synchronized(VM.globalVMData){
    		                    targetstrs = (String[])VM.globalVMData.get(new Integer(questInfo[targets]));
    						}
    						if(targetstrs != null){
    							if(qs.targetStates == null){//目标信息还没初始化
    								qs.targets = new String[targetstrs.length];
    								qs.targetStates = new int[targetstrs.length];
    								for (int j = 0; j < targetstrs.length; j++) {
    									int currTargetState = q.getTargetStatus(j)?VM.TRUE:VM.FALSE;
    	                    			qs.targetStates[j] = currTargetState;
    	                    			String currTargetText = new String(translateText(id, targetstrs[j]));
    									qs.targets[j] = currTargetText;
    								}
                        		} else {
                        			for (int j = 0; j < targetstrs.length; j++) {
        	                    		int currTargetState = q.getTargetStatus(j)?VM.TRUE:VM.FALSE;
        	                    		if(qs.targetStates[j] != currTargetState){//目标完成状态改变
        	                    			qs.targetStates[j] = currTargetState;
        	                    			sendQuestEvent(id, QUEST_EVENT_TARGET_COMPLETED, currTargetState);
        	                    		} else {
        	                    			String currTargetText = new String(translateText(id, targetstrs[j]));
        	                    			if((qs.targets[j] == null && currTargetText != null) || !qs.targets[j].equals(currTargetText)){//目标需求数量改变
        										qs.targets[j] = currTargetText;
        										sendQuestEvent(id, QUEST_EVENT_TARGET_CHANGED, j);
        									}
        	                    		}
        	                    	}
                        		}
    ...
    public static final int QUEST_EVENT_ADDED = 0;
    public static final int QUEST_EVENT_REMOVED = 1;
    public static final int QUEST_EVENT_STATE_CHANGED = 2;
    public static final int QUEST_EVENT_TARGET_CHANGED = 3;
    public static final int QUEST_EVENT_TARGET_COMPLETED = 4;
    
    
    private static final int[] questEventParams = new int[3];
     ...
     * @param eventType
     */
    public static void sendQuestEvent(int id,int eventType,int targetIndex){
    	VMGame vmg = VMGame.getVMGame("game_panel");
    	if(vmg != null){
    		synchronized(vmg.gtvm){
    			questEventParams[0] = id;
    			questEventParams[1] = eventType;
    			questEventParams[2] = targetIndex;
            	vmg.gtvm.callback("ProcessQuestEvent", questEventParams);
            }
    	}
