						...
						try {
							...
							Server.server.getServiceRegistry()
									.getMailService().sendSystemMail(
											player.id, "系统", "激活码奖励", "", 0,
											item, 1, "ACTCODE");
						}
					}else{
						Server.server.getServiceRegistry().getMailService()
							.sendSystemMail(player.id, "系统", "激活码奖励",
								"", 0, item, 1, "ACTCODE");
					}
				} else if (retCode == 1) {
					error(null, "您输入的激活码不正确，请核实后再试。");
				} else if (retCode == 2) {
					error(null, "您输入的激活码不是“明珠三国”的激活码，请核实后再试。");
				} else if (retCode == 3) {
					error(null, "您输入的激活码在这个兑换员处无法兑换，请找另一个兑换员。");
				} else if (retCode == 4) {
					error(null, "您输入的激活码已经兑换过了。");
				} else if (retCode == 5) {
					error(null, "您输入的激活码已过期。");
				} else if (retCode == 6) {
				    error(null, "兑换活动已暂停。");
				} else if (retCode == 7) {
				    error(null, "一个帐号只能兑换一次。");
				}
			}
		} catch (Exception ex) {
			log.error(ex, ex);
