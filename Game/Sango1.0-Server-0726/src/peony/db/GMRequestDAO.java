...
public class GMRequestDAO extends GenericHibernateDAO<GMRequest, Integer>{
	...
	@SuppressWarnings("unchecked")
	public List<GMRequest> getGMRequestsByDate(Date date, Date date2, int begin, int count) {
		return limitList(
				"from GMRequest m where m.createTime>? and m.createTime<? order by m.createTime desc",
				begin,count,date,date2);
	}
	
	public int getGMRequestByDateCount(Date date, Date date2){
		Long l = (Long) uniqueResult(
				"select count(*) from GMRequest m where m.createTime>? and m.createTime<?", date,date2);
		return l.intValue();
	}
