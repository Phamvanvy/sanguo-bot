	...
	public void run() {
		...
		if(p!=null){
			if(requestId==1){
				...
				if(p.getAccount().getCity()!=null && p.getAccount().getCity().equals("温州") && Server.server.REVISION_TYPE_CMCC.equals(Server.server.revision)){
					if(p.level<40){
				...
				if(fame!=null){
					...
					try {
						...
						tx.rollback();
						ErrorHandler.sendErrorMessage(session, serial, OpCode.GETITEM_FROM_NPC_CLIENT, "背包已满");
					}
				}else{
