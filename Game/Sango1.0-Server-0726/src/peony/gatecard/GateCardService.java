...
//		}
		...
		if("0".equals(result.getAnyType().get(0).toString())){
			...
			addBalance(accountId,imoney,call,serial);
...
//		Server.server.getServiceRegistry().getAccountService().postMessage(new AddBalanceMessage(accountId,imoney,"gatecard"));
		Server.server.getServiceRegistry().getAccountService().sendAndRegister(new AddBalanceMessage(accountId,imoney,"gatecard_"+cardNum),call);
	}
