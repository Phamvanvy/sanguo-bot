	...
	public void callFinish() throws Exception {
		if(success){
			...
			if(player != null){
				...
				packet.putString(MessageFormat.format("尊敬的{0}: 您已经提出申请和您的伴侣{1}解除婚姻关系，经判定，离婚申请获得批准。", service.find(player.id).name,service.find(annotherPersonId).name));
				player.send(packet);
				player.send(packet0);
				MailService mailService = Server.server.getServiceRegistry().getMailService();
				mailService.sendSystemMail(player.id, "系统", "您已离婚", MessageFormat.format("您与{0}婚姻宣告破裂", service.find(annotherPersonId).name), 0, null, 0, "");
				if(ObjectAccessor.getPlayer(annotherPersonId) != null){
					Packet packet1 = new Packet(OpCode.MARRIAGE_DIVORCE_SERVER);
					packet1.putString(MessageFormat.format("尊敬的{0}: 您的伴侣{1}已经提出申请和您解除婚姻关系，经判定，离婚申请获得批准。", service.find(annotherPersonId).name,player.name));
...
//					mailService.sendSystemMail(annotherPersonId, "系统", "您已离婚", "您的伴侣"+service.find(player.id).name+"提出申请和您解除婚姻关系，经判定，离婚申请获得批准", 0, null, 0);
					mailService.sendSystemMail(annotherPersonId, "系统", "您已离婚", MessageFormat.format("您与{0}婚姻宣告破裂", service.find(player.id).name), 0, null, 0, "");
				}else if(type == 0){
					mailService.sendSystemMail(annotherPersonId, "系统", "您已离婚", MessageFormat.format("您与{0}婚姻宣告破裂", service.find(player.id).name), 0, null, 0, "");
				}
				chatService.addChatMessage(new ChatMessage(ChatOption.FACTION,-1,player.faction,"系统",player.faction,MessageFormat.format("{0}和{1}已离婚.", player.name,service.find(annotherPersonId).name),null));
