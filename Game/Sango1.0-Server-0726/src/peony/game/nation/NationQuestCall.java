	...
	public void run() {
		...
		if (p != null) {
			...
			if (nation.getOfficerByPlayerId(p.id)!= null) {
					...
					if (day != Time.day) {
						if (nation.money < Nation.QUEST_MIN_MONEY) {
							...
							if (p.bag.removeGameItem(ItemUtil.ITEM_NATION_QUEST,
								...
								ms.sendSystemMail(p.id, "系统", "您已成功发布了国家任务。敬请领取礼品！", MessageFormat.format("您已成功发布了国家任务。由于您的勤政爱民，特此发放贺礼如下：{0}x1", itemName)
										, 0, gameItem, 1, "KING");
								...
								Server.server
										...
										.sendFactionSystemMessage(p.faction,
												MessageFormat.format("好消息," + nation.getOfficerByPlayerId(p.id).getName() +
														"发布丰厚奖励的{0}任务，大家可以去大司农处领取", quest.getGameQuest().getName()));
				...
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.NATION_QUEST_CLIENT, "您不是国家官员，不能发布国家任务");
			}
		}
	}
