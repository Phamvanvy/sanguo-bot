...
public class NationConvoyService implements Service {
	...
	public void startConvoy(Nation nation) throws ConvoyException{
		if(!Time.betweenHour(Time.currDate, 19, 24))
			throw new  ConvoyException("必须在19点至24点之间进行国家押送");
		if(convoys[nation.faction] != null){
			throw new ConvoyException("当前正有一个国家押送没完成");
		}
		synchronized(nation){
			if(nation.money<DEPOSITE){
				throw new ConvoyException("需要20万国库资金才能发起国家押送，请呼吁民众捐款支援。");
			}
			nation.decMoney(DEPOSITE);
		}
		if(nation.pool.getInt(Nation.PROPERTY_NATION_CONVOY, 0)==Time.day){
			throw new ConvoyException("国家押运每天只能一次");
		}
		...
		Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(nation.faction, "为了保障前线战事，国公下令押送军备物资送往前线， 36级以上的玩家前往江陵营地押送，5分钟后押送开始。");
		for(int i=1;i<4;i++){
			if(i != nation.faction){
				Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(i, 
						MessageFormat.format("{0}发起了国家押送，5分钟后大家集结去江陵拦截对方的物资车，保家卫国。", GameObject.getFactionName(nation.faction)));
			}
		}
	}
	...
	public void success(NationConvoy convoy) {
		...
		for(Player p:l){
			...
			mailService.sendSystemMailAsync(p.id, "系统", "国家押送奖励", "", 0, item1, 2, "COV");
			mailService.sendSystemMailAsync(p.id, "系统", "国家押送奖励", "", 0, item2, 1, "COV");
		}
		if (l.size() > 0) {
		...
		Server.server.getServiceRegistry().getChatService()
				.sendFactionSystemMessage(convoy.nation.faction,
						"国家押送成功，国库资金增加20万；参与国家押送的玩家平分20万，另每人获得神秘礼包1个和珍珠2颗");
		Server.server.getServiceRegistry().getChatService().sendWorldMessage(
				MessageFormat.format("{0}国民齐心协力，成功将物资送往了前线。", GameObject.getFactionName(convoy.nation.faction)));
	...
	public void fail(NationConvoy convoy){
		...
		for(Player p:l){
			...
			GameItem item1 = ObjectAccessor.createGameItem(1311);
			GameItem item2 = ObjectAccessor.createGameItem(1683);
			mailService.sendSystemMailAsync(p.id, "系统", "劫取国家押送奖励", "", 0, item1, 1, "COV");
			mailService.sendSystemMailAsync(p.id, "系统", "劫取国家押送奖励", "", 0, item2, 1, "COV");
		}
		if (l.size() > 0) {
			int v = DEPOSITE / l.size();
			for (Player p : l) {
		...
		Server.server.getServiceRegistry().getChatService().sendWorldMessage(
				MessageFormat.format("{0}的镖车已经被摧毁。", GameObject.getFactionName(convoy.nation.faction)));
		for(int i=1;i<bs.length;i++){
			if(bs[i]){
				Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(i, 
						MessageFormat.format("{0}送往前线的物资已被我国截获，举国欢腾", 
