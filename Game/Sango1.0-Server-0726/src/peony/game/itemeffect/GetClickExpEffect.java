...
//		}
		...
		if(totalOnlineTime>=ONLINE_TIME){
			...
			if(times>12)
				throw new UseItemException("已经达到每天领取的最大次数");
			p.pool.setLong(Player.PROPERTY_CLICKEXP_START_TIME, System.currentTimeMillis());
			p.pool.setLong(Player.PROPERTY_CLICKEXP_CUMULATE_TIME, 0);
			...
			if(min==0) min = 1;
			p.message(-1, MessageFormat.format("还需要{0}分钟才能得到在线奖励", min), -1, -1);
		}
	}
	...
	public void useItem(Unit source, GameItem item, Unit target, int useIB, PlayerTransaction tx_ext ,ClientSession session)
		...
		if(t==0){
			p.pool.setLong(Player.PROPERTY_CLICKEXP_START_TIME, System.currentTimeMillis());
			p.pool.setLong(Player.PROPERTY_CLICKEXP_CUMULATE_TIME, 0);
			ErrorHandler.sendErrorMessage(session, OpCode.FUXING_BAG_CLIENT, OpCode.FUXING_BAG_CLIENT, MessageFormat.format("还需要{0}分钟才能得到在线奖励", ONLINE_TIME/60000));
...
//		}
		if(System.currentTimeMillis()-lastUseTime>=CD && totalOnlineTime>=ONLINE_TIME){
			...
			int times = p.pool.getInt(Player.PROPERTY_CLICKEXP_TIMES,0);
			if(times>12){
				ErrorHandler.sendErrorMessage(session, OpCode.FUXING_BAG_CLIENT, OpCode.FUXING_BAG_CLIENT, "已经达到每天领取的最大次数");
				throw new UseItemException("");
			}
			p.pool.setLong(Player.PROPERTY_CLICKEXP_START_TIME, System.currentTimeMillis());
			p.pool.setLong(Player.PROPERTY_CLICKEXP_CUMULATE_TIME, 0);
...
//				}
				...
				for (GainItem gi : gain.getGainItems()) {
                	...
                	mailService.sendSystemMail(p.id, "系统", "福星赐福礼物", "", 0, gi.getItem(), gi.getCount(), "CEX");
	            }
				p.message(-1, "恭喜您获得福星赐福礼物，请查收邮件", -1, -1);
			}else{
				synchronized (this) {
					...
					if(useIB == 1){
						...
						if(service.getItemPrice(NoItemShopBuy.YIYUANBAO) >imoney){
							tx.rollback();
							ErrorHandler.sendErrorMessage(session, OpCode.FUXING_BAG_CLIENT, OpCode.FUXING_BAG_CLIENT, "i币不足");
							throw new UseItemException("");
						}
			...
			if(lastUseTime==0 || (System.currentTimeMillis()-lastUseTime>=CD && totalOnlineTime<ONLINE_TIME)){
				...
				if(min==0) 
					min = 1;
				ErrorHandler.sendErrorMessage(session, OpCode.FUXING_BAG_CLIENT, OpCode.FUXING_BAG_CLIENT, MessageFormat.format("还需要{0}分钟才能得到在线奖励", min));
				throw new UseItemException("");
			}else{
				...
				min = min%60;
				if(hour>0 && min==0){
					ErrorHandler.sendErrorMessage(session, OpCode.FUXING_BAG_CLIENT, OpCode.FUXING_BAG_CLIENT, MessageFormat.format("还需要{0}小时才能继续使用", hour));
					throw new UseItemException("");
				}
				else if(hour>0){
					ErrorHandler.sendErrorMessage(session, OpCode.FUXING_BAG_CLIENT, OpCode.FUXING_BAG_CLIENT, MessageFormat.format("还需要{0}小时{1}分钟才能继续使用", hour, min));
					throw new UseItemException("");
				}
				else{
					ErrorHandler.sendErrorMessage(session, OpCode.FUXING_BAG_CLIENT, OpCode.FUXING_BAG_CLIENT, MessageFormat.format("还需要{0}分钟才能继续使用", min));
					throw new UseItemException("");
				}
			}
		}
