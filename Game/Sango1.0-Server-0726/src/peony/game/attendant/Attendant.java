 ...
 * @author dchen
 */
public class Attendant extends Unit {
	...
	public void equip(int itemId, int instanceId) throws AttendantException {
		if(owner!=null){
			if(Server.server.getServiceRegistry().getCandidateService().isKingEquipment(itemId))
				throw new AttendantException("此装备不可以装备给您的随从");
			...
			if (grid != null && grid.getItem().template.isEquipment()) {
				...
				if (equ.template.hasDuration() && equ.duration <= 0) {
					tx.rollback();
					throw new AttendantException("装备已损坏，不能装配");
				}
	...
	public void unEquip(int itemId, int instanceId) throws AttendantException {
		...
		if(old!=null){
			...
			try {
				...
				equip(old, owner);
				tx.rollback();
				throw new AttendantException("没有足够的包位");
			}
			unSuiteEffect(old.template.equipment);
		}
	}
	...
	public void addSkill(Skill skill, int index) {
		...
		if (oldSkill != null) {
			...
			owner.changed.addChangedItem(new AttendantSkillChangedItem(this, oldSkill, false, false));
		}
		LogUtil.logAttendantStudySkill(owner, id, instanceId, skill.getId(), index);
		if (skill.getLevel() > 0) {
		...
		owner.changed.addChangedItem(new AttendantSkillChangedItem(this, skill, true, false));
	}
	...
	public void update(int diffTime) {
		if ((Time.tick % 5) == 0) {
			try {
				if(ObjectAccessor.getPlayer(owner.id)!=null){
					cycle++;
					processMove(owner);
					if (attack != null) {
