...
public class RemoveAllJewelCall1 extends SyncIbuyCall {
	...
	public static Packet getRequest(ItemEnhance enhance, ClientSession session ,int serial) throws Exception{
		...
		for(int i = 0; i < jewelNum; i ++){
			...
			if(jewelTemp == null){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.REMOVE_ALLJEWEL_RESOURCE_CLIENT, "宝石数据错误");
			...
			if(player != null){
				...
				for(int j = 0; j < jewelNum; j ++){
				    ...
				    if(removeSuccess==0){
					    ...
					    try {
					    	waitBuy(syncShopItemBuy(player, 0, service.getShopByItemId(js.removeItemIDs[lvl - 1<2?2:lvl-1]).id, js.removeItemIDs[jewelTemp.useLevel - 1], 1, this));
					    } catch (Exception e) {
					    	tx.rollback();
					    	addToClientSession();
					    	return;
				    ...
				    if (!player.bag.addGameItem(jewel, 1, tx, true)) {
				    	...
				    	if(ms.sendSystemMail(player.id, player.name, "摘除宝石", "摘除宝石", 0, jewel, 1, "背包已满") == -1){
				    		tx.rollback();
				    		error("飞鸽发送失败");
				    		addToClientSession();
				    		return;
				    	}
				    	ErrorHandler.sendErrorMessage(session, serial, OpCode.DECORATE_REMOVE_ALLJEWEL_CLIENT, "背包已满 宝石已通过飞鸽发送");
				    }
				    // 删除镶嵌信息
