			...
			if(player != null){
				...
				for(int j = 0; j < jewelNum; j ++){
				    ...
				    try {
				        ...
				        tx.rollback();
				        ErrorHandler.sendErrorMessage(session,  serial,OpCode.REMOVE_ALLJEWEL_RESOURCE_CLIENT,  "金钱不足");
				    ...
				    if(removeSuccess==0){
					    ...
					    if(needImoney>0){
							...
							if(needImoney>imoney){
								tx.rollback();
								ErrorHandler.sendErrorMessage(session, serial,OpCode.DECORATE_REMOVE_ALLJEWEL_CLIENT, "i币不足");
