...
public class AdminPacketHandler implements PacketHandler{
	...
	protected void addActLeader(Packet packet, ClientSession session){
		...
		try {
			if(mapId<=0 || x<=0 || y<=0 || minLevel<=0 || minLevel>=100 || maxLevel>100 || maxLevel<=0){
				ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_NEWACTLEADER_CLIENT, "参数配置错误");
				return;
			}
	...
	protected void deleteActivity(Packet packet, ClientSession session) {
		...
		if (act == null) {
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_DELETE_ACTIVITY_CLIENT, "不存在的活动");
			return;
		}
	...
	protected void enableActivity(Packet packet, ClientSession session) {
		...
		Activity act = service.getActivityByName(name);
		if (act == null) {
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_ENABLE_ACTIVITY_CLIENT, "不存在的活动");
			return;
		}
	...
	protected void newActivity(Packet packet, ClientSession session) {
		...
		try {
			if (service.getActivityByName(name) != null) {
				throw new Exception("已经有一个同名的活动了，每个活动必须有一个唯一的内部名称。");
			}
			...
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_ACTIVITY_DETAIL_CLIENT, "创建活动失败：" + e);
		}
	}
	...
	protected void activityDetail(Packet packet, ClientSession session) {
		...
		if (act == null) {
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_ACTIVITY_DETAIL_CLIENT, "不存在的活动ID");
			return;
		}
	...
	protected void reload(Packet packet, ClientSession session){
		...
		try {
			...
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_RELOAD_CLIENT, "重载服务器信息失败");
		}
	}
	...
	protected void gotoMap(Packet packet, ClientSession session){
		...
		if(p==null){
			ErrorHandler.sendAdminErrorMessage(session, serial,
					OpCode.ADMIN_GOTO_CLIENT, "指定的玩家没有在线");
			return;
		}
		int[] point = {mapId,x,y};
		if (mapId == -1) {
		...
		try {
			...
			ErrorHandler.sendAdminErrorMessage(session, serial,
					OpCode.ADMIN_GOTO_CLIENT, MessageFormat.format("无法传送{0}", e.getMessage()));
			return;
		}
	}
	...
	protected void chat(Packet packet, ClientSession session) {
		...
		Admin admin = (Admin) session.getClient();
		if (admin != null) {
			ChatMessage cm = new ChatMessage(channel, -admin.id, -1 , "系统", message, null);
	...
	protected void adminLogin(Packet packet,ClientSession session){
		...
		if(session.getClient()==null){
			...
			if(!service.getPassword().equals(password)){
				ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_LOGIN_CLIENT, "登陆密码错误");
			...
			try {
				...
				ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_LOGIN_CLIENT, "没有可登录ID");
			}
