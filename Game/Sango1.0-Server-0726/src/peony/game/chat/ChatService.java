...
public class ChatService implements Service, ServiceEventListener, Runnable {
	...
	//中国二级区划
	protected static final String[] NATIVES = { "辽宁", "吉林", "黑龙江", "北京", "天津",
			"河北", "山西", "内蒙古", "湖北", "湖南", "河南", "江西", "上海", "江苏", "浙江", "山东",
			"安徽", "广东", "广西", "海南", "福建", "四川", "重庆", "贵州", "云南", "西藏", "陕西",
			"甘肃", "新疆", "青海", "宁夏", "香港", "澳门", "台湾",
...
//			cm.message = s.replace('\n', ' ');
	...
	public void sendSystemMessage(int ch, String name, String message) {
		ChatMessage cm = new ChatMessage(ch, -1, -1, "系统", message, null);
		addChatMessage(cm);
	}
	
	public void sendPrivateMessage(int destId,String message){
		ChatMessage cm = new ChatMessage(ChatOption.PRIVATE, -1, -1, "系统",
				destId, message, null);
		addChatMessage(cm);
	}
	
	public void sendFactionSystemMessage(int faction, String message) {
		ChatMessage cm = new ChatMessage(ChatOption.FACTION, -1, faction, "系统", message, null);
		cm.destId = faction;
		addChatMessage(cm);
	}
	
	public void sendAreaSystemMessage(String message,int destId){
		ChatMessage cm = new ChatMessage(ChatOption.AREA,-1,-1,"系统",message,null);
		cm.destId = destId;
		addChatMessage(cm);
	}
	
	public void sendFactionShout(int faction,String message,int color,int duration){
		ChatMessage cm = new ChatMessage(ChatOption.FACTION_SHOUT, -1, faction, "系统", message, null);
		cm.destId = faction;
		cm.shoutColor = color;
		cm.shoutDuration = duration;
		addChatMessage(cm);
	...
	public void sendWorldMessage(String message) {
		ChatMessage cm = new ChatMessage(ChatOption.WORLD,-1,-1,"系统",message,null);
		addChatMessage(cm);
	}
	
	public void sendGuildSystemMessage(String message, int destId){
		ChatMessage cm = new ChatMessage(ChatOption.GUILD,-1,-1,"系统",message,null);
		cm.destId = destId;
		addChatMessage(cm);
	}
