...
//				accountDepotDAO.newEntity(accountDepot);
	...
	public void getItemFromBagToDepot(Player p, ClientSession session, int serial, 
		...
		if (p != null) {
			...
			if (count > currentCount) {
				ErrorHandler.sendErrorMessage(session, serial, OpCode.DEPOT_GETFROMBAG_CLIENT, "物品数量不足!");
				return;
			}
	...
	public void getItemFromDepotToBag(Player p, ClientSession session, int serial,  
		...
		if (p != null) {
			...
			if (count > currentCount) {
				ErrorHandler.sendErrorMessage(session, serial, OpCode.DEPOT_GETFROMDEPOT_CIENT, "物品数量不足!");
				return;
			}
...
//		}
	...
	public void loadAccountDepot(int accountId){
		...
		if(depot==null){
			List<AccountDepot> list = accountDepotDAO.getAccountDepots(accountId);
						...
						try {
							...
							for(int gridId=0;gridId<otherDepot.depot.getGrids().size();gridId++){
								if(otherDepot.depot.getGrid(gridId)!=null && otherDepot.depot.getGrid(gridId).item!=null 
									...
									if(!b){
										log.info("[ACCDEPOTMISS]"+LogUtil.getGameItemString(item, count)+"ACC["+accountId+"]");
									}else{
										log.info("[ACCDEPOT]"+LogUtil.getGameItemString(item, count)+"ACC["+accountId+"]");
									}
								}
							}
