...
//			player.mapCell.broadcast(player, player.getMovePacket(GameObject.MOVE_DETAIL), false, false);
	...
	protected void horseUpSkillLevel(Packet packet, ClientSession session){
		...
		if(player!=null){
			...
			if(horse==null){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.HORSE_UP_SKILL_CLIENT, "没有找到相应的坐骑");
				return;
			}
			int oldSkillIndex = -1;
			Skill oldSkill = null;
			try {
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.HORSE_UP_SKILL_CLIENT, "没有找到相应的技能");
				return;
			}
			if(oldSkillIndex>-1 && oldSkill!=null){
				...
				if(oldSkillLevel<2){
					...
					if(newSkill==null){
						ErrorHandler.sendErrorMessage(session, serial, OpCode.HORSE_UP_SKILL_CLIENT, "已达到最高等级");
						return;
					}
					...
					if(player.bag.removeGameItemIngoreInstanceId(ItemUtil.HORSE_UP_SKILLLEVEL_ITEM, 1, tx, false)!=null){
						...
						ErrorHandler.sendErrorMessage(session, serial, OpCode.HORSE_UP_SKILL_CLIENT, MessageFormat.format("缺少{0}", itemName));
					}
				}else{
					ErrorHandler.sendErrorMessage(session, serial, OpCode.HORSE_UP_SKILL_CLIENT, "已达到最高等级");
					return;
	...
	protected void removeAllJewelRequest(Packet packet,ClientSession session) throws Exception{
		...
		if(player != null){
			...
			Object[] obj = ItemUtil.findPlayerEquipment(player, equItemID,
					equInstanceID);
			if (obj == null) {
				ErrorHandler.sendErrorMessage(session,  serial,OpCode.REMOVE_ALLJEWEL_RESOURCE_CLIENT, "找不到此装备");
				return;
			}
			GameItem gi = (GameItem) obj[0];
			if (gi.template.equipment == null) {
				ErrorHandler.sendErrorMessage(session,  serial,OpCode.REMOVE_ALLJEWEL_RESOURCE_CLIENT, "拆除对象必须是装备");
				return;
			}
			if (gi.object == null) {
			...
			if (!(gi.object instanceof ItemEnhance)) {
				ErrorHandler.sendErrorMessage(session, serial, OpCode.REMOVE_ALLJEWEL_RESOURCE_CLIENT,  "装备数据错误");
				return;
			}
			ItemEnhance itemEn = (ItemEnhance)gi.object;
			if(itemEn.getJewelCount() == 0){
				ErrorHandler.sendErrorMessage(session, serial ,OpCode.REMOVE_ALLJEWEL_RESOURCE_CLIENT, "此装备上没有镶嵌宝石");
				return;
			}
			Packet pt = null;
			if(SyncExecutorService.autoMergerAndRemoveFlag==1)
	...
	protected void mergeJewelRequest(Packet packet, ClientSession session){
		...
		if(player!=null){
			...
			try {
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.MERGE_JEWEL_REQUEST_CLIENT, "宝石数据错误");
				return;
			}
			if(jewelLevel<=2){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.MERGE_JEWEL_REQUEST_CLIENT, "达到3级才能自动合成");
				return;
			}
			if(SyncExecutorService.autoMergerAndRemoveFlag==1){
	...
	protected void attendantAddLoyal(Packet packet, ClientSession session){
		...
		if(player!=null){
			...
			if(attendant!=null){
				...
				if(item==null){
					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_ADDLOYAL_SERVER, MessageFormat.format("没有{0}", item0.name));
					return;
				}
...
//				}
				...
				if(oldValue>0 && value<=0){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_EQUIP_CLIENT, "目前随从忠诚度已满，请下次提升");
					return;
				}
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_EQUIP_CLIENT, "没有发现你所要寻找的指定随从");
			}
		}
	}
	...
	protected void attendantDelete(Packet packet, ClientSession session){
		...
		if(player!=null){
			int serial = packet.getInt();
			int instanceId = packet.getInt();
			if(player.attendant!=null && player.attendant.instanceId==instanceId){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_DELETE_CLIENT, "正在跟随状态的随从，无法放生");
				return;
			}
			...
			if(attendant!=null){
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_DELETE_CLIENT, "没有发现你所要寻找的指定随从");
			}
		}
	}
		...
		if(player!=null){
			...
			if(attendant!=null){
				if(!attendant.canLight(skillIndex)){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_LIGHTSKILL_CLIENT, "您的随从天赋稍逊，无法激活此技能位");
					return;
				}
				...
				if(item!=null){
					...
					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_LIGHTSKILL_CLIENT, MessageFormat.format("没有{0}", item0.name));
					return;
				}
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_LIGHTSKILL_CLIENT, "没有发现你所要寻找的指定随从");
			}
		}
	}
		...
		if(player!=null){
			...
			if(attendant!=null){
				if(StringUtil.isValidText(name)!=IStringValidator.OK || StringUtil.hasBadWord(name)){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_RENAME_CLIENT, "您所起的名字有损您随从的威名，请换一个吧");
					return;
				}
				...
				if(item==null){
					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_RENAME_CLIENT, MessageFormat.format("没有{0}无法进行改名", item0.name));
					return;
				}
				tx.commit();
				attendant.name = name;
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_RENAME_CLIENT, "没有找到指定随从");
			}
		}
	}
	...
	protected void attendantUnequip(Packet packet, ClientSession session){
		...
		if(player!=null){
			...
			if(attendant!=null){
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_UNEQUIP_CLIENT, "没有发现你所要寻找的指定随从");
			}
		}
	}
	...
	protected void cancelAttendantFollow(Packet packet, ClientSession session){
		...
		if(player!=null){
			...
			if(attendant!=null){
				...
				if(attendant==null){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_CANCELFOLLOW_CLIENT, "您的随从已经是收回状态");
					return;
				}
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_CANCELFOLLOW_CLIENT, "没有发现你所要寻找的指定随从");
			}
		}
	}
	...
	protected void attendantFollow(Packet packet, ClientSession session){
		...
		if(player!=null){
			...
			if(attendant!=null){
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.ATTENDANT_FOLLOW_CLIENT, "没有发现你所要寻找的指定随从");
			}
		}
	}
		...
		if(player!=null){
			...
			if(type==0){
				...
				if(association!=null){
					...
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ASSOCIATION_EXECISE_CLIENT, "您没有自己的血盟");
				}
			}else if(type==4){
				//主动退出
				...
				try {
					...
					if(association!=null && association.getMember(player.id)!=null && association.getMember(player.id).duty==AssociationMember.DUTY_LEADER){
						ErrorHandler.sendErrorMessage(session, serial, OpCode.ASSOCIATION_EXECISE_CLIENT, "您是盟主不能退出");
						return;
					}
					...
					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ASSOCIATION_EXECISE_CLIENT, "没有足够的声望");
				}
			}
		}
	}
		...
		if(player!=null){
			...
			try {
				...
				if(item!=null){
					...
					throw new AssociationException("你没有血盟改名符");
				}
			} catch (AssociationException e) {
				tx.rollback();
		...
		if(p!=null){
			...
			if(tarPlayer!=null){
				...
				if(p.getVMap()!=null && tarPlayer.getVMap()!=null && p.getVMap().getId()==tarPlayer.getVMap().getId()){
					...
					ErrorHandler.sendErrorMessage(session, serial, OpCode.REPORT_CLIENT, "非法举报");
				}
			}else{
				ErrorHandler.sendErrorMessage(session, serial, OpCode.REPORT_CLIENT, "玩家不在线");
			}
		}
	}
	...
	protected void duelGetTitle(Packet packet, ClientSession session){
		...
		if(p!=null){
			...
			if(index==-1){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.DUEL_GETTITLE_CLIENT, "只有驸马才能领取");
				return;
			}else{
				int itemId = service.itemIds[index];
				if(p.pool.getInt("DUELTILE",0)==1){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.DUEL_GETTITLE_CLIENT, "不能重复领取");
					return;
				}
				...
				try {
					...
					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial, OpCode.DUEL_GETTITLE_CLIENT, "您的背包已满");
				}
			}
		}
	}
	...
	protected void duelScore(Packet packet, ClientSession session){
		...
		if(p!=null){
			...
			for(int i=0;i<4;i++){
				...
				if(id==0){
					pt.putString(duelService.npcNames[i]+":暂无");
				}else{
...
//					player.equip(equ.template.id, equ.instanceId, -1);
	...
	protected void weddingSendGift(Packet packet,ClientSession session){
		...
		if(p!=null){
			...
			if(p.id!=instance.man.id){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_GIFT_CLINT, "只有新郎才可以发放红包");
				return;
			}
	...
	protected void weddingOpen(Packet packet, ClientSession session) throws VMapException{
		...
		if(p!=null){
			...
			if(mateId<=0){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_OPEN_CLIENT, "请先结婚");
				return;
			}
			Player mate = ObjectAccessor.getPlayer(mateId);
			if(mate==null){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_OPEN_CLIENT, "配偶不在线");
				return;
			}
			if(p.sex!=0){
			...
			if(p.money < (2*level - 1)*600000){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_OPEN_CLIENT, "您的金钱不足");
				return;
			}
			if(p.map.id != mate.map.id){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_OPEN_CLIENT, "新郎新娘在同一场景才可以开启婚礼");
				return;
			}
			if(p.party == null || mate.party == null || !p.party.contains(mate.id)){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_OPEN_CLIENT, "新郎新娘组队后才可以开启婚礼");
				return;
			}
	...
	protected void autoAddHole(Packet packet, ClientSession session){
		...
		if(p!=null){
			...
			if(os==null){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.AUTO_ADDHOLE_CLIENT, "没有找到装备");
				return;
			}else{
				GameItem item = (GameItem) os[0];
				if(!item.template.isEquipment()){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.AUTO_ADDHOLE_CLIENT, "打孔目标必须是一件装备");
					return;
				}
				if (item.object == null) {
				...
				if (currentHoles >= maxHoles) {
					ErrorHandler.sendErrorMessage(session, serial, OpCode.AUTO_ADDHOLE_CLIENT,"已达到最大孔数");
					return;
				}
				if(maxHoles-currentHoles < wantHole || wantHole>maxHoles){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.AUTO_ADDHOLE_CLIENT, "请选择正确的孔数");
					return;
				}
	...
	protected void extendBag(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if (player.bag.isSizeLocked()) {
				ErrorHandler.sendErrorMessage(session, serial, OpCode.EXTEND_BAG_CLIENT, "操作正在进行中");
				return;
			}
		...
		if(p!=null){
			...
			if(tong!=null && applyService.isWinner(tong.id)){
				if(p.map.getId()!=applyService.getWinnerMapId(tong.id)){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.TONG_BATTLE_GETEXP_CLIENT, "不属于此城池胜利军团,不能领取经验");
					return;
				}
				if(p.pool.getInt(Player.PROPERTY_TONGBATTLE_EXPDAY,0)==Time.day){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.TONG_BATTLE_GETEXP_CLIENT, "每天只可领取一次");
					return;
				}
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.TONG_BATTLE_GETEXP_CLIENT, "不属于胜利军团,不能领取经验");
			}
		}
	}
			...
			if(tong!=null){
				...
				try {
					if(!tong.getChairmanName().equals(p.name)){
						ErrorHandler.sendErrorMessage(session, serial, OpCode.TONG_BATTLE_ABANDON_CLIENT, "您没有权限");
						return;
					}
	...
	protected void tongBattleMakeTax(Packet packet, ClientSession session){
		...
		if(p!=null){
			...
			if(tong!=null && tong.getChairmanName().equals(p.name) && service.isWinner(tong.id)){
				...
				if(tax>10 || tax<5){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.TONG_BATTLE_MAKETAX_CLIENT, "税率必须在5-10之间");
				}else{
	...
	protected void tongBattleWinnerInfo(Packet packet, ClientSession session){
		...
		if(p!=null){
			...
			pt.putInt(serial);
			if(tong!=null){
				pt.putString(winTongName+"\t军团都督:<cFF0000>"+tong.getChairmanName()+"</c>\t军团等级:<cFC00FF>"+tong.level+"</c>\t城战纪录:<cFC00FF>"+tong.pool.getInt(Tong.PROPERTY_TONGBATTLE_WIN, 0)
						+"</c>\t军团人数:<cFF0000>"+tong.members.size()+"</c>\t军团资金:<cFF0000>"+tong.money+"</c>\t竞价价格:<cFC00FF>"+appService.getApplyByTongId(tong.id).money+"</c>");
			}else{
				pt.putString("暂无");
			}
			p.send(pt);
	...
	protected void worldShout(Packet packet, ClientSession session){
		...
		if(p!=null){
			...
			if(p.level<20){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WORLD_SHOUT_CLIENT, "不满20级不能世界喊话");
				return;
			}
		...
		if(p!=null){
			...
			if(!gameItem.isBound()){
				...
				if(item!=null){
					...
					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial, OpCode.HORSE_EQUIPMENT_UNBIND_CLIENT
							, MessageFormat.format("您没有{0},不能解绑", ObjectAccessor.getItemTemplate(491).name));
				}
			}
		}
	}
		...
		if(p!=null){
			...
			try {
				...
				if(p.horseBag.maxSize==15){
					...
					.sendPrivateMessage(p.id, "恭喜您拥有了15个坐骑栏。");
				}else{
					Server.server.getServiceRegistry().getChatService()
					.sendPrivateMessage(p.id, 
							MessageFormat.format("您成功扩大了坐骑栏，能容纳更多坐骑，目前拥有{0}个还有{1}个可扩展。", 
				...
				tx.rollback();
				ErrorHandler.sendErrorMessage(session, serial, OpCode.HORSEBAG_EXTEND_CLIENT, "金钱不足");
			}
		}
	}
		...
		if(player != null){
			...
			if (os != null) {
				...
				if(item.template.equipment.markCharCount <= 0){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.EQUIPEMENT_MARK_CLIENT, "此装备不能刻字");
					return;
				}
				if(item.template.equipment.markCharCount < msg.length()){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.EQUIPEMENT_MARK_CLIENT, MessageFormat.format("此装备最多可刻{0}个字", item.template.equipment.markCharCount));
					return;					
				}
				ItemEnhance ie = (ItemEnhance)item.object;
				if(ie != null){
					if(ie.getMarkString().length()>0){
						ErrorHandler.sendErrorMessage(session, serial, OpCode.EQUIPEMENT_MARK_CLIENT, "装备只能刻字一次");
						return;	
					}
				}
	...
	protected void nationConvoy(Packet packet, ClientSession session){
		...
		if(player != null){
			if(player.isKing()==1){
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.NATION_CONVOY_CLIENT, "只有国公能够发起国家押镖");
			}
		}
	}
					...
					if (rate >= 5 && rate <= 15) {
						...
						if(oldRate>nation.taxRate){
							Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(player.faction, 
									MessageFormat.format("国公{0}施仁政，下调税率，可去国家信息菜单查询", player.name));
						...
						ErrorHandler.sendErrorMessage(session, serial, OpCode.KING_TAXRATE_CLIENT, "税率必须在5~15之间");
					}
				} else {
					ErrorHandler.sendErrorMessage(session, serial, OpCode.KING_TAXRATE_CLIENT, "尊敬的国公,当日税率只可调整一次");
				}
			}else{
				ErrorHandler.sendErrorMessage(session, serial, OpCode.KING_TAXRATE_CLIENT, "只有国公才能调整税率");
			}
		}
	}
	...
	protected void autoNaturalEnhance(Packet packet, ClientSession session){
		...
		if (player != null) {
			...
			if (os != null) {
				...
				if (!item.template.isEquipment()) {
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.AUTO_NATURALENHANCE_CLIENT, "非装备不允许进行资质鉴定。");
					return;
				}
				if (!item.template.equipment.canJudgePotential) {
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.AUTO_NATURALENHANCE_CLIENT, "此装备不允许进行资质鉴定。");
					return;
				}
				if(player.bag.getGameItemCount(ItemUtil.ITEM_NATURAL_ENHANCE)==0){
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.AUTO_NATURALENHANCE_CLIENT, "没有资质鉴定符。");
					return;
				}
...
//	 * 数据							byte[](如果是否存在数据字段是0，这个字段不存在)
	...
	protected void get_File(Packet packet, ClientSession session){
		...
		if(fd == null){
			ErrorHandler.sendErrorMessage(session, serial, OpCode.GET_FILE_CLIENT, "没找到指定文件");
		}else{
	...
	protected void agentHorseList(Packet packet, ClientSession session){
		...
		if(exps < needExp){
			String msg = MessageFormat.format("<cff0000>提示：</c>您有<cff0000>{0}</c>离线经验,剩余经验不够兑换", exps);
...
//		}
		...
		String msg;
		if(count < count1){
			msg = MessageFormat.format("您有<cff0000>{0}</c>个一盒酥，消耗后可兑换<cff0000>{1}</c>离线经验", count,canChangeExp);
		} else {
		    msg = MessageFormat.format("您有<cff0000>{0}</c>个一盒酥，兑换所有可兑换的离线经验需消耗一盒酥<cff0000>{1}</c>个", count,count1);
		}
...
//		}
	...
	protected void nationRel(Packet packet, ClientSession session) {
		...
		if (p != null) {
			...
			if (service.isKing(p)) {
				...
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.NATION_REL_CLIENT, "不是国公，没有权限");
			}
		}
	}
		...
		if (p != null) {
			...
			try {
				...
				if (type == NationRel.TYPE_WAR_REQUEST) {
					if (!service.isKing(p)) {
						throw new NationDeclareException("您不是国公，没有权利使用此项!");
					}
					...
					Server.server.getServiceRegistry().getChatService()
							.sendFactionSystemMessage(
									p.faction,
									MessageFormat.format("全体国民请注意，我国已向{0}宣战", 
					...
					Server.server
							...
							.sendFactionSystemMessage(
									p.faction,
									MessageFormat.format("全体国民请注意，我国已向{0}发起反击，请全体国民速到国都烽火台处参战，反击战将在21:30分开始", 
											GameObject.getFactionName(faction)));
					Server.server
							.getServiceRegistry()
							.getChatService()
							.sendFactionSystemMessage(
									faction,
									MessageFormat.format("{0}对我国发起反击，请全体国民速到国都烽火台处参战,反击战将在21:30分开始", 
											GameObject.getFactionName(p.faction)));
				} else {
	...
	protected void nationDeclareAccept(Packet packet, ClientSession session) {
		...
		if (p != null) {
			...
			if (service.isKing(p)) {
				...
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.NATION_DECLARE_ACCEPT_CLIENT, "不是国公，没有此权限");
			}
		}
	}
	...
	protected void instanceClear(Packet packet, ClientSession session) {
		...
		if (player != null) {
			if (player.getVMap().instance == null) {
				...
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.INSTANCE_CLEAR_CLIENT, "当前状态不能清除副本进度");
			}
		}
	}
			...
			if (Server.server.getServiceRegistry().getChinaJoyService()
				...
				try {
					...
					Server.server.getServiceRegistry().getMailService()
							.sendSystemMailAsync(p.id, "系统", "短信投票奖励", "", 0, item,
									1, "CHINAJOY");
					log.info("[CHINAJOY]" + LogUtil.getPlayerLogString(p)
							+ "MAIL");
				}
				...
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.CHINAJOY_GIFT_CLIENT, "每个帐号一天只能投两次票");
			}
		}
	}
	...
	protected void depotArrange(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if (player.depot.arrange()) {
				...
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.DEPOT_ARRANGE_SERVER, "现在暂时不能整理仓库，请稍后再试");
			}
		}
	}
		...
		if (player != null) {
			...
			if (os != null) {
				...
				if (!item.template.equipment.canJudgeStar) {
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.NATURAL_ENHANCE_CLIENT, "此装备不允许进行星级鉴定。");
					return;
				}
				if (item.object != null) {
					ItemEnhance ie = (ItemEnhance) item.object;
					if (ie != null && ie.getStar() != 0) {
						ErrorHandler.sendErrorMessage(session, serial,
								OpCode.NATURAL_ENHANCE_CLIENT, "此装备已经鉴定过星级。");
						return;
					}
				}
				int fuId = -1;
				...
				if (fuId != -1) {
					if (player.bag.removeGameItem(fuId, -1, 1, tx, true) == null) {
						...
						ErrorHandler.sendErrorMessage(session, serial,
								...
								MessageFormat.format("你没有{0}，请准备好后再来进行鉴定。", it.name));
						return;
					}
				}
				...
				try {
					...
					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.NATURAL_ENHANCE_CLIENT, "你的金钱数额不足，无法鉴定星级。");
				}
			} else {
				ErrorHandler.sendErrorMessage(session, serial,
	...
	protected void naturalEnhance(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if (os != null) {
				...
				if (!item.template.isEquipment()) {
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.NATURAL_ENHANCE_CLIENT, "非装备不允许进行资质鉴定。");
					return;
				}
				if (!item.template.equipment.canJudgePotential) {
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.NATURAL_ENHANCE_CLIENT, "此装备不允许进行资质鉴定。");
					return;
				}
				...
				try {
					...
					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.NATURAL_ENHANCE_CLIENT, "你的金钱数额不足，无法资质鉴定。");
				}
			} else {
				ErrorHandler.sendErrorMessage(session, serial,
	...
	protected void naturalEnhanceMoney(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if (os != null) {
				...
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.NATURAL_ENHANCE_MONEY_CLIENT, "没有找到指定物品");
			}
		}
	}
	...
	protected void startEnhanceMoney(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if (os != null) {
				...
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.START_ENHANCE_MONEY_CLIENT, "没有找到指定物品");
			}
		}
	}
	...
	protected void bagArrange(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if (player.bag.arrange()) {
				...
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.BAG_ARRANGE_CLIENT, "现在暂时不能整理背包，请稍后再试");
			}
		}
	}
	...
	protected void ishopList(Packet packet, ClientSession session) {
		...
		if (Server.server.REVISION_TYPE_CMCC.equals(Server.server.revision)|| Server.server.REVISION_TYPE_TEL.equals(Server.server.revision)) {
			...
			pt.putString("186,139,140,141,142,187");
		}else if(Server.server.REVISION_TYPE_JAPAN.equals(Server.server.revision)){
			pt.putString("184,180,181,182,183,188");
		}else {
		...
		if (player != null) {
			...
			if (player.bag.removeGameItem(HorseUtil.CHANGE_SKILL_ITEMID, -1, 1,
				...
				if (skill != null) {
					...
					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.HORSE_CHANGE_SKILL_CLIENT, "没找到指定技能");
					return;
				}
			} else {
				tx.rollback();
		...
		if(p!=null){
			...
			if (item!= null) {
				if(((h.lockSkillId>>skillIndex)&1)==1){
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.HORSE_CHANGE_SKILL_CLIENT, "不能重复锁定");
					return;
				}
				tx.commit();
	...
	protected void unlockHorseSkill(Packet packet, ClientSession session){
		...
		if(p!=null){
			...
			try {
				...
				tx.rollback();
				ErrorHandler.sendErrorMessage(session, serial, OpCode.UNLOCK_HORSESKILL_CLIENT, "您的金钱不足");
				return;
			}
													...
													if (marriageService.isFriend(
														...
														if (p.money >= MarriageService.MARRY_MONEY) {
															if (p.map.id == annotherPerson.map.id) {
																...
																message = "您的意中人不在同一地图，不能结婚";
															}
														} else {
															message = "需要" + MarriageService.MARRY_MONEY + "注册费，您的金钱不足";
														}
													} else if (marriageService
															.isFriend(p.id,
																	annotherPersonId) == 0) {
														message = "对方未加您为好友,不能申请结婚";
													} else if (marriageService
															.isFriend(p.id,
																	annotherPersonId) == 1) {
														message = "未加对方为好友,不能申请结婚";
													} else if (marriageService
															.isFriend(p.id,
																	annotherPersonId) == -1) {
														message = "必须互相加为好友才能申请结婚";
													}
												} else {
													message = "对方等级达到10级后才能和您结婚";
												}
											} else {
												message = "结婚等级必须达到10级";
											}
										...
										if(p.honor>=500){
											if(annotherPerson.money >= 20000){
												if(annotherPerson.honor >= 500){
													...
													message = "离婚需要扣除500声望，对方的声望不足";
											    }
											} else {
												message = "离婚需要扣除20000金钱，对方的金钱不足";
											}
										} else {
											message = "离婚需要扣除500声望，您的声望不足";
										}
								    } else {
									    message = "离婚需要金钱20000，您的金钱不足";
								}
							}else {
									message = "双方不在同场景下不能解除婚约";
								}
							} else {
								message = "对方不是您的伴侣";
							}
		...
		if (player != null) {
			...
			if (player.titles.getCurrentTitle() != null) {
				if (player.titles.getCurrentTitle().type != Title.TYPE_OFFICIAL) {
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.TITLE_SALARY_CLIENT, "只有朝廷称号才能领取俸禄");
					return;
				}
				if (player.titles.lastSalaryTime == 0
					...
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.TITLE_SALARY_CLIENT, "同一天只能领取一次俸禄");
				}
			} else {
				ErrorHandler.sendErrorMessage(session, serial,
	...
	protected void horsePacket(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if (h == null) {
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.HORSE_PACK_CLIENT, "未找到指定坐骑");
				return;
			}
			if (h.agentHorse==1){
				ErrorHandler.sendErrorMessage(session, serial,
			...
			if (!h.equs.isEmpty()) {
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.HORSE_PACK_CLIENT, "指定坐骑还存在装备");
				return;
			}
			...
			try {
				...
				tx.rollback();
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.HORSE_PACK_CLIENT, "没有足够的包格");
				return;
			}
		}
	...
	protected void horseThrow(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if(isAgent){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.HORSE_THROW_CLIENT, "不能放生代理饲养的马");
				return;
			}
	...
	protected void horseChangeName(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if (StringUtil.isValidText(name) != IStringValidator.OK) {
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.HORSE_CHANGENAME_CLIENT, "坐骑的名字中存在非法字符");
				return;
			}
	...
	protected void titleBuy(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if (player.titles.hasTitle(titleId)) {
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.TITLE_BUY_CLIENT, "已经有此称号");
				return;
			}
			...
			if (player.level < t.level) {
				ErrorHandler.sendErrorMessage(session, serial,
						...
						MessageFormat.format("此称号需要{0}级", t.level));
				return;
			}
			if (t.price > 0) {
				...
				try {
					...
					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.TITLE_BUY_CLIENT, MessageFormat.format("此称号声望需要{0}点声望", t.price));
					return;
				}
			}
			player.addTitle(t);
			...
			if (gridId == -1) {
				if (ex.decMoney(player.id, count)) {
					...
					if(target!=null && targetState==1){
						target.message(-1, "对方交易内容已变动", -1, -1);
					}
					// 记录日志
					LogUtil.logExchangeRemoveItem(ex.id, player, count, null, 0);
				} else {
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.EXCHANGE_REMOVEITEM_CLIENT, "金钱数量错误");
				}
			} else {
		...
		if (player != null && ex != null && ex.state == Exchange.STATE_STARTED) {
			...
			if (itemId == -1) { // 金钱
				...
				try {
					...
					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.EXCHANGE_ADDITEM_CLIENT, "没有足够金钱");
				}
			} else {
				PlayerTransaction tx = player.newTransaction("EXC");
				try {
					...
					if (item.isBound()) {
						tx.rollback();
						ErrorHandler.sendErrorMessage(session, serial,
								OpCode.EXCHANGE_ADDITEM_CLIENT, "指定物品已经被绑定");
						return;
					}
					if (ex.addGameItem(player.id, item, count) == false) {
						tx.rollback();
						ErrorHandler.sendErrorMessage(session, serial,
								OpCode.EXCHANGE_ADDITEM_CLIENT, "交易栏已满");
						return;
					}
					tx.commit();
					...
					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.EXCHANGE_ADDITEM_CLIENT, "没有足够物品");
				}
			}
		}
	}
	...
	protected void exchangeInvit(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if (player.exchange != null) {
				player.exchangeRefuse(serial, "已经有交易存在");
				return;
			}
			Player p = ObjectAccessor.getPlayer(targetId);
			if (p == null) {
				player.exchangeRefuse(serial, "交易目标不存在");
				return;
			}
			...
			if (p.faction != player.faction) {
				player.exchangeRefuse(serial, "交易目标无效");
				return;
			}
			if (p.exchange != null) {
				player.exchangeRefuse(serial, "目标正忙");
				return;
			}
			...
			if(!p.isAllowTrade()){
				player.exchangeRefuse(serial, "对方开启了免打扰模式");
				return;
			}
		...
		if (player != null) {
			...
			if (exites == null || exites.length == 0) {
				log.error("[FINDPATHERROR]FROM[" + player.map.id + "," + player.x
						...
						+ y * 8 + "]");
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.FINDPATH_CLIENT, "当前没有有效路径");
			} else {
...
//					// }
				...
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.CHANGE_CLASS_CLIENT, "不能修改国家");
			}
	...
	protected void changeclass(Packet packet, ClientSession session) {
		...
		if (clazz >= 0 && clazz <= 3) {
			...
			ErrorHandler.sendErrorMessage(session, serial,
					OpCode.CHANGE_CLASS_CLIENT, "不能修改职业");
		}
	}
	...
	protected void changeSex(Packet packet, ClientSession session) {
		...
		if (sex == 0 || sex == 1) {
			...
			if (player != null && player.level < 6) {
				...
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.CHANGE_SEX_CLIENT, "不能修改性别");
			}
		}
	...
	protected void changeName(Packet packet, ClientSession session) {
		...
		if (player != null) {
			if (StringUtil.isValidName(name) != 0){
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.CHANGE_NAME_CLIENT, "错误的角色名");
			...
			if (player.pkInfo == null) {
				...
				if (player.getMoney() >= wager) {
					...
					if (target != null
						...
						if (rel != null && rel.blackList.exists(player.id)) {
							Packet pt = new Packet(OpCode.PK_REFUSE_SERVER);
							pt.putString("对方拒绝你的切磋请求");
							player.send(pt);
							return;
						}
						if(!target.isAllowPK()){
							Packet pt = new Packet(OpCode.PK_REFUSE_SERVER);
							pt.putString("对方开启了免打扰模式");
							player.send(pt);
							return;
						}
...
//					: new byte[0]);
	...
	public void itemInfo(Packet packet, ClientSession session) {
		...
		if (instanceId == -1) {
			...
			if (template != null) {
				...
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.ITEMINFO_CLIENT, "没找到指定物品");
			}
		} else {
			GameItem item = ObjectAccessor
	...
	public void mailPost(Packet packet, ClientSession session) {
		...
		if (player != null) {
			if (destName.equals(player.name)) {
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.MAIL_POST_CLIENT, "不能发送邮件给自己");
				return;
			}
			Server.server.getServiceRegistry().getDbService().schedule(
	...
	public void partyTransLeader(Packet packet, ClientSession session) {
		...
		if (player != null) {
			if (player.party != null
				...
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.PARTY_TRANSFER_LEADER_CLIENT, "不是队长，无需转让");
			}
		}
	}
	...
	public void partyJoinAnswer(Packet packet, ClientSession session){
		...
		if (player != null) {
			...
			if(target == null){
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.PARTY_JOIN_ANSWER_CLIENT, "目标已消失");
				return;
			}
			if(Server.server.getServiceRegistry().getPartyService().getParty(joinerId)!=null){
				ErrorHandler.sendErrorMessage(session, -1, OpCode.PARTY_INVIT_CLIENT, "对方已经加入别的队伍");
				return;
			}
			if(answer == 1){//拒绝此人加入队伍
				Packet pt = new Packet(OpCode.PARTY_INVIT_REJECT_SERVER);
				pt.putString(player.name);
				pt.putString("队长拒绝您入队");
				target.send(pt);
				return;
			}
			PartyService partyService = Server.server.getServiceRegistry()
			...
			if (player.party != null) {
				if (!player.party.isFull()
					...
					ErrorHandler.sendErrorMessage(session, -1,
							OpCode.PARTY_INVIT_CLIENT, "队伍已满");
					return;
				}
			} else {
	...
	public void partyInvit(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if (player.party != null
					&& player.party.leader.getId() != player.id) {
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.PARTY_INVIT_CLIENT, "不是队长，不能发送组队邀请");
				return;
			}
			Player target = (Player) ObjectAccessor.getPlayer(targetId);
			if (target == null) {
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.PARTY_INVIT_CLIENT, "目标不存在");
				return;
			}
			if (target.faction != player.faction) {
				ErrorHandler.sendErrorMessage(session, -1,
			...
			if (target.party != null) {
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.PARTY_INVIT_CLIENT, "目标已经在组队状态");
				return;
			}
			if(!target.isAllowParty()){
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.PARTY_INVIT_CLIENT, "对方开启了免打扰模式");
				return;
			}
			...
			if (rel.blackList.exists(player.id)) {
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.PARTY_INVIT_CLIENT, "目标拒绝你的邀请");
				return;
			}
			PartyService partyService = Server.server.getServiceRegistry()
					.getPartyService();
	...
	public void partyJoin(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if (player.party != null && player.party.getCount() > 0) {
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.PARTY_JOIN_CLIENT, "您已经在组队状态");
				return;
			}
			...
			if(target == null){
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.PARTY_JOIN_CLIENT, "目标不存在");
				return;
			}
			if (target.faction != player.faction) {
				ErrorHandler.sendErrorMessage(session, -1,
			...
			if(target.party == null || target.party.getCount() == 0){
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.PARTY_JOIN_CLIENT, "目标未组队");
				return;
			}
			...
			if (rel.blackList.exists(player.id)) {
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.PARTY_JOIN_CLIENT, "目标拒绝你的邀请");
				return;
			}
			
			if(target.party.leader == null){
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.PARTY_JOIN_CLIENT, "没找到队长");
				return;
			}
...
//			PartyRequest request = partyService.newPartyRequest(player, target);
	...
	public void chat(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if((!(session instanceof AdminDispatchClientSession)) && message.length()>30){
				player.message(-1, "简短的语言更明白，请缩短输入内容", -1, -1);
				return;
			}
			ChatService chatService = Server.server.getServiceRegistry()
					.getChatService();
			...
			if (cm != null) {
				...
				if (ch == ChatOption.NATIVE) {
					if (player.chatOptions.nativeName == null
						...
						cm = new ChatMessage(ChatOption.PRIVATE, -1, -1, "系统",
								player.id, "当前没有设置家乡，不能使用此频道", null);
					} else {
					...
					if (tm != null) {
						if (tm.forbid) {
							cm = new ChatMessage(ChatOption.PRIVATE, -1, -1,
									"系统", player.id, "你已经被禁言，不能使用此频道", null);
						} else
							cm.destId = tm.tongID;
					} else {
						cm = new ChatMessage(ChatOption.PRIVATE, -1, -1, "系统",
								player.id, "当前没有加入军团，不能使用此频道", null);
					}
							...
							if (player.bag
								...
								if (player.level > 10) {
									player
											.message(
													-1,
													"免费国家聊条数已用完，拥有国家喊话符才能继续在国家频道聊天，每条聊天扣除1个国家喊话符",
													-1, -1);
									return;
								} else {
									player.message(-1, "11级之前不能使用国家聊。", -1, -1);
									return;
								}
							}
					...
					if (player.bag.removeGameItem(ItemUtil.ITEM_WORLD_CHAT, -1,
						...
						tx.rollback();
						player.message(-1, "拥有世界喊话符才能在世界频道聊天，每条聊天扣除1个世界喊话符",
								-1, -1);
						return;
					}
	...
	public void gatherStart(Packet packet, ClientSession session) {
		...
		if (p != null && p.isAlive()) {
			...
			if (gu != null && gu.isAlive()) {
				...
				if(gu.level>p.level){
					p.message(-1, MessageFormat.format("您的级别不能采集此材料，{0}级后可采集", gu.level), -1, -1);
					p.cancelGather(4);
					return;
				}
	...
	public void versionCompare(Packet packet, ClientSession session) {
		...
		if (version == null) {
			ErrorHandler.sendErrorMessage(session, -1,
					OpCode.VERSION_COMPARE_CLIENT, "该版本暂未开放");
			return;
		} else {
...
//		}
	...
	public void skillDesc(Packet packet, ClientSession session) {
		...
		if (skill != null && player != null) {
			...
			ErrorHandler.sendErrorMessage(session, serial,
					OpCode.SKILL_DESC_CLIENT, "没有此技能");
		}
	}
	...
	public void actorCreate(Packet packet, ClientSession session) {
		...
		Player player = PlayerUtil.createPlayer(name, sex, clazz, randomFaction,
	...
	public void itemDesc(Packet packet, ClientSession session) {
		...
		if (instanceId == -1) {
			...
			if (p != null) {
				...
				if (item == null) {
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.ITEM_DESC_CLIENT, "没找到对应的物品");
				} else {
					Packet pt = new Packet(OpCode.ITEM_DESC_SERVER);
					pt.putInt(serial);
					pt.putInt(itemId);
		...
		if (player != null) {
			...
			if(gameItem != null && gameItem.template.isEquipment()){
				...
				if(itemEnhance != null && itemEnhance.getJewelCount() > 0 && itemEnhance.findJewelByLevel(ItemEnhance.CANT_REMOVE_LEVEL)){
					ErrorHandler.sendErrorMessage(session, -1,
							OpCode.REMOVEITEM_CLIENT, "镶嵌4级(含)以上宝石的物品无法丢弃,请妥善保管");
					return;
				}
			}
			...
			if ((grid = player.bag.removeGridGameItem(gridId, itemId,
				...
				if (itemEffect != null
					...
					tx.rollback();
					ErrorHandler.sendErrorMessage(session, -1,
							OpCode.REMOVEITEM_CLIENT, "物品不能丢弃");
				} else {
					tx.commit();
				}
			} else {
	...
	public void splitItem(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			try {
				...
				ErrorHandler.sendErrorMessage(session, -1,
						OpCode.SPLITITEM_CLIENT, "背包已满，不能拆分");
			} catch (Exception e) {
				ErrorHandler.sendErrorMessage(session, -1,
			...
			if (exit != null && exit.exitType < GameMapExit.TYPE_INTERNAL) {
				...
				try {
					...
					if (player.level < c.minLevel || player.level > c.maxLevel) {
						throw new VMapException("你没有达到可以通过这里的级别");
					}
