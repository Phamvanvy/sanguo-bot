...
public class DropTestCall extends ClientSessionAsyncCall {
	...
	public void callFinish() throws Exception {
		if (map.size() > Short.MAX_VALUE) {
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_GM_DROP_TEST, MessageFormat.format("掉落物品太多，超过协议范围：{0}", map.size()));
			return;
		}
	...
	public static String qualityName[] = {"白色装备","绿色装备","蓝色装备","紫色装备","黄色装备"};
	public int qualityCount[] = new int[qualityName.length];
	public void run() {
		GroupDrop d = ObjectAccessor.getGroupDrop(dropGroupId);
		if (d == null) {
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_GM_DROP_TEST, MessageFormat.format("没有相应掉落组：{0}", dropGroupId));
			return;
		}
