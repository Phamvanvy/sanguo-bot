...
public class AdminRemoveItemCall extends ClientSessionAsyncCall {
	...
	public void callFinish() throws Exception {
		if (p != null) {
			...
			ErrorHandler.sendAdminErrorMessage(session, serial,
					OpCode.ADMIN_REMOVE_ITEM_CLIENT, "找不到指定物品");
		} else {
			ErrorHandler.sendAdminErrorMessage(session, serial,
					OpCode.ADMIN_REMOVE_ITEM_CLIENT, "找不到指定角色");
		}
	}
	...
	public void run() {
		...
		if(actor==null){
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_REMOVE_ITEM_CLIENT, "找不到指定角色");
			return;
		}
