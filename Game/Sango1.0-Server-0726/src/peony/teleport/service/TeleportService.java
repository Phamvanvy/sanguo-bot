...
//							}else 
							if(buyRequirement.type == Shop.TYPE_MONEY && buyRequirement.toString() != null){
								strb.append(MessageFormat.format("金钱{0}、", buyRequirement.amount));
								...
								strb.append(MessageFormat.format("军衔{0}、", rank.title));
							}else if(buyRequirement.type == Shop.TYPE_LEVEL){
								strb.append(MessageFormat.format("级别{0}、", buyRequirement.amount));
								...
								strb.append(MessageFormat.format("i币{0}、", buyRequirement.amount));
						...
						for(BuyRequirement buyRequirement : requirements){
							...
							if (buyRequirement.type == Shop.TYPE_MONEY && buyRequirement.deduct) {
								try {
									...
									ErrorHandler.sendErrorMessage(session, serial, OpCode.TELEPORT_CLIENT,"您的金钱不足");
									return;
								}
