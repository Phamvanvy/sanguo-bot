		...
		try {
			...
			if(type==1){
				...
				if(!Server.server.getServiceRegistry().getAccountBindingService().validIdcard(accountId, idcard)){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNTBINDING_CLIENT, "身份证号错误");
					return;
				}
				sms = Server.server.getServiceRegistry().getAccountBindingService().bindPhone(accountId, value1);
				retMessage = "绑定手机申请提交成功，请确认您的手机可以正常发送短信。";
			}else if(type==2){
				if(!validIdcard(value1)){
					ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ACCOUNTBINDING_CLIENT, "身份证格式错误");
					return;
				}
...
//				retMessage = "恭喜您绑定成功，您的奖励已通过飞鸽发送。请稍候查收。";
				...
				if(!validQna(value1, value2)){
					ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ACCOUNTBINDING_CLIENT, "问题和答案字数必须在3到30之间");
					return;
				}
				if(!Server.server.getServiceRegistry().getAccountBindingService().validIdcard(accountId, idcard)){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNTBINDING_CLIENT, "身份证号错误");
...
//				Server.server.getServiceRegistry().getMailService().sendSystemMail(playerId, "系统", "绑定问题奖励", "", 0, ObjectAccessor.createGameItem(REWARDS[3][0]), REWARDS[3][1], "ACCBIND");
				...
				if(!validMail(value1)){
					ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ACCOUNTBINDING_CLIENT, "邮件格式错误");
					return;
				}
				if(!Server.server.getServiceRegistry().getAccountBindingService().validIdcard(accountId, idcard)){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNTBINDING_CLIENT, "身份证号错误");
				...
				retMessage = "现在需要你去邮箱里确认哦，才可以最终完成绑定，赶紧去吧。";
			}
			if (type == 1) {
