			...
			timer.schedule(new TimerTask(){
				public void run() {
					...
					Server.server.getServiceRegistry().getChatService().sendWorldMessage(
							"大汉公主殿下要举行比武招驸马了，现在正在司隶中心城内火热报名中，英雄您还不快去！");
				}
...
//		removeStatue();
			...
			if(player.getVMap().instance instanceof DuelInstance){
				...
				if(instance!=null){
					...
					try {
						...
						tx.rollback();
						Server.server.getServiceRegistry().getMailService().sendSystemMailAsync(player.id, "系统", "比武奖励", "", 0, item, 1, "DUEL");
					}
				}
			}
			if(instance!=null){
	...
	public void signUp(Player p) throws DuelException{
		if(p!=null){
			...
			try {
				...
				tx.rollback();
				throw new DuelException("勇士您的金钱不足，无法报名参加比武招亲");
			}
		}
	}
	...
	protected void processWinner(int playerId){
		...
		if(p!=null){
			Server.server.getServiceRegistry().getChatService().sendPrivateMessage(playerId, "勇士，恭喜您已获得比武招亲的头名，公主正在等您");
