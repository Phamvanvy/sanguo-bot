...
//	protected List<Actor> topWeekRanks = null;
	...
	public String[] questName = {"情牵七夕","情牵七夕","情牵七夕","中秋佳节","感恩戴义","感恩戴义","感恩戴义","为国效力","摸金校尉","大破天龙阵"};
	...
	public void playerFinishQuest(Player p,int questId,int branch){
		if(p!=null){
			...
			if(achs!=null && achs.size()>0){
				for(int i=0;i<size1+ach2.size();i++){
					Achievement ach;
					if (i < size1) {
						ach = achs.get(i);
					} else {
						ach = ach2.get(i - size1);
					}
...
//		}.start();
						...
						if(items != ""){
							...
							for(int m=0;m<strs.length;m++){
							    try{
							        ...
							        log.error(type + " , " + achievementType + " , " + achieveId + items, e);
							    }
							}
						}
	...
	public void detailAchieveList(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			if(person != null){
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.PERSONAL_ACHIEVEMENT_DETAIL_CLIENT, "玩家不在线");
			}
		}
	}
	...
	public void achievementList(Packet packet, ClientSession session) {
		...
		if (player != null) {
			...
			pt.putInt(serial);
			pt.putShort(size);
			String[] catagoryNames = { "常规", "荣誉", "生活" ,"成长历程","穿金戴银","经商有道","刺杀国君","伯乐之才","富甲天下","打造","装备","称号","卡片","击杀怪物","任务","其它","击杀统计","被击杀统计"};
			Player person = (Player)ObjectAccessor.getPlayer(personId);
			if(person == null){
				person =  Server.server.getServiceRegistry().getFameService().getStatue(personId);
			}
			if(person == null){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.PERSONAL_ACHIEVEMENT_CLIENT, "玩家已下线");
				return;
			}
	...
	public void getReward(Player p,int type,int achieveId) throws Exception{
		if(p!=null){
			...
			if(achs!=null && achs.size()>0){
				...
				if(ach != null){
					if(ach.acomplish == 0){
						throw new Exception("对不起，您未完成该成就");
					}
					if(p.pool.getString(getPropertyOfGetGift(ach.achievementName))!=""){
						throw new Exception("对不起，您已经领取过奖励了");
					}
					...
					if(itemIds != null && itemIds.size()>0){
						for(int i = 0;i<itemIds.size();i+=2){
							...
							try {
								...
								String content = MessageFormat.format(
										"恭喜你完成{0}成就时获得{1}个{2}的奖励。", ach.achievementName,count,
										item.template.name);
								Server.server.getServiceRegistry().getMailService()
								.sendSystemMail(p.id, "系统", "完成成就奖励", content, 0,
						...
						throw new Exception("对不起，完成该成就没有奖励");
					}
				}
			}
		}
	...
	public String setMessage(Player p,Achievement achieve,boolean worldMessage){
		if(p!=null){
			if(achieve != null){
				...
				.sendAreaSystemMessage(MessageFormat.format("恭喜英雄<cff0000>{0}</c>达成<cff0000>{1}</c>成就", p.name,achieve.achievementName),p.map.id);
				if(worldMessage){
					Server.server.getServiceRegistry().getChatService()
					.sendWorldMessage(MessageFormat.format("恭喜英雄<cff0000>{0}</c>达成<cff0000>{1}</c>成就", p.name,achieve.achievementName));
				}
			}
		}
		return "";
					...
					case 2:
						   ...
						   case 2://生活其它
								   for(Achievement a:achs){
									   ...
									   }else if(a.param1.equals("beauty")){
