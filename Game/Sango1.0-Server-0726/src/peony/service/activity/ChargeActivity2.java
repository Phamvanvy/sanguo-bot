	...
	protected void sendGift(Player p, int itemId, int count){
		if(p!=null){
			...
			try {
				...
				tx.commit();
				String content = MessageFormat.format("恭喜你，{0}，今天充值达到{1}（奖励已放入背包，请查收", p.name, GIFT_MONEY);
				p.message(-1, content, -1, -1);
			} catch (Exception e) {
				tx.rollback();
				String content = MessageFormat.format("恭喜你，{0}，今天充值达到{1}", p.name, GIFT_MONEY);
				Server.server.getServiceRegistry().getMailService().sendSystemMail(
					p.id, "系统", "充值奖励！", content, 0, item, 1, "ACTV");
				content = MessageFormat.format("恭喜你，{0}，今天充值达到{1}（您的背包已满，奖励已经以飞鸽附件的方式发送到你的飞鸽里了，请及时查收。）", p.name, GIFT_MONEY);
				p.message(-1, content, -1, -1);
			}
		}
	}
