...
public class AccountLoginCall extends AccountAsyncCall{
    ...
    public AccountLoginCall(ClientSession session, String name, String password, String model, String version, int serial, String realPhone, int playerID, String IMEI){
        super(session);
        this.name = name;
    ...
    public void run(){
    	if(ObjectAccessor.players.size()>=Server.server.maxPlayer){
            ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNT_LOGIN_CLIENT, "已经到达最大登录数量");
        ...
        if(v == null || v.status == Version.STATUS_OBSOLETE){
            ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNT_LOGIN_CLIENT, v == null? "版本号错误": v.message);
        ...
        if (Server.server.REVISION_TYPE_TW.equals(Server.server.revision)) {
            ...
            try {
                ...
                if(code==200){
                    ...
                    ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNT_LOGIN_CLIENT, "账号不存在");
                    log.info("[ACCLOGIN]NAME[" + name + "]HANGAMEID[password]MODEL[" + model + "]VERSION[" + version + "]PHONE[" + this.realPhone + "]IP[" + session.getClientIP() + "]FAIL[HANGAME]");
                    return;
                }
            } catch (Exception ex) {
	        ...
	        try {
	            ...
	            if(code==200){
                    ...
                    if(result.contains("1")){
                        ...
                        AccountRegMessage regMsg = new AccountRegMessage(password, "", "", -1, model, Server.server.gameCode, version, realPhone, "123");
