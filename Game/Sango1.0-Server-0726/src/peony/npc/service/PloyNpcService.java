			...
			if(creature.battleContribList!=null){
				...
				if(owner!=null){
					...
					if(owner instanceof GameObjectRef){
						...
						chatService.sendWorldMessage(MessageFormat.format("{0}英勇难当,成功击杀了反贼{1}。", p.name, name));
					}else{
						Party party = (Party)owner;
						if(party.leader!=null){
							chatService.sendWorldMessage(MessageFormat.format("{0}带领小队英勇难当,成功击杀了反贼{1}。", party.leader.getName(), name));
						}
					}
				}
			}
		...
		if(u1.id==3473573 || u1.id==3342503 ||u1.id==3670249 ||u1.id==3539122){
			...
			if(creature.battleContribList!=null){
				...
				if(owner!=null){
					...
					if(owner instanceof GameObjectRef){
						Player p = (Player)ObjectAccessor.getGameObject((GameObjectRef)owner);
						chatService.sendFactionSystemMessage(u2.faction, MessageFormat.format("我国的{0}英勇击杀了{1}的雪人，获得了12生肖兔年感情符。", p.name, map));
					}else{
						Party party = (Party)owner;
						if(party.leader!=null){
							chatService.sendFactionSystemMessage(u2.faction, MessageFormat.format("我国的{0}带领小队英勇击杀了{1}的雪人，获得了12生肖兔年感情符。", party.leader.getName(), map));
						}
					}
				}
			}
