...
public class IGBC extends MIDlet implements CommandListener , Runnable
{
  ...
  public void perform()
  {
      ...
      {                
        ...
        Loging = new Loging();
        ///測試用透過IGBKernel來更新tsi取得opid等資訊
        String igbxx = IGBKernel.kernelSyncTsi("ping","1111",null,null);
        System.out.println("get tsi="+igbxx);
        String[] igbyy = Utilities.stringTokenize(igbxx, ','); 
//      /取出igbuc local欄位資訊
        System.out.println("igbus loacl ="+IGBKernel.getUC_Local());        
        System.out.println("igbus getUc_Local_Data_Country ="+IGBKernel.getUc_Local_Data_Country());
        System.out.println("igbus getUc_Local_Data_OP ="+IGBKernel.getUc_Local_Data_OP());
        System.out.println("igbus getUc_Local_Data_Sid ="+IGBKernel.getUc_Local_Data_Sid());
        System.out.println("igbus getUc_Local_Data_Mode ="+IGBKernel.getUc_Local_Data_Mode());
        //執行月租認證
        System.out.println("authorize = "+IGBKernel.getAuthorizeResult(igbyy[4], igbyy[5], IGBKernel.getUc_Local_Data_Sid(), null));
        //進入付費金流區塊/////////////////
        gamelist = new Vector();
        ...
        {
        	//模擬登入行為取得tsi
        	System.out.println("模擬登入行為取得tsi");
        	Loging.synctsi();
        	System.out.println("更新完tsi資訊");
        	gamelist = billing.getStoreType( usd.getsyncData_tsi());
        }
        ...
        {        	
        	...
        	String testresport = billing.sendPayChoose( tsi, Integer.parseInt(paylist[0][0]) );
