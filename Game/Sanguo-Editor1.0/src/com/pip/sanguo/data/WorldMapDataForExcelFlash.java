...
public class WorldMapDataForExcelFlash {
    /**世界地图ID*/
    ...
    /**地图上城池的宽度*/
    private final static byte WW           =        8;
    /**地图上城池的高度*/
    private final static byte HH           =        9;
    /**通行方向UP*/
    private final static byte UP           =        10;
    /**通行方向DOWN*/
    private final static byte DOWN         =        11;
    /**通行方向LEFT*/
    private final static byte LEFT         =        12;
    /**通行方向RIGHT*/
    private final static byte RIGHT        =        13;
    /**共享地图标识*/
    private final static byte SHARE_ID     =        14;
    /**mapID键步长*/
    private final static byte MAPID_STEP   = 2;
    /**worldID键步长*/
    private final static byte WORLDID_STEP = 14;
    ...
    public static void save(String filePath){
        try{
            ...
            for(int i=0; i<cells.length; i++){
                ...
                keyWorldID.addElement(cells[i][WW]);
                keyWorldID.addElement(cells[i][HH]);
            ...
            System.out.println("worldID键数据量(14倍)：" + keyWorldID.size());
            System.out.println("worldID键数据保存完毕");

            FileOutputStream fos = new FileOutputStream(new File(new File(filePath).getParent(), "worldMapInfo.data"));
            ...
            for(int i=0; i<keyWorldID.size(); i+=WORLDID_STEP){
                ...
                dos.writeInt(Integer.parseInt(keyWorldID.elementAt(i + WW)));
                dos.writeInt(Integer.parseInt(keyWorldID.elementAt(i + HH)));
