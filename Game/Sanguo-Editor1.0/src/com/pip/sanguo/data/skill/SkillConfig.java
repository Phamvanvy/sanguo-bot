...
public class SkillConfig extends DataObject {
    ...
    public void generateJava(PrintWriter out, String packageName, String classPrefix) {
        ...
        out.println("import peony.game.attendant.*;");
        out.println();
        ...
        case 5:
            ...
            break;
        case 6:
            out.println("        clazz = Attendant.SKILLCLASS;");
            break;  
        default:
        ...
        if (type != TYPE_PASSIVE && type != TYPE_BUFF) {
            ...
            for (EffectConfig eff : effects.getAllEffects()) {
                ...
                case EffectConfig.DYNAMIC_CHANGE_PHYICAL_AP:
                    ...
                    out.println("        if("+p1+"==1)");
                    out.println("           context.attackPower += context.getSkillParam(this, \"" + p1 + "\",  context.source.strength * "+p2+"/100.0f);");
                    out.println("        if("+p1+"==2)");
                    out.println("           context.attackPower += context.getSkillParam(this, \"" + p1 + "\",  context.source.agility * "+p2+"/100.0f);");
                    out.println("        if("+p1+"==3)");
                    out.println("           context.attackPower += context.getSkillParam(this, \"" + p1 + "\",  context.source.intellect * "+p2+"/100.0f);");
                    out.println("        if("+p1+"==4)");
                    out.println("           context.attackPower += context.getSkillParam(this, \"" + p1 + "\",  context.source.stamina * "+p2+"/100.0f);");
                    break;
            ...
            for (EffectConfig eff : effects.getAllEffects()) {
                ...
                case EffectConfig.ATTENDANTONHIT_ADDBUFF_OWNER:
                    ...
                    out.println("            }");
                    out.println("            if (hit) {");
                    out.println("                if(context.source.type == GameObject.TYPE_ATTENDANT){");
                    out.println("                    Attendant att = (Attendant)context.source;");          
                    out.println("                    if(att.owner!=null){");
                    out.println("                         att.owner.buffs.addBuff(BuffUtil.createBuff(" + p3 + ", "
                                                           + p4 + ", context.source, context.target, context.damage));");
                    out.println("                    }");
                    out.println("                 }");
                    out.println("            }");
                ...
                case EffectConfig.RELIVE_TARGET:
                    ...
                    out.println("        }");
                    break;
                case EffectConfig.RELIVE_MATE:
                    // Effect_PercentAdd: 百分比float
                    out.println("        if (context.source != context.target) {");
                    out.println("            if (isActive) {");
                    out.println("                if (!context.target.isAlive()) {");
                    out.println("                    // TODO: 向死亡的人发送一个复活请求");
                    out.println("                    if (context.target.type == GameObject.TYPE_PLAYER&&((Player)context.source).getMateID()==context.target.id) {");
                    out.println("                        ReliveOption option = new ReliveOption(");
                    out.println("                                ReliveOption.SKILL_ACTIVE, getName(), 14,");
                    out.println("                               context.target.map.id, context.source.x,");
                    out.println("                               context.source.y);");
