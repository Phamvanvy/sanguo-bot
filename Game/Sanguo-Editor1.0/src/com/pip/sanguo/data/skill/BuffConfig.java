...
public class BuffConfig extends DataObject {
    ...
    public boolean hadWeight;//是否有权重
    public String weights;//对应效果的权重数值表
    ...
    public void load(Element elem) {
        ...
        hadWeight="true".equals(elem.getAttributeValue("hadweight"));
        weights = elem.getAttributeValue("weights");
        try {
    ...
    public Element save() {
        ...
        ret.addAttribute("hadweight", hadWeight ? "true" : "false");
        ret.addAttribute("weights",String.valueOf(weights));
    ...
    public void generateJava(PrintWriter out, String packageName, String classPrefix) {
        ...
        out.println("import peony.game.itemenhance.ItemEnhance;");
        out.println();
        ...
        out.println();
        out.println("    };");
        out.println("    public int buffover_addbuff = 0;");
        out.println("    public int buffover_addbuff_level = 0;");
        ...
        // 如果有恐惧激活BUFF的效果，需要记录是否已经加过BUFF了
        if (effects.findEffect(EffectConfig.FEAR_ADD_BUFF_OWNER) != null) {
            out.println("    boolean fearBuffActive;");
        }
        
        // 如果有麻痹激活BUFF的效果，需要记录是否已经加过BUFF了
        if (effects.findEffect(EffectConfig.PARALYZE_ADD_BUFF_OWNER) != null) {
            out.println("    boolean paralyzeBuffActive;");
        }
        
        // 如果有定身激活BUFF的效果，需要记录是否已经加过BUFF了
        if (effects.findEffect(EffectConfig.STAY_ADD_BUFF_OWNER) != null) {
            out.println("    boolean stayBuffActive;");
        }
        
        // 如果有沉默激活BUFF的效果，需要记录是否已经加过BUFF了
        if (effects.findEffect(EffectConfig.DUMB_ADD_BUFF_OWNER) != null) {
            out.println("    boolean dumbBuffActive;");
        }
        if (effects.findEffect(EffectConfig.ANTI_DAMAGE) != null) {
            out.println("   int lastEffectTime;");
        }
        ...
        // 支持限制场景效果
        EffectConfig limitMapConfig = effects.findEffect(EffectConfig.LIMIT_MAP);
        if (limitMapConfig != null) {
            out.println("    IntHashSet limitMaps = new IntHashSet();");
        }
        
        //支持不生效场景效果
        EffectConfig noteffectMapConfig = effects.findEffect(EffectConfig.NOTEFFECT_MAP);
        if(noteffectMapConfig != null){
            out.println("   IntHashSet noteffectMaps = new IntHashSet();");
        }
        ...
        if (buffType == BUFF_TYPE_STATIC) {
            ...
            if(effects.findEffect(EffectConfig.BUFFOVER_ADDBUFF)!=null){
                EffectConfig eff = effects.findEffect(EffectConfig.BUFFOVER_ADDBUFF);
                String name = EffectConfig.TYPE_PARAMS[eff.getType()][2];
                out.println("        buffover_addbuff = " + name.toUpperCase() + "[lvl];");
                out.println("        buffover_addbuff_level = lvl;");
            }
        } else if (buffType == BUFF_TYPE_EQUIP) {
            if(hadWeight){
                String[] allWeight=weights.split(";");
                Map<Integer, int[]> weightMap=new Hashtable<Integer, int[]>();
                for(String str:allWeight){
                    String[] oneWeight=str.split(",");
                    int effIndex=Integer.parseInt(oneWeight[0]);
                    int weight1=Integer.parseInt(oneWeight[1]);
                    int weight2=Integer.parseInt(oneWeight[2]);
                    weightMap.put(effIndex, new int[]{weight1,weight2});
                }
                String weightsText = "WEIGHTS";
                
                out.println("    private static final float[] "+weightsText+" = {");
                for(int i=0;i<allWeight.length;i++){
                    out.println("        "+allWeight[i]+",");
                }
                out.println();
                out.println("    };");
                
                out.println("    public " + className + "(int lvl, int realWeight) {");
                out.println("        int dmg = 0;");
                out.println("        instanceID = BuffUtil.getNextID();");
                out.println("        level = lvl;");
                out.println("        multiple = 1;");
                out.println("        this.realWeight = realWeight;");
                
                String weight1="weight1";
                String weight2="weight2";
                out.println("        float "+weight1+" = 0;");
                out.println("        float "+weight2+" = 0;");
                int changeParamIndex=0;
                boolean useNewMethod=false;
                for (EffectParamRef pr : effects.getAllParams()) {
                    int effectType=pr.effect.getType();//效果类型
                    String name = getFieldName(pr.effect, pr.index, false);
                    String staticName = getFieldName(pr.effect, pr.index, true);
                    int[] weightsTT=weightMap.get(effectType);
                    
                    if(pr.effect instanceof Effect_ChangeParam){
                        if(pr.effect.getParam(pr.index) instanceof ParamIndicator[]){
                            int buffId=((ParamIndicator[])pr.effect.getParam(pr.index))[0].id;
                            int type11=((ParamIndicator[])pr.effect.getParam(pr.index))[0].type;
                            int type2=((ParamIndicator[])pr.effect.getParam(pr.index))[0].paramIndex;
                            if(buffId==6&&type11==0&&type2==6){
                                useNewMethod=true;
                            }
                        }
                        if(weightsTT!=null&&useNewMethod&&changeParamIndex%3==2){
                            useNewMethod=false;
                            if(pr.getParamClass()==Integer.class || pr.getParamClass()==Float.class){
                                String type=(pr.getParamClass()==Integer.class?"(int)":"");
                                out.println("        for(int i=0;i<"+weightsText+".length;i+=3){");
                                out.println("           if("+effectType+" == "+weightsText+"[i]){");
                                out.println("               "+weight1+" = "+weightsText+"[i+1]"+";");
                                out.println("               "+weight2+" = "+weightsText+"[i+2]"+";");
                                out.println("           }");
                                out.println("        }");
                                out.println("        if("+weight2+">0 && realWeight>="+weight2+"){");
                                out.println("           " + name + " = "+ type + staticName + "[level] * (1f+"+weight1+"*(Math.min(700, realWeight)/100/7f-1f"+"));");
                                out.println("        }");
                            }else{
                                out.println("        " + name + " = " + staticName + "[level];");
                            }
                            ...
                            if(pr.getParamClass()==Integer.class || pr.getParamClass()==Float.class){
                                ...
                                out.println("        }");
                                out.println("        if("+weight2+">0 && realWeight>="+weight2+"){");
                                out.println("           " + name + " = "+ type + staticName + "[level] * (1f+"+weight1+"*(realWeight/700f-1f"+"));");
                                out.println("        }");
                            }else{
                                out.println("        " + name + " = " + staticName + "[level];");
                            }
                        ...
                        changeParamIndex++;
                    }else if(pr.effect instanceof Effect_AddBuff){
                        if(weightsTT!=null&&pr.index==3){
                            if(pr.getParamClass()==Integer.class){
                                out.println("        "+name+"=realWeight/100;");
                            }else if(pr.getParamClass()==Float.class){
                                out.println("        "+name+"=realWeight/100;");
                            }else{
                                out.println("        " + name + " = " + staticName + "[level];");
                            }
                        ...
                        if(weightsTT!=null){
                            if(pr.getParamClass()==Integer.class){
                                ...
                                out.println("        }");
                                out.println("        if("+weight2+">0 && realWeight>="+weight2+"){");
                                out.println("           " + name + " = (int)(" + staticName + "[level] * (1f+"+weight1+"*(realWeight/700f-1f"+")));");
                                out.println("        }");
                            }else if(pr.getParamClass()==Float.class){
                                out.println("        for(int i=0;i<"+weightsText+".length;i+=3){");
                                out.println("           if("+effectType+" == "+weightsText+"[i]){");
                                ...
                                out.println("        }");
                                out.println("        if("+weight2+">0 && realWeight>="+weight2+"){");
                                out.println("           " + name + " = " + staticName + "[level] * (1f+"+weight1+"*(realWeight/700f-1f"+"));");
                                out.println("        }");
                            }else{
                                out.println("        " + name + " = " + staticName + "[level];");
                            }
            ...
            if(effects.findEffect(EffectConfig.ADD_FEARDEBUFF) != null){
                out.println("        if(owner!=null){");
                out.println("           Player player = ObjectAccessor.getPlayer(owner.id);");
                out.println("           if(player!=null)");
                out.println("               player.buffs.addBuff(new FearDebuff(player, endTime-Time.currTime<=0?0:endTime-Time.currTime));");
                out.println("        }");
            }
            if(effects.findEffect(EffectConfig.ADD_PARALYZEDEBUFF) != null){
                out.println("        if(owner!=null){");
                out.println("           Player player = ObjectAccessor.getPlayer(owner.id);");
                out.println("           if(player!=null)");
                out.println("               player.buffs.addBuff(new ParalyzeDebuff(player, endTime-Time.currTime<=0?0:endTime-Time.currTime));");
                out.println("        }");
            }
            if(effects.findEffect(EffectConfig.ADD_STAYDEBUFF) != null){
                out.println("        if(owner!=null){");
                out.println("           Player player = ObjectAccessor.getPlayer(owner.id);");
                out.println("           if(player!=null)");
                out.println("               player.buffs.addBuff(new StayDebuff(player, endTime-Time.currTime<=0?0:endTime-Time.currTime));");
                out.println("        }");
            }
            if(effects.findEffect(EffectConfig.ADD_DUMBDEBUFF) != null){
                out.println("        if(owner!=null){");
                out.println("           Player player = ObjectAccessor.getPlayer(owner.id);");
                out.println("           if(player!=null)");
                out.println("               player.buffs.addBuff(new DumbDebuff(player, endTime-Time.currTime<=0?0:endTime-Time.currTime));");
                out.println("        }");
            }
            out.println("        multiple = 1;");
            for (EffectParamRef pr : effects.getAllParams()) {
                ...
                String staticName = getFieldName(pr.effect, pr.index, true);
                out.println("        " + name + " = " + staticName + "[level];");
                if((id>=633&&id<=644 || id==648)&&name.equals("change_param_value_2")){
                    continue;
                }
                if (pr.getParamClass() == Integer.class || pr.getParamClass() == Float.class) {
        ...
        List<EffectConfig> hots = effects.findEffects(new int[] { EffectConfig.DOT, EffectConfig.HOT, EffectConfig.MPDOT, 
                EffectConfig.MPHOT, EffectConfig.ADD_HP_ENHANCE, EffectConfig.ADD_MP_ENHANCE });
        if (hots.size() != 0) {
            ...
            while (itor.hasNext()) {
                ...
                } else if (hot.getType() == EffectConfig.ADD_HP_ENHANCE) {
                    out.println("        maxhppercent = " + varNameT + " / 100.0f;");
                } else if (hot.getType() == EffectConfig.ADD_MP_ENHANCE) {
                    out.println("        maxmppercent = " + varNameT + " / 100.0f;");
                }
            }
        }
        ...
        if (shield != null) {
            ...
            out.println("        if(src!=null){");
            out.println("           mp_shield_total += src.spellpower * MP_SHIELD_ADDPERCENT[level] / 100.0f;");
            out.println("           remainAbsorb += src.spellpower * MP_SHIELD_ADDPERCENT[level] / 100.0f;");
            out.println("        }");
        }
        ...
        // 处理场景限制
        if (limitMapConfig != null) {
            String name = getFieldName(limitMapConfig, 0, false);
            out.println("        String[] secs = " + name + ".split(\",\");");
            out.println("        for (String s : secs) {");
            out.println("            limitMaps.add(Integer.parseInt(s));");
            out.println("        }");
        }
        
        // 处理不生效场景限制
        if (noteffectMapConfig != null) {
            String name = getFieldName(noteffectMapConfig, 0, false);
            out.println("        String[] secs = " + name + ".split(\",\");");
            out.println("        for (String s : secs) {");
            out.println("            noteffectMaps.add(Integer.parseInt(s));");
            out.println("        }");
        }
        ...
        if(id==695||id==696||id==697||id==698){
                ...
                for (int i = 0; i < secs.length; i++) {
                    if ((i & 1) == 0) {
                        ...
                        if (type == 0) {
                            ...
                            paramStrings.add("CommonUtil.formatPercent(Math.round(" + varRef + " * multiple))");
                        } else if (type == 2) {
        ...
        // getMergeStrategy()
        out.println("    public int getMergeStrategy() {");
        out.println("        return " + this.mergeStrategy + ";");
        out.println("    }");
        out.println();
        ...
        if (buffType != BUFF_TYPE_DYNAMIC) {
            if(hadWeight){
                ...
                for (EffectParamRef pr : effects.getAllParams()) {
                    ...
                    String name = getFieldName(pr.effect, pr.index, false);
                    String staticName = getFieldName(pr.effect, pr.index, true);
                    if(pr.effect instanceof Effect_AddBuff){
                        if(weightsTT!=null&&pr.index==3){
                            if(pr.getParamClass()==Integer.class){
                                out.println("        "+name+"=realWeight/100;");
                            }else if(pr.getParamClass()==Float.class){
        ...
        if (ifs.contains("Updatable")) {
            ...
            if (hots.size() != 0) {
                ...
                out.println("                }");
                out.println("            }");
                EffectConfig maxhpConfig = effects.findEffect(EffectConfig.ADD_HP_ENHANCE);
                if (maxhpConfig != null) {
                    out.println("            if (maxhppercent > 0) {");
                    out.println("                Unit targetUnit = (Unit)ObjectAccessor.getGameObject(owner);");
                    out.println("                if (targetUnit != null && targetUnit.isAlive()) {");
                    out.println("                    float addHp = targetUnit.hp + targetUnit.maxhp * maxhppercent;");
                    out.println("                    targetUnit.setHp((int) Math.min(addHp, targetUnit.maxhp), true);");
                    out.println("                }");
                    out.println("            }");
                }
                EffectConfig maxmpConfig = effects.findEffect(EffectConfig.ADD_MP_ENHANCE);
                if (maxmpConfig != null) {
                    out.println("            if (maxmppercent > 0) {");
                    out.println("                Unit targetUnit = (Unit)ObjectAccessor.getGameObject(owner);");
                    out.println("                if (targetUnit != null && targetUnit.isAlive()) {");
                    out.println("                    float addMp = targetUnit.mp + targetUnit.maxmp * maxmppercent;");
                    out.println("                    targetUnit.setMp((int) Math.min(addMp, targetUnit.maxmp), true);");
                    out.println("                }");
                    out.println("            }");
                }
                out.println("        }");
...
//                out.println("        }");
            ...
            if (effects.findEffect(EffectConfig.HP_ACTIVE_BUFF) != null) {
                ...
                out.println("            hpBuffActive = false;");
                ...
                out.println("            hpBuffActive = true;");
                out.println("        }");
                
            }
            if (effects.findEffect(EffectConfig.FEAR_ADD_BUFF_OWNER) != null) {
                EffectConfig eff = effects.findEffect(EffectConfig.FEAR_ADD_BUFF_OWNER);
                String p1 = getFieldName(eff, 0, false);
                String p2 = getFieldName(eff, 1, false);
                String p3 = getFieldName(eff, 2, false);
                ...
                out.println("            return true;");
                out.println("        }");
                out.println("        boolean hit = CommonUtil.hit(RND, (int)" + p1 + ", 100);");
                out.println("        if (fearBuffActive && !targetUnit.isFear()) {");
//                out.println("            targetUnit.buffs.removeBuff((int)" + p3 + ");");
                out.println("            fearBuffActive = false;");
                out.println("        } else if (!fearBuffActive && targetUnit.isFear() && hit) {");
                out.println("            targetUnit.buffs.addBuff(BuffUtil.createBuff((int)" + p3 + ", "
                        + p4 + ", targetUnit, targetUnit, 0));");
                out.println("            fearBuffActive = true;");
                out.println("        }");
                
            }
            if (effects.findEffect(EffectConfig.PARALYZE_ADD_BUFF_OWNER) != null) {
                EffectConfig eff = effects.findEffect(EffectConfig.PARALYZE_ADD_BUFF_OWNER);
                String p1 = getFieldName(eff, 0, false);
                String p2 = getFieldName(eff, 1, false);
                String p3 = getFieldName(eff, 2, false);
                String p4 = getFieldName(eff, 3, false);
                ...
                out.println("        }");
                out.println("        boolean hit = CommonUtil.hit(RND, (int)" + p1 + ", 100);");
                out.println("        if (paralyzeBuffActive && !targetUnit.isParalyze()) {");
//                out.println("            targetUnit.buffs.removeBuff((int)" + p3 + ");");
                out.println("            paralyzeBuffActive = false;");
                out.println("        } else if (!paralyzeBuffActive && targetUnit.isParalyze() && hit) {");
                out.println("            targetUnit.buffs.addBuff(BuffUtil.createBuff((int)" + p3 + ", "
                        + p4 + ", targetUnit, targetUnit, 0));");
                out.println("            paralyzeBuffActive = true;");
                out.println("        }");
                
            }
            if (effects.findEffect(EffectConfig.STAY_ADD_BUFF_OWNER) != null) {
                EffectConfig eff = effects.findEffect(EffectConfig.STAY_ADD_BUFF_OWNER);
                String p1 = getFieldName(eff, 0, false);
                String p2 = getFieldName(eff, 1, false);
                String p3 = getFieldName(eff, 2, false);
                String p4 = getFieldName(eff, 3, false);
                ...
                out.println("        }");
                out.println("        boolean hit = CommonUtil.hit(RND, (int)" + p1 + ", 100);");
                out.println("        if (stayBuffActive && !targetUnit.isStay()) {");
//                out.println("            targetUnit.buffs.removeBuff((int)" + p3 + ");");
                out.println("            stayBuffActive = false;");
                out.println("        } else if (!stayBuffActive && targetUnit.isStay() && hit) {");
                out.println("            targetUnit.buffs.addBuff(BuffUtil.createBuff((int)" + p3 + ", "
                        + p4 + ", targetUnit, targetUnit, 0));");
                out.println("            stayBuffActive = true;");
                out.println("        }");
                
            }
            if (effects.findEffect(EffectConfig.DUMB_ADD_BUFF_OWNER) != null) {
                EffectConfig eff = effects.findEffect(EffectConfig.DUMB_ADD_BUFF_OWNER);
                String p1 = getFieldName(eff, 0, false);
                String p2 = getFieldName(eff, 1, false);
                String p3 = getFieldName(eff, 2, false);
                String p4 = getFieldName(eff, 3, false);
                ...
                out.println("        }");
                out.println("        boolean hit = CommonUtil.hit(RND, (int)" + p1 + ", 100);");
                out.println("        if (dumbBuffActive && !targetUnit.isDumb()) {");
//                out.println("            targetUnit.buffs.removeBuff((int)" + p3 + ");");
                out.println("            dumbBuffActive = false;");
                out.println("        } else if (!dumbBuffActive && targetUnit.isDumb() && hit) {");
                out.println("            targetUnit.buffs.addBuff(BuffUtil.createBuff((int)" + p3 + ", "
                        + p4 + ", targetUnit, targetUnit, 0));");
                out.println("            dumbBuffActive = true;");
                out.println("        }");
        ...
        if (ifs.contains("SkillEnhancer")) {
            ...
            if (limitMapConfig != null) {
                out.println("        if (owner != null) {");
                out.println("            GameObject ownerUnit = ObjectAccessor.getGameObject(owner);");
                out.println("            if (ownerUnit != null && !limitMaps.contains(ownerUnit.map.id)) {");
            ...
            if (noteffectMapConfig != null) {
                out.println("        if (owner != null) {");
                out.println("            GameObject ownerUnit = ObjectAccessor.getGameObject(owner);");
                out.println("            if (ownerUnit != null && noteffectMaps.contains(ownerUnit.map.id)) {");
                out.println("                return cd;");
                out.println("            }");
                out.println("        }");
            }
        ...
        if (ifs.contains("PropertyEnhancer")) {
            ...
            for (EffectConfig eff : effects.getAllEffects()) {
                ...
                case EffectConfig.CHANGE_PHYICAL_AP:
                    ...
                    break;
                case EffectConfig.DYNAMIC_CHANGE_PHYICAL_AP:
                    // Effect_MultiAdd: 数额int，百分比float
                    out.println("        if("+p1+"==1)");
                    out.println("           pc.attackpowerup += multiple * pc.unit.strength * "+p2+"/100.0f;");
                    out.println("        if("+p1+"==2)");
                    out.println("           pc.attackpowerup += multiple * pc.unit.agility * "+p2+"/100.0f;");
                    out.println("        if("+p1+"==3)");
                    out.println("           pc.attackpowerup += multiple * pc.unit.intellect * "+p2+"/100.0f;");
                    out.println("        if("+p1+"==4)");
                    out.println("           pc.attackpowerup += multiple * pc.unit.stamina * "+p2+"/100.0f;");
                    break;
                case EffectConfig.CHANGE_MAGIC_AP:
                    ...
                    break;
                case EffectConfig.DYNAMIC_CHANGE_MAGIC_AP:
                    // Effect_MultiAdd: 数额int，百分比float
                    out.println("        if("+p1+"==1)");
                    out.println("           pc.spellpower += multiple * pc.unit.strength * " + p2 + "/100.0f;");
                    out.println("        if("+p1+"==2)");
                    out.println("           pc.spellpower += multiple * pc.unit.agility * " + p2 + "/100.0f;");
                    out.println("        if("+p1+"==3)");
                    out.println("           pc.spellpower += multiple * pc.unit.intellect * " + p2 + "/100.0f;");
                    out.println("        if("+p1+"==4)");
                    out.println("           pc.spellpower += multiple * pc.unit.stamina * " + p2 + "/100.0f;");
                    break;
                case EffectConfig.CHANGE_LASTTIME:
                    // Effect_ChangeSpecialParam: 数额int，百分比float
                    out.println("        switch (level) {");
                    for (int lvl = 0; lvl < maxLevel; lvl++) {
                        // Effect_ChangeParam: 重复4次（数额int，百分比float）
                        out.println("        case " + (lvl + 1) + ":");
                        for (int i = 0; i < 4; i++) {
                            String p11 = getFieldName(eff, i * 2, false);
                            String p22 = getFieldName(eff, i * 2 + 1, false);
                            if(p11.equals("0"))
                                continue;
                            out.println("        if(pc.unit instanceof Player){");
                            out.println("            Player p = (Player)pc.unit;");
                            out.println("            if(p!=null){");
                            out.println("               p.timeRatio["+p11+"-1] *= (1-"+p22+"/100.0f);");
                            out.println("            }");
                            out.println("        }");
                        }
                        out.println("            break;");
                ...
                case EffectConfig.CHANGE_PHYSICAL_CRIT:
                    ...
                    break;
                case EffectConfig.CHANGE_CRITED:
                    // Effect_PercentAdd：百分比float
                    out.println("        pc.anticrit += multiple * " + p1 + " / 100.0f;");
                    break;
                ...
                case EffectConfig.CHANGE_SPEED:
                    ...
                    out.println("        }");
                    break;
                case EffectConfig.ADD_SPEED:
                    // Effect_PercentAdd：百分比float
                    out.println("        if (" + p1 + " > 0.0f) {");
                    out.println("            pc.fast2(multiple * " + p1 + " / 100.0f);");
                    out.println("        }");
                    break;
                case EffectConfig.ADD_NATUAL_ENHANCE:
                    // Effect_PercentAdd：百分比float
                    out.println("        if (" + p1 + " > 0.0f) {");
                    out.println("            pc.natualEnhance = (multiple * " + p1 + " / 100.0f);");
                    out.println("        }");
                    break;
                case EffectConfig.ADD_JEWEL_ENHANCE:
                    // Effect_PercentAdd：百分比float
                    out.println("        if (" + p1 + " > 0.0f) {");
                    out.println("            pc.jewelEnhance = (multiple * " + p1 + " / 100.0f);");
                    out.println("        }");
                    break;
                case EffectConfig.ADD_PLAYER_JEWELENHANCE:
                    // Effect_PercentAdd：百分比float
                    out.println("        if (" + p1 + " > 0.0f) {");
                    out.println("            pc.playerJewelEnhance += (multiple * " + p1 + " / 100.0f);");
                    out.println("        }");
                    break;
                case EffectConfig.ADD_HORSE_JEWELENHANCE:
                    // Effect_PercentAdd：百分比float
                    out.println("        if (" + p1 + " > 0.0f) {");
                    out.println("            pc.horseJewelEnhance += (multiple * " + p1 + " / 100.0f);");
                    out.println("        }");
                    break;     
                case EffectConfig.ADD_STAR_ENHANCE:
                    // Effect_PercentAdd：百分比float
                    out.println("        if (" + p1 + " > 0.0f) {");
                    out.println("            ItemEnhance.starCoefficient = (multiple * " + p1 + " / 100.0f);");
                    out.println("        }");
                    break;
                ...
                case EffectConfig.CHANGE_EXP_RATE:
                    ...
                    break;
                case EffectConfig.CHANGE_REWARD_RATE:
                    // Effect_PercentAdd: 百分比float
                    out.println("        pc.rewardRation += multiple * " + p1 + " / 100.0f;");
                    break;
                ...
                case EffectConfig.CHANGE_MONEY_RATE:
                    ...
                    break;
                case EffectConfig.CHANGE_PEOPLEEXP_RATE_BY_TONG_SKILL:
                    //Effect_PercentAdd: 百分比float:
                    out.println("        peony.service.tong.TongService ts = Server.server.getServiceRegistry().getTongService();");
                    out.println("        peony.service.tong.TongMember tm = ts.getPlayerInfo(pc.unit.id);");
                    out.println("        if(tm!=null && tm.skills!=null && tm.skills.get(9)!=null){");
                    out.println("           peony.service.tong.TongSkill9 tskill = (peony.service.tong.TongSkill9)tm.skills.get(9);");
                    out.println("           int ration = tskill.getRatios();");
                    out.println("           pc.expRatio += ration / 100.0f;");
                    out.println("         }");
                    break;
                case EffectConfig.CHANGE_HRSEEXP_RATE_BY_TONG_SKILL:
                    //Effect_PercentAdd: 百分比float:
                    out.println("        peony.service.tong.TongService ts = Server.server.getServiceRegistry().getTongService();");
                    out.println("        peony.service.tong.TongMember tm = ts.getPlayerInfo(pc.unit.id);");
                    out.println("        if(tm!=null && tm.skills!=null && tm.skills.get(9)!=null){");
                    out.println("           peony.service.tong.TongSkill9 tskill = (peony.service.tong.TongSkill9)tm.skills.get(9);");
                    out.println("           int ration = tskill.getRatios();");
                    out.println("           pc.horseExpRatio+= ration / 100.0f;");
                    out.println("         }");
                    break;
                }
            }
        ...
        if (ifs.contains("CombatEffect")) {
            ...
            for (EffectConfig eff : effects.getAllEffects()) {
                ...
                case EffectConfig.IGNORE_ARMOR:
                    ...
                    out.println("        }");
                    break;
                case EffectConfig.IGNORE_IGNORE_ARMOR:
                    // Effect_MultiAdd: 数额int，百分比float
                    out.println("        if (context.damageType == CombatContext.DAMAGE_PHYSICAL && context.armorRate<0) {");
                    out.println("            context.armor += multiple * " + p1 + ";");
                    out.println("            context.armorRate *= (multiple * (1-" + p2 + " / 100.0f));");
                    out.println("            context.armorRate = context.armorRate>0 ? 0 : context.armorRate;");
                    out.println("            effectTimes++;");
                    out.println("        }");
                    break;
                case EffectConfig.IGNORE_MAGIC_ARMOR:
                    ...
                    out.println("        }");
                    break;
                case EffectConfig.IGNORE_IIGNORE_MAGIC_ARMOR:
                    // Effect_MultiAdd: 数额int，百分比float
                    out.println("        if (context.damageType == CombatContext.DAMAGE_MAGIC && context.armorRate<0) {");
                    out.println("            context.armor += multiple * " + p1 + ";");
                    out.println("            context.armorRate *= (multiple * (1-" + p2 + " / 100.0f));");
                    out.println("            context.armorRate = context.armorRate>0 ? 0 : context.armorRate;");
                    out.println("            effectTimes++;");
                ...
                case EffectConfig.CHANGE_BATTLE_MAGIC_ARMOR:
                    ...
                    out.println("        }");
                    break;
                case EffectConfig.IGNORE_PHYSICS_IMMUNE:
                    // Effect_PercentAdd: 百分比float                                        
                    out.println("        if (isActive && context.damageType == CombatContext.DAMAGE_PHYSICAL) {");
                    out.println("            float value = context.damage * multiple * ignore_physics_percent / 100.0f;");
                    ...
                    out.println("         }");
                    break;
                case EffectConfig.IGNORE_MAGIC_IMMUNE:
                    // Effect_PercentAdd: 百分比float
                    out.println("        if (isActive && context.damageType == CombatContext.DAMAGE_MAGIC) {");
                    out.println("            float value = context.damage * multiple * ignore_magic_percent / 100.0f;");
                    out.println("            if (value > context.damage) {");
                    out.println("               value = context.damage;");
                    out.println("             }");
                    out.println("             if (context.damage > 0) {");
            ...
            for (EffectConfig eff : effects.getAllEffects()) {
                ...
                String p4 = getFieldName(eff, 3, false);
                switch (eff.getType()) {
                case EffectConfig.ANTI_DAMAGE:
                    out.println("        if ((context.damageType==context.DAMAGE_PHYSICAL || context.damageType==context.DAMAGE_MAGIC)) {");
                    out.println("           if((context.skill==null || context.skill.getGroupId()!=9 && !(context.skill instanceof peony.game.skill.FixedDamageSkill)) && !(context.source instanceof peony.game.attendant.Attendant)){");
                    out.println("               if(context.target!=null && context.target.buffs!=null && context.target.buffs.getBuffByID(getId())!=null){");
                    out.println("                   if(ratio>context.ratio)");
                    out.println("                       context.ratio = ratio;");
                    out.println("                   if(timeDis>0 && (context.timeDis==0 || timeDis<context.timeDis)){");
                    out.println("                       context.timeDis = timeDis;");
                    out.println("                   }");
                    out.println("               }");
                    out.println("           }");
                    out.println("        }");
                ...
                case EffectConfig.DOUBLE_DAMAGE_ON_HIT:
                    ...
                    out.println("        }");
                    break;
                case EffectConfig.DOUBLE_HEAL_ON_HIT:
                    // Effect_PercentAdd: 百分比float
                    out.println("        if (isActive && context.damageType == CombatContext.DAMAGE_HEAL && !context.isAdditive) {");
                    out.println("            int rate = (int)(multiple * 100 * " + p1 + ");");
                    out.println("            if (CommonUtil.hit(RND, rate, 10000)) {");
                    out.println("                context.damage *= 2;");
                    out.println("                context.threat *= 2;");
...
//                    out.println("                context.target.setMp(context.target.mp - needMp, true);");
                    out.println("                context.needMp = needMp;");
                    out.println("            }");
                    out.println("            effectTimes++;");
                    out.println("        }");
                    break;
            ...
            for (EffectConfig eff : effects.getAllEffects()) {
                ...
                case EffectConfig.ADD_MP_ON_HIT:
                    ...
                    out.println("        if (isActive && context.hited() && context.isDamage() && (context.target instanceof Player || context.target instanceof Creature)) {");
                    out.println("            int rate = (int)(multiple * 100 * " + p1 + ");");
                    out.println("            if (CommonUtil.hit(RND, rate, 10000)) {");
...
//                        }
//                    }
                    out.println("            float temp_rate = context.target.buffs.getParamEnhances().enhance(ParamEnhanceSet.TYPE_BUFF_OWNER, this.getId(), \"" + p1 + "\","+p1+");");
                    out.println("            boolean hit = false;");
                    out.println("            if (" + p2 + ".length() == 0) {");
                    out.println("               if ("+p1 + "!=temp_rate) {");
                    out.println("                  hit = CommonUtil.hit(RND, (int)(multiple * 100 * temp_rate), 10000);");
                    out.println("               }else {");
                    out.println("                  hit = CommonUtil.hit(RND, (int)(multiple * 100 * " + p1 + "), 10000);");
                    out.println("               }");
                    out.println("            } else {");
                ...
                case EffectConfig.DUMB_ON_HIT:
                    ...
                    out.println("            "+p1+" = context.target.buffs.getParamEnhances().enhance(ParamEnhanceSet.TYPE_BUFF_OWNER, this.getId(), \"" + p1 + "\","+p1+");");
                    out.println("            boolean hit = false;");
                    out.println("            if (" + p2 + ".length() == 0) {");
                    out.println("                hit = CommonUtil.hit(RND, (int)(multiple * 100 * " + p1 + "), 10000);");
                    out.println("            } else {");
                    ...
                    out.println("                if(!(context.skill instanceof FixedDamageSkill))");
                    out.println("                   effectTimes++;");
                    out.println("            }");
                    out.println("        }");
                    break;
                ...
                case EffectConfig.ADD_DEBUFF_ON_HITED:
                    ...
                    out.println("        }");
                    break;
                case EffectConfig.ATTENDANTONHIT_ADDBUFF_OWNER:
                	 // Effect_AddBuff: 概率float，概率补充变量String，BUFFID int，BUFF级别int
                    out.println("        if (!isActive && context.hited() && context.isAttack()) {");
                    out.println("            boolean hit = false;");
                    out.println("            if (" + p2 + ".length() == 0) {");
                    ...
                    out.println("            }");
                    out.println("            if (hit) {");
                    out.println("                if(context.source.owner!=null){");
                    out.println("                        context.source.owner.buffs.addBuff(BuffUtil.createBuff(" + p3 + ", "
                                                  + p4 + ", context.target, context.source, context.damage));");
                    out.println("                        effectTimes++;");
                    out.println("                }");
                    out.println("            }");
    ...
    private Set<String> getJavaInterface() {
        ...
        for (EffectConfig eff : effects.getAllEffects()) {
            ...
            case EffectConfig.ADD_HP_ENHANCE:
            case EffectConfig.ADD_MP_ENHANCE:
                ret.add("Updatable");
                break;
            case EffectConfig.MP_SHIELD:
            case EffectConfig.VAMPIRE_ON_HIT:
            ...
            case EffectConfig.FEAR_ADD_BUFF_OWNER:
            case EffectConfig.PARALYZE_ADD_BUFF_OWNER:
            case EffectConfig.DUMB_ADD_BUFF_OWNER:
            case EffectConfig.STAY_ADD_BUFF_OWNER:
                ret.add("Updatable");
                break;
            case EffectConfig.CRIT_ACTIVE_BUFF:
            case EffectConfig.CRITED_ACTIVE_BUFF:
