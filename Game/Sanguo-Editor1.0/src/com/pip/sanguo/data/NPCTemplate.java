...
public class NPCTemplate extends DataObject {
    ...
    public void load(Element elem) {
        ...
        if(typeID != 3){
            ...
            isPvp = "1".equals(elem.getAttributeValue("isPvp"));
            ...
            try {
                String cardIdString = elem.getAttributeValue("cardId");
                cardId = Integer.parseInt(cardIdString);
            } catch (Exception e) {
            }
            
            try {
                String cardRatioString = elem.getAttributeValue("cardRatio");
                cardRatio = Integer.parseInt(cardRatioString);
            } catch (Exception e) {
            }
    ...
    public Element save() {
        ...
        if(type != null && type.id == 3){
            ...
            ret.addAttribute("cardId", String.valueOf(cardId));
            ret.addAttribute("cardRatio", String.valueOf(cardRatio));
        }
        else{
            ret.addAttribute("clazz", String.valueOf(clazz));
            ret.addAttribute("powerful", powerful ? "1" : "0");
            ret.addAttribute("isPvp",isPvp ? "1" : "0");
