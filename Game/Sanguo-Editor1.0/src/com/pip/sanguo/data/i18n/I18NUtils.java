...
public class I18NUtils {
            ...
            if (existStrings.containsKey(str)) {
                if (i18nMode) {
                    ...
                    if (ret.contains(" ") && "Quest Variable".equals(source)) {
                        throw new IllegalArgumentException("Ambigous variable name " + str);
                    }
                    return ret;
                }
            }
    ...
    private static void processProject(ProjectData proj, I18NContext context) {
        ...
        // GameArea
        //   GameMapInfo: name
        //   GameMapNPC: name, functionName, searchName,functionScript
        //   GameMapExit: positionVarName
        System.out.println("process GameArea...");
        for (DataObject dobj : proj.getDataListByType(GameArea.class)) {
            ...
            try {
                ...
                System.out.println("Error in GameArea " + dobj.id);
                e.printStackTrace();
            }
        }
        ...
        // HorseType: showName
        changed = false;
        System.out.println("process HorseType...");
        ...
        // Equipment: title
        changed = false;
        System.out.println("process Equipment...");
        for (DataObject dobj : proj.getDataListByType(Equipment.class)) {
            ...
            tmp = context.input(equipment.produceArea, "Equipment");
            if (tmp != null) {
                equipment.produceArea = tmp;
                changed = true;
            }
        }
        if (changed) {
        ...
        // Formula: title, description
        changed = false;
        System.out.println("process Formula...");
        ...
        //   needVarMessage, giveOKMessage, bagFullMessage
        changed = false;
        System.out.println("process GiftGroup...");
        ...
        for (DataObject dobj : proj.getDataListByType(Item.class)) {
            ...
            tmp = context.input(item.useConfirm==null?"":item.useConfirm, "Item");
            if (tmp != null) {
                item.useConfirm = tmp;
                changed = true;
            }
            ...
            tmp = context.input(item.produceArea, "Item");
            if(tmp != null){
                item.produceArea = tmp;
                changed = true;
            }
        }
        if (changed) {
        ...
        // Shop: title
        changed = false;
        System.out.println("process Shop...");
        ...
        // Suite: title
        changed = false;
        System.out.println("process SuiteConfig...");
        ...
        //   Teleport: name
        changed = false;
        System.out.println("process TeleportSet...");
        ...
        //   Rule: message
        changed = false;
        System.out.println("process NPCTemplate...");
        ...
        for (DataObject dobj : proj.getDataListByType(Quest.class)) {
            ...
            try {
                ...
                tmp = context.input(quest.title, "Quest Title(" + quest.id + ")");
                if (tmp != null) {
                    quest.title = tmp;
                    changed = true;
                }
                tmp = context.input(quest.description, "Quest(" + quest.id + ")");
                if (tmp != null) {
                    quest.description = tmp;
                    changed = true;
                }
                tmp = context.input(quest.preDescription, "Quest(" + quest.id + ")");
                if (tmp != null) {
                    quest.preDescription = tmp;
                    changed = true;
                }
                tmp = context.input(quest.postDescription, "Quest(" + quest.id + ")");
                if (tmp != null) {
                    quest.postDescription = tmp;
                    changed = true;
                }
                tmp = context.input(quest.unfinishDescription, "Quest(" + quest.id + ")");
                if (tmp != null) {
                    quest.unfinishDescription = tmp;
                    changed = true;
                }
                ExpressionList exprList = ExpressionList.fromString(quest.finishCondition);
                if (processExpressionList(exprList, context, quest.id)) {
                ...
                for (QuestTarget target : quest.targets) {
                    ...
                    tmp = context.input(target.description, "Quest Target(" + dobj.id + ")");
                    if (tmp != null) {
                        target.description = tmp;
                        changed = true;
                    }
                    tmp = context.input(target.hint, "Quest Target(" + dobj.id + ")");
                    if (tmp != null) {
                        target.hint = tmp;
                        changed = true;
                    }
                ...
                System.out.println("Error in Quest " + dobj.id);
                e.printStackTrace();
            }
        }
        if (changed) {
        ...
        // BuffConfig: title, description
        changed = false;
        System.out.println("process BuffConfig...");
        ...
        // SkillConfig : title, description
        changed = false;
        System.out.println("process SkillConfig...");
        ...
        // 处理所有活动引导
        // DirectoryType: title
        List<DataObject> directoryList = (List<DataObject>)proj.getDataListByType(DirectoryType.class);
        System.out.println("process Directory...");
        for (DataObject dobj : directoryList) {
            DirectoryType directory = (DirectoryType)dobj;
            tmp = context.input(directory.title, "directory");
            if (tmp != null) {
                directory.title = tmp;
                changed = true;
            }
            tmp = context.input(directory.categoryName, "directory");
            if (tmp != null) {
                directory.categoryName = tmp;
                changed = true;
            }
            tmp = context.input(directory.description, "directory");
            if (tmp != null) {
                directory.description = tmp;
                changed = true;
            }
            tmp = context.input(directory.description1, "directory");
            if (tmp != null) {
                directory.description1 = tmp;
                changed = true;
            }
            tmp = context.input(directory.directoyList, "directory");
            if (tmp != null) {
                directory.directoyList = tmp;
                changed = true;
            }
            tmp = context.input(directory.rewardDesc, "directory");
            if (tmp != null) {
                directory.rewardDesc = tmp;
                changed = true;
            }
        }
        if (changed) {
            Element root = new Element("directorys");
            Document doc = new Document(root);
            for (DataObject dobj : directoryList) {
                root.addContent(dobj.save());
            }
            try {
                Utils.saveDOM(doc, new File(proj.baseDir, "directorys/index.xml"));
            } catch (Exception e) {
            }
        }
        ...
        // salary.xml
        try {
            System.out.println("process salary.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "salary.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("salaryType");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("name"), "Salary");
                if (tmp != null) {
                    elem.getAttribute("name").setValue(tmp);
                    changed = true;
                }
                List list1 = elem.getChildren("salary");
                for(int j = 0; j < list1.size(); j++){
                    Element elem1 = (Element)list1.get(j);
                    tmp = context.input(elem1.getAttributeValue("salaryName"), "Salary");
                    if (tmp != null) {
                        elem1.getAttribute("salaryName").setValue(tmp);
                        changed = true;
                    }
                    tmp = context.input(elem1.getAttributeValue("salaryDec"), "Salary");
                    if (tmp != null) {
                        elem1.getAttribute("salaryDec").setValue(tmp);
                        changed = true;
                    }
                }
            }
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "salary.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        // 处理成就系统
        // achievement.xml : 所有文本
        try {
            System.out.println("process achievement.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "achievement.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("achievement");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("name"), "Achievement");
                if (tmp != null) {
                    elem.getAttribute("name").setValue(tmp);
                    changed = true;
                }
                
                List list1 = elem.getChildren("item");
                for(int j = 0; j < list1.size(); j++){
                    Element elem1 = (Element)list1.get(j);
                    tmp = context.input(elem1.getAttributeValue("name"), "Achievement");
                    if (tmp != null) {
                        elem1.getAttribute("name").setValue(tmp);
                        changed = true;
                    }
                    tmp = context.input(elem1.getAttributeValue("dec"), "Achievement");
                    if (tmp != null) {
                        elem1.getAttribute("dec").setValue(tmp);
                        changed = true;
                    }
//                    List list2 = elem1.getChildren("item");
//                    for(int k = 0; k < list2.size(); k++){
//                        Element elem2 = (Element)list2.get(k);
//                        tmp = context.input(elem2.getAttributeValue("name"), "Achievement");
//                        if (tmp != null) {
//                            elem2.getAttribute("name").setValue(tmp);
//                            changed = true;
//                        }
//                        
//                        tmp = context.input(elem2.getAttributeValue("dec"), "Achievement");
//                        if (tmp != null) {
//                            elem2.getAttribute("dec").setValue(tmp);
//                            changed = true;
//                        }
//                    }
                }
            ...
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "achievement.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        // 处理城战
        // tongbattleapply.xml: name
        try {
            System.out.println("process tongbattleapply.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "tongbattleapply.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("city");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("name"), "TongBattleApply");
                if (tmp != null) {
                    elem.getAttribute("name").setValue(tmp);
                    changed = true;
                }
            ...
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "tongbattleapply.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        // sendgift.xml : 所有文本
        try {
            System.out.println("process sendgift.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "sendgift.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("gift");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("name"), "SendGift");
                if (tmp != null) {
                    elem.getAttribute("name").setValue(tmp);
                    changed = true;
                }
                
                List list1 = elem.getChildren("type");
                for(int j = 0; j < list1.size(); j++){
                    Element elem1 = (Element)list1.get(j);
                    tmp = context.input(elem1.getAttributeValue("name"), "SendGift");
                    if (tmp != null) {
                        elem1.getAttribute("name").setValue(tmp);
                        changed = true;
                    }
            ...
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "sendgift.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        // ploynpc.xml : message
        try {
            System.out.println("process ploynpc.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "ploynpc.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("map");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("message"), "PloyNPC");
                if (tmp != null) {
                    elem.getAttribute("message").setValue(tmp);
                    changed = true;
                }
            }
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "ploynpc.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        // plotterboss.xml : pos
        try {
            System.out.println("process plotterboss.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "plotterboss.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("npc");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                
                List list1 = elem.getChildren("position");
                for(int j = 0; j < list1.size(); j++){
                    Element elem1 = (Element)list1.get(j);
                    tmp = context.input(elem1.getAttributeValue("pos"), "PlotterBoss");
                    if (tmp != null) {
                        elem1.getAttribute("pos").setValue(tmp);
                        changed = true;
                    }
                }
            }
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "plotterboss.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        // npctypes.xml : title
        try {
            System.out.println("process npctypes.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "npctypes.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("npctype");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("title"), "NpcTypes");
                if (tmp != null) {
                    elem.getAttribute("title").setValue(tmp);
                    changed = true;
                }
            }
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "npctypes.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        // factions.xml : title
        try {
            System.out.println("process factions.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "factions.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("faction");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("title"), "Factions");
                if (tmp != null) {
                    elem.getAttribute("title").setValue(tmp);
                    changed = true;
                }
            ...
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "factions.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        // 处理活动指引
        // actleader.xml : mapName
        try {
            System.out.println("process actleader.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "actleader.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("map");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("mapName"), "ActLeader");
                if (tmp != null) {
                    elem.getAttribute("mapName").setValue(tmp);
                    changed = true;
                }
            }
            
            list = root.getChildren("act");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("name"), "ActLeader");
                if (tmp != null) {
                    elem.getAttribute("name").setValue(tmp);
                    changed = true;
                }
            ...
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "actleader.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        // 处理公会战场
        // tongbattlefield.xml : name
        try {
            System.out.println("process tongbattlefield.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "Areas/tongbattlefield.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("tongbattlefield");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("name"), "TongBattleField");
                if (tmp != null) {
                    elem.getAttribute("name").setValue(tmp);
                    changed = true;
                }
            ...
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "Areas/tongbattlefield.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        // 处理随从
        // attendanttype.xml : title
        try {
            System.out.println("process attendanttype.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "Attendant/attendanttype.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("attendant");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("title"), "Attendant");
                if (tmp != null) {
                    elem.getAttribute("title").setValue(tmp);
                    changed = true;
                }
            ...
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "Attendant/attendanttype.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        // talismen.xml : title
        try {
            System.out.println("process talisman.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "Items/talisman.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("talisman");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("title"), "Talisman");
                if (tmp != null) {
                    elem.getAttribute("title").setValue(tmp);
                    changed = true;
                }
            ...
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "Items/talisman.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     // 处理每日福利
        // welfare.xml : name, welfareDec,welfareName
        try {
            System.out.println("process welfare.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "welfare.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("welfareType");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("name"), "Welfare");
                if (tmp != null) {
                    elem.getAttribute("name").setValue(tmp);
                    changed = true;
                }
                List<Element> eleList = elem.getChildren("welfare");
                for(int j=0;j<eleList.size();j++){
                    Element el = (Element)eleList.get(j);
                    tmp = context.input(el.getAttributeValue("welfareName"), "Welfare");
                    if (tmp != null) {
                        el.getAttribute("welfareName").setValue(tmp);
                        changed = true;
                    }
                    tmp = context.input(el.getAttributeValue("welfareDec"), "Welfare");
                    if (tmp != null) {
                        el.getAttribute("welfareDec").setValue(tmp);
                        changed = true;
                    }
                }
            }
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "welfare.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     // 包月
        // monthlybuy.xml : name, dec
        try {
            System.out.println("process monthlybuy.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "monthlybuy.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("item");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("name"), "monthlybuy");
                if (tmp != null) {
                    elem.getAttribute("name").setValue(tmp);
                    changed = true;
                }
                tmp = context.input(elem.getAttributeValue("dec"), "monthlybuy");
                if (tmp != null) {
                    elem.getAttribute("dec").setValue(tmp);
                    changed = true;
                }
            }
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "monthlybuy.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     // 处理新新手引导系统
        try {
            System.out.println("process ClientEvent index.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "ClientEvent/index.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("event");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                List list1 = elem.getChildren("eventitem");
                for(int j = 0; j < list1.size(); j++){
                    Element elem1 = (Element)list1.get(j);
                    tmp = context.input(elem1.getAttributeValue("desKey"), "ClientEvent");
                    if (tmp != null) {
                        elem1.getAttribute("desKey").setValue(tmp);
                        changed = true;
                    }
                    tmp = context.input(elem1.getAttributeValue("desTouch"), "ClientEvent");
                    if (tmp != null) {
                        elem1.getAttribute("desTouch").setValue(tmp);
                        changed = true;
                    }
                    tmp = context.input(elem1.getAttributeValue("signDesc"), "ClientEvent");
                    if (tmp != null) {
                        elem1.getAttribute("signDesc").setValue(tmp);
                        changed = true;
                    }
                    tmp = context.input(elem1.getAttributeValue("signDescTouch"), "ClientEvent");
                    if (tmp != null) {
                        elem1.getAttribute("signDescTouch").setValue(tmp);
                        changed = true;
                    }
                }
            }
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "ClientEvent/index.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            ...
            for (DataObject dobj : proj.getDataListByType(BookConfig.class)) {
                BookConfig book = (BookConfig)dobj;
                tmp = context.input(book.title, "BookConfig");
                if (tmp != null) {
                    book.title = tmp;
                    changed = true;
                }
                tmp = context.input(book.dec, "BookConfig");
                if (tmp != null) {
                    book.dec = tmp;
                    changed = true;
                }
      ...
      //处理副本扫荡
        //instancesweep:name,reward
        try {
            System.out.println("process Areas instancesweep.xml...");
            changed = false;
            Document doc = Utils.loadDOM(new File(proj.baseDir, "Areas/instancesweep.xml"));
            Element root = doc.getRootElement();
            List list = root.getChildren("instance");
            for (int i = 0; i < list.size(); i++) {
                Element elem = (Element)list.get(i);
                tmp = context.input(elem.getAttributeValue("name"), "InstanceSweep");
                if (tmp != null) {
                    elem.getAttribute("name").setValue(tmp);
                    changed = true;
                }
                tmp = context.input(elem.getAttributeValue("reward"), "InstanceSweep");
                if (tmp != null) {
                    elem.getAttribute("reward").setValue(tmp);
                    changed = true;
                }
                
            }
            if (changed) {
                Utils.saveDOM(doc, new File(proj.baseDir, "Areas/instancesweep.xml"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private static boolean processExpressionList(Object exprObj, I18NContext context, int questID) {
        boolean ret = false;
        if (exprObj instanceof ExpressionList) {
            ExpressionList list = (ExpressionList)exprObj;
            for (int i = 0; i < list.getExprCount(); i++) {
                if (processExpressionList(list.getExpr(i), context, questID)) {
                    ret = true;
                }
            }
        } else if (exprObj instanceof Expression) {
            Expression expr = (Expression)exprObj;
            if (processExpressionList(expr.getLeftExpr(), context, questID)) {
                ret = true;
            }
            if (expr.getRightExpr() != null) {
                if (processExpressionList(expr.getRightExpr(), context, questID)) {
                    ret = true;
                }
            }
            ...
            if (expr0.type == Expr0.TYPE_IDENTIFIER) {
                ...
                String tmp = context.input(str, isVar ? "Quest Variable" : "Quest(" + questID + ")");
                if (tmp != null) {
                ...
                for (int i = 0; i < fc.getParamCount(); i++) {
                    if (processExpressionList(fc.getParam(i), context, questID)) {
                        ret = true;
                    }
                }
            }
    ...
    private static void processJava(File root, I18NContext context, String encoding1, String encoding2) {
        ...
        javaFiles.addAll(findFiles(root, ".gtl"));
        javaFiles.addAll(findFiles(root, ".h"));
