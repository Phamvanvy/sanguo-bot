...
import com.pip.sanguo.editor.quest.expr.A_AddItem;
import com.pip.sanguo.editor.quest.expr.A_Dec;
import com.pip.sanguo.editor.quest.expr.A_Empty;
import com.pip.sanguo.editor.quest.expr.A_GetReward;
import com.pip.sanguo.editor.quest.expr.A_Inc;
import com.pip.sanguo.editor.quest.expr.A_Question;
import com.pip.sanguo.editor.quest.expr.A_RemoveItem;
import com.pip.sanguo.editor.quest.expr.A_Set;
import com.pip.sanguo.editor.quest.expr.AbstractNotifyAction;
import com.pip.sanguo.editor.quest.expr.C_AnswerQuestion;
import com.pip.sanguo.editor.quest.expr.C_CloseChat;
import com.pip.sanguo.editor.quest.expr.C_CloseMessage;
import com.pip.sanguo.editor.quest.expr.C_GlobalVar;
import com.pip.sanguo.editor.quest.expr.C_HasItem;
import com.pip.sanguo.editor.quest.expr.C_LocalVar;
import com.pip.sanguo.editor.quest.expr.C_True;
import com.pip.sanguo.editor.quest.expr.C_UseItem;
import com.pip.sanguo.editor.quest.expr.IExpr;
import com.pip.sanguo.editor.quest.expr.TemplateManager;
import com.pip.sanguo.editor.quest.flow.ActionGroupNode;
import com.pip.sanguo.editor.quest.flow.ActionNode;
import com.pip.sanguo.editor.quest.flow.ConditionNode;
import com.pip.sanguo.editor.quest.flow.Flow;
import com.pip.sanguo.editor.quest.flow.FlowNode;
import com.pip.sanguo.editor.quest.flow.IFlowIterator;
...
public class QuestFlowViewer extends Canvas implements DropTargetListener, PaintListener {
	 ...
	 * @param expr
	 */
	public void loadFlowsFromExpression(String expr) {
	    mode = 1;
	    if (expr.trim().length() >= 0) {
    	    QuestTrigger dumbTrigger = new QuestTrigger();
    	    dumbTrigger.condition = expr;
	...
	public void layout() {
	    ...
	    if (mode == 1) {
	        ...
	        if (flows.size() > 0) {
    	        ConditionNode startNode = flows.get(0).getStartNode();
    	        while (true) {
    	            while (startNode.getChildren().size() > 0) {
    	                if (startNode.getChildren().get(0) instanceof ConditionNode) {
    	            ...
    	            startNode = (ConditionNode)startNode.getChildren().get(0);
    	        }
	        }
	    }
...
//            gc.drawRectangle(0, 0, clip.width - 1, clip.height - 1);
//	    }
    ...
    public boolean preSaveCheck() {
        ...
        if (flatShow) {
            ...
            int missCount = 0;
            for (Flow flow : flows) {
                IExpr expr = flow.getStartNode().getCondition();
            ...
            if (missCount > 0) {
                ...
                if (!MessageDialog.openQuestion(getShell(), "确认", msg)) {
                    ...
                    for (int i = 0; i < flows.size(); i++) {
                        IExpr expr = flows.get(i).getStartNode().getCondition();
                        if (expr instanceof C_AnswerQuestion && !idMap.contains(((C_AnswerQuestion)expr).param1)) {
                            flows.remove(i);
                            i--;
                        }
	...
	public String saveCondition() {
	    ...
	    while (true) {
	        ...
	        buf.append(startNode.getCondition().getExpression());
	        if (startNode.getChildren().size() > 0 && startNode.getChildren().get(0) instanceof ConditionNode) {
	            startNode = (ConditionNode)startNode.getChildren().get(0);
	        } else {
	            break;
	        }
	    ...
	    if (selectedNode.getParent() == null) {
	        ...
	        for (FlowNode child : selectedNode.getChildren()) {
	            if (child instanceof ActionGroupNode) {
	                ConditionNode fakeNode = new ConditionNode();
	                ...
	                Flow newFlow = new Flow();
	                newFlow.setStartNode((ConditionNode)child);
	...
	public void drop(DropTargetEvent event) {
	    ...
	    IExpr expr = TemplateManager.recognize(list.getExpr(0), questInfo);
	    ...
	    if (hoverNode != null) {
	        ...
	        if (expr.isCondition()) {
	            Flow newFlow = new Flow();
	            ConditionNode newCond = new ConditionNode();
	            newCond.setCondition(expr);
	            newFlow.setStartNode(newCond);
	            flows.add(newFlow);
	            ActionGroupNode gn = new ActionGroupNode();
            ...
            if (node.getBounds() != null) {
                ...
                if (nbounds.intersects(clip)) {
                    ...
                    if (hoverNode == node) {
                        ...
                        if (isDraggingTemplate && node instanceof ConditionNode && 
                            ...
                            SWTUtils.drawRoundRect(gc, bounds2, 0xDADADA, 0xF9F9F9, 0xE6E6E6);
                        }
                    } else if (selectedNode == node) {
                        SWTUtils.drawRoundRect(gc, nbounds, 0x9ADFFE, 0xF2F9FE, 0xD6F0FD);
                    } else if (!(node instanceof ActionGroupNode)) {
