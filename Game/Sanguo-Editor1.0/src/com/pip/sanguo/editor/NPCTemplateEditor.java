...
import com.pip.sanguo.editor.property.ChooseItemByTypeDialog;
...
public class NPCTemplateEditor extends DefaultDataObjectEditor {
    ...
    private Text textPartyModelRatio;
    private Text textCardId;
    private Text textCardRatio;
    ...
    private Button buttonPartyModel;
    private Button buttonCard;
    
    private Button buttonIsPvp;
    ...
    public void createPartControl(Composite parent) {
        ...
        buttonIsPvp = new Button(container, SWT.CHECK);
        final GridData gd_buttonIsPvp = new GridData(SWT.LEFT, SWT.CENTER, false, false, 3, 1);
        buttonIsPvp.setLayoutData(gd_buttonIsPvp);
        buttonIsPvp.setText("这是一只PVP怪");
        buttonIsPvp.addSelectionListener(new SelectionAdapter(){
            public void widgetSelected(SelectionEvent e) {
                ((NPCTemplate)getEditObject()).isPvp = buttonIsPvp.getSelection();
                setDirty(true);
            }
        });
        ...
        textRank = new Text(container, SWT.BORDER);
        ...
        label_29.setText("填队模式增幅：");
        
        textPartyModelRatio = new Text(container, SWT.BORDER);
        final GridData gd_textPartyModelRatio = new GridData(SWT.FILL, SWT.CENTER, true, false, 2, 1);
        textPartyModelRatio.setLayoutData(gd_textPartyModelRatio);
        textPartyModelRatio.addFocusListener(AutoSelectAll.instance);
        textPartyModelRatio.addModifyListener(this);
        
        buttonPartyModel = new Button(container, SWT.CHECK);
        final GridData gd_buttonPartyModel = new GridData(SWT.LEFT, SWT.CENTER, false, false, 3, 1);
        buttonPartyModel.setLayoutData(gd_buttonPartyModel);
        buttonPartyModel.setText("填队模式");
        buttonPartyModel.addSelectionListener(new SelectionAdapter(){
            public void widgetSelected(SelectionEvent e) {
                ((NPCTemplate)getEditObject()).partyModel = buttonPartyModel.getSelection();
                setDirty(true);
            }
        });
        
        final Label label_30 = new Label(container, SWT.NONE);
        label_30.setText("卡片掉落：");
        
        textCardId = new Text(container, SWT.BORDER);
        final GridData gd_textCardModelRatio = new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1);
        textCardId.setLayoutData(gd_textCardModelRatio);
        textCardId.addFocusListener(AutoSelectAll.instance);
        textCardId.addModifyListener(this);
        
        buttonCard = new Button(container, SWT.Activate);
        final GridData gd_buttonCard = new GridData(SWT.LEFT, SWT.LEFT, false, false, 1, 1);
        buttonCard.setLayoutData(gd_buttonCard);
        buttonCard.setText("选择卡片");
        buttonCard.addSelectionListener(new SelectionAdapter(){
          int itemID = 0;
            public void widgetSelected(SelectionEvent e) {
                ChooseItemByTypeDialog dlg = new ChooseItemByTypeDialog(textCardId.getShell(),Item.TYPE_CARD);
                ...
                if (dlg.open() == Dialog.OK) {
                    ...
                    if(item != null){   
                        String value = Integer.toString(item.id);
                        if(textCardId.getText().equals(""))
                            textCardId.append(value);
                        else if(textCardId.getText().equals("0"))
                            textCardId.setText(value);
                        else
                            textCardId.append("," + value);
                    }
                    else{
                        textCardId.setText("无效的物品");
                    }
                }
        
//                int selGroup = 0;
//                ChooseDropGroupDialog2 dlg = new ChooseDropGroupDialog2(textCardRatio.getShell());
//                dlg.setSelectedDropGroup(selGroup);
//                if(dlg.open() == IDialogConstants.OK_ID){
//                    selGroup = dlg.getSelectedDropGroup();
//                    String value = Integer.toString(selGroup);
//                    if(textCardRatio.getText().equals(""))
//                        textCardRatio.append(value);
//                    else
//                        textCardRatio.append("," + value);
//                }
            }
        ...
        label_31.setText("怪物卡片抗率(%)：");
        textCardRatio = new Text(container, SWT.BORDER);
        final GridData gd_textCardRatio = new GridData(SWT.FILL, SWT.LEFT, true, false, 2, 1);
        textCardRatio.setLayoutData(gd_textCardRatio);
        textCardRatio.addFocusListener(AutoSelectAll.instance);
        textCardRatio.addModifyListener(this);
    ...
    private void updateView() {
        ...
        if (dataDef.type.id == 3) {
            ...
            buttonPartyModel.setSelection(dataDef.partyModel);
            comboLevel.select(dataDef.level);
            buttonIsPvp.setSelection(dataDef.isPvp);
        ...
        textRank.setText(String.valueOf(dataDef.rank));
        textPartyModelRatio.setText(String.valueOf(dataDef.partyModelRatio));
        textCardId.setText(String.valueOf(dataDef.cardId));
        textCardRatio.setText(String.valueOf(dataDef.cardRatio));
    ...
    private void setType(NPCType type){
        ...
        buttonIsPvp.setEnabled(flag);
        buttonPartyModel.setEnabled(flag);
//        comboLevel.setEnabled(flag);
    ...
    protected void saveData() throws Exception {
        ...
        if(dataDef.type.id == 3){
            ...
            dataDef.partyModel = buttonPartyModel.getSelection();
            dataDef.isPvp = buttonIsPvp.getSelection();
            try {
            ...
            try {
                ...
                throw new Exception("请输入正确的战功。");
            }
            try {
                dataDef.partyModelRatio = Integer.parseInt(textPartyModelRatio.getText());
            } catch (Exception e) {
                throw new Exception("请输入正确的增幅。");
            }
            try {
                String cardIdString = textCardId.getText();
                dataDef.cardId = Integer.parseInt(cardIdString);
            } catch (Exception e) {
                throw new Exception("请输入正确的卡片信息。");
            }
            try {
                String cardRatioString = textCardRatio.getText();
                dataDef.cardRatio = Integer.parseInt(cardRatioString);
            } catch (Exception e) {
                throw new Exception("请输入正确的卡片信息。");
            }
        }
