...
public class ChooseBuffDialog extends Dialog {
    class ListContentProvider implements IStructuredContentProvider {
        public Object[] getElements(Object inputElement) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (q.buffType != BuffConfig.BUFF_TYPE_EQUIP&&q.buffType != BuffConfig.BUFF_TYPE_DYNAMIC) {
                    continue;
                }
                if (matchCondition(q)) {
                    retList.add(q);
     ...
     * @param parentShell
     */
    public ChooseBuffDialog(Shell parentShell) {
        super(parentShell);
    }
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        if (selectedBuff != -1) {
            try {
                BuffConfig q = (BuffConfig)EditorApplication.getProj().findObject(BuffConfig.class, selectedBuff);
                if (q != null) {
