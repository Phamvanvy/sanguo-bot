...
public class AutoNewObjectWizard {
    ...
    public static void newClientEvent(Shell shell) {
        ProjectData proj = EditorApplication.getInstance().getProjectData();
        int maxEventId = 0;

        List<DataObject> events = proj.getDataListByType(ClientEvent.class);

        for (int i = 0; i < events.size(); i++) {
            ClientEvent event = (ClientEvent) events.get(i);
        ...
        String newArea = (maxEventId + 1) + "-" + (maxEventId + 101);
