...
public class EquipmentEditor extends DefaultDataObjectEditor implements IValueChanged {
    ...
    private Text textMaxCardCount;
    private Text textHoleCount;
    private Text textCardCount;
    ...
    /** 装备升级消耗数量 */
    private Text textEquipLeveUpEffect;
    /** 装备保留属性消耗数量 */
    private Text textEquipLeveUpSavePropEffect;
    /** 时效类型 */
    private Combo comboTimeType;
    /** 装备升级消耗类型（无，战功，元宝，材料.....） */
    private Combo comboEquipLevelUpType;
    /** 保留属性消耗物品*/
    private Combo comboSaveProp;
    
    /** 装备价格 */
    private Text textPrice;
    /** 下一级装备ID */
    private Text textNextLevelEquipId;
    /** 绑定类型 */
    ...
    private Button canLevelUpButton;
    ...
    public void createPartControl(Composite parent){
        ...
        //装备升级消耗类型
        final Label label_EquipLevelUp = new Label(container, SWT.NONE);
        label_EquipLevelUp.setLayoutData(new GridData());
        label_EquipLevelUp.setText("装备升级消耗类型：");
        
        comboEquipLevelUpType = new Combo(container, SWT.READ_ONLY);
        comboEquipLevelUpType.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
        comboEquipLevelUpType.setItems(Constants.COMBO_EQUILEVELUP_TYPE);
        comboEquipLevelUpType.addSelectionListener(eventHandle);
        comboEquipLevelUpType.addModifyListener(this);
        
        final Label label_EquipLeveUp_Price = new Label(container, SWT.NONE);
        label_EquipLeveUp_Price.setLayoutData(new GridData());
        label_EquipLeveUp_Price.setText("消耗数量：");
        
        
        textEquipLeveUpEffect = new Text(container, SWT.BORDER);
        textEquipLeveUpEffect.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
        textEquipLeveUpEffect.addModifyListener(this);
        
        final Label label_SaveProp = new Label(container, SWT.NONE);
        label_SaveProp.setLayoutData(new GridData());
        label_SaveProp.setText("保留装备属性消耗类型：");
        
        comboSaveProp = new Combo(container, SWT.READ_ONLY);
        comboSaveProp.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
        comboSaveProp.setItems(Constants.COMBO_SAVEPROP_TYPE);
        comboSaveProp.addSelectionListener(eventHandle);
        comboSaveProp.addModifyListener(this);
        
        final Label label_EquipLeveUpSaveProp_Price = new Label(container, SWT.NONE);
        label_EquipLeveUpSaveProp_Price.setLayoutData(new GridData());
        label_EquipLeveUpSaveProp_Price.setText("保留属性消耗数量：");
        
        textEquipLeveUpSavePropEffect = new Text(container, SWT.BORDER);
        textEquipLeveUpSavePropEffect.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
        textEquipLeveUpSavePropEffect.addModifyListener(this);
        ...
        label_21.setText("卡槽数：");
        
        textCardCount = new Text(container, SWT.BORDER);
        final GridData gd_textCardCount = new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1);
        textCardCount.setLayoutData(gd_textCardCount);
        textCardCount.addFocusListener(AutoSelectAll.instance);
        textCardCount.addModifyListener(this);
        
        final Label label_22 = new Label(container, SWT.NONE);
        label_22.setText("最大卡槽数：");
        
        textMaxCardCount = new Text(container, SWT.BORDER);
        final GridData gd_textMaxCardCount = new GridData(SWT.FILL, SWT.CENTER, true, false);
        textMaxCardCount.setLayoutData(gd_textMaxCardCount);
        textMaxCardCount.addFocusListener(AutoSelectAll.instance);
        textMaxCardCount.addModifyListener(this);
        ...
        final Label label_NextLevelEquip = new Label(container, SWT.NONE);
        label_NextLevelEquip.setText("下一等级装备ID：");

        
        canLevelUpButton = new Button(composite, SWT.CHECK);
        canLevelUpButton.setBackground(SWTResourceManager.getColor(0xff, 0, 0));
        canLevelUpButton.setText("允许装备升级");
        canLevelUpButton.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(final SelectionEvent e) {
                if (!updating) {
                    setDirty(true);
                }
                textNextLevelEquipId.setEnabled(canLevelUpButton.getSelection());
                if(!textNextLevelEquipId.isEnabled()){
                    textNextLevelEquipId.setText("");
                }
            }
        });
        
        textNextLevelEquipId = new Text(container, SWT.BORDER);
        textNextLevelEquipId.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
        textNextLevelEquipId.addFocusListener(AutoSelectAll.instance);
        textNextLevelEquipId.addModifyListener(this);
        textNextLevelEquipId.setEnabled(canLevelUpButton.getSelection());
        ...
        textCity = new Text(container, SWT.BORDER);
        textCity.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
        textCity.addFocusListener(AutoSelectAll.instance);
        textCity.addModifyListener(this);
        ...
        final Label label_Type = new Label(group, SWT.NONE);
        label_Type.setLayoutData(new GridData());
        label_Type.setText("类型：");
        
        comboType = new Combo(group, SWT.READ_ONLY);
        comboType.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
        comboType.setItems(Constants.COMBO_TYPE_EXT);
    ...
    protected void saveData(boolean ignoreError) throws Exception {
        ...
        try {
            equipment.cardCount = Integer.parseInt(textCardCount.getText());
        } catch (NumberFormatException e) {
            if (!ignoreError) {
                throw new Exception("卡槽数输入错误！");
            }
        }
        try {
            equipment.maxCardCount = Integer.parseInt(textMaxCardCount.getText());
        } catch (NumberFormatException e) {
            if (!ignoreError) {
                throw new Exception("最大卡槽数输入错误！");
            }
        }
        ...
        equipment.canLevelUp=canLevelUpButton.getSelection();
        try {
            equipment.nextEquipId = Integer.parseInt(textNextLevelEquipId.getText());
            if(equipment.nextEquipId>0){
                textNextLevelEquipId.setEnabled(true);
            }
        } catch (NumberFormatException e) {
            if (!ignoreError&&equipment.canLevelUp) {
                throw new Exception("下一等级装备ID错误！");
            }else if(!equipment.canLevelUp){
                equipment.nextEquipId=0;
            }
        }
        
        if(equipment.nextEquipId < 0&&equipment.canLevelUp){
            if (!ignoreError) {
                throw new Exception("下一等级装备ID必须是正数！");
            }
        }
        ...
        //装备升级消耗
        equipment.equipLevelUpPriceType=comboEquipLevelUpType.getSelectionIndex();
        if(equipment.equipLevelUpPriceType>Equipment.EQUIPLEVELUP_TYPE_UNDEFINE){
        	 try {
//                 if(equipment.equipLevelUpPriceType == Equipment.EQUIPLEVELUP_TYPE_CREDIT){
                     equipment.equipLevelUpPriceNum=Integer.parseInt(textEquipLeveUpEffect.getText());
//                 }
             } catch (Exception e) {
                 if (!ignoreError) {
                     throw new Exception("格式输入错误！");
                 }
             }
        }
        
        //保留属性消耗
        equipment.equipLevelUpSavePropType=comboSaveProp.getSelectionIndex();
        if(equipment.equipLevelUpSavePropType>Equipment.SAVEEQUIPPROP_TYPE_UNDEFINE){
            try {
                if(equipment.equipLevelUpSavePropType == Equipment.SAVEEQUIPPROP_TYPE_IMONEY){
                    equipment.equipLeveUpSavePropPriceNum=Integer.parseInt(textEquipLeveUpSavePropEffect.getText());
                }
            } catch (Exception e) {
                if (!ignoreError) {
                    throw new Exception("格式输入错误！");
        ...
        equipment.limitType = comboType.getSelectionIndex()-1;
        ...
        equipment.produceArea = textCity.getText().trim();
    }
    ...
    private void setCurrentData(Equipment equi) {
        ...
        comboEquipLevelUpType.select(equi.equipLevelUpPriceType);
        if(equi.equipLevelUpPriceType != Equipment.EQUIPLEVELUP_TYPE_UNDEFINE){
        	textEquipLeveUpEffect.setText(String.valueOf(equi.equipLevelUpPriceNum));
        }else{
        	textEquipLeveUpEffect.setEditable(false);
        }
        
        comboSaveProp.select(equi.equipLevelUpSavePropType);
        if(equi.equipLevelUpSavePropType != Equipment.SAVEEQUIPPROP_TYPE_UNDEFINE){
            textEquipLeveUpSavePropEffect.setText(String.valueOf(equi.equipLeveUpSavePropPriceNum));
        }else{
            textEquipLeveUpSavePropEffect.setEditable(false);
        }
        if(equi.equipLevelUpPriceType==Equipment.EQUIPLEVELUP_TYPE_MATERIAL){
            comboSaveProp.setEnabled(false);
            textEquipLeveUpSavePropEffect.setEnabled(false);
        }
        ...
        textCardCount.setText(String.valueOf(equi.cardCount));
        textMaxCardCount.setText(String.valueOf(equi.maxCardCount));
        ...
        canLevelUpButton.setSelection(equi.canLevelUp);
        
        iconChooser.setIcon(equi.specialIconIndex, Item.ICON_IMAGE_ABILITY);
        if(equi.nextEquipId>0){
            canLevelUpButton.setSelection(true);
            textNextLevelEquipId.setText(equi.nextEquipId+"");
        }else{
            canLevelUpButton.setSelection(false);
        }
        
        // 装备限制
        textPlayerLevel.setText(String.valueOf(equi.playerLevel));
        comboJob.select(equi.job + 1);
        comboType.select(equi.limitType+1);
        ...
        textCity.setText(equi.produceArea);
    ...
    private void updatePreview() {
        ...
        if(equ.limitType>=0){
            sb.append("类型：" + Constants.COMBO_TYPE[equ.limitType] + "\n");
        }
        if (equ.job >= 0) {
    ...
    class SelectionEventHandle extends SelectionAdapter implements ISelectionChangedListener {
        public void widgetSelected(SelectionEvent event) {
            ...
            }else if(event.getSource() == comboEquipLevelUpType){
                if(comboEquipLevelUpType.getSelectionIndex()== Equipment.EQUIPLEVELUP_TYPE_UNDEFINE){
                	textEquipLeveUpEffect.setEditable(false);
                }else{
                    if(comboEquipLevelUpType.getSelectionIndex()== Equipment.EQUIPLEVELUP_TYPE_MATERIAL){
                        comboSaveProp.select(0);
                        comboSaveProp.setEnabled(false);
                        textEquipLeveUpSavePropEffect.setText("0");
                        textEquipLeveUpSavePropEffect.setEnabled(false);
                    }else{
                        comboSaveProp.setEnabled(true);
                        textEquipLeveUpSavePropEffect.setEnabled(true);
                    }
                    textEquipLeveUpEffect.setEditable(true);
                }
            }else if(event.getSource() == comboSaveProp){
                if(comboSaveProp.getSelectionIndex()== Equipment.SAVEEQUIPPROP_TYPE_UNDEFINE){
                    textEquipLeveUpSavePropEffect.setEditable(false);
                }else{
                    textEquipLeveUpSavePropEffect.setEditable(true);
                }
