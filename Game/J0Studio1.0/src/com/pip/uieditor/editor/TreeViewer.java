...
import com.pip.uieditor.editor.tree.TreeViewerTransferDragListener;
import com.pip.uieditor.editor.tree.TreeViewerTransferDropListener;
...
public class TreeViewer extends AbstractEditPartViewer {
	...
	public Control createControl(Composite parent) {
		Tree tree = new Tree(parent, SWT.SINGLE | SWT.H_SCROLL | SWT.V_SCROLL | SWT.CHECK);
		setControl(tree);
		return tree;
	}
		...
		tree.addSelectionListener(new SelectionListener() {
			public void widgetSelected(SelectionEvent e) {
				if(e.detail != SWT.CHECK) {
					...
					EditPart ep = (EditPart)item.getData();
					Object model = ep.getModel();
					if(model != null && model instanceof UIObject) {
						UIObject uo = (UIObject)model;
						uo.setShowInEditing(item.getChecked());
					}
				}
			}
