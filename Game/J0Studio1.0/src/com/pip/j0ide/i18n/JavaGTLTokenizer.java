...
/**
 * 简单Java/GTL语言词法分析器。可以把源代码无损解析成一个个token，根据这些token可以完全恢复源文件。
 * @author lighthu
 */
public class JavaGTLTokenizer {
    // 输入流
    protected Reader in;
    // 缓冲的最后一个字符，-1表示没有
    protected int cachedChar = -1;
    
    public JavaGTLTokenizer(Reader r) {
        in = r;
    }
    ...
    public static List<String> parse(Reader r) throws IOException {
        JavaGTLTokenizer tokens = new JavaGTLTokenizer(r);
        String token;
