...
public class GTLDebugSession implements IDebugVMListener {
    ...
    public void syncState(boolean pause) {
    	try {
    		vm.syncState();
    		if (pause) {
    ...
    public void modify(int op, int addr, int index, int intValue, String strValue) {
    	try {
    		vm.modify(op, addr, index, intValue, strValue);
    	} catch (Exception e) {
    	}
    }
    
    public void toggleFuncReport(boolean send) {
    	try {
    		vm.toggleFuncReport(send);
    	} catch (Exception e) {
    	}
    }
    ...
    public Object[] getCurrentLine() {
        ...
        return getCurrentLine2();
    }
    
    public Object[] getCurrentLine2() {
        GTLDebugSession ss;
        ETFDebugInfo dinfo;
        int curFunc;
    ...
    public void syncStateOver(boolean isClosed) {
    	// 重新生成变量表
    	...
    	for (int i = 0; debugInfo.libraries != null && i < debugInfo.libraries.length; i++) {
            ...
            if (ss != null) {
            	...
            	for (int j = 0; j < arr.length; j++) {
            		...
            		newItem.name = var.name;
            		newItem.typeName = var.typeName;
            		newItem.address = (var.address & 0xC0000000) | ((var.address & 0x3FFFFFFF) + offset);
            		list.add(newItem);
            	}
            }
    	}
    	...
    	if (isClosed) {
    		debugManager.closeSyncOver(this);
    	}
	...
	/**
	 * 遍历静态堆和栈，找出所有引用到的内存单元的引用计数。
	 * @param target 保存引用计数的目标session（因为在版本2以上的VM中用到了全局动态堆，计算引用计数时需要遍历所有session，而所有引用计数都保存在当前session里）
	 */
	public void updateRefCount(GTLDebugSession target) {
		Set<Integer> walkedPointers = new HashSet<Integer>();
		for (VariableItem var : variables) {
			if ((var.address & 0x40000000) != 0) {
				// 变量是指针类型
				int value = vm.memLoad(var.address);
				try {
					updateRefCount(target.vm, value, var.type, var.typeName, walkedPointers);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void updateRefCount(GTLDebugVMServerStub target, int ptr, int type, String typeName, Set<Integer> walkedPointers) {
		if (ptr == 0) {
			return;
		}
		addRefCount(target, ptr);
		
		// 同一个指针不要遍历超过1次
		if (walkedPointers.contains(ptr)) {
			return;
		} else {
			walkedPointers.add(ptr);
		}

		// 如果这个指针指向一个结构、结构数组、结构Vector或者结构Hashtable，需要继续向下遍历
		Object objValue = null;
		try {
			objValue = vm.followPointer(ptr);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (objValue == null) {
		...
		ETFDebugInfo.StructDef stdef = (ETFDebugInfo.StructDef)debugInfo.structDefs.get(typeName);
		if (stdef == null) {
			return;
		}
		if ((type & 16) > 0 || type == 9 || type == 10) {
			// 结构体的数组/Vector/Hashtable
			Object[] arr = (Object[])objValue;
			updateRefCount(target, arr, stdef, walkedPointers);
		} else {
			// 普通结构体，这是value应该是一个int[]，查找运行时信息
			updateRefCount(target, (int[])objValue, stdef, walkedPointers);
		}
	}
	
	private void updateRefCount(GTLDebugVMServerStub target, Object[] arr, ETFDebugInfo.StructDef stdef, Set<Integer> walkedPointers) {
		int len = arr.length;
		for (int i = 0; i < len; i++) {
			if (arr[i] == null) {
				continue;
			...
			if (arr[i] instanceof int[]) {
				// 当结构解释
				updateRefCount(target, (int[])arr[i], stdef, walkedPointers);
			} else if (arr[i] instanceof Object[]) {
				// 当结构数组解释
				updateRefCount(target, (Object[])arr[i], stdef, walkedPointers);
			}
		}
	}
	
	private void updateRefCount(GTLDebugVMServerStub target, int[] stValue, ETFDebugInfo.StructDef stdef, Set<Integer> walkedPointers) {
		try {
			stdef = getRuntimeType(stdef, ((int[])stValue)[0]);
		} catch (Exception e) {
		}
		...
		for (int i = 0; i < ret.length; i++) {
			...
			if ((member.address & 0x40000000) != 0) {
				// 变量是指针类型
				int value = stValue[member.address & 0x3FFFFFFF];
				updateRefCount(target, value, member.type, member.typeName, walkedPointers);
			}
		}
	}
	
	private void addRefCount(GTLDebugVMServerStub target, int address) {
		int addr;
		...
		synchronized (target.refCount) {
			if (addr < target.refCount.length) {
				target.refCount[addr]++;
			}
		}
	}
	...
	/** 查找etf文件 */
    public File findETF(String name, String model) {
    	File scriptsDir = debugManager.getTargetDir();
    	File target = new File(scriptsDir, model + "/" + name + "_" + model + ".etf.gz");
    	if (target.exists()) {
    		return target;
    	} else {
    		return null;
