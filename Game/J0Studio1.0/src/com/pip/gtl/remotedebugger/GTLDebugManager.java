...
import com.pip.gtl.remotedebugger.ui.MemoryLeakView;
...
public class GTLDebugManager implements IGTLDebugManager {
	...
	protected MemoryLeakView memoryLeakView;
	...
	public void closeSyncOver(GTLDebugSession session) {
		session.updateRefCount(session);
		if (getMemoryView() != null) {
			getMemoryView().forceCheckLeak(session);
		}
	}
   ...
   private class ActivateDebugSessionViewJob implements Runnable {
	...
	private class RefreshVariableViewJob implements Runnable {
		public void run() {
			...
			if(getMemoryLeakView() != null){
				getMemoryLeakView().refresh();
			}
		}
	}
	...
	private class ShowAllocTraceJob implements Runnable {
		...
		public void run() {
			...
			getMemoryLeakView().showAllocTrace(items);
		}
	}
	
	public MemoryLeakView getMemoryLeakView(){
		return memoryLeakView;
	}
	
	public void setMemoryLeakView(MemoryLeakView filterMemoryView) {
		this.memoryLeakView = filterMemoryView;
	}
