...
public class GTLEditorImpl extends TextEditor implements ITextListener, Runnable {
	...
	/** the parser object */
	protected GTLPreCompiler parser;
	...
	public GTLEditorImpl() {
		...
		parserThread.setPriority(Thread.MIN_PRIORITY);
		parserThread.start();
	}
	...
	protected String getCursorPosition() {
		try{
			if(parser != null){
				...
				int caret = widgetOffset2ModelOffset(viewer, styledText.getCaretOffset());
				int line = document.getLineOfOffset(caret);
				GTLPreCompiler.FunctionDef func = parser.findFunction(getEditingFile(), line);
				if(func != null){
					System.out.println("func name:" + func.name);
					setCursorFunc(func.name);
				}
			}
		}catch(Exception e){
			e.printStackTrace();
			...
			if (textDirty && display != null) {
				synchronized (parserLock) {
					...
					if (edFile != null && doc != null) {
						System.gc();
						System.out.println("parse begin: used memory = " + (Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()));
						try {
						...
						System.gc();
						System.out.println("parse over: used memory = " + (Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()));
						if (newparser.isSucc()) {
							parser = newparser;
	...
	public void setCursorFunc(String selectFunction){
		if (fOutlinePage != null)
			fOutlinePage.setCursorFunc(selectFunction);
	}
}
