...
public class ChooseDropGroupDialog  extends Dialog {
        ...
        public Object[] getChildren(Object parentElement) {
            if (parentElement instanceof ProjectData) {
                ...
                if(node.type == DropNode.TYPE_ITEM){
                    //物品根节点
                    return EditorApplication.getProj().getCategoryListByType(Item.class).toArray();
                }
                else if(node.type == DropNode.TYPE_EQUIPMENT){
                    //装备根节点
                    return EditorApplication.getProj().getCategoryListByType(Equipment.class).toArray();
                }
                else if(node.type == DropNode.TYPE_DROPGROUP){
                    //掉落组根节点
                    return EditorApplication.getProj().getCategoryListByType(DropGroup.class).toArray();
                }
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        label_5.setText("示例：1% = 1/100   1%% = 1/10,000   1%%% = 1 = 1/1,000,000");
