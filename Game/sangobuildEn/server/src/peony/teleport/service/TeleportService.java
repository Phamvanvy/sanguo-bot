...
//							}else 
							if(buyRequirement.type == Shop.TYPE_MONEY && buyRequirement.toString() != null){
								strb.append(MessageFormat.format("金錢{0}、", buyRequirement.amount));
								...
								strb.append(MessageFormat.format("軍銜{0}、", rank.title));
							}else if(buyRequirement.type == Shop.TYPE_LEVEL){
								strb.append(MessageFormat.format("級別{0}、", buyRequirement.amount));
							}else if(buyRequirement.type == Shop.TYPE_HONOR){
								strb.append(MessageFormat.format("榮譽{0}、", buyRequirement.amount));
							}else if(buyRequirement.type == Shop.TYPE_IMONEY){
								strb.append(MessageFormat.format("i幣{0}、", buyRequirement.amount));
							}else if(buyRequirement.type == Shop.TYPE_ITEM){
								strb.append(MessageFormat.format("物品{0}、", buyRequirement.amount));
							}
						}
		...
		if(p != null){
			...
			if(npc != null && npc.type == GameObject.TYPE_CREATURE ){
				if(((Creature)npc).touchAction==null){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.TELEPORT_CLIENT,"傳送被打斷");
					return;
				}
...
//					}
						...
						for(BuyRequirement buyRequirement : requirements){
							...
							if (buyRequirement.type == Shop.TYPE_MONEY && buyRequirement.deduct) {
								try {
									...
									ErrorHandler.sendErrorMessage(session, serial, OpCode.TELEPORT_CLIENT,"您的金錢不足");
									return;
								}
								...
								try {
									...
									tx.rollback();
									log.info("[TELEPORTFAILED]"+LogUtil.getPlayerLogString(p)+"BALANCE["+p.money+"]");
									ErrorHandler.sendErrorMessage(session, serial, OpCode.TELEPORT_CLIENT,"您的戰功不足");
									return;
								}
							} else if(buyRequirement.type == Shop.TYPE_IMONEY){
								if(amount > account.getIMoney() / 100){
									tx.rollback();
									log.info("[TELEPORTFAILED]"+LogUtil.getPlayerLogString(p)+"BALANCE["+p.money+"]");
									ErrorHandler.sendErrorMessage(session, serial, OpCode.TELEPORT_CLIENT, "您的i幣不足");
									return;
								}else{
								...
								if (rank == null) {
									ErrorHandler.sendErrorMessage(session, serial, OpCode.TELEPORT_CLIENT, "商品配置錯誤");
									tx.rollback();
									log.info("[TELEPORTFAILED]"+LogUtil.getPlayerLogString(p)+"BALANCE["+p.money+"]");
									return;
								}else{
									if(p.getRank() < rank.id){
										...
										ErrorHandler.sendErrorMessage(session, serial, OpCode.TELEPORT_CLIENT, 
												MessageFormat.format("必須達到軍銜\"{0}\"以上才能驛站傳送", rank.title));
										return;
									}
								}
								...
								if (buyRequirement.item != null) {
									...
									if (p.bag.getGameItemCount(itemId0) < amount) {
										ErrorHandler.sendErrorMessage(session, serial, OpCode.TELEPORT_CLIENT, 
												MessageFormat.format("所需材料{0}數量不足", buyRequirement.item.title));
										tx.rollback();
										log.info("[TELEPORTFAILED]"+LogUtil.getPlayerLogString(p)+"BALANCE["+p.money+"]");
										return;
									} else {
					...
					ErrorHandler.sendErrorMessage(session, serial, OpCode.TELEPORT_CLIENT, "距离驛站NPC太遠");
				}
			}
		}
	}
