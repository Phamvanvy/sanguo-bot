...
public class NationBattleFieldInstance implements Instance {
	...
	public void addPlayer(Player player) throws VMapException {
		if(state!=STATE_STARTED)
			throw new VMapException("國戰已經結束");
	}
	...
	protected void checkBattleValue(){
		...
		if(sourceNation.pool.getInt(Nation.PROPERTY_DEFENSE_VALUE)>=HEALTH_VALUE){
			...
			Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(def.sourceFaction, 
					MessageFormat.format("{0}國民齊心協力抵御外敵,我方前線受傷將士得到救治生命值提高200,前線將士歡欣鼓舞.", 
		...
		if(destNation.pool.getInt(Nation.PROPERTY_ATTACK_VALUE)>=HEALTH_VALUE){
			...
			Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(def.destFaction, 
					MessageFormat.format("{0}國民齊心協力殲滅反賊,我方前線受傷將士得到救治生命值提高200,前線將士歡欣鼓舞.", 
	...
	protected void fail(){
		...
		chatService.sendFactionSystemMessage(def.destFaction, "進攻失利,鳴金收兵,眾將不可自亂陣腳.");
		chatService.sendFactionSystemMessage(def.destFaction, "進攻失利,大量輜重遺失");
		chatService.sendFactionSystemMessage(def.sourceFaction, "入侵賊寇敗走,鳴金收兵,清點戰利所得.");
		chatService.sendFactionSystemMessage(def.sourceFaction, "防守胜利,繳獲敵人大量戰略物資.");
		...
		sendMails(sourcePlayerRefs,itemAccount);
		transPlayers();
	}
	...
	protected void win(){
		...
		ChatService chatService = Server.server.getServiceRegistry().getChatService();
		chatService.sendFactionSystemMessage(def.destFaction, "敵虜防線潰敗,將士豪勇無倫,破城殺敵無數.");
		chatService.sendFactionSystemMessage(def.destFaction,"進攻胜利,舉國歡慶.搶回大量金銀財寶,已入國庫!");
		chatService.sendFactionSystemMessage(def.sourceFaction, "城門失守,敵軍勢大,全國軍隊士气低迷.");
		chatService.sendFactionSystemMessage(def.sourceFaction,"防守失敗.,國庫被劫掠.");
		Nation nation = manager.getNationByFaction(def.destFaction);
		NationSkill skill = nation.skills.get(5);
		int itemAccount = ((NationSkill5)skill).getItemAccount(skill.level);
		sendMails(destPlayerRefs,itemAccount);
		transPlayers();
		
	}
	
	protected void sendMails(Set<GameObjectRef> refs, int itemAccount) {
		MailService mailService = Server.server.getServiceRegistry()
				.getMailService();
		for (GameObjectRef ref : refs) {
			mailService.sendSystemMailAsync(ref.id, "系統", "國戰戰場胜利獎勵", "", 0,
					ObjectAccessor
							.createGameItem(ItemUtil.ITME_NATIONBATTLE_WIN), itemAccount, "NBT");
			//国战胜利事件
			Server.server.getEventManager().fireEvent(new ServiceEvent(ServiceEvent.EVENT_BATTLE_WIN,ref.id,0));
		}
	}
