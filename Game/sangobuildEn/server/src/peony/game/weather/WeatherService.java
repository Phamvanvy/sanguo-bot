...
public class WeatherService implements Service, ServiceEventListener {
	...
	public void schedule(WeatherDef def){
		...
		for(Period p:def.periods){
			...
			timer.schedule(new ChangeWeatherTask(def.mapId,def.type,true),getScheduleTime(cal,p.startHour,p.startMinute),ONEDAY);
			timer.schedule(new ChangeWeatherTask(def.mapId,def.type,false),getScheduleTime(cal,p.endHour,p.endMinute),ONEDAY);
		}
	}
	
	protected Date getScheduleTime(Calendar cal,int hour,int minu){
		Calendar cal1 = Calendar.getInstance();
		cal1.set(Calendar.HOUR_OF_DAY,hour);
		cal1.set(Calendar.MINUTE, minu);
	...
	class ChangeWeatherTask extends TimerTask{
