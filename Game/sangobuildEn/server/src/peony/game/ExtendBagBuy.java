...
public class ExtendBagBuy implements IBuyObject {
	GameObjectRef player;
	int serial;
	int price;
	
	public ExtendBagBuy(Player player, int serial, int price){
	...
	public void notifyClient(boolean succ, String message) {
		...
		if (succ) {
			Packet pt = new Packet(OpCode.EXTEND_BAG_SERVER);
			pt.putInt(serial);
			p.send(pt);
		} else {
			ErrorHandler.sendErrorMessage(p.session, serial, OpCode.EXTEND_BAG_CLIENT, message);
		}
	}
	...
	public void receive(PlayerTransaction tx, boolean supportMail)
	    ...
	    p.bag.extend(p.bag.getAddedSize() + 1, true);
	}
