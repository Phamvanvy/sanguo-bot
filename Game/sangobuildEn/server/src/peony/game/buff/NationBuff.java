...
//		本周内免暴率提升了XX%
		...
		if(defense!=0.0f){
			sb.append(String.format("本周內護甲值和法術防御提高%3.1f%%",defense*100));
		}
		if(antiCrti!=0.0f){
			sb.append(String.format("本周內免暴率提升%3.1f%%",antiCrti*100));
		}
		if(expratio!=0.0f){
			sb.append(String.format("本周內經驗獲得速度提高%3.1f%%",expratio*100));
		}
		return sb.toString();
	}
