		...
		if (actor != null) {
			if(count<=0){
				ErrorHandler.sendAdminErrorMessage(session, serial,
						OpCode.ADMIN_SENDMAIL_CLIENT, "物品數量錯誤,不能為:"+count);
				return;
			}
				
			if (itemId == -1) {
				Server.server.getServiceRegistry().getMailService()
						.sendSystemMail(playerId, "系統", title, content, count,
								null, 0, "GM");
			} else {
				...
				if (item != null) {
					...
					if (enhanceOk) {
						...
						ErrorHandler.sendAdminErrorMessage(session, serial,
								OpCode.ADMIN_SENDMAIL_CLIENT, "數据錯誤");
					}
				} else {
					ErrorHandler.sendAdminErrorMessage(session, serial,
							OpCode.ADMIN_SENDMAIL_CLIENT, "沒找到指定物品");
				}
			}
		} else {
			ErrorHandler.sendAdminErrorMessage(session, serial,
					OpCode.ADMIN_SENDMAIL_CLIENT, "該用戶已下線");
		}
	}
