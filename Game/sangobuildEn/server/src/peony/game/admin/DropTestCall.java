...
public class DropTestCall extends ClientSessionAsyncCall {
	...
	public void callFinish() throws Exception {
		if (map.size() > Short.MAX_VALUE) {
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_GM_DROP_TEST, MessageFormat.format("掉落物品太多,超過協議范圍：{0}", map.size()));
			return;
		}
	...
	public static String qualityName[] = {"白色裝備","綠色裝備","藍色裝備","紫色裝備","黃色裝備"};
	public int qualityCount[] = new int[qualityName.length];
	public void run() {
		GroupDrop d = ObjectAccessor.getGroupDrop(dropGroupId);
		if (d == null) {
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_GM_DROP_TEST, MessageFormat.format("沒有相應掉落組：{0}", dropGroupId));
			return;
		}
