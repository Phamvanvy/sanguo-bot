	...
	public void run() {
		...
		if (p != null) {
			...
			if (grid == null || grid.item == null
				...
				p.sendUseItemFail(itemUse.itemId, "沒找到物品");
				tx.rollback();
				return;
			}
			...
			if (grid.item.template.useType.occasion == UseType.OCCASION_BATTLE) {
				if (p.getThreatCount() == 0) {
					p.sendUseItemFail(itemUse.itemId, "只能在戰斗中使用");
					tx.rollback();
					return;
				}
			} else if (grid.item.template.useType.occasion == UseType.OCCASION_NOBATTLE) {
				if (p.getThreatCount() > 0) {
					p.sendUseItemFail(itemUse.itemId, "不能在戰斗中使用");
					tx.rollback();
					return;
				}
			}
			...
			if (itemUse.target != null) {
				...
				if (target == null) {
					p.sendUseItemFail(itemUse.itemId, "無效目標");
					tx.rollback();
					return;
				} else {
					if (!target.isAlive()) {
					...
					try {
						...
						if (mitems != null) {
					        ...
					        p.message(-1, "您背包已滿,獲得的物品已經通過郵件發送了,請查收.", -1, -1);
					        for (GainItem gitem : mitems) {
					            ...
					            Server.server.getServiceRegistry().getMailService().sendSystemMailAsync(p.id, "系統", itemTitle, "", 0,
