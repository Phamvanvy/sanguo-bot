...
public class IBuyActivity1 implements IActivityImpl, ServiceEventListener {
	...
	protected void sendReward(int playerID, int type) {
		...
		try {
			...
			String content;
			if (type == 0) {
				content = MessageFormat.format("感謝您的支持,本商城特在此贈送您超值回饋大禮包一個.（獎勵已放入背包,請查收.）", p.name);
			} else {
				content = MessageFormat.format("感謝您的支持,本商城特在此贈送您真情回饋大禮包一個.（獎勵已放入背包,請查收.）", p.name);
			}
			...
			String content;
			if (type == 0) {
				content = MessageFormat.format("感謝您的支持,本商城特在此贈送您超值回饋大禮包一個.", p.name);
			} else {
				content = MessageFormat.format("感謝您的支持,本商城特在此贈送您真情回饋大禮包一個.", p.name);
			}
			Server.server.getServiceRegistry().getMailService().sendSystemMail(
				p.id, "系統", "系統獎勵", content, 0, item, 1, "ACTV");
			if (type == 0) {
				content = MessageFormat.format("感謝您的支持,本商城特在此贈送您超值回饋大禮包一個.（您的背包已滿,獎勵已經以飛鴿附件的方式發送到你的飛鴿里了,請及時查收.）", p.name);
			} else {
				content = MessageFormat.format("感謝您的支持,本商城特在此贈送您真情回饋大禮包一個.（您的背包已滿,獎勵已經以飛鴿附件的方式發送到你的飛鴿里了,請及時查收.）", p.name);
			}
			p.message(-1, content, -1, -1);
		}
