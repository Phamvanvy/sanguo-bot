...
public class ExpansionNation {
	...
	public void update(int diff){
		if(flag_die==0){
			if(getNpcCount(ExpansionNpcTemplate.NPC_FLAG, true)==0){
				Server.server.getServiceRegistry().getChatService().
					sendAreaSystemMessage(MessageFormat.format("悲呼!{0}軍陣營的大旗被砍斷了,{1}軍無法取得最終的胜利.他們發誓在下一場戰斗中奮起反擊!",
		...
		if(!getExpansionNpc(ExpansionNpcTemplate.NPC_DOOR).npc.isAlive() && 
			...
			chatService.sendWorldMessage(MessageFormat.format("勇猛無敵的{0}軍戰士砍倒了董卓軍的大旗,取得了本場戰斗的胜利,讓我們一起歡呼吧!10分鐘后華雄來襲!", GameObject.getFactionName(faction)));
			// 十分钟之后刷华雄、任务NPC
