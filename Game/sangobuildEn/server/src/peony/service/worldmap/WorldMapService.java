	...
	public void worldTeleport(Packet packet, ClientSession session){
		...
		if(p!=null){
			...
			if(p.getThreatCount()>0){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WORLD_TELEPORT_CLIENT, "戰斗狀態不可傳送");
				return;
			}
			if(mapId==p.map.id){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WORLD_TELEPORT_CLIENT, "已經在此場景中");
				return;
			}
			int[] positionArr = getPosition(mapId,p.faction);
			if(positionArr==null){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WORLD_TELEPORT_CLIENT, "此場景不能到達");
				return;
			}
			int x = positionArr[0];
			int y = positionArr[1];
			...
			if(item!=null){
				try {
					...
					for(int id : neutra){
						if(mapId==id){
							for (GameMapExit exit : p.getVMap().exits.values()) {
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WORLD_TELEPORT_CLIENT, 
						MessageFormat.format("您沒有{0}無法前往,請進入充值商店購買", 
