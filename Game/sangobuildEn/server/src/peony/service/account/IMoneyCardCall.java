		...
		if(!success){
			...
			if(p != null){
				...
				if (p.session != null) {
					...
					if (account != null) {
						account.setIMoney(msg.getBalance());
						p.addIntPropertyChangedItem(ChangedItem.IMONEY, account
								.getIMoney() / 100, true, true);
					}
				}
			}
			if(!added){ //如果没有加入到背包中，那么邮寄
				Server.server.getServiceRegistry().getMailService().sendSystemMail(playerId, "系統", "元寶卡", "", 0, item, 1, "IMCARD");
				if(p != null){
					p.message(-1, "你兌換的元寶卡已經郵寄到信箱中,請註意查收.", -1, -1);
				}
			}
		}
	}
