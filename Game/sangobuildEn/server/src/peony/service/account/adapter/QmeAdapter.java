	...
	public static QmeAccount register(String name, String password, String email) throws QmeException {
		...
		try {
            ...
            if (code == 200) {
                ...
                if (secs.length < 3) {
                    throw new QmeException("註冊時發生系統內部錯誤");
                }
                ...
                if (retID > 0) {
                	...
                	switch (retID) {
                	case -1:
                		throw new QmeException("用戶名不合法");
                	case -2:
                		throw new QmeException("包含不允許註冊的詞語");
                	case -3:
                		throw new QmeException("用戶名已經存在");
                	case -4:
                		throw new QmeException("Email格式有誤");
                	case -5:
                		throw new QmeException("Email不允許註冊");
                	case -6:
                		throw new QmeException("該Email已經被註冊");
                	case -7:
                		throw new QmeException("缺少帳號/密碼/email必填欄位");
            		default:
                		throw new QmeException("未定義錯誤");	
                	}
                }
            } else {
	...
	public static QmeAccount login(String name, String password) throws QmeException {
		...
		try {
            ...
            if (code == 200) {
                ...
                String[] secs = result.trim().split(",");
                if (secs.length < 4) {
                    throw new QmeException("登入時發生系統內部錯誤");
                }
                ...
                if (lastSec.startsWith("sms://")) {
                	...
                	try {
                		...
                		throw new QmeException(MessageFormat.format("登入時發生錯誤：{0}", lastSec));
                	}
                	QmeAccount acc = new QmeAccount();
                	acc.name = name;
                	acc.tsi = secs[2];
		...
		try {
            ...
            if (code == 200) {
                ...
                if ("100000015".equals(retCode)) {
                	...
                	throw new QmeException(MessageFormat.format("支付時發生錯誤：{0}", retMsg));
                }
            } else {
                log.info("[QME_PAY]code[" + code + "]ERROR");
                throw new QmeException("支付時發生系統內部錯誤");
            }
		} catch (QmeException qe) {
			throw qe;
        } catch (Exception ex1) {
