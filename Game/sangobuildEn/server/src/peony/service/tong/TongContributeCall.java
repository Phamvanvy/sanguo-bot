...
public class TongContributeCall extends ClientSessionAsyncCall {
	...
	public void run() {
		...
		if(p != null){
			if(money < 5000){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.TONG_CONTRIBUTE_CLIENT, "捐獻金額必須大于5000");
				return;
			}
			TongService tongService = Server.server.getServiceRegistry().getTongService();
			Tong tong = tongService.getPlayerTong(p.id);
			if(tong == null){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.TONG_CONTRIBUTE_CLIENT, "你沒有所屬的幫派");
				return;
			}
...
//				}
				...
				tx.rollback();
				ErrorHandler.sendErrorMessage(session, serial, OpCode.TONG_CONTRIBUTE_CLIENT, "沒有足夠的金錢捐獻");
			}
		}
	}
