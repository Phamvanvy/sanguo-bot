...
public class TongBattleApplyService implements Service, ServiceEventListener {
	...
	public void startup() throws Exception {
		...
		for (final int mapId : controllers.keySet()) {
			...
			if (controller != null) {
				...
				}, getScheduleTime(cal, weekDay, beginTime), ONEWEEK);
				new Timer().schedule(new TimerTask() {
					public void run() {
				...
				}, getScheduleTime(cal, weekDay, endTime), ONEWEEK);
			}
		}
		loadApplys();
	...
	public void tagTongBattle(Player p, int playerId) throws TongBattleException{
		...
		if(tong!=null && tong.getChairmanName().equals(p.name)){
			...
			if(tm.actor!=null && tm.actor.level<TongBattleVMapManager.MINLEVEL)
				throw new TongBattleException(MessageFormat.format("不能標記{0}級以下的軍團成員", TongBattleVMapManager.MINLEVEL));
			tm.battleTag = 1;
		}else{
			throw new TongBattleException("您不是都督不能行使此權利");
		}
	}
	...
	public String getWinner(int mapId){
		...
		if(apply==null){
			...
			String factionName = "";
			try {
				int faction = tong.members.get(0).actor.faction;
	...
	public void abandon(Player p, int tongId) throws TongBattleException{
		if(isWinner(tongId)){
			if(p.map.getId()!=getWinnerMapId(tongId)){
				throw new TongBattleException("沒有權利操作此項");
			}
			int mapId = 0;
			...
			if(isPreBattleSide(tongId)){
				throw new TongBattleException("此時間不能放棄占領");
			}
			owners.remove(mapId);
			TongService tongService = Server.server.getServiceRegistry().getTongService();
			Tong tong = tongService.getTong(tongId);
			Server.server.getServiceRegistry().getChatService().sendWorldMessage(
					MessageFormat.format("{0}軍團都督放棄了原屬于他們的領地{1}", tong.name, getMapName(mapId)));
			Server.server.getServiceRegistry().getChatService().sendGuildSystemMessage(
					MessageFormat.format("都督{0}放棄了原屬于你們的領地{1}", p.name, getMapName(mapId)), tongId);
		}else{
			throw new TongBattleException("您沒有占領該城市");
		}
	}
	...
	public String getCanApplyTime(int mapId){
		...
		if(controller==null)
			return "";
		return MessageFormat.format("{0}申請爭奪的時間:{1}\n{2}城戰開始時間:{3}", controller.name,
	...
	public void apply(Player p, int mapId, int money) throws TongBattleApplyException {
		if (p != null) {
			...
			Tong tong = tongService.getPlayerTong(p.id);
			if (tong == null) {
				throw new TongBattleApplyException("尚未加入軍團");
			} else if (tongService.getPlayerInfo(p.id).duty != TongService.CHAIRMAN) {
				throw new TongBattleApplyException("對不起,您不是都督不能行使此權利");
			}
			if (hasApplyed(tong.id)) {
				throw new TongBattleApplyException("尊敬的都督,您已經報名成功,不能再次報名.");
			}
			if(isWinner(tong.id)){
				throw new TongBattleApplyException("尊敬的都督,您已經占領一個城池,不可報名.");
			}
			if (canApply.get(mapId)==null || !canApply.get(mapId)) {
				throw new TongBattleApplyException("尊敬的都督,當前時間不可以發起對該城市的爭奪");
			}
			if(money<MIN_BATTLEAPPLY_MONEY){
				throw new TongBattleApplyException("參加軍團競价最少需要交納30万軍團資金");
			}
			...
			try {
				...
				throw new TongBattleApplyException("尊敬的都督,您的軍團資金不足");
			}
	...
	public void tongBattleBid(Player p, int mapId, int money) throws TongBattleApplyException{
		if(p!=null){
			if(money<=0){
				throw new TongBattleApplyException("請輸入正确的金額");
			}
			TongService tongService = Server.server.getServiceRegistry().getTongService();
			Tong tong = tongService.getPlayerTong(p.id);
			if (tong == null) {
			...
			if(battleApply==null){
				throw new TongBattleApplyException("尊敬的都督,您尚未申請城戰");
			}
			boolean can = canApply.get(mapId);
			if (!can) {
				throw new TongBattleApplyException("尊敬的都督,當前時間不可以競价");
			}
			try {
				tong.decMoney(money);
				battleApply.addMoney(money);
	...
	protected void notifyBattles(TongBattleApply[] apps, TongBattleApply owner){
		...
		for(TongBattleApply apply : apps){
			...
			service.sendGuildSystemMessage(MessageFormat.format(
					"{0}軍團在今天的攻城競价中獲得最終胜利,將于晚上19點參加{1}爭奪戰", apply.tongName, getMapName(apply.mapId)), tongId);
			service.sendFactionSystemMessage(apply.faction
					, MessageFormat.format("{0}軍團在今天的攻城競价中獲得最終胜利,將于晚上19點參加{1}爭奪戰", apply.tongName, getMapName(apply.mapId)));
		}
		if(owner!=null){
			if(apps.length==2){
				service.sendGuildSystemMessage(MessageFormat.format(
						"{0}軍團及{1}軍團將于19：00對你們的城池發起突襲,請註意防守", apps[0].tongName, apps[1].tongName), owner.tongId);
			}else if(apps.length==1){
				service.sendGuildSystemMessage(MessageFormat.format(
						"{0}軍團將于19：00對你們的城池發起突襲,請註意防守", apps[0].tongName), owner.tongId);
			}
		}
	}
	...
	public void preWin(int mapId, TongBattleApply apply, int type){
		...
		if(type==0){
			chatService.sendGuildSystemMessage(MessageFormat.format(
					"{0}軍團進攻{1}胜利,獲得該城池的占領權", apply.tongName,getMapName(mapId)), apply.tongId);
			chatService.sendFactionSystemMessage(apply.faction, MessageFormat.format(
					"{0}軍團進攻{1}胜利,獲得該城池的占領權", apply.tongName,getMapName(mapId)));
		}else if(type==1){
			chatService.sendGuildSystemMessage(MessageFormat.format(
					"{0}軍團防守{1}胜利,獲得該城池的占領權", apply.tongName,getMapName(mapId)), apply.tongId);
			chatService.sendFactionSystemMessage(apply.faction, MessageFormat.format(
					"{0}軍團防守{1}胜利,獲得該城池的占領權", apply.tongName,getMapName(mapId)));
		}
		if(applys.get(mapId)!=null){
	...
	private void notifyBattle(final TongBattleApply apply){
		new Timer().schedule(new TimerTask(){
			public void run() {
				Server.server.getServiceRegistry().getChatService().sendGuildSystemMessage(MessageFormat.format(
						"軍團\"{0}\"將于19:00參加{1}的爭奪戰", apply.tongName,getMapName(apply.mapId)), apply.tongId);
			}
		}, 55*60*1000l);
		...
		timer.schedule(new TimerTask(){
			public void run() {
				Server.server.getServiceRegistry().getChatService().sendGuildSystemMessage(
						MessageFormat.format("軍團全体勇士,請大家速去{0}的都尉處參加城市爭奪戰", getMapName(apply.mapId)), apply.tongId);
			}
		...
		for(int mapId : applys.keySet()){
			...
			while(it.hasNext()){
				...
				if(!has(battles, a)){
					...
					tong.addMoney(money);
					ChatService chatService = Server.server.getServiceRegistry().getChatService();
					chatService.sendGuildSystemMessage("城戰競標失敗,競標資金已返還", a.tongId);
...
//				list.clear();
	...
	private Date getScheduleTime(Calendar cal, int weekDay, int hour) {
	...
	protected void notifyEnterTongOccupy(VMap map, Player player){
		...
		if(winner!=null){
			String factionName = "";
			try {
				int faction = winner.members.get(0).actor.faction;
				factionName = GameObject.getFactionName(faction);
			} catch (Exception e) {
			}
			ChatService chatService = Server.server.getServiceRegistry().getChatService();
			chatService.sendPrivateShout(player.id, 0xff4700, 11000, player.faction, MessageFormat.format(
					"你已經進入{0}軍團{1}的領土.", winner.name, factionName=="" ? "" : ("("+factionName+")")));
		}
	}
