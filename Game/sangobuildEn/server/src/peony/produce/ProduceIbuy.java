				...
				try {
					...
					if(movePoint > 0){
						try {
							...
							ErrorHandler.sendErrorMessage(session, serial, OpCode.PRODUCE_CLIENT,"行動力不足");
							throw new ShopException("行動力不足"); 
						}
					}
				} catch (NoEnoughSpaceException e) {
//					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial, OpCode.PRODUCE_CLIENT, "您的背包已滿");
					throw new ShopException("背包已滿");
				}
			...
			p.send(packet1);
			Server.server.getEventManager().fireEvent(new ServiceEvent(ServiceEvent.EVENT_PRODUCE,player));
