...
public class PipAnimateSet {
	...
	public int hashCode;
	/**
	 * 会将帧引用的值用short保存; JHKang
	 */
	private boolean useExtFr;
	...
	public void save(DataOutputStream dos, boolean saveFullName) throws IOException {
		...
		if (saveFullName) {
			...
			short head;
			if(useExtFr){//前3为,frameRef的版本号,再三位framePiece的版本号,后10位,帧数
				head = (short) ((version << 10) | frames.size());
				head |= 1<<13;
			} else {
				head = (short) ((version << 14) | frames.size());
			}
			dos.writeShort(head);
		}
		for (int i = 0; i < frames.size(); i++) {
	...
	public HashMap<Point, int[]> reflectNewPieces(PipAnimateSet animateSet, HashSet<Point> usedFrames) throws ColorsExceedException {
	    ...
	    for (Point p1 : usedFrames) {
	        ...
	        int[] newID = addImageToAnimateSetReal(imgData);
	    ...
	    while (candidateImages.size() > 0) {
	        ...
	        if (bestMatch != -1) {
	            ...
	            try {
	                ...
	                if(targetImage.getImgCount()==255){
	                	continue;
	                }
