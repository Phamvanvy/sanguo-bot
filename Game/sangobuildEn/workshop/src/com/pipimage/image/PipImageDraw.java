...
public class PipImageDraw{
    ...
    private ImageData make(PipImagePalette palette, int trans){
        ...
        if (palette != null) {
	        if (trans < 4) {
	        	bufferImg = new ImageData(w, h, 32, palette.createSWTPalette());
	        } else {
	        	bufferImg = new ImageData(h, w, 32, palette.createSWTPalette());
	        }
        	...
        	bufferImg.alphaData = new byte[idata.length];
        	for (int i = 0; i < idata.length; i++) {
        		bufferImg.data[i * 4] = (byte)(idata[i] >> 24);
        		bufferImg.data[i * 4 + 1] = (byte)(idata[i] >> 16);
        		bufferImg.data[i * 4 + 2] = (byte)(idata[i] >> 8);
        		bufferImg.data[i * 4 + 3] = (byte)idata[i];
        		bufferImg.alphaData[i] = (byte)(idata[i] >> 24);
	        }
        }
        return bufferImg;
    }
