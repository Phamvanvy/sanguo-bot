...
public class TileConfigTool implements IMapEditTool {
    ...
    // 设置按钮的文本
    private String[] buttonTexts = { "设置", "清除" ,"1x1", "3x3", "5x5", "7x7", "9x9"};
    private int brushSizeShift[] = new int[]{0, 1, 2, 3, 4};
    private int brushSize[] = new int[]{1, 3, 5, 7, 9};
    // 设置按钮的位置和大小
     ...
     * @param tv 贴图查看器
     */
    public TileConfigTool(MapViewer viewer, byte mask) {
        this.viewer = viewer;
        lastX = -1;
        lastY = -1;
...
//                return;
    ...
    private void response(int x, int y){
     ...
     * 可飞行可行走绘制黄色点框.<br/>
     * 视野遮挡时不绘制表示遮挡 */
    private void drawCellMask(GC gc) {
        ...
        for (; i < map.tileInfo.length; i++) {
            for (int j = 0; j < map.tileInfo[i].length; j++) {
                ...
                if ((value & configMask) == 0) {//没有任何设置,绘制黑色
                }else{//有设置
                	switch (configMask) {
                	case CONFIG_MASK_SKY:
                	case CONFIG_MASK_GROUND:
						if((value & (CONFIG_MASK_SKY|CONFIG_MASK_GROUND)) == (CONFIG_MASK_SKY|CONFIG_MASK_GROUND)){
                		...
                		break;
                	case CONFIG_MASK_SAFE_AREA:
                		if((value & CONFIG_MASK_SAFE_AREA) == CONFIG_MASK_SAFE_AREA){
                			continue;
                		}
                		break;
					default:
	...
	private void drawCoordinate(GC gc) {
    	int cellSize = viewer.getMap().parent.getCellSize();
	...
	private void drawMouseOverRect(GC gc) {
	...
	public void setConfigMask(byte configMaskV) {
		configMask = configMaskV;
	}
}
