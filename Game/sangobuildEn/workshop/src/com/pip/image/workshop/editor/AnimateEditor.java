...
import com.pip.image.workshop.ApplicationActionBarAdvisor;
...
import com.pip.image.workshop.editor.AnimateFrameEditor;
import com.pip.image.workshop.editor.AnimateFrameSelector;
...
public class AnimateEditor extends EditorPart implements ImageViewerListener, IFileModificationListener {
	...
	private Action newFrameAction, dupFrameAction, delFrameAction, hflipFrameAction, vflipFrameAction, moveUpFrameAction, moveDownFrameAction, checkFrameRefAction, cleanPiece;
	...
	public void createPartControl(Composite parent) {
		...
		addImageItem.setImage(ResourceManager.getPluginImage(WorkshopPlugin.getDefault(), "icons/open_large.gif"));
		addImageItem.setText("载入图片");
        ...
        refImageItem.setImage(ResourceManager.getPluginImage(WorkshopPlugin.getDefault(), "icons/open_large.gif"));
        refImageItem.setText("参考图片");
        ...
        mergeItem.setImage(ResourceManager.getPluginImage(WorkshopPlugin.getDefault(), "icons/merge.gif"));
        mergeItem.setText("合并文件");
		...
		enlarge2Item.setImage(ResourceManager.getPluginImage(WorkshopPlugin.getDefault(), "icons/enlarge_large.gif"));
		...
		smallItem.setImage(ResourceManager.getPluginImage(WorkshopPlugin.getDefault(), "icons/enlarge_large.gif"));
		smallItem.setText("整体缩小");
	...
	private void createActions() {
		...
		newImageAction = new Action("载入文件") {
			public void run() {
				onLoadFile();
			}
		};
...
//					contentChanged(imageViewer);
   ...
   private void onVflipFrame() {
	...
	protected void reloadAnimateFile() {
		try{
			boolean ret = true;//MessageDialog.openConfirm(getSite().getShell(), "确认", "cts文件已经被修改,是否重新加载?");
			if(ret){
				animateSet = new PipAnimateSet();
				animateSet.load(animateFile);		
