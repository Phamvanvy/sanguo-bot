                ...
                implements Runnable
{
  ...
  public CMCCSplashCanvas(MIDlet midlet, Displayable d) {
    ...
    deviceScreenWidth = World.viewWidth;
    deviceScreenHeight = World.viewHeight;
...
//               (deviceScreenHeight - screenHeight)/2);
   g.setClip(0,0,screenWidth,screenHeight);
    ...
    if(showGameLogo){
      ...
      g.drawImage(gamelogo,screenWidth/2,screenHeight/2 - gamelogo.getHeight() / 2, Graphics.HCENTER | Graphics.TOP);
      return;
    }
...
//        Display.getDisplay(theMidlet).setCurrent(this);
      ...
      if(backCanvas != null){
        ...
        new Thread((World)backCanvas).start();
        clear();
      ...
      if (logoInfo[index][i * 2] != -1) {
        ...
        if (logoInfo[index][i * 2] == 5) {
          ...
          if (logoInfo[index][i * 2 + 1] == heioff1[0])
            ...
            g.drawImage(logo[logoInfo[index][i * 2]], www + xoff, 100 + yoff - logo[logoInfo[index][i * 2]].getHeight() / 2, g.HCENTER | g.TOP);
          else if (logoInfo[index][i * 2 + 1] == heioff1[2])
            g.drawImage(transImg2, www + jiaozhengx[i] + xoff, 72 + yoff, g.HCENTER | g.TOP);
        }
        else
          g.drawImage(logo[logoInfo[index][i * 2]], www + xoff,
                      logoInfo[index][i * 2 + 1] + 100 + yoff - logo[logoInfo[index][i * 2]].getHeight() / 2, g.HCENTER | g.TOP);
      }
  }
