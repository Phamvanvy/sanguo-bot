...
public class DiscountShopLoader {
	public DiscountShopLoader(File file,int iMoneyType,int IMONEY_TYPE_CMCC) throws Exception{
		SAXReader reader = new SAXReader();
	    Document doc = reader.read(file);
	    loadDiscountShop(doc,iMoneyType,IMONEY_TYPE_CMCC);
	}
	
	public void loadDiscountShop(Document doc,int iMoneyType,int IMONEY_TYPE_CMCC){
		...
		for(Iterator i=root.elementIterator("group");i.hasNext();){
            ...
            DiscountShopGroups.addGroup(group);
        }
	}
	 ...
	 private IStoreItem[] loadItems(Element node,int iMoneyType,int IMONEY_TYPE_CMCC){
	        ...
	        for(Iterator i=node.elementIterator("Item");i.hasNext();){
	            ...
	            if(item!=null){
	                ...
	                int imoneyPrice = Integer.parseInt(Utils.getWholeDataPrice(n.attributeValue("imoney")));
	                imoneyPrice = imoneyPrice > 0?imoneyPrice:price;
	                int jmoneyPrice = Integer.parseInt(n.attributeValue("jmoney"));
	                int credit = Integer.parseInt(n.attributeValue("credit"));
	                ...
	                storeItem.iPrice = imoneyPrice;
	                storeItem.jPrice = jmoneyPrice;
