...
public class DownloadPointShopLoader {
	public DownloadPointShopLoader(File file,int iMoneyType,int IMONEY_TYPE_CMCC) throws Exception{
		SAXReader reader = new SAXReader();
	    Document doc = reader.read(file);
	    loadDownloadPointShop(doc,iMoneyType,IMONEY_TYPE_CMCC);
	}
	
	private void loadDownloadPointShop(Document doc,int iMoneyType,int IMONEY_TYPE_CMCC){
		Element root = doc.getRootElement();
	    Element tmpLimit = root.element("downloadPointUseLimit");
	    int useMax = Integer.parseInt(tmpLimit.attributeValue("limit"));
	    DownloadPointShopGroups.setUseMax(useMax);
	    for(Iterator i=root.elementIterator("group");i.hasNext();){
	        ...
	        DownloadPointShopGroups.addGroup(group);
	    }
	}
