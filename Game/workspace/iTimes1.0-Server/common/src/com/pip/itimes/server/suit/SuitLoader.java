...
public class SuitLoader{
    public SuitLoader(File file) throws Exception{
    ...
    private void loadSuits(Document doc){
        ...
        for(Iterator i=root.elementIterator("suit");i.hasNext();){
            ...
            Element equips = suitNode.element("equips");
            
            if(equips != null){
                for(Iterator j = equips.elementIterator("equip"); j.hasNext();){
                    Element equipNode = (Element)j.next();

                    Attribute attrEquip = equipNode.attribute("itemId");
                    int itemId = Integer.parseInt(attrEquip.getValue());
                    suit.addEquip(itemId);
                    
//                	IEquipment item = (IEquipment) Items.getTemplate(itemId)
//    				.newInstance();
                	
                	String itemName = Items.getTemplate(itemId).getName();
                	itemName = ItemUtils.splitItemDescSting(itemName);
                	suit.addEquipName(itemName, itemId);
