...
public class LyricsConfig {
	static private Logger log = Logger.getLogger(LyricsConfig.class);
    ...
    public static ArrayList<Lyric> lyrics;
    ...
    public static Hashtable<Integer, Lyric> oldSing = new Hashtable<Integer, Lyric>();		//记录着唱过的歌
...
//    public static final long HOURTIME = 60 * 60 * 1000;	//一小时
    ...
    public static final int GIFT_ID = 201124;		//掉落组ID
    public static final int GIFT5_ID = 201021;		//前5名送的物品ID
    ...
    public static void startCurrent(){
    	Lyric lyric = (Lyric)lyrics.get(currentIndex);
		    		...
		    		for(int i=0; i<list.size(); i++){
		    			...
		    			if(player != null){
			    				...
			    				if(rndValue == 1){
			    					...
			    					if(iit != null){
				        				...
				        				Server.instance.mailService.sendMail(player.getId(), player.getPlayerName(), -1, "系统",
				        						iit.getName() + "*" + 1, "您在对歌词活动中获得了一个" + iit.getName() + "！这可是能一次性获得10000点荣誉的荣誉包啊！", att, 0, true);
				        				Server.instance.chatService.sendRoarMessage( -1, "狮子吼", "哇~！这不是真的吧！" + player.getPlayerName() + "在我爱对歌词中获得了一个" + iit.getName() + "！这可是能一次性获得10000点荣誉的荣誉包啊！", true, 10, 50, Utils.CLR_RED_ROAR, Utils.WORLD, (short)0);
				        				sendCredit = true;
				        				log.info("LyricsConfig sendGiftCredit to playerId[" + player.getId()
			    				...
			    				if(player.getPlayerName().equals(right5Player.get(n))){
			    					...
			    					if(rndValue == 100){
			    						...
			    						if(iit != null){
			    							...
			    							Server.instance.mailService.sendMail(player.getId(), player.getPlayerName(), -1, "系统",
			    									iit.getName() + "*" + count, "您在我爱对歌词活动中进入了前五名，这是额外奖励，请再接再厉哦~", att, 0, true);
			    							log.info("LyricsConfig sendGift5 to playerId[" + player.getId()
			    									+ "] playerName[" + player.getPlayerName() + "] get ItemId[" +
	    					...
	    					if(iit != null){
		    					byte[] att = ItemUtils.item2dbAttachment(iit, 1);
		        				Server.instance.mailService.sendMail(player.getId(), player.getPlayerName(), -1, "系统",
		        						iit.getName() + "*" + 1, "您在对歌词活动中对中了歌词，这是奖励，请再接再厉哦~", att, 0, true);
		        				log.info("LyricsConfig sendGift to playerId[" + player.getId()
		        						+ "] playerName[" + player.getPlayerName() + "] get ItemId[" +
		        						iit.getItemId() + "] ItemName [" + iit.getName() + "] ItemCount[" + 1 + "]");
	    					}
		    			}
    		...
    		if(currentDayTime >= hourTime){
    			if((hourCount + 1) * HOURTIME + dayStartTime >= dayEndTime){
    				if(dayStartTime + 24 * 60 * 60 * 1000 >= endTime){
    					sb.append("本次的记歌词活动于今天告一段落，尽请期待下次更精彩。");
    				}else{
    ...
    public static void startNoticeMessage(){
    	state = WAITACTION;
    	Server.instance.chatService.sendRoarMessage( -1, "狮子吼", "5分钟后开始“我爱记歌词”比赛，3分钟内在世界聊里接对歌词就有丰厚的奖励哦，前5名还有额外奖励哦。", true, 10, 50, Utils.CLR_RED_ROAR, Utils.WORLD, (short)0);
    }
	...
	static public void action(long now){
		...
		//记歌词活动前五分钟的提示
        if(LyricsConfig.state == LyricsConfig.NOTICEMESSAGE && now < LyricsConfig.dayEndTime - TIME * hourTime && now + TIME5 >= LyricsConfig.dayStartTime){
        	log.info("state[NOTICEMESSAGE]:startNoticeMessage()");
        	LyricsConfig.startNoticeMessage();
        }else if(LyricsConfig.state == LyricsConfig.NOTICEMESSAGE && now >= LyricsConfig.dayStartTime){
        	log.info("state[NOTICEMESSAGE]:state = WAITACTION");
        	state = WAITACTION;
        }else if(LyricsConfig.state == LyricsConfig.WAITACTION && now >= LyricsConfig.dayStartTime + hourCount * HOURTIME && now < LyricsConfig.dayEndTime){
        	log.info("state[WAITACTION]:startAction()");
        	LyricsConfig.startAction();
        }else if(LyricsConfig.state == LyricsConfig.ACTIONSTART && !LyricsConfig.endAction && now >= LyricsConfig.endActionTime){
        	log.info("state[ACTIONSTART]:nextAction()");
        	LyricsConfig.nextAction();
        }else if(LyricsConfig.state == LyricsConfig.ACTIONEND && now < LyricsConfig.dayEndTime - TIME * hourTime && now + TIME5 >= LyricsConfig.dayStartTime + hourCount * HOURTIME){
