...
public class RiddlesConfig {
	static private Logger log = Logger.getLogger(RiddlesConfig.class);
...
//    public static final long HOURTIME = 60 * 60 * 1000;	//一小时
    ...
    public static final int GIFT_ID = 201145;		//豆沙元宵
    public static final int FIRST_ID = 201147;		//玫瑰元宵 第一名获得物品
    public static final int GIFT5_ID = 201146;		//前5名送的物品ID 黑芝麻元宵 除第一名
		    		...
		    		for(int i=0; i<list.size(); i++){
		    			...
		    			if(player != null){
			    				...
			    				if(rndValue == 1){
			    					...
			    					if(iit != null){
				        				...
				        				Server.instance.mailService.sendMail(player.getId(), player.getPlayerName(), -1, "系统",
				        						iit.getName() + "*" + 1, "您在中国好声音答题活动中获得了一个" + iit.getName() + "！这可是能一次性获得10000点荣誉的荣誉包啊！", att, 0, true);
				        				Server.instance.chatService.sendRoarMessage( -1, "狮子吼", "哇~！这不是真的吧！" + player.getPlayerName() + "在中国好声音答题活动中获得了一个" + iit.getName() + "！这可是能一次性获得10000点荣誉的荣誉包啊！", true, 10, 50, Utils.CLR_RED_ROAR, Utils.WORLD, (short)0);
				        				sendCredit = true;
				        				log.info("RiddlesConfig sendGiftCredit to playerId[" + player.getId()
			    			...
			    			for(int n=0; n<right5Player.size(); n++){
			    				if(player.getPlayerName().equals(right5Player.get(n))){
			    					if(n == 0){
		    							...
		    							Server.instance.mailService.sendMail(player.getId(), player.getPlayerName(), -1, "系统",
		    									iit.getName() + "*" + count, "您在中国好声音答题活动中获得了第一名，这是额外奖励，请再接再厉哦~", att, 0, true);
		    							log.info("RiddlesConfig sendGift5 to playerId[" + player.getId()
		    									+ "] playerName[" + player.getPlayerName() + "] get ItemId[" +
			    						...
			    						byte[] att = ItemUtils.item2dbAttachment(iit, count);
			    						Server.instance.mailService.sendMail(player.getId(), player.getPlayerName(), -1, "系统",
			    								iit.getName() + "*" + count, "您在中国好声音答题活动中获得了前五名，这是额外奖励，请再接再厉哦~", att, 0, true);
			    						log.info("RiddlesConfig sendGift5 to playerId[" + player.getId()
			    								+ "] playerName[" + player.getPlayerName() + "] get ItemId[" +
			    								iit.getItemId() + "] ItemName [" + iit.getName() + "] ItemCount[" + count + "]");
			    					}
							...
							if(iit != null){
		    					byte[] att = ItemUtils.item2dbAttachment(iit, 1);
		        				Server.instance.mailService.sendMail(player.getId(), player.getPlayerName(), -1, "系统",
		        						iit.getName() + "*" + 1, "您在中国好声音答题活动中猜中了答案，这是奖励，请再接再厉哦~", att, 0, true);
		        				log.info("RiddlesConfig sendItem to playerId[" + player.getId()
		        						+ "] playerName[" + player.getPlayerName() + "] get ItemId[" +
		        						iit.getItemId() + "] ItemName [" + iit.getName() + "] ItemCount[" + 1 + "]");
							}
		    			}
    		...
    		if(currentDayTime >= hourTime){
    			if((hourCount + 1) * HOURTIME + dayStartTime >= dayEndTime){
    				if(dayStartTime + 24 * 60 * 60 * 1000 >= endTime){
    					sb.append("本次的中国好声音答题活动于今天全部结束了，明年再会吧。");
    				}else{
    					sb.append("今日的活动已经结束，明天晚8点继续哦~");
    				}
    			}else{
    				sb.append("本轮活动结束，下一个整点继续。");
    			}
    		}else{
	    		sb.append("下面开始下一道中国好声音答题。");
	    		setNextRiddle();
    ...
    public static void playerChat(String msg, WorldPlayer player){
    	...
    	Riddles riddle = (Riddles)riddles.get(currentIndex);
    	if(msg.equals(riddle.getAnswer()) || msg.equalsIgnoreCase(riddle.getAnswer())){
    ...
    public static void startNoticeMessage(){
    	state = WAITACTION;
    	Server.instance.chatService.sendRoarMessage( -1, "狮子吼", TIME5 / 60000 + "分钟后开始“中国好声音答题”活动，" + "每道问题出现后 " + TIME / 60000 + "分钟内在世界聊里喊出正确选项(A.B.C.D单选)就有丰厚的奖励哦，前5名还有额外奖励哦。", true, 10, 50, Utils.CLR_RED_ROAR, Utils.WORLD, (short)0);
    }
...
//		log.info("dayStartTime:" + new Date(dayStartTime).toString() + " dayEndTime:" + new Date(dayEndTime));
        ...
        if(RiddlesConfig.state == RiddlesConfig.NOTICEMESSAGE && now < RiddlesConfig.dayEndTime - TIME * hourTime && now + TIME5 >= RiddlesConfig.dayStartTime){
        	log.info("state[NOTICEMESSAGE]:startNoticeMessage()");
        	RiddlesConfig.startNoticeMessage();
        }else if(RiddlesConfig.state == RiddlesConfig.NOTICEMESSAGE && now >= RiddlesConfig.dayStartTime){
        	log.info("state[NOTICEMESSAGE]:state = WAITACTION");
        	state = WAITACTION;
        }else if(RiddlesConfig.state == RiddlesConfig.WAITACTION && now >= RiddlesConfig.dayStartTime + hourCount * HOURTIME && now < RiddlesConfig.dayEndTime){
        	log.info("state[WAITACTION]:startAction()");
        	RiddlesConfig.startAction();
        }else if(RiddlesConfig.state == RiddlesConfig.ACTIONSTART && !RiddlesConfig.endAction && now >= RiddlesConfig.endActionTime){
        	log.info("state[ACTIONSTART]:nextAction()");
        	RiddlesConfig.nextAction(true);
        }else if(RiddlesConfig.state == RiddlesConfig.ACTIONEND && now < RiddlesConfig.dayEndTime - TIME * hourTime && now + TIME5 >= RiddlesConfig.dayStartTime + hourCount * HOURTIME){
