 ...
 * @version 1.0
 */
public class Ai90003_1 extends BaseMonsterAI{
    public static final IItem life = Items.getTemplate(6).newInstance(); //强效生命药剂s
    public static final Ability ability = Ability.getAbility(4); //缤纷连击4
    ...
    public boolean action(BattleSprite bs, int index, BattleSprite[] our,
        ...
        if(!bs.canAction()){
            ...
            if(bs.hp<=getPercentValue(bs.attributes[BattleSprite.ATTR_HPMAX],20)){  //生命低于20％，使用强效生命
                if(!life_used){
                    life_used = true;
            ...
            if(Utils.hit(rnd,40,100)&&bs.mp>=ability.getMpUse(bs.level, bs.mp)){ //40％使用缤纷连击4
