 ...
 * 手机钱包普通8元续费产品。
 */
public class UMPay_Product_300 implements ProductHandler {
	private static Logger log = Logger.getLogger(UMPay_Product_300.class);
	private Server server;
	
	public UMPay_Product_300(Server s) {
		server = s;
	}
	...
	public int placeOrder(String productID, String phone, StringBuffer remarkBuf) {
		...
		// 创建订单
		Fee fee = server.newFee(a.getId(), Const.UNIPAY_FEE_300, getOrderType());
        if (fee == null) {
            return -1;
        } else {
    		String retRemark = Const.UNIPAY_REMARK30.replace("%account%", a.getUserName());
	...
	public String getWAPAddress() {
		return Const.UNIPAY_ORDER_URL_30;
	}
}
