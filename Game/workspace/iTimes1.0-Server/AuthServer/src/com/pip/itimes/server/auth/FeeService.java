...
public class FeeService {
    ...
    private JettyServer jServer;
    private AccountService accountService;
    private ConnectService connectService;
    private Set trustips = new HashSet();

    public FeeService(FeeDao feeDao, Configuration conf,AccountService accountService,JettyServer jServer) throws Exception {
        this.feeDao = feeDao;
        this.jServer = new JettyServer(conf.getString("feehttpurl"),
                                       conf.getInt("feehttpport"), 3, 50);
    ...
    private boolean updateAccount(Account newAcc, Fee newFee) {
        try {
            ...
            if (newAcc.getiMoney() != 0) {
                UWAPSegment seg = new UWAPSegment(ServerConstants.SYNC_IMONEY);
...
//                if(acc.getMonthFee()>=Const.MONTH_MAX||acc.getSubscribeStatus()==Account.SUBSCRIBED){
//                    seg.writeBoolean(false);
//                }else{
//                    seg.writeBoolean(true);
//                }
//                seg.writeBoolean((acc.getMonthFee()<Const.MONTH_MAX)||(acc.getSubscribeStatus()==Account.SUBSCRIBED));
    ...
    class BillingServlet extends HttpServlet {
        protected void service(HttpServletRequest request,
            ...
            if ("findAccount".equals(cmd)) {
                ...
                int accountId = Integer.parseInt(request.getParameter(
                        "accountId"));
                ...
                Account newAcc = (Account) Const.stringToObject(request.
                        getParameter("newAcc"));
                Fee newFee = (Fee) Const.stringToObject(request.getParameter(
                        "newFee"));
                ...
                if (!a.getPassword().equals(password)) {
                    if (a.getPassword().startsWith("#")) {
                        if (!SecurityUtils.verifyMD5(password,
                            ...
                            log.info("Login Error Name[" + name +
                                     "]Pass[" + password + "]");
                            response.setStatus(HttpServletResponse.SC_OK);
                            response.getWriter().println("1");
                        ...
                        log.info("Login Error Name[" + name + "]Pass[" +
                                 password + "]");
                        response.setStatus(HttpServletResponse.SC_OK);
                        response.getWriter().println("1");
                        response.getWriter().println("帐号或者密码错误");
