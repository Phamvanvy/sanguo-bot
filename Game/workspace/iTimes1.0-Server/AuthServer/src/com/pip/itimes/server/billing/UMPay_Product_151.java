...
public class UMPay_Product_151 implements ProductHandler {
	...
	public int placeOrder(String productID, String phone, StringBuffer remarkBuf) {
		...
		// 检查：一个手机号只能为1个帐号订购包月
		if (server.findAccountBySubscribePhone(phone) != null) {
			return -2;
		}

		// 创建订单
		Fee fee = server.newFee(a.getId(), 0, getOrderType() + phone);
        if (fee == null) {
            return -1;
        } else {
        	String retRemark = Const.UNIPAY_REMARK151.replace("%account%", a.getUserName());
	...
	public boolean fulfilOrder(int orderID, String phone, StringBuffer remarkBuf) {
		// 检查手机号是否已经绑定了帐号了
		Account a = server.findAccountBySubscribePhone(phone);
		if (a != null) {
			return false;
		}
		...
		a.setiMoney(0);
		a.setSubscribeStatus(Account.SUBSCRIBED);
		a.setSubscribeBill(fee.getId());
		a.setSubscribePhone(phone);
		fee.setCharged(true);
