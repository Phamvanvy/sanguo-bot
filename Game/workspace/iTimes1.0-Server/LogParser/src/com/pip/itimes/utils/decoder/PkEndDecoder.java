...
import com.pip.itimes.utils.action.PkEndAction;
 ...
 * @version 1.0
 */
public class PkEndDecoder
    extends RegexLineDecoder {
    public PkEndDecoder() {
        super("ID\\[(\\d+)\\]Money\\[(\\d+)\\]Dest\\[(\\d+)\\]Money\\[(\\d+)\\]Wager\\[(\\d+)\\]BEGIN");
        //ID[45555]Money[8871]Dest[47666]Money[10791]Wager[0]END
    }


    protected PkEndAction createAction(Matcher matcher, Date date) {
        ...
        return new PkEndAction(source,dest,sourceMoney,destMoney,wager,date);
    }
}
