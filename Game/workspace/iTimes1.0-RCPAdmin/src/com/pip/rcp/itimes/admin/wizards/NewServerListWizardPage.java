...
public class NewServerListWizardPage extends WizardPage{
    ...
    public NewServerListWizardPage(){
        super("新建服务器");
        setTitle("请输入服务器信息");
        setDescription("");
    }
    ...
    public boolean makeInputData(){
        ...
        try{
            ...
            serverList.setIp("" + ip_1 + "." + ip_2 + "." + ip_3 + "." + ip_4);
        ...
        if(result){
            ...
            if(desc.trim().length() == 0 || user.trim().length() == 0 || password.trim().length() == 0){
                ...
                serverList.setDesc(desc);
                serverList.setUser(user);
                serverList.setPassword(password);
            }
        }
