...
public class Wulin_DataFetcher implements DataFetcher {
	...
	public void run() {
		try {
			...
			progressText = "正在查询数量";
			String condition = (dateType == 0 ? "createtime" : "lastlogintime") + " between ? and ? and level >= " + minLevel;
			Object[] params = new Object[] { startDate, endDate };
			String sql = "select count(1) from tbl_userdata where " + condition;
			...
			while (start < count) {
				...
				sql = "select id, accountid, playername, level, mapid, x, y, sex, exp, money, tongid, createtime, lastlogintime, credit, leavepoints, " +
					"basicitems, pets, metaitems, equipments, usedequipments, friends, abilitypoints, point, petid, restvalue, addedgridsize, " +
					"abilitytimes, valid, feats, banksize, bankitems, unionid, leveluppoint, laststation, tongmoney, jbcount, tongcredit, " + 
					"foes, unioncredit, kmoney, achievecount,transferLevel,bablecredit,vipcardtype,imoneypoint,statistics from tbl_userdata where " + 
				...
				sb.append("select id, current, finished from tbl_task where id in (");
				...
				sb.append("select playerid, itemid, type, imoney, giftflag from tbl_ibuy where playerid in (");
				for (int i = 0; i < ids.length; i++) {
					if (i > 0) {
						sb.append(',');
					}
				...
				// 读取这些用户的心法数据
				sb = new StringBuilder();
				sb.append("select id,hanatk,yanatk,shaatk,handef,yandef,shadef,main,lastdoubletrade from tbl_xinfa where id in (");
				for (int i = 0; i < ids.length; i++) {
					if (i > 0) {
						sb.append(',');
					}
