...
public class ITimes_DataFetcher implements DataFetcher {
	...
	public void run() {
		try {
			...
			while (start < count) {
				...
				sql = "select id, accountid, playername, level, mapid, x, y, sex, exp, moeny, tongid, createtime, lastlogintime, credit, leavepoints, " +
					"basicitems, pets, metaitems, equipments, usedequipments, friends, abilitypoints, point, petid, addedgridsize, " +
					"abilitytimes, valid, houselevel, face, abilities, techskills, petsize, kills, sneaks, arenalevel, arenapoint, arenalevel2, "  +
					"arenalevel3, camp, campwin, camplost, campcredit, roleface from tbl_userdata where " + 
				...
				sb.append("select playerid, itemid, type, imoney, giftflag, level from tbl_ibuy where playerid in (");
				for (int i = 0; i < ids.length; i++) {
					if (i > 0) {
						sb.append(',');
					}
