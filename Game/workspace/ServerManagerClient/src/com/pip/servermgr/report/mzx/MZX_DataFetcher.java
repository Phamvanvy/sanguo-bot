...
public class MZX_DataFetcher implements DataFetcher {
	...
	public void run() {
		try {
			...
			while (start < count) {
				...
				sql = "select p.id, p.accountid, p.name, p.mapid, p.x, p.y, p.level, p.exp, p.sex, p.faction, p.tong, p.money, p.lastLoginTime, " +
					"p.lastLogoutTime, p.createTime, p.exist, pe.horsebag, pe.petbag, p.shenfa, p.lidao, p.gengu, p.tizhi, " +
					"pe.bags, pe.skills, p.equs, pe.tasks from player p, player_ext pe where p.id = pe.playerid and " +
				...
				sb.append("select playerid, itemtemplateid, imoney, boundimoney, level from ibuy where playerid in (");
