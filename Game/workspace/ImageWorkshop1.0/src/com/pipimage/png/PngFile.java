...
public class PngFile {
    ...
    private void writeIDATChunk(DataOutputStream os, boolean bestCompress) throws IOException {
    	int totalSize = scanlines.size();
    	if (totalSize > 0) {
    		totalSize *= scanlines.get(0).length;
    	}
    	if (totalSize < 32 * 1024 * 1024) {
    		...
    		// Special process for large images(multiple blocks)
    		ByteArrayOutputStream zipBytes = new ByteArrayOutputStream();
    		ZOutputStream zout = new ZOutputStream(zipBytes, JZlib.Z_BEST_COMPRESSION, 12);
    		for (int i = 0; i < scanlines.size(); i++) {
    			byte[] scanline = (byte[])scanlines.get(i);
    			zout.write(0);
    			writeScanline(scanline, zout);
    			if (zipBytes.size() > 64 * 1024) {
    				// 64k for each block
    				PngTrunk trunk = new PngTrunk("IDAT", zipBytes.toByteArray());
    		        zipBytes.reset();
    		        trunk.writeTrunk(os);
    			}
    		}
