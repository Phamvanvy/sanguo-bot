...
public class ImageUtil {
	 ...
	 * @param frame1
	 * @param frame2
	 * @param offset 输出参数，用来保存最佳匹配的偏移量（frame2相对于frame1）
	...
	public static double compareFrame(int[][] frame1, int[][] frame2, int[] offset) {
		...
		for (int xoff = xoffmin; xoff <= xoffmax; xoff++) {
			for (int yoff = yoffmin; yoff <= yoffmax; yoff++) {
				double value = compareFrame(frame1, frame2, xoff, yoff);
				if (value > maxMatch) {
					maxMatch = value;
					offset[0] = xoff;
					offset[1] = yoff;
	 ...
	 */
	public static double compareFrame(int[][] frame1, int[][] frame2, int xoff, int yoff) {
		int h1 = frame1.length;
