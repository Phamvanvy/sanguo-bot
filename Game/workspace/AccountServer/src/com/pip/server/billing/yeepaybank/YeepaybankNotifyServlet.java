...
public class YeepaybankNotifyServlet extends HttpServlet {
	private static final Logger log = Logger.getLogger(YeepaybankNotifyServlet.class);
	...
	static String callbackURL = "http://218.206.80.188/umpayfee/yeepaybank_result.do";
	...
	public YeepaybankNotifyServlet(Server s, PayInfoDAO dao) {
	    this.server = s;
		this.dao = dao;
	}
...
//        String rq_CardNo = request.getParameter("rq_CardNo");
        ...
        log.info("[yeepaybank_notify]orderid[" + r6_Order + "]amount[" + r3_Amt + 
		...
		try {
			...
			if (pi == null) {
			    // 订单不存在
			    tx.rollback();
			    log.info("[yeepaybank_notify] order not found.");
			    errorcode = 2;
			} else if (!pi.isValid()) {
			    // 订单存在，且尚未成功，则标记为成功
			    int realAmount = (int)Double.parseDouble(r3_Amt);
...
//		        pi.setI_sum(realAmount * 360 );
		        ...
		        try {
		            ...
		            log.info("[yeepaybank_notify]orderid[" + r6_Order + "] add i money success.");
		            pi.setAddIFail(false);
		        } catch (Exception e) {
		            log.info("[yeepaybank_notify]orderid[" + r6_Order + "] add i money fail.");
		        }
    ...
    private int addIMoney(PayInfo order) throws Exception {
        ...
        Fee fee = server.newFee(order.getUserName(), (int)(order.getI_sum() * 100 *
        		ConstYeepay.randomGiftRatio(server, order)), "YEEPAYBK_" + order.getMoney());
