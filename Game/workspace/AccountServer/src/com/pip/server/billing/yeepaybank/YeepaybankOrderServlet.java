...
public class YeepaybankOrderServlet  extends HttpServlet {
	private static final Random RND = new Random();
	private static Logger log = Logger.getLogger(YeepaybankOrderServlet.class);
...
//	public static String TEST_MerchantKEY = "8UPp0KE8sq73zVP370vko7C39403rtK1YwX40Td6irH216036H27Eb12792t";
	...
	public YeepaybankOrderServlet(Server s, PayInfoDAO dao) throws Exception{
	    server = s;
		this.dao = dao;
	}
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
        ...
        String channel = request.getParameter("channel");
         if (name == null) {
            log.info("[yeepaybank_order]accountid[" + id + "]amount[" + amount + "]gamecode[" + gameCode + 
                    "]banktype[" + banktype + "]");
        } else {
            log.info("[yeepaybank_order]accountname[" + name + "]amount[" + amount + "]gamecode[" + gameCode + 
            		"]banktype[" + banktype + "]");
        }
        ...
        if(banktype ==null ){
        	log.info("[yeepaybank_order] 缺少交易银行参数");
            out.println("6");
        ...
        if(banktype.equals("icbc")){
        	banktype = "ICBC-WAP";
        }else if(banktype.equals("cmb")){
        	banktype = "CMBCHINA-WAP";
        }else{
        	log.info("[yeepaybank_order] 不支持的银行类型");
            out.println("7");
        ...
        if (acc == null) {
            log.info("[yeepaybank_order] 帐号不存在");
            out.println("1");
            out.println("帐号不存在");
            return;
        }
        ...
        try{
            ...
            dao.create(pi);
            tx.commit();
            log.info("[yeepaybank_order]payid[" + orderId + "] created ok");
            insertok = true;
        } catch(Exception ex) {
            log.error(ex,ex);
            tx.rollback();
        ...
        if (!insertok) {
            log.info("[yeepaybank_order] 创建订单失败");
            out.println("3");
            out.println("创建订单失败");
            return;
        }
        ...
        // 向易宝支付平台发起支付请求
        String notifyURL = server.getServerURL() + "/yeepaybank_notify";
        ...
        try {
            ...
            if (code == 200) {
                ...
                out.println(0);
                out.println(result);
                log.info("[yeepaybank_order] " + "0");
                YeepaybankNotifyServlet.orderMap.put(orderId, returnhttp==null?"":returnhttp);
            } else {
                log.info("[yeepaybank_order] code=" + code);
                out.println("4");
