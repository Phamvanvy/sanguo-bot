 ...
 * 手机钱包普通2元续费产品。
 */
public class UMPay_Product_020 implements ProductHandler {
	private static Logger log = Logger.getLogger(UMPay_Product_020.class);
	private Server server;
	
	public UMPay_Product_020(Server s) {
		server = s;
	}
...
//		}
		...
		// 创建订单
		Fee fee = server.newFee(a.getName(), Const.UNIPAY_FEE_020, getOrderType());
        if (fee == null) {
            return -1;
        } else {
    		String retRemark = Const.UNIPAY_REMARK2.replace("%account%", a.getName());
