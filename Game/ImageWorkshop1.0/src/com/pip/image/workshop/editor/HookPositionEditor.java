...
public class HookPositionEditor extends EditorPart implements ImageViewerListener, FramePieceSelectedListener, IFileModificationListener{
	...
	protected ImageDrawCache imageCache = new ImageDrawCache(1000);
...
//		});
		...
		Button changeCTSBtn = new Button(positionOps, SWT.PUSH);
		changeCTSBtn.setText("更换CTS");
		changeCTSBtn.setToolTipText("修改挂接的CTS文件");
		changeCTSBtn.addSelectionListener(new SelectionAdapter(){
			public void widgetSelected(SelectionEvent e) {
				changeCTS();
			}
		});
		
		Button resetLayerBtn = new Button(positionOps, SWT.PUSH);
		resetLayerBtn.setText("自动调层");
		resetLayerBtn.setToolTipText("根据CTS里的层级自动设置挂接点层级");
		resetLayerBtn.addSelectionListener(new SelectionAdapter(){
			public void widgetSelected(SelectionEvent e) {
				autoSetLayer();
			}
		});
	...
	protected void hookSelected() {
		...
		for (int j = 0; j < idxes.length; j++) {
			...
			for(int i=0; i<curFrame.getPieceCount(); i++){
				...
				if(piece.getFrame() == 0xFFFF && piece.getImageID() == hook.getImageID()){
	...
	synchronized protected void removeHook() {
		...
		for(int i=0; i<animateSet.getFrameCount(); i++){
			...
			for(int j=0; j<frame.getPieceCount(); j++){
				...
				if(pieceInFrame.getFrame() == 0xFFFF && pieceInFrame.getImageID() == piece.getImageID()){
	...
	private void createTopLeft(Composite parent){
		...
		frameEditor.setImageCache(imageCache);
	}
...
//			String hookFilePath = "E:\\workspace\\ImageWorkshop1.0\\equipEditorDev\\male3.hk";
			...
			SWTUtils.showError(getSite().getShell(), "错误", "初始化错误。", e);
		}
	}
...
//	@Override
	...
	private void changeCTS() {
		FileDialog fdlg = new FileDialog(getSite().getShell(), SWT.OPEN);
		fdlg.setFilterExtensions(new String[] { "*.cts" });
		fdlg.setFilterNames(new String[] { "动画文件(*.cts)" });
		String restrictDir = new File(hookFilePath).getParentFile()
				.getAbsolutePath();
		fdlg.setFilterPath(restrictDir);
		String file = fdlg.open();
		if (file != null) {
			File ctsFile = new File(file);
			if (!ctsFile.getParentFile().equals(new File(hookFilePath).getParentFile())) {
				MessageDialog.openError(getSite().getShell(), "错误", "HK文件和CTS文件必须在同一个目录。");
				return;
			}
			File hookFile = new File(hookFilePath);
			bodyDef = new BodyDef();
			try {
				...
				MessageDialog.openError(getSite().getShell(), "错误", "重加载素体文件失败。");
				return;
			}
			...
			try {
				...
				MessageDialog.openError(getSite().getShell(), "错误", "加载CTS文件失败。");
				return;
			}
			animateSet = pas;
			bodyDef.embedHookPieces(animateSet);
	...
	/**
	 * 根据cts里的pip图片的层级自动设置挂接点的层级。
	 */
	private void autoSetLayer() {
		if (hooks.size() == 0 || animateSet.getFileCount() == 0) {
			MessageDialog.openInformation(getSite().getShell(), "信息", "没有挂接点或动画文件中没有PIP文件。");
			return;
		}
		
		// 首先选择挂接点
		List<String> hookNames = new ArrayList<String>();
		for (int i = 0; i < hooks.size(); i++) {
			hookNames.add((i + 1) + "." + hooks.get(i).name);
		}
		GenericChooseDialog dlg = new GenericChooseDialog(getSite().getShell(), "挂接点", "请选择要调整的挂接点：", hookNames);
		if (dlg.open() != Dialog.OK) {
			return;
		}
		int hookIndex = dlg.getSelectionIndex();
		int hookId = hooks.get(hookIndex).getImageID();
		
		// 选择pip文件名
		List<String> fileNames = new ArrayList<String>();
		for (int i = 0; i < animateSet.getFileCount(); i++) {
			fileNames.add((i + 1) + "." + animateSet.getFileName(i));
		}
		dlg = new GenericChooseDialog(getSite().getShell(), "PIP文件", "请选择要挂接点对应的PIP文件：", fileNames);
		if (dlg.open() != Dialog.OK) {
			return;
		}
		int imageIndex = dlg.getSelectionIndex();
		
		// 扫描所有的帧，找到对应pip文件的图块，并把挂接点调整到这个图块的后面
		int matchCount = 0;
		for (int i = 0; i < animateSet.getFrameCount(); i++) {
			PipAnimateFrame frame = animateSet.getFrame(i);
			
			// 找到挂接点的索引
			int hookPieceIndex = -1;
			for (int j = 0; j < frame.getPieceCount(); j++) {
				if (frame.getPiece(j) instanceof PipAni4AniFramePiece) {
					PipAni4AniFramePiece pp = (PipAni4AniFramePiece)frame.getPiece(j);
					if (pp.getImageID() == hookId) {
						hookPieceIndex = j;
						break;
					}
				}
			...
			// 找到图片对应的图块索引
			int imagePieceIndex = -1;
			for (int j = 0; j < frame.getPieceCount(); j++) {
				if (!(frame.getPiece(j) instanceof PipAni4AniFramePiece)) {
					if (frame.getPiece(j).getImageID() == imageIndex) {
						imagePieceIndex = j;
						break;
					}
				}
			}
			if (imagePieceIndex == -1) {
				continue;
			}
			
			// 把挂接点移动到图块后面
			matchCount++;
			PipAni4AniFramePiece hookP = (PipAni4AniFramePiece)frame.getPiece(hookPieceIndex);
			frame.removePiece(hookPieceIndex);
			if (hookPieceIndex > imagePieceIndex) {
				frame.addPieceAt(imagePieceIndex + 1, hookP);
			} else {
				frame.addPieceAt(imagePieceIndex, hookP);
			}
		}
		
		MessageDialog.openInformation(getSite().getShell(), "信息", "调整完成，修改了" + matchCount + "帧。");
	}
}
