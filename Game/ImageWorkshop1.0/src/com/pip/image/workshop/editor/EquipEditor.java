...
public class EquipEditor extends EditorPart implements
	...
	private void createLeftBottom(Composite parent) {
		...
		eqpAniView.setImageCache(imageCache);
	}
	...
	private void createLeftTop(Composite parent) {
		...
		bodyAnimateViewer.setImageCache(imageCache);
		...
		allBodyViewer.setImageCache(imageCache);
	}
	...
	protected void saveEquipAnimateSet() {
		...
		try {
			...
			SWTUtils.showError(getSite().getShell(), "错误", "保存装备动画出错。", e);
			equipSaving = false;
		}
	}
	...
	private void createRightBottomOperation(Composite parent) {
		...
		equipCtsNameLabel.setLayoutData(gridData);

		Button renameEquipCtsName = new Button(container, SWT.PUSH);
		renameEquipCtsName.setText("CTS改名");
		...
		Button changeBodyPathBtn = new Button(container, SWT.PUSH);
		changeBodyPathBtn.setLayoutData(new GridData());
		changeBodyPathBtn.setText("更换素体");
		changeBodyPathBtn.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				changeBodyPath();
			}
		});
		...
		gridData.horizontalSpan = 4;
		curHookLabel.setLayoutData(gridData);
		...
		bindToSelBodyFrame.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false, 3, 1));
	...
	protected void changeBodyPath() {
		...
		File equipFile = new File(equipFilePath);
		File dir = equipFile.getParentFile();
		File hkFile = new File(dir, selBodyName);
		...
		String restrictDir = hkFile.getParentFile().getAbsolutePath();
		fdlg.setFilterPath(restrictDir);
		String file = fdlg.open();
		if (file != null) {
			// 素体文件和装备文件必须在同一个盘上
			String relatePath = Utils.getRelatePath(file, dir.getAbsolutePath());
			if (relatePath == null) {
				MessageDialog.openInformation(getSite().getShell(), "Info",
						"素体文件必须和装备文件位于同一磁盘中。");
				return;
			}
			eqp2hook.setHookFileName(idx, relatePath);
			BodyDef bodyDef = new BodyDef();
			PipAnimateSet newBodyPas = null;
			try {
				newBodyPas = loadBody(dir.getAbsolutePath(), relatePath, bodyDef);
			} catch (IOException e) {
				SWTUtils.showError(getSite().getShell(), "错误", "加载素体文件错误。", e);
				return;
			}
			bodyAniSets.set(idx, newBodyPas);
			bodyDefs.set(idx, bodyDef);
			bodyAniTree.refresh();
			bodyAniSet = bodyAniSets.get(idx);
			bodyDef.embedHookPieces(bodyAniSet, selHookId, true);
			eqp2hook.doEquip(bodyAniSet, equipAniSet, idx);
			setDirty(true);
		}
	}
	...
	protected void bindToBodyFrame(PipAnimateFrame bodyFrame) {
		...
		if (hook == null) {
			...
			MessageDialog.openError(getSite().getShell(), "错误", alertMsg);
			return;
		}
	...
	public void doSave(IProgressMonitor arg0) {
		...
		try {
			...
			SWTUtils.showError(getSite().getShell(), "错误", "保存文件错误。", e);
		}
		setDirty(false);
