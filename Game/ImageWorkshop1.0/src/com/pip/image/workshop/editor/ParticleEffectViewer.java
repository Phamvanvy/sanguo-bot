...
public class ParticleEffectViewer extends AbstractImageViewer implements Runnable {
	...
	protected void paintInput(GC gc) {
		...
		if (refAnimate != null) {
			refAnimate.drawAnimateFrame(gc, playTimer, size.x / 2 + paintOffset.x, size.y / 2 + paintOffset.y, ratio, null);
		}
		if (input == null) {
			return;
		}
		...
		for (PipParticle p : arr) {
			...
			if (time >= p.startTime && time < p.startTime + p.path.length) {
				...
				animate.drawAnimateFrame(gc, time - p.startTime, (int)(size.x / 2 + paintOffset.x + pos[0] * ratio), 
						(int)(size.y / 2 + paintOffset.y + pos[1] * ratio), ratio, MapEditor.imageCache);
			}
		}
	}
