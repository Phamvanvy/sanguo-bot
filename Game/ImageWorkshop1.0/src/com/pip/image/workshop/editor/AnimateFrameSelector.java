...
public class AnimateFrameSelector extends AbstractImageViewer {
	...
	protected void paintInput(GLGraphics gc) {
		...
		if (input == null) {
	        ...
	        int mapw = (int)(pos[0].x * ratio);
	        int maph = (int)(pos[0].y * ratio);
	        int offx = (size.x - mapw) / 2 + paintOffset.x;
	        int offy = (size.y - maph) / 2 + paintOffset.y;
	        ...
	        for (int i = 0; i < animate.getFrameCount(); i++) {
	            ...
	            int dy = (int)(pos[i + 1].y * ratio);
	            Rectangle frameRec = zoom(fr.getBounds());
	            fr.draw(gc, dx - frameRec.x + offx, dy - frameRec.y + offy, ratio, cache);
	            gc.setColor(0xFF000000);
	            gc.drawRect(dx + offx, dy + offy, frameRec.width, frameRec.height);
	        }
		}
	}
			...
			if (pos != null) {
				...
				for (int i = 0; i < animate.getFrameCount(); i++) {
					...
					if (selectedFrames.contains(i)) {
						...
						gc.drawRect(dx + paintOffset.x, dy + paintOffset.y, frameRec.width, frameRec.height);
						gc.setColor(0xFF000000);
					} else {
//						gc.drawRectangle(dx + paintOffset.x, dy + paintOffset.y, frameRec.width, frameRec.height);
					}
		...
		if (input != null && selectedFrame != -1 && needDrawTip) {
			String str = "(双击添加)";
			Point ts = gc.textExtent(str);
			gc.drawRect(70 + 1, size.y - ts.y - 8, ts.x + 7, ts.y + 6);
			gc.drawText(str, 70 + 4, size.y - ts.y - 5);
		}
        ...
        str = "按住SHIFT/CTRL可以多选";
        ts = gc.textExtent(str);
        gc.drawRect(size.x - ts.x - 8, size.y - ts.y - 8, ts.x + 7, ts.y + 6);
        gc.drawText(str, size.x - ts.x - 5, size.y - ts.y - 5);
	}
