...
public class ImageEditor extends EditorPart implements ImageViewerListener {
	...
	public void createPartControl(Composite parent) {
        ...
        comboMode.setItems(new String[] {"256色模式", "256色合并模式", "65536色模式", "真彩色模式", "JPEG合并模式", "PVRTC4", "分离透明度的PVRTC4", "ETC1", "带透明度的ETC1"});
        if (allImage.isTrueColor()) {
        	if (allImage.getJPEGOption() != null) {
        		comboMode.select(4);
        	} else if (allImage.getCompressTextureOption() != null && 
        			allImage.getCompressTextureOption().format.equals(CompressTextureOption.PVRTC_4BPP)) {
        		comboMode.select(5);
        	} else if (allImage.getCompressTextureOption() != null && 
        			allImage.getCompressTextureOption().format.equals(CompressTextureOption.PVRTC_4BPP2)) {
        		comboMode.select(6);
        	} else if (allImage.getCompressTextureOption() != null && 
        			allImage.getCompressTextureOption().format.equals(CompressTextureOption.ETC1)) {
        		comboMode.select(7);
        	} else if (allImage.getCompressTextureOption() != null && 
        			allImage.getCompressTextureOption().format.equals(CompressTextureOption.ETC2)) {
        		comboMode.select(8);
        	} else {
        		comboMode.select(3);
        	}
        ...
        comboMode.addModifyListener(new ModifyListener() {
		    public void modifyText(final ModifyEvent e) {
                ...
                case 0:
                    ...
                    allImage.setJPEGOption(null);
                    allImage.setCompressTextureOption(null);
                    break;
                case 1:
                ...
                case 5:
                    ...
                    allImage.setMergeMode(false);
                    allImage.setTrueColor(true);
                    allImage.setCompressTextureOption(new CompressTextureOption(CompressTextureOption.PVRTC_4BPP));
                    allImage.getCompressTextureOption().borderWidth = CompressTextureOptionDialog.choose(2);
                    break;
                case 6:
                	allImage.setSupportColorOp(false);
                	allImage.setSupportMoreColors(false);
                    allImage.setMergeMode(false);
                    allImage.setTrueColor(true);
                    allImage.setCompressTextureOption(new CompressTextureOption(CompressTextureOption.PVRTC_4BPP2));
                    allImage.getCompressTextureOption().borderWidth = CompressTextureOptionDialog.choose(2);
                    break;
                case 7:
                	allImage.setSupportColorOp(false);
                    ...
                    allImage.setMergeMode(false);
                    allImage.setTrueColor(true);
                    allImage.setCompressTextureOption(new CompressTextureOption(CompressTextureOption.ETC1));
                    allImage.getCompressTextureOption().borderWidth = CompressTextureOptionDialog.choose(2);
                    break;
                case 8:
                	allImage.setSupportColorOp(false);
                    ...
                    allImage.setMergeMode(false);
                    allImage.setTrueColor(true);
                    allImage.setCompressTextureOption(new CompressTextureOption(CompressTextureOption.ETC2));
                    allImage.getCompressTextureOption().borderWidth = CompressTextureOptionDialog.choose(2);
                    break;
                }
		        setDirty(true);
	...
	public void doSave(IProgressMonitor monitor) {
		try {
            ...
            case 7:
            	...
            	// ETC1格式不支持透明色，如果图片中包含透明色，则不能使用这种模式
            	if (!allImage.isOpaque()) {
		    ...
		    if (allImage.getCompressTextureOption() == null && !allImage.testAddFrame(-1, -1)) {
		    	// 图片过大
		    	String msg = "图片过大，单个pip文件的所有帧必须可以合并到一个1024x" + allImage.maxMergeHeight + "的区域里，是否确认继续？";
		    	if (!MessageDialog.openConfirm(getSite().getShell(), "确认", msg)) {
		    		if (monitor != null) {
					    monitor.setCanceled(true);
					}
	...
	private void saveState() {
		if (allImage.getCompressTextureOption() != null) {
			// 压缩纹理不支持undo，因为转换太慢了
			return;
		}
		try {
	...
	public void dispose() {
		super.dispose();
		if (importImage != null) {
			GLUtils.unloadImage(importImage);
		}
	}
