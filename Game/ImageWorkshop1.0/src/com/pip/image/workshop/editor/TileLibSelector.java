...
public class TileLibSelector extends AbstractImageViewer implements Runnable {
	...
	protected int replaceTrans;
	
	// 预览PipAnimateSet的时候，把每一帧都缓存起来
	protected Map<Integer, Image> frameCache;
	
	// 预览PipImage时，允许只显示部分帧
	protected boolean[] pipImageFrameVisibleFlag;
	
	protected ImageDrawCache cache = MapEditor.imageCache; 
	...
	public void setPipImageFrameVisibleFlag(boolean[] flag) {
		pipImageFrameVisibleFlag = flag;
	}
	
	public void setReplaceImage(int imageID, int sourceFrame, int targetFrame, int offsetX, int offsetY, int trans) {
	...
	protected void paintInput(GC gc) {
		...
		if (input == null) {
			...
			for (int i = 0; i < img.getImgCount(); i++) {
				...
				rect.x += offx + paintOffset.x;
				rect.y += offy + paintOffset.y;
				if (pipImageFrameVisibleFlag != null && !pipImageFrameVisibleFlag[i]) {
					// 用黑色盖上表示已处理
                ...
                if (cachedImg == null) {
                	...
                	if (replaceImageID == -1) {
                		...
                		ani.getFrame(rframe).draw(fgc, -bounds.x, -bounds.y, 1.0,
                    			...
                    			replaceTargetFrame, replaceOffsetX, replaceOffsetY, replaceTrans);
                	}
                	fgc.dispose();
                }
	...
	protected void paintInput(GLGraphics gc) {
		...
		if (input == null) {
		    ...
		    for (int i = 0; i < imgs.length; i++) {
				...
				gc.drawTexture(GLUtils.loadImage(cachedImg), 0, 0, rect.x, rect.y, (int)(tileSize * 6 * ratio), (int)(tileSize * 6 * ratio));
		    }
		} else if (input instanceof PipImage) {
			PipImage img = (PipImage)input;
			Rectangle[] pos = getBestLayout(img);
			...
			for (int i = 0; i < img.getImgCount(); i++) {
				...
				if (pipImageFrameVisibleFlag != null && !pipImageFrameVisibleFlag[i]) {
					...
					Image frameImg = null;
					if (cache != null) {
						frameImg = cache.get(img, i, 0);
					}
					if (frameImg == null) {
						frameImg = img.getImageDraw(i).createSWTImage(getDisplay(), 0);
						if (cache != null) {
							cache.add(img, i, 0, frameImg);
						}
					}
					gc.drawTexture(GLUtils.loadImage(frameImg), 0, 0, rect.x, rect.y, rect.width, rect.height);
					if (cache == null) {
		    ...
		    for (int i = 0; i < ani.getAnimateCount(); i++) {
                ...
                PipAnimateFrameRef frameRef = animate.getFrame(frame);
                Rectangle rect2 = animate.getBounds();
                if (replaceImageID == -1) {
	                animate.drawAnimateFrame(gc, currentTime, (int)(rect.x - rect2.x * ratio), 
	                		(int)(rect.y - rect2.y * ratio), ratio, cache);
                } else {
                	PipAnimateFrame frameObj = frameRef.realize();
                	frameObj.draw(gc, (int)(rect.x - rect2.x * ratio + frameRef.getDx() * ratio), 
                			(int)(rect.y - rect2.y * ratio + frameRef.getDy() * ratio), 
                			ratio, replaceImageID, replaceSourceFrame,
                			replaceTargetFrame, replaceOffsetX, replaceOffsetY, replaceTrans);
                }
		    }
		} else if (input instanceof PipAnimateSet[]) {
...
//						rect.height -= 1;
				...
				for (int i = 0; i < pos.length - 1; i++) {
					...
					rect.y += offy + paintOffset.y;
					if (i == hoverFrame) {
						gc.setColor(getDisplay().getSystemColor(SWT.COLOR_RED));
						gc.drawRect(rect);
						gc.setColor(getDisplay().getSystemColor(SWT.COLOR_BLACK));
					} else if (selectedFrames.contains(posIndex - 1)) {
						gc.drawRect(rect);
					}
				}
		...
		if (isDragging) {
			// gc.setXORMode(true);
			// gc.setColor(SWTResourceManager.getColor(SWT.COLOR_WHITE));
