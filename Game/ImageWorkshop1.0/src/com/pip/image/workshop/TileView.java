...
public class TileView extends ViewPart implements ImageViewerListener {
	...
	private Object loadImage(String file) throws Exception {
		try {
			...
			SWTUtils.showError(getSite().getShell(), "错误", "加载图片失败。", e);
	...
	public void openTileFile(File file) throws Exception {
		...
		if (image != null && image instanceof Image) {
			GLUtils.unloadImage((Image)image);
		}
		image = img;
		imageFile = file;
...
//				}
			...
			for (int i = 0; i < frames.length; i++) {
				if (image instanceof Image) {
					...
					PipAnimateSet[] arr = (PipAnimateSet[])image;
					PipAnimateSet ani = getAnimateOfIndex(arr, frames[i]);
					int index = getAnimateIndexOfIndex(arr, frames[i]);
					((MapEditor)editor).addAnimate(ani, index);
				} else if (image instanceof LandformImage[]) {
					curFileHashCode = hashCodesForFiles[frames[i]];
				    ((MapEditor)editor).addLandform(((LandformImage[])image)[frames[i]]);
				    return;
				}
			}
		}
	...
	private PipAnimateSet getAnimateOfIndex(PipAnimateSet[] arr, int index) {
		for (PipAnimateSet animate : arr) {
			if (index < animate.getAnimateCount()) {
				return animate;
			}
			index -= animate.getAnimateCount();
		}
		return null;
	}
	
	private int getAnimateIndexOfIndex(PipAnimateSet[] arr, int index) {
		for (PipAnimateSet animate : arr) {
			if (index < animate.getAnimateCount()) {
				return index;
			}
	 ...
	 * @throws Exception
	 */
	public void openDir(File dir) throws Exception {
		// 找出目录下所有cts文件，不含子目录
		File[] files = dir.listFiles();
		List<File> ctsFiles = new ArrayList<File>();
		for (File f : files) {
			if (f.isFile() && f.getName().toLowerCase().endsWith(".cts")) {
				ctsFiles.add(f);
			}
		}
		
		// 如果目录中存在cts，则预览cts文件，否则查找ldf文件
		if (ctsFiles.size() > 0) {
			// 载入cts文件
			List<PipAnimateSet> animates = new ArrayList<PipAnimateSet>();
			for (File f : ctsFiles) {
			...
			PipAnimateSet[] arr = new PipAnimateSet[animates.size()];
			animates.toArray(arr);
			image = arr;
			curFilePath = dir.getAbsolutePath();
			setupViewer();
		} else {
			// 查找ldf文件
			List<File> ldfFiles = new ArrayList<File>();
			for (File f : files) {
				if (f.isFile() && f.getName().toLowerCase().endsWith(".ldf")) {
					ldfFiles.add(f);
				}
			}
			...
			// 载入ldf文件
			List<LandformImage> images = new ArrayList<LandformImage>();
			for (File f : ldfFiles) {
				LandformImage image = new LandformImage();
				try {
					image.load(f.getAbsolutePath());
					images.add(image);
				} catch (Exception e) {
				}
			}
			
			LandformImage[] arr = new LandformImage[images.size()];
			images.toArray(arr);
			image = arr;
			curFilePath = dir.getAbsolutePath();
			hashCodesForFiles = new int[ldfFiles.size()];
			for (int i = 0; i < ldfFiles.size(); i++) {
				hashCodesForFiles[i] = ProjectParser.getFileHashCode(ldfFiles.get(i));
			}
			setupViewer();
		}
	}
