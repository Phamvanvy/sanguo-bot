...
public class MapNPCLayer implements IMapLayer {
    ...
    public void cut(int x, int y, int w, int h) {
		for (int i = 0; i < npcs.size(); i++) {
			...
			if (npc instanceof MultiAnimNPC) {
				...
				for (int j = 0; j < mnpc.children.size(); j++) {
					if (isNPCInArea(mnpc.children.get(j), x, y, w, h)) {
						mnpc.children.get(j).x -= x;
						mnpc.children.get(j).y -= y;
					} else {
						mnpc.children.remove(j);
						j--;
					}
				}
				if (mnpc.children.size() > 0) {
					mnpc.x -= x;
					mnpc.y -= y;
				} else {
					npcs.remove(i);
    ...
    /*
     * 判断一个NPC是否和指定区域有交集（不支持组合NPC）。
     */
    private boolean isNPCInArea(MapNPC npc, int x, int y, int w, int h) {
	...
	public int getNPCIndex(MapNPC npc) {
		for (int i = 0; i < npcs.size(); i++) {
			if (npcs.get(i) == npc) {
				return i;
			}
