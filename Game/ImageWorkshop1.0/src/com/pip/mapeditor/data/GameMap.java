...
public class GameMap {
	...
	/**
	 * 地图阻挡信息 值解释:第0位(视线遮挡 1表示遮挡) 1位(地面层阻挡 2表示可通过) 2位(天空层阻挡 1表示可通过) 3位(安全区 0表示安全区)
	 */
    ...
    public void load(Element elem) {    		
        ...
        //这里做一个地图大小合理性的检查，将问题截获在编辑状态，不要遗留到客户端处理时导致问题
        int blurTileWidth = parent.getBlurTileWidth();
        int blurTileHeight = parent.getBlurTileHeight();
        if ((width % blurTileWidth) != 0 ||	(height % blurTileHeight) != 0) {
        	String tip = "地图大小("+width+")不正确，不是地形大小(宽："+blurTileWidth+"，高："+blurTileHeight+")的整倍数。\n可参考如下修改：";
        	if((width % blurTileWidth) != 0){
        		float cnt = width / (float)blurTileWidth;
        		tip += "\n将宽度改为："+ blurTileWidth * Math.floor(cnt)+"或者"+ blurTileWidth * Math.ceil(cnt);
        	}
        	if((height % blurTileHeight) != 0){
        		float cnt = height /(float) blurTileHeight;
        		tip += "\n将高度改为："+ Math.floor(cnt)+"或者"+Math.ceil(cnt);
        	}
