...
//        lines.toArray(layerData);
	...
	/**
	 * 放大地图数据，用左上角的1/4扩大一倍填充整个地图。
	 */
	public void enlargeData() {
		int cw = layerData[0].length;
		int ch = layerData.length;
		byte[][] newData = new byte[ch][cw];
		for (int row = ch / 2 - 1; row >= 0; row--) { 
			for (int col = cw / 2 - 1; col >= 0; col--) {
				newData[row * 2][col * 2] = layerData[row][col];
				newData[row * 2][col * 2 + 1] = layerData[row][col];
				newData[row * 2 + 1][col * 2] = layerData[row][col];
				newData[row * 2 + 1][col * 2 + 1] = layerData[row][col];
			}
		}
		layerData = newData;
		clearBuffer();
