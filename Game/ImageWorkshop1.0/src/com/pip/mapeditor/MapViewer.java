...
import com.pip.mapeditor.tool.IUpdatableTool;
...
public class MapViewer extends AbstractImageViewer {
    ...
    protected MapNPC tempShowNPC = null;  // EmulateWalkTool使用来临时在人物层加入一个玩家
    protected PipAnimate tempShowNPCAnimate = null;
    protected int[] refPalette;
    protected Image backgroundImg;
    
    // OpenGL模式下，缓存地面tile层的绘图，提高速度
    protected GLGraphics groundLayerBuffer;
    protected ImageDrawCache tileCache = new ImageDrawCache(1000);
    
    public void setBackgroundImage(Image img) {
    	if (backgroundImg != null) {
    		GLUtils.unloadImage(backgroundImg);
    	}
    	backgroundImg = img;
    }
    ...
    public void setRefPalette(int[] pal) {
    	refPalette = pal;
    	MapEditor.imageCache.setPalette(refPalette);
    }
    
    public void setTempShowNPC(PipAnimate animate, int x, int y) {
    	tempShowNPC = new MapNPC();
    	tempShowNPC.animateSetRef = -1;
    	tempShowNPC.animate = -1;
    	tempShowNPC.x = x;
    	tempShowNPC.y = y;
    	tempShowNPCAnimate = animate;
    }
     ...
     * @param area
     */
    public void makeDirty(Rectangle area) {
    	if (npcDirtyList.size() == 0) {
			npcDirtyList.add(area);
		} else {
			npcDirtyList.get(0).add(area);
		}
    }
	...
	public void setInput(Object input) {
		...
		mapBuffer = null;
		if (groundLayerBuffer != null) {
        	groundLayerBuffer.dispose();
        	groundLayerBuffer = null;
        }
		activeLayer = 0;
		if (map == null) {
				...
				if (e.button == 1) {
				    if (tool != null) {
				        ...
				        tool.mouseDown(pt.x, pt.y, e.stateMask);
				        	...
				        	SWTUtils.showError(getShell(), "错误", t);
				        }
				    }
				}
			}
	 ...
	 * @param rh
	 */
	public static void paintTransparentBackground(GLGraphics gc, int rx, int ry, int rw, int rh) {
		int cx = gc.getClipX();
		int cy = gc.getClipY();
		int cw = gc.getClipWidth();
		int ch = gc.getClipHeight();
		gc.setClip(rx, ry, rw, rh);
		gc.setColor(0xFFFFFFFF);
		gc.fillRect(rx, ry, rw, rh);
		gc.setColor(0xFFDFDFDF);
		int i = 0;
		for (int x = rx - 2; x < rx + rw; x += 4, i++) {
			int j = 0;
			for (int y = ry - 2; y < ry + rh; y += 4, j++) {
	...
	// 绘制地图到一个虚拟缓存上（原始大小）
	public void drawMapOnBuffer(GC gc, boolean includeAnimate) {
		for (int i = 0; i < map.layers.size(); i++) {
            IMapLayer layer = map.layers.get(i);
            if (showLayers[i]) {
            	double oldRatio = ratio;
            	ratio = 1.0;
                drawMapLayer(gc, layer, 0, 0, new Rectangle(0, 0, map.width, map.height), null, includeAnimate);
                ratio = oldRatio;
            }
        }
	}
	...
	// 绘制地图的一层。
	protected void drawMapLayer(GLGraphics gc, IMapLayer layer, int offx, int offy, Rectangle visibleRange, List<Rectangle> dirtyList, boolean includeAnimate) {
	    if (layer instanceof AccurateMapLayer) {
	        drawAccurateMapLayer(gc, (AccurateMapLayer)layer, offx, offy, visibleRange);
	    } else if (layer instanceof MapNPCLayer) {
	        drawNPCLayer(gc, (MapNPCLayer)layer, offx, offy, visibleRange, dirtyList, includeAnimate);
	...
	// 绘制精确贴图层
	protected void drawAccurateMapLayer(GLGraphics gc, AccurateMapLayer layer, int offx, int offy, Rectangle visibleRange) {
	    int tw = map.parent.getTileWidth();
        int th = map.parent.getTileHeight();
        ArrayList<TileInfo> tiles = map.parent.getTileImage().tileInfo;
        PipImage imageSet = map.parent.getTileImage().image;
    	...
    	gc.setClip((int)(offx + visibleRange.x * ratio), (int)(offy + visibleRange.y * ratio), 
    			(int)(visibleRange.width * ratio), (int)(visibleRange.height * ratio));
        for (int cy = sy; cy < ey; cy++) {
            for (int cx = sx; cx < ex; cx++) {
                int cc = mapData[cy][cx];
                ...
                int celly = (int)(cy * th * ratio) + offy;
                TileInfo cinfo = tiles.get(cc);
                Image img = tileCache.get(imageSet, cinfo.frameID, cinfo.transit);
                if (img == null) {
                	img = imageSet.getImageDraw(cinfo.frameID).createSWTImage(null, cinfo.transit);
                	tileCache.add(imageSet, cinfo.frameID, cinfo.transit, img);
                }
                gc.drawTexture(GLUtils.loadImage(img), 0, 0, cellx, celly, (int)(tw * ratio), (int)(th * ratio));
            }
        }
	...
	protected void drawNPCLayer(GC gc, MapNPCLayer layer, int offx, int offy, Rectangle visibleRange, List<Rectangle> dirtyList, boolean includeAnimate) {
		...
		if (tempShowNPC != null && layer == map.groundLayer) {
		    npcs = new MapNPC[layer.getNpcs().size() + 1];
		    layer.getNpcs().toArray(npcs);
		    npcs[npcs.length - 1] = tempShowNPC;
		} else {
		    npcs = new MapNPC[layer.getNpcs().size()];
		    layer.getNpcs().toArray(npcs);
		}
	...
	protected void drawRawNPC(GLGraphics gc, int offx, int offy, Rectangle visibleRange, MapNPC npc, List<Rectangle> dirtyList, boolean includeAnimate){
		if(npc instanceof MultiAnimNPC){
			if (((MultiAnimNPC) npc).isShow == false) {
				return;
			}
	...
	private void drawNPC(GC gc, int offx, int offy, Rectangle visibleRange, MapNPC npc, List<Rectangle> dirtyList, boolean includeAnimate) {
		...
		PipAnimate animate;
    	if (map.parent.isLibMode) {
    		if (npc.animateSetRef == -1) {
    			animate = tempShowNPCAnimate;
    		} else {
    			animate = map.parent.getAnimate(npc.animateSetRef, npc.animate);
    		}
    	} else {
	...
	private void drawNPC(GLGraphics gc, int offx, int offy, Rectangle visibleRange, MapNPC npc, List<Rectangle> dirtyList, boolean includeAnimate) {
		if(npc.isShow == false) {
			return;
		}
		PipAnimateSet animates = map.parent.getAnimates();
        ...
        int clipw = gc.getClipWidth();
        int cliph = gc.getClipHeight();
        gc.setClip((int)(offx + rect.x * ratio), (int)(offy + rect.y * ratio), (int)(rect.width * ratio), (int)(rect.height * ratio));
        animate.drawAnimateFrame(gc, getCurrentTime(), rx, ry, ratio, MapEditor.imageCache);
        gc.setClip(clipx, clipy, clipw, cliph);
	}
 ...
 // 绘制模糊贴图层
    protected void drawBlurMapLayer(GLGraphics gc, BlurMapLayer layer, int offx, int offy, Rectangle visibleRange) {
        int tw = map.parent.getBlurTileWidth();
        int th = map.parent.getBlurTileHeight();
        int[][][] mapData = layer.getMapData();
        int sx = 0;
        ...
        for (int cy = sy; cy < ey; cy++) {
            for (int cx = sx; cx < ex; cx++) {
                for (int cz = 0; cz < 3; cz++) {
                    ...
                    int celly = (int)(cy * th * ratio) + offy;
                    PipImage image = map.parent.getLandforms().get(lfid).image;
                    Image img = tileCache.get(image, fid, trans);
                    if (img == null) {
                    	img = image.getImageDraw(fid).createSWTImage(null, trans);
                    	tileCache.add(image, fid, trans, img);
                    }
                    gc.drawTexture(GLUtils.loadImage(img), 0, 0, cellx, celly, (int)(tw * ratio), (int)(th * ratio));
                }
            }
	...
	protected void createMapBuffer() {
		int mapw = (int)(map.width);
		int maph = (int)(map.height);
		...
		// 绘制背景图（如果有）
		if (backgroundImg != null) {
			int bw = backgroundImg.getBounds().width;
			int bh = backgroundImg.getBounds().height;
			gc.drawImage(backgroundImg, (mapw - bw) / 2, (maph - bh) / 2);
		}

		// 绘制在底部的贴图层
		drawMapOnBuffer(gc, false);
		gc.dispose();
	}
	...
	protected void updateMapBuffer() {
		...
		// 绘制底层透明背景
		paintTransparentBackground(gc, (int)(r.x), (int)(r.y), 
		...
		if (backgroundImg != null) {
			int bw = backgroundImg.getBounds().width;
			int bh = backgroundImg.getBounds().height;
			gc.drawImage(backgroundImg, r.x, r.y, r.width, r.height, 
					(map.width - bw) / 2 + r.x, (map.height - bh) / 2 + r.y, r.width, r.height);
		}
	...
	protected void paintInput(GC gc) {
		...
		gc.drawImage(mapBuffer, 0, 0, map.width, map.height, 
				mapx, mapy, (int)(map.width * ratio), (int)(map.height * ratio));
	...
	protected void paintInput(GLGraphics gc) {
		...
		for (int i = 0; i < map.layers.size(); i++) {
            ...
            if (i == 0) {
            	if (groundLayerBuffer == null) {
            		groundLayerBuffer = new GLGraphics();
            		groundLayerBuffer.setZ(map.layers.size() * -10000.0f);
            		drawMapLayer(groundLayerBuffer, layer, 0, 0, new Rectangle(0, 0, map.width, map.height), null, true);
            	}
            	gc.drawBatch(groundLayerBuffer.getHandle(), mapx, mapy);
            } else {
            	gc.setZ((i - map.layers.size()) * 10000.0f);
            	gc.setAlphaTest(true);
                drawMapLayer(gc, layer, mapx, mapy, new Rectangle(0, 0, map.width, map.height), null, true);
                gc.setAlphaTest(false);
            }
        }
	...
	public void widgetDisposed(DisposeEvent evt) {
		if (tool != null && tool.getMenu() != null) {
			tool.getMenu().dispose();
		}
    ...
    public void step() {
        this.currentTime++;
        if (this.getTool() != null && this.getTool() instanceof IUpdatableTool) {
        	((IUpdatableTool)getTool()).update();
        }
    }
    ...
    public void setTool(IMapEditTool tool) {
    	...
    	this.tempShowNPC = null;
    	this.tempShowNPCAnimate = null;
    	if (this.tool != null && this.tool.getMenu() != null) {
    		this.tool.getMenu().dispose();
    	}
        this.tool = tool;
        if (tool != null) {
