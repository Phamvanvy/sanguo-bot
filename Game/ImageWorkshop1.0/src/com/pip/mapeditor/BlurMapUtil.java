...
public class BlurMapUtil {
     ...
     * @param ratio 缩放比例
     */
    public static void drawLandform(GLGraphics gc, LandformImage img, Random rand, byte[][] data, int x, int y, int tw, int th, double ratio) {
        img.generateSearchTable();
        for (int i = 0; i < data.length; i++) {
            for (int j = 0; j < data[i].length; j++) {
                if (data[i][j] == 0) {
                ...
                Image timg = MapEditor.imageCache.get(img, tile[0], tile[1]);
                if (timg == null) {
                	timg = img.getImageDraw(tile[0]).createSWTImage(null, tile[1]);
                	MapEditor.imageCache.add(img, tile[0], tile[1], timg);
                }
                gc.drawTexture(GLUtils.loadImage(timg), 0, 0, tx, ty, rtw, rth);
            }
        }
    }
}
