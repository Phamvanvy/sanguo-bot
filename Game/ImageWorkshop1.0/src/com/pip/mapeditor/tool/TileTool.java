...
public class TileTool implements IMapEditTool {
    ...
    public void draw(GLGraphics gc) {
        ...
        if (tileView.getSelectedFrame() == 0) {
            ...
            TileSet ts = map.parent.getTileImage();
            TileInfo ti = ts.tileInfo.get(tileView.getSelectedFrame() - 1);
            tileImage = MapEditor.imageCache.get(ts.image, ti.frameID, ti.transit);
            if (tileImage == null) {
            	tileImage = ts.image.getImageDraw(ti.frameID).createSWTImage(null, ti.transit);
            	MapEditor.imageCache.add(ts.image, ti.frameID, ti.transit, tileImage);
            }
        }
        if (isDragging) {
            // 绘制本次拖动影响到的Tile
     ...
     * @param image 如果为null，表示绘制透明
     */
    private void drawTile(GLGraphics gc, Point pt, Image image) {
        int tw = viewer.getMap().parent.getTileWidth();
        int th = viewer.getMap().parent.getTileHeight();
        Rectangle rect = new Rectangle(pt.x * tw, pt.y * th, tw, th);
        viewer.map2screen(rect);
        if (image != null) {
            gc.drawTexture(GLUtils.loadImage(image), 0, 0, rect.x, rect.y, rect.width, rect.height);
        } else {
            MapViewer.paintTransparentBackground(gc, rect.x, rect.y, rect.width, rect.height);
        }
    }
