...
public class TileConfigTool implements IMapEditTool {
    ...
    // 设置按钮的文本
    private String[] buttonTexts = { "设置", "清除" ,"1x1", "3x3", "5x5", "7x7", "30x30", "全部"};
    ...
    public static final byte CONFIG_MASK_VISIBILITY = 16;
...
//            viewer.fireContentChanged();//这句话执行太慢了，鼠标移动时就不做这个处理了
        }
        lastX = x;
        lastY = y;
...
//        }	
        ...
        //合并连续的格子，优化绘制格子的次数，提高一些性能
        gc.setForeground(SWTResourceManager.getColor(SWT.COLOR_RED));
//        gc.setAlpha(100);
        gc.setBackground(SWTResourceManager.getColor(SWT.COLOR_RED)); 
        gc.setLineStyle(SWT.LINE_SOLID);
        ArrayList<Rectangle> rects = handleDrawMapCell();
        for(Rectangle rect : rects) {
        	Rectangle rect2 = new Rectangle(rect.y * cellSize, rect.x * cellSize, rect.width * cellSize, rect.height * cellSize);
            viewer.map2screen(rect2);
...
//            gc.fillRectangle(rect2);
	...
	private ArrayList<Rectangle> handleDrawMapCell() {
		ArrayList<Rectangle> beans = new ArrayList<Rectangle>(); // 收集整合后的格子信息
		GameMap map = viewer.getMap();
		
		int iStep = 4;
		int jStep = 4;
		
        for (int i=0; i < map.tileInfo.length; i+=iStep) {
            for (int j = 0; j < map.tileInfo[i].length; j+=jStep) {
                
                boolean canMerge = true;
                for(int ii = 0; ii<iStep; ii++) {
                    for(int jj=0; jj<jStep; jj++) {
                        if(needDraw(map, i + ii, j + jj) == false) {
                            canMerge = false;
                            break;
                        }
                    }
                ...
                if(canMerge) {
                    Rectangle rect = new Rectangle(i, j, jStep, iStep);
                    beans.add(rect);
                } else {
                    for(int ii = 0; ii<iStep; ii++) {
                        for(int jj=0; jj<jStep; jj++) {
                            if(needDraw(map, i + ii, j + jj)) {
                                Rectangle rect = new Rectangle(i + ii, j + jj, 1, 1);
                                beans.add(rect);
                            }
                        }
                    }
    ...
    private boolean needDraw(GameMap map, int i, int j) {
        if(i < 0 || j < 0 || i > map.tileInfo.length - 1 || j > map.tileInfo[0].length - 1) {
            return false;
        }
     ...
     * 可飞行可行走绘制黄色点框.<br/>
     * 视野遮挡时不绘制表示遮挡 */
    protected void drawCellMask(GLGraphics gc) {
    	GameMap map = viewer.getMap();
        int cellSize = map.parent.getCellSize();
        int i = 0;
        for (; i < map.tileInfo.length; i++) {
	...
	protected void drawCoordinate(GLGraphics gc) {
    	int cellSize = viewer.getMap().parent.getCellSize();
    	Point size = viewer.getSize();
        String coordStr = lastX + "," + lastY + "," + (lastX / cellSize) + "," + (lastY / cellSize);
        Point ts = gc.textExtent(coordStr);
