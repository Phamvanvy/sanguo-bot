...
public class LandformTool implements IMapEditTool {
    ...
    public void mouseUp(int x, int y, int mask) {
        if (isDragging) {
            ...
            if (renderCells.size() > 0) {
                ...
                // 计算重画区域
                int tw = viewer.getMap().parent.getBlurTileWidth();
                int th = viewer.getMap().parent.getBlurTileHeight();
                int cols = viewer.getMap().width / tw;
                int rows = viewer.getMap().height / th;
                if (minx > 0) {
                	minx--;
                }
                if (miny > 0) {
                ...
                if (viewer.isOpenGLMode()) {
                	viewer.refresh();
                } else {
	                Rectangle rect = new Rectangle(minx * tw, miny * th, (maxx - minx) * tw, (maxy - miny) * th);
	                viewer.makeDirty(rect);
	                viewer.redraw();
                }
            }
        }
    ...
    public void draw(GLGraphics gc) {
        ...
        gc.drawString(coordStr, size.x - ts.x - 5, size.y - ts.y - 5, GLGraphics.TOP | GLGraphics.LEFT);
    }
