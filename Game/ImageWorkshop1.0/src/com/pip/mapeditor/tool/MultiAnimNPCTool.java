...
public class MultiAnimNPCTool implements IMapEditTool, SelectionListener {
    ...
    public void mouseDown(int x, int y, int mask) {
        ...
        // 检查是否点中了当前选中的组合NPC的参考点
        if (selectedNPC != null && selectedNPC instanceof MultiAnimNPC) {
        	if (x >= selectedNPC.x - 4 && x <= selectedNPC.x + 4 && y >= selectedNPC.y - 4 && y <= selectedNPC.y + 4) {
        		// 点中NPC
...
//            selectedLayer.getNpcs().add(selectedNPC);
    ...
    private void drawNPC(GLGraphics gc, GameMap map, MapNPC npc) {
    	PipAnimate animate;
        if (map.parent.isLibMode) {
        	animate = map.parent.getAnimate(npc.animateSetRef, npc.animate);
        } else {
    ...
    private void paintNPCBox(GC gc, GameMap map, MapNPC selectedNPC2, boolean isCurSel) {
        ...
        // 如果选中的是组合NPC，显示参考点调整按钮
        if (selectedNPC2 instanceof MultiAnimNPC && isCurSel) {
        	Point pt = new Point(selectedNPC2.x, selectedNPC2.y);
        	viewer.map2screen(pt);
        	gc.setForeground(SWTResourceManager.getColor(0xFF, 0x99, 0x00));
        	gc.drawLine(pt.x - 4, pt.y, pt.x + 4, pt.y);
        	gc.drawLine(pt.x, pt.y - 4, pt.x, pt.y + 4);
        	gc.drawRectangle(pt.x - 4, pt.y - 4, 8, 8);
        }
	}
    
    private void paintNPCBox(GLGraphics gc, GameMap map, MapNPC selectedNPC2, boolean isCurSel) {
        Rectangle bounds = makeNPCBounds(map, selectedNPC2);
        int color = SWT.COLOR_RED;
        viewer.map2screen(bounds);
        if(!isCurSel){
        ...
        gc.setColor(SWTResourceManager.getColor(color));
        gc.drawRect(bounds);
