...
public class WindowViewTool implements IMapEditTool {
    ...
    public void draw(GLGraphics gc) {
    	...
    	// 绘制蒙版
    	gc.setColor(0xE0000000);
		gc.fillRect(0, 0, size.x, rect.y);
		gc.fillRect(0, rect.y, rect.x, rect.height);
		gc.fillRect(rect.x + rect.width, rect.y, size.x - rect.x - rect.width, rect.height);
		gc.fillRect(0, rect.y + rect.height, size.x, size.y - rect.y - rect.height);
        gc.setColor(0);
    }
