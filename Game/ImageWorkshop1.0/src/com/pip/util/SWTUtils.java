...
public class SWTUtils {
            ...
            for (int j = 0; j < samples.length; j++) {
            	...
            	if (imageData.palette.isDirect) {
            		...
            		if (imageData.alphaData != null) {
            			...
            			int offset = mlsize * (area.y + i) + (area.x + j) / 8;
            			int offset2 = 7 - ((area.x + j) % 8);
        ...
        for (int i = 0; i < area.height; i++) {
            ...
            for (int j = 0; j < samples.length; j++) {
            	...
            	if (imageData.palette.isDirect) {
            		...
            		RGB rgb = imageData.palette.colors[samples[j]];
            		clr = (rgb.blue << 16) | (rgb.green << 8) | rgb.red;
            		if (imageData.alphaData != null) {
            			byte alpha = imageData.alphaData[(area.y + i) * imageData.width + area.x + j];
            			clr |= alpha << 24;
            		} else if (samples[j] != imageData.transparentPixel) {
     ...
     * @return
     */
    public static Image createImage(int[][] rgbData) {
    	ImageData idata = new ImageData(rgbData[0].length, rgbData.length, 32, new PaletteData(0xFF0000, 0x00FF00, 0x0000FF));
    	idata.alphaData = new byte[rgbData[0].length * rgbData.length * 4];
    	int index = 0;
    	for (int i = 0; i < rgbData.length; i++) {
    		for (int j = 0; j < rgbData[0].length; j++) {
				int clr = rgbData[i][j];
				idata.data[index * 4] = (byte)(clr >> 24);
				idata.data[index * 4 + 1] = (byte)(clr >> 16);
				idata.data[index * 4 + 2] = (byte)(clr >> 8);
				idata.data[index * 4 + 3] = (byte)clr;
				idata.alphaData[index] = (byte)(clr >> 24);
				index++;
	    	}
    	}
    ...
    public static Image createImage(int[] rgbData, int width, int height) {
    	ImageData idata = new ImageData(width, height, 32, new PaletteData(0xFF0000, 0x00FF00, 0x0000FF));
    	idata.alphaData = new byte[width * height * 4];
    	for (int i = 0; i < width * height; i++) {
			int clr = rgbData[i];
			idata.data[i * 4] = (byte)(clr >> 24);
			idata.data[i * 4 + 1] = (byte)(clr >> 16);
			idata.data[i * 4 + 2] = (byte)(clr >> 8);
			idata.data[i * 4 + 3] = (byte)clr;
			idata.alphaData[i] = (byte)(clr >> 24);
    	}
    	return new Image(null, idata);
    }
    ...
    public static void showError(Shell shell, String title, String message, Throwable exception) {
    	ExceptionDialog dlg = new ExceptionDialog(shell, title, message, exception);
    	dlg.open();
    }
    
    public static void showError(Shell shell, String title, Throwable exception) {
    	ExceptionDialog dlg = new ExceptionDialog(shell, title, null, exception);
    	dlg.open();
    }
}
