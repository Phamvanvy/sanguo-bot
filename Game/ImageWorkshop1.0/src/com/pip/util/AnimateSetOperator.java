...
public class AnimateSetOperator extends PipAnimateSet {
	...
	protected int maxMergeHeight;
	
	public AnimateSetOperator(int borderSize, int maxMergeHeight) {
		this.borderSize = borderSize;
		this.maxMergeHeight = maxMergeHeight;
	}
	    ...
	    for (Point p1 : usedFrames) {
			...
			if(similarImg.perfectMatch){
				...
				if (animateSet.getSourceImage(p1.x).isTrueColor()) {
					newID = addImageToAnimateSetReal(imgData, -1, null, 0, borderSize, maxMergeHeight);
				} else {
					newID = addImageToAnimateSetReal(imgData, -1, animateSet.getSourceImage(p1.x), p1.y, borderSize, maxMergeHeight);
				}
			}
...
//			pipName = ctsName+pipName;
	...
	public static void mergeCtsInDir(String path) throws Exception{
		...
		AnimateSetOperator oper = new AnimateSetOperator(0, 2048);
