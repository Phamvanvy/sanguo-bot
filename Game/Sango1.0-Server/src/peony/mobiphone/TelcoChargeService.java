		...
		if(player!=null){
			...
			try {	
				...
				//If MOBI then cardcode=cardcode + ":VMS" 
				  ...
				  else if((cardCode.length()<=12 || cardCode.length()==14) && type==TYPE_VINA)
				...
				//Function get price 
				  ...
				  if(status==1){
					  ...
					  addBalance(player.getAccount().getId(), imoney, Math.round(value), call, cardCode);
...
//				log.error(e, e);
				if(e instanceof MobilePhoneChargeException)
					...
					throw new MobilePhoneChargeException(peony.Messages.STRING_00620);
			}
		}
	}
	
	protected int webCharge(String accountName, int accountId, int type,String cardCode,AccountAsyncCall call) throws MobilePhoneChargeException{
		String cardNum = cardCode;
		log.info("[WEBTELCOCARDTRY]ACC["+accountName+"]TYPE["
				+getTypeString(type)+"]CARDCODE["+cardCode+"]");
		...
		try {	
			  ...
			  if(status==1){
				  ...
				  float value = Float.parseFloat(tripleDES.DecryptVMS(lr.getSessionid(), chr.getDRemainAmount()));
				  int imoney = getimoney(Math.round(value));
				  addBalance(accountId, imoney, Math.round(value), call, cardNum);
				  log.info("[WEBTELCOCARDDOK]ACC["+accountName+"]TYPE["
						  +getTypeString(type)+"]AMOUNT["+imoney+"]CARDCODE["+cardCode+"]");
				  return Math.round(value);
			  }else{
				  log.info("[WEBTELCOCARDDFAIL]ACC["+accountName+"]TYPE["
						  +getTypeString(type)+"]CARDCODE["+cardCode+"]STATUS["+getResponseMessage(status)+"]");
				  throw new MobilePhoneChargeException(getResponseMessage(status));
			  }
		} catch (Exception e) {
	...
	protected void addBalance(int accountId, int imoney, int money, AccountAsyncCall call, String cardNum) {
		Server.server.getServiceRegistry().getAccountService().sendAndRegister(new AddBalanceMessage(accountId,imoney,"telco_"+cardNum,"",money),call);
	}
