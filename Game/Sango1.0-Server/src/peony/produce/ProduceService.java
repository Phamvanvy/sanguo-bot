...
//	public Map<Integer, List<Integer>> map = new HashMap<Integer, List<Integer>>();
	...
	public static String getLevelToString(int level) {
		if (level == 1) {
			return peony.Messages.STRING_01965;
		} else if (level == 2) {
			return peony.Messages.STRING_01966;
		} else if (level == 3) {
			return peony.Messages.STRING_01967;
		} else if (level == 4) {
			return peony.Messages.STRING_01968;
		} else if (level == 5) {
			return peony.Messages.STRING_01969;
		} else if (level == 6) {
			return peony.Messages.STRING_01970;
		} else if (level == 7) {
			return peony.Messages.STRING_01971;
		} else if (level == 8) {
			return peony.Messages.STRING_01972;
		} else if (level == 9) {
			return peony.Messages.STRING_01973;
		} else if (level == 10) {
			return peony.Messages.STRING_01974;
		}
		return "";
	}
...
//	}
	public static int enhancePractice(Player p, int formulaOrItemLevel, int produceOrgatherLevel, int currentPractice, int playerLevel) {
		...
		if(num < ratio && currentPractice < maxPractice){
			...
			if(getPracticeLevel(playerLevel, (currentPractice+1)) > (produceOrgatherLevel+1)){
				...
				pt.putInt(0);
				pt.putString(peony.Messages.STRING_01975);
				pt.putInt(0);
				pt.putInt(0);
				if(p != null){
					p.send(pt);
	...
	public void produce(int formulaId, Player p, ClientSession session, int serial) {
		...
		if(!p.formulaList.contains(formulaId)){
			ErrorHandler.sendErrorMessage(session, serial, OpCode.PRODUCE_CLIENT, peony.Messages.STRING_01976);
			return;
		}
		Formula formula = (Formula) projectData.findObject(Formula.class, formulaId);
			//ObjectAccessor.formulas.get(formulaId); 
		...
		if (ProduceService.getPracticeLevel(p.level, practice) < level1) {
			ErrorHandler.sendErrorMessage(session, serial, OpCode.PRODUCE_CLIENT, peony.Messages.STRING_01977);
			return;
		}
	...
	public static void deleteFormula(Packet packet, ClientSession session) {
		...
		if(p != null){
			try{
				...
				ErrorHandler.sendErrorMessage(session, serial, OpCode.FORMULA_DELETE_CLIENT, peony.Messages.STRING_01978);
			}
		}
	}
	...
	public String getLevelCause(int playerLevel, int producePractice, int formulaLevel) {
		...
		if(practiceLevel < formulaLevel){
			if(getLevel(producePractice) < formulaLevel && getMaxLevelByPlayerLevel(playerLevel) >= formulaLevel){
				return peony.Messages.STRING_01979;
			}else if(getLevel(producePractice) >= formulaLevel && getMaxLevelByPlayerLevel(playerLevel) < formulaLevel){
				return peony.Messages.STRING_01980;
			}else if(getLevel(producePractice) < formulaLevel && getMaxLevelByPlayerLevel(playerLevel) < formulaLevel){
				return peony.Messages.STRING_01981;
			}
		}
		return peony.Messages.STRING_01982;
	}
