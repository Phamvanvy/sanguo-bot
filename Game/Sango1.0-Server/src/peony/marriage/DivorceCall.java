	...
	public void callFinish() throws Exception {
		if(success){
			...
			if(player != null){
				...
				packet.putString(MessageFormat.format(peony.Messages.STRING_01745, service.find(player.id).name,service.find(annotherPersonId).name));
				player.send(packet);
				player.send(packet0);
				MailService mailService = Server.server.getServiceRegistry().getMailService();
				mailService.sendSystemMail(player.id, peony.Messages.STRING_00004, peony.Messages.STRING_01520, MessageFormat.format(peony.Messages.STRING_01521, service.find(annotherPersonId).name), 0, null, 0, "");
				if(ObjectAccessor.getPlayer(annotherPersonId) != null){
					Packet packet1 = new Packet(OpCode.MARRIAGE_DIVORCE_SERVER);
					packet1.putString(MessageFormat.format(peony.Messages.STRING_01523, service.find(annotherPersonId).name,player.name));
...
//					mailService.sendSystemMail(annotherPersonId, "系统", "您已离婚", "您的伴侣"+service.find(player.id).name+"提出申请和您解除婚姻关系，经判定，离婚申请获得批准", 0, null, 0);
					mailService.sendSystemMail(annotherPersonId, peony.Messages.STRING_00004, peony.Messages.STRING_01520, MessageFormat.format(peony.Messages.STRING_01521, service.find(player.id).name), 0, null, 0, "");
				}else if(type == 0){
					mailService.sendSystemMail(annotherPersonId, peony.Messages.STRING_00004, peony.Messages.STRING_01520, MessageFormat.format(peony.Messages.STRING_01521, service.find(player.id).name), 0, null, 0, "");
				}
				chatService.addChatMessage(new ChatMessage(ChatOption.FACTION,-1,player.faction,peony.Messages.STRING_00004,player.faction,MessageFormat.format(peony.Messages.STRING_01522, player.name,service.find(annotherPersonId).name),null));
				Server.server.getEventManager().fireEvent(new ServiceEvent(ServiceEvent.EVENT_DIVORCE,player,annotherPersonId));
			}
		}else{
