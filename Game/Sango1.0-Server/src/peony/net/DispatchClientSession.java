...
//		}
		if (state == State.CONNECTED || state == State.AUTHENTICATED) {
			...
			if(getClient() instanceof Player){
				Player player = (Player)getClient();
				dp.accountId = player.accountId;
				dp.playerId = player.id;
				if(Server.isStepServer && !player.isInStep && packet.getOpCode()!=OpCode.STEPSERVER_CLIENTSERVER_INFO_SERVER 
						&& packet.getOpCode()!=OpCode.STEPSERVER_BATTLE_SIGNUP_SERVER 
						&& packet.getOpCode()!=OpCode.STEPSERVER_BATTLE_SCORE_SERVER
						&& packet.getOpCode()!=OpCode.STEPBATTLE_FINALS_BETANDWATCH_SERVER)
					return;
			}
			session.write(dp);
		}
