...
public class DispatchClientSessionService extends AbstractClientSessionService {
	 ...
	 */
	public Map<Long, DispatchClientSession> sessions = new ConcurrentHashMap<Long, DispatchClientSession>();
	/*
	 * 代理ID生成器
	 */
	...
	public void removeClientSession(ClientSession session) {
		if(Server.isStepServer){
			try {
				if(session!=null && session.getClient()!=null && session.getClient() instanceof Player){
					Player player = (Player)session.getClient();
					if(player!=null){
						long sessionId = StepServer.getStepBattleSessionId(player.accountId, player.id);
						ClientSession s = sessions.remove(sessionId);
						if (s != null) {
	...
	public void removeAllClientSessions(){
		sessions.clear();
	}
	
	public DispatchClientSession removeClientSession(long id) {
		return sessions.remove((Long)id);
	}

	public DispatchClientSession getSession(long id) {
...
//						log.info("session disconnect");
						...
						ds.setDisconnected();
						break;
					case OpCode.SET_ENCRYPT_KEY: // 设定加密key
						ds.setupEncryption(dp.packet.getBytes());
						break;
					default:
