 ...
 * @author lighthu
 */
public class AdminDispatchClientSessionService extends AbstractClientSessionService implements Runnable {
	...
	public void startup() throws Exception {
		...
		notifyProxy();
		notifyProxyThread = new Thread(this);
		notifyProxyThread.start();
	...
	protected void notifyProxy() throws IOException {
        // 用UDP向代理服务器发送注册包
		String proxyIP = config.getString("proxy_address");
		int proxyPort;
		...
		if (proxyIP != null) {
			...
			proxyIP = "221.179.216.53";
			proxyPort = 7001;
			proxyPassword = "proxy@pip2008";
		}
		InetAddress addr = InetAddress.getByName(proxyIP);
    	String cmd = proxyPassword + ":addserver:" + localIP + ":" + localPort + ":3";
