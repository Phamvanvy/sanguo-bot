	...
	public void callFinish() throws Exception {
		...
		if (success) {
			...
			if(auction.getCurrentPrice() == -1){
				pt.putString(peony.Messages.STRING_00813);
			}else{
				if((auction.getEndPrice()>0 && price>=auction.getEndPrice())){
					String msg = MessageFormat.format(peony.Messages.STRING_01420, auction.getName());
					 pt.putString(msg);
				}else{
					 pt.putString(peony.Messages.STRING_00814);
				}
			}
			session.send(pt);
		} else {
	...
	public void run() {
		...
		synchronized(auctionService){
			...
			try {
				...
				error(null, peony.Messages.STRING_00815);
				addToClientSession();
				return;
			}
