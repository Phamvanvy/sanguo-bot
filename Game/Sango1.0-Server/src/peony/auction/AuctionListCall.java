	...
	public void run() {
		...
		if(p != null){
			if (sortfeild != 1 && sortfeild != 2 && sortfeild != 3) {
				error(null, peony.Messages.STRING_00709);
			} else if (asc != 0 && asc != 1) {
				error(null, peony.Messages.STRING_00710);
			} else if(pageNum <= 0){
				error(null, peony.Messages.STRING_00711);
			}else if(amount <= 0){
				error(null, peony.Messages.STRING_00712);
			}else {
				try {
				...
				if (this.auctions == null) {
					error(null, peony.Messages.STRING_00713);
				}
			}
		}
		addToClientSession();
