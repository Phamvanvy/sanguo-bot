...
public class ExamAnswerCall extends ClientSessionAsyncCall {

	private Logger log = Logger.getLogger(ExamAnswerCall.class);
	...
	public ExamAnswerCall(ClientSession session, Packet packet) {
		super(session);
		this.player = (Player)session.getClient();
		this.serial = packet.getInt();
		this.examId = packet.getInt();
		this.answerId = packet.getByte();
	}
	...
	public void callFinish() throws Exception {
		if(player!=null){
			...
			if(!examService.isLegalExam(player.id, examId)){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.EXAM_ANSWER_CLIENT, "很抱歉，您的答题时间已失效");
				return;
			}
			Packet packet = new Packet(OpCode.EXAM_ANSWER_SERVER);
			packet.putInt(serial);
			int examType = examService.getAllocExamType(player.id);
			if(examType==ExamService.EXAM_TYPE_IDLE){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.EXAM_ANSWER_CLIENT, "很抱歉，您没有资格参加比赛");
				return;
			}
			Exam exam = examService.getExamById(examType, examId);
			if(exam==null){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.EXAM_ANSWER_CLIENT, "对不起，您回答的题目不存在");
				return;
			}
			if(answerId==exam.answerIndex){
				packet.put(1);
				...
				log.info("[EXAM-ANSWER]PLAYER["+player.id+"]EXAMID["+examId+"]ANS["+answerId+"]OK");
			}else{
				packet.put(0);
				ExamBoard board = examService.getExamBoard(player.id);
				if(board!=null)
					board.freshDayData();
				log.info("[EXAM-ANSWER]PLAYER["+player.id+"]EXAMID["+examId+"]ANS["+answerId+"]FALSE");
			}
			ExamBoard board = examService.getExamBoard(player.id);
			if(board!=null){
				...
				log.info("[EXAM-ANSWERCOUNT]PLAYER["+board.getPlayerId()+"]COUNT["+board.getPassCount()+"]");
			}else{
				packet.put(0);
				packet.put(0);
				packet.putInt(0);
			...
			if(ansCount==ExamService.countOfDay){
				...
				if(board!=null){
					board.setTotalTime(totalTime + board.getTotalTime());
				}
