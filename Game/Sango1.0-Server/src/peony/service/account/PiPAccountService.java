...
public class PiPAccountService extends Connector implements Service,IMessageHandler,ServiceEventListener,AccountService {
	...
	public void handle(IMessage message) throws Exception {
		...
		case ServerMessageType.ERROR: {
			...
			if (call != null) {
				if (call instanceof IMoneyBuyCall) {
					log.info("[ACCOUNT_RECV_IBUY]SERIAL[" + message.getSerial() + "]CALL[" + call + "]SUCC[false]");
		...
		case GameAccountMessageType.LEGACY_BUY_RESULT:{
			AccountAsyncCall call = calls.remove(message.getSerial());
			if(call != null){
				log.info("[ACCOUNT_RECV_IBUY]SERIAL[" + message.getSerial() + "]CALL[" + call + "]SUCC[true]");
				call.setMessage(message);
				call.callFinish();
			} else {
				log.info("[ACCOUNT_RECV_IBUY]SERIAL[" + message.getSerial() + "]CALL[null]SUCC[true]");
			}
		}
			break;
		...
		case GameAccountMessageType.SYNC_BALANCE: {
			...
			long balance = sbmsg.getLongBalance();
			Account account = getAccount(accountID);
			if (account != null) {
				account.setLongIMoney(balance);
			}
	...
	public void sendAndRegister(IMessage message,AccountAsyncCall call){
		...
		send(message);
		if (message instanceof LegacyBuy1Message) {
			log.info("[ACCOUNT_SEND_IBUY]SERIAL[" + message.getSerial() + "]");
		}
//		executor.execute(call);
	...
	public void sessionRemoved(ClientSession session) {
		...
		if(identity!=null&&identity instanceof Account){
			...
			if(session.getClient() instanceof Player){
				Date date = new Date();
				((Player)session.getClient()).lastLogoutTime = date;
				((Player)session.getClient()).lastLogoutTimeMills = date.getTime();
			}
		}
	}
