 ...
 * 中兴SDK支付接入，请求创建订单。
 */
public class ZhongXingGetOrderCall extends ClientSessionAsyncCall {
	
	protected static final Logger log = LoggerFactory.getLogger(ZhongXingGetOrderCall.class);
	private int serial;
	private int amount;
	private String channel;
	private Player player;
	
	public ZhongXingGetOrderCall(ClientSession session, Packet pt) {
		super(session);
	...
	public void run() {
		...
		// 向billing服务器发起请求
		PostMethod method = new PostMethod(Server.server.billingURL + "zhongxing_get_order");
		...
		try {
			...
			if (code == 200) {
				...
				if (retCode != 0) {
					// 提交出错
					ErrorHandler.sendErrorMessage(getSession(), serial, OpCode.ZHONGXING_GET_ORDER_CLIENT, br.readLine());
				} else {
					// 提交成功
					String orderID = br.readLine();
					Packet pt = new Packet(OpCode.ZHONGXING_GET_ORDER_SERVER);
					pt.putInt(serial);
				...
				// 网络错误
				ErrorHandler.sendErrorMessage(getSession(), serial, OpCode.ZHONGXING_GET_ORDER_CLIENT, "网络故障，请稍后重试");
			}
		} catch (Exception ex) {
