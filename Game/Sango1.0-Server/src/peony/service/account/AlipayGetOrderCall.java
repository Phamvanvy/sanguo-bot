 ...
 * 支付宝支付接入，请求一个订单信息。
 */
public class AlipayGetOrderCall extends ClientSessionAsyncCall {
	
	protected static final Logger log = LoggerFactory.getLogger(AlipayGetOrderCall.class);
	private int serial;
	...
	public AlipayGetOrderCall(ClientSession session, Packet pt) {
		super(session);
		this.serial = pt.getInt();
		this.amount = pt.getString();
		this.subject = pt.getString();
		this.desc = pt.getString();
	...
	public void run() {
		...
		// 向billing服务器发起请求
		PostMethod method = new PostMethod(Server.server.billingURL + "alipay2_order");
		...
		try {
			...
			if (code == 200) {
				...
				if (retCode != 0) {
					// 提交出错
					ErrorHandler.sendErrorMessage(getSession(), serial, OpCode.ALIPAY_GETORDER_CLIENT, br.readLine());
				} else {
					// 提交成功
					String orderInfo = br.readLine();
					Packet pt = new Packet(OpCode.ALIPAY_GETORDER_SERVER);
					pt.putInt(serial);
				...
				// 网络错误
				ErrorHandler.sendErrorMessage(getSession(), serial, OpCode.ALIPAY_GETORDER_CLIENT, "网络故障，请稍后重试");
			}
		} catch (Exception ex) {
