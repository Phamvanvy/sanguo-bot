...
public class IMoneyCardCall extends AccountAsyncCall {
	...
	public void callFinish() throws Exception {
		if(!success){
			...
			if(!added){ //如果没有加入到背包中，那么邮寄
				Server.server.getServiceRegistry().getMailService().sendSystemMail(playerId, peony.Messages.STRING_00004, peony.Messages.STRING_01291, "", 0, item, 1, "IMCARD");
				if(p != null){
					p.message(-1, peony.Messages.STRING_01292, -1, -1);
				}
			}
		}
	}
	...
	public void run() {
		...
		if(p != null){
			this.playerId = p.id;
            if(!Server.createIMoneyCard||p.getAccount().getChannel().equals(Server.CHANNEL_APP_IPHONE)||
            		p.getAccount().getChannel().equals(Server.CHANNEL_APP_IPAD)){
            	ErrorHandler.sendErrorMessage(session, serial, OpCode.IMONEYCARD_CREATE_CLIENT,  "此功能暂未开通，敬请期待");
            	log.info("[IMONEYCARD]ID["+playerId+"]FAIL");
            	return;
			}
