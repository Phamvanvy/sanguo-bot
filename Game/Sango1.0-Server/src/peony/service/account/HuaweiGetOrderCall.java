 ...
 * 华为SDK支付接入，请求创建一个订单。
 */
public class HuaweiGetOrderCall extends ClientSessionAsyncCall {
	
	protected static final Logger log = LoggerFactory.getLogger(HuaweiGetOrderCall.class);
	private int serial;
	private int amount;
	private String channel;
	private Player player;
	
	public HuaweiGetOrderCall(ClientSession session, Packet pt) {
		super(session);
	...
	public void run() {
		...
		// 向billing服务器发起请求
		PostMethod method = new PostMethod(Server.server.billingURL + "huawei_get_order");
		...
		try {
			...
			if (code == 200) {
				...
				if (retCode != 0) {
					// 提交出错
					ErrorHandler.sendErrorMessage(getSession(), serial, OpCode.HUAWEI_GETORDER_CLIENT, br.readLine());
				} else {
					// 提交成功
					...
					Packet pt = new Packet(OpCode.HUAWEI_GETORDER_SERVER);
					pt.putInt(serial);
				...
				// 网络错误
				ErrorHandler.sendErrorMessage(getSession(), serial, OpCode.HUAWEI_GETORDER_CLIENT, "网络故障，请稍后重试");
			}
		} catch (Exception ex) {
