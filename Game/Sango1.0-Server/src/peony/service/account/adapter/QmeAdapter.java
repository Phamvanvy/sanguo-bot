...
public class QmeAdapter {
	...
	private static String IGB_URL_NEW = "http://igb.qme.tw/igb/";
	...
	public static boolean useNewLoginProtocol = true; //临时字段：是否启用新的登陆协议
	...
	public static QmeAccount register(String name, String password, String email) throws QmeException {
		...
		try {
            ...
            if (code == 200) {
                ...
                if (secs.length < 3) {
                    throw new QmeException(peony.Messages.STRING_01177);
                }
                ...
                if (retID > 0) {
                	...
                	switch (retID) {
                	case -1:
                		throw new QmeException(peony.Messages.STRING_01178);
                	case -2:
                		throw new QmeException(peony.Messages.STRING_01179);
                	case -3:
                		throw new QmeException(peony.Messages.STRING_01180);
                	case -4:
                		throw new QmeException(peony.Messages.STRING_01181);
                	case -5:
                		throw new QmeException(peony.Messages.STRING_01182);
                	case -6:
                		throw new QmeException(peony.Messages.STRING_01183);
                	case -7:
                		throw new QmeException(peony.Messages.STRING_01184);
            		default:
                		throw new QmeException(peony.Messages.STRING_01185);	
                	}
                }
            } else {
	...
	public static QmeAccount login(String name, String password) throws QmeException {
		...
		try {
            ...
            if (code == 200) {
                ...
                String[] secs = result.trim().split(",");
                if (secs.length < 4) {
                    throw new QmeException(peony.Messages.STRING_01186);
                }
                ...
                if (lastSec.startsWith("sms://")) {
                	...
                	try {
                		...
                		throw new QmeException(MessageFormat.format(peony.Messages.STRING_01187, lastSec));
                	}
                	QmeAccount acc = new QmeAccount();
                	acc.name = name;
                	acc.tsi = secs[2];
	 ...
	 * 最新登录QME帐号。
	 */
	public static QmeAccount newLogin(String name, String password) throws QmeException {
		GetMethod method = new GetMethod(IGB_URL_NEW);
		method.addRequestHeader( "Connection", "close");
		method.getParams().setParameter(HttpMethodParams.RETRY_HANDLER, new DefaultHttpMethodRetryHandler(0, false));
		log.info("[QME_LOGIN]name[" + name + "]password[" + password + "]");
		try {
            ...
            httpclient.getParams().setSoTimeout(30000);
            List<NameValuePair> params = new ArrayList<NameValuePair>();
            params.add(new NameValuePair("c", "ver_6_2"));
            params.add(new NameValuePair("m", "sync_tsi"));
            params.add(new NameValuePair("smbody", "55123joyaa0c01p13d"));
            params.add(new NameValuePair("qmeacc", name));
            params.add(new NameValuePair("qmepwd", password));
            NameValuePair[] arr = new NameValuePair[params.size()];
            params.toArray(arr);
            ...
            if (code == 200) {
                String result = method.getResponseBodyAsString();
                log.info("[QME_LOGIN]result[" + result.trim() + "]");
                JSONObject res = JSONObject.fromString(result);
                if (res.getString("qchest_msg")!=null && res.getString("qchest_msg").contains("LoginOK")) {
                	QmeAccount acc = new QmeAccount();
                	acc.name = name;
                	acc.tsi = res.getString("tsi");
                	String sms = res.getString("qchest_sms");
                	if(sms.equalsIgnoreCase("null"))
                		acc.smsCode = null;
                	else
                		acc.smsCode = sms;
                	acc.qbID = 0;
                	try {
						acc.qbID = Integer.parseInt(res.getString("qchest_userid"));
					} catch (Exception e) {
						e.printStackTrace();
					}
                	return acc;
	...
	public static void pay(int userID, int amount) throws QmeException {
		if (!contentIDMap.containsKey(amount)) {
			throw new QmeException(peony.Messages.STRING_01188);
		}
		...
		try {
            ...
            if (code == 200) {
                ...
                if ("100000015".equals(retCode)) {
                	...
                	throw new QmeException(MessageFormat.format(peony.Messages.STRING_01189, retMsg));
                }
            } else {
                log.info("[QME_PAY]code[" + code + "]ERROR");
                throw new QmeException(peony.Messages.STRING_01190);
            }
		} catch (QmeException qe) {
			throw qe;
        } catch (Exception ex1) {
