...
public class AppStoreService implements Service {
	...
	//App充值成功后通知对方平台的url参数
	public static String url = "http://igb.qme.tw/igb/storeValue/ios/";
    public static String acc = "gameta";
    public static String pwd = "8244d77a246df9b8648228065c942c1d";
    public static Map<String, String> receipt2contentId = new HashMap<String, String>();
    
    static {
    	receipt2contentId.put("60", "614");
		receipt2contentId.put("122", "615");
		receipt2contentId.put("248", "616");
		receipt2contentId.put("498", "617");
		receipt2contentId.put("998", "618");
    }
	...
	private static class AppStoreCharge2Call extends ClientSessionAsyncCall {
		...
		public AppStoreCharge2Call(ClientSession session, Account acc, Player p, int serial, String bid, byte[] receipt, String clientID) {
			...
			this.serial = serial;
			this.acc.setReceipt(this.receipt);
			log.info("[APPSTORE_CHARGE]" + LogUtil.getPlayerLogString(p) + "BID[" + bid + "]RECEIPT[" + this.receipt + "]TRY");
		}
...
//			method.setParameter("partition", Server.server.usePartitionBalance ? Server.server.gameCode : "");
	...
	private static class AppStoreListProduct2Call extends ClientSessionAsyncCall {
		...
		public void callFinish() throws Exception {
			if (success) {
				...
				if(p != null && p.level >= Player.LIMITSHOWOTHERCHARGELEVEL && (limit==-1 || p.pool.getInt(Player.ISSHOWPIPCHARGEFLAG, 0)==1)){
					packet.put(products.size()+1);
					log.info("[APPSTORESERVICESENDPRODUCTS1]COUNT["+(products.size()+1)+"]LIMIT["+limit+"]FLAG["+p.pool.getInt(Player.ISSHOWPIPCHARGEFLAG, 0)+"]");
				} else {
					packet.put(products.size());
					log.info("[APPSTORESERVICESENDPRODUCTS2]COUNT["+(products.size())+"]LIMIT["+limit+"]FLAG["+p.pool.getInt(Player.ISSHOWPIPCHARGEFLAG, 0)+"]");
				}
				...
				if(p != null && p.level >= Player.LIMITSHOWOTHERCHARGELEVEL && (limit==-1 || p.pool.getInt(Player.ISSHOWPIPCHARGEFLAG, 0)==1)){
					packet.putString("chargeinweb");
					packet.putString("其它充值方式");
					packet.putInt(1);
					packet.putInt(1);
				}
				packet.putInt(limit);
