...
public class AccountQuickRegisterCall extends AccountAsyncCall {
	...
	public AccountQuickRegisterCall(ClientSession session,Packet pt){
		...
		session.setInitChannel(AccountLoginCall.cutChannel(version));
		try {
	...
	public void callFinish() throws Exception {
		if (success) {
			...
			log.info("[QUICKREGERROR]VERSION["+version+"]IP["+session.getClientIP()+"]MESS["+ErrorMessages.getErrorMesssage(msg)+"]MSG["+msg+"]");
		}
	}
	...
	public void run() {
		...
		if (Server.server.REVISION_TYPE_TW.equals(Server.server.revision)) {
			ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNT_QUICK_REG_CLIENT, peony.Messages.STRING_00959);
			return;
		}
		if (Server.server.REVISION_TYPE_CMCC.equals(Server.server.revision)||Server.server.REVISION_TYPE_TEL.equals(Server.server.revision)) {
			if(model.contains("Android") || cmccUserId.equals("") && cmccUserKey.equals("")){
