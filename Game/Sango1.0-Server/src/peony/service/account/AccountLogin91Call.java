 ...
 * @author dchen
 */
public class AccountLogin91Call extends AccountAsyncCall {
	
	protected static final Logger log = LoggerFactory.getLogger(AccountLogin91Call.class);

	protected String uin,clientSessionId,version,model,uiModel,realPhone;
	protected String accountName;
	protected int serial,playerId;
	protected Version v;
	protected static final String PUBLIC_PASSWORD = "0f04z34jw";
	
	public AccountLogin91Call(ClientSession session,String uin,String clientSessionId,
			String version,String model,String uiModel,String realPhone,int playerId,int serial){
		super(session);
		this.uin = uin;
		this.accountName = "91:" + uin;
	...
	public void callFinish() {
		if (success) {
			...
			// 其他错误通知客户端
			ErrorHandler.sendErrorMessage(getSession(), serial, OpCode.ACCOUNT_LOGIN_91_CLIENT, ErrorMessages.getErrorMesssage(em));
		}
	}
	...
	public void run() {
    	...
    	if (!session.checkOnlineCount(0)) {
            ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNT_LOGIN_91_CLIENT, "已经到达最大登录数量");
            return;
        }
		try {
			v = Server.server.getServiceRegistry().getVersionService().getVersion(version);
		} catch (Exception e) {
			ErrorHandler.sendErrorMessage(getSession(), serial, OpCode.ACCOUNT_LOGIN_91_CLIENT, e.getMessage());
			return;
		}
		...
		if (result != null) {
			ErrorHandler.sendErrorMessage(getSession(), serial, OpCode.ACCOUNT_LOGIN_91_CLIENT, result);
			return;
		}
	...
	// 验证91用户登录，如果成功返回null，错误返回错误信息。
	protected String verifyLogin() {
		PostMethod method;
		BufferedReader br = null;
		method = new PostMethod(Server.server.billingURL + "91_login");
		method.addRequestHeader("Connection", "close");
		method.setParameter("gamecode", "6");
		method.setParameter("uin", uin);
