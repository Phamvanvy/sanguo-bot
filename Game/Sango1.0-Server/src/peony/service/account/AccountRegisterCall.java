...
public class AccountRegisterCall extends AccountAsyncCall {
	...
	public AccountRegisterCall(ClientSession session,Packet pt){
		...
		if(Server.server.REVISION_TYPE_CMCC.equals(Server.server.revision)||Server.server.REVISION_TYPE_TEL.equals(Server.server.revision)){
			...
			if(Server.server.REVISION_TYPE_CMCC.equals(Server.server.revision) && this.cmccUserId.equals(""))
				this.cmccUserId = CmccAccountService.CMCC_ANDROID_USERID;
			this.cmccUserKey = pt.getString();
			if(Server.server.REVISION_TYPE_CMCC.equals(Server.server.revision) && this.cmccUserKey.equals(""))
				this.cmccUserKey = CmccAccountService.CMCC_ANDROID_USERKEY;
		}
	...
	public void callFinish() throws Exception {
		if (success) {
		    ...
		    if (Server.server.REVISION_TYPE_TW.equals(Server.server.revision)) {
	...
	public void run() {
		if(!Server.server.getConfig().getBoolean("AllowReg", true)){
			ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNT_REG_CLIENT, peony.Messages.STRING_00959);
			return;
		}
		
		if(initPass.length()==0){
			ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNT_REG_CLIENT, peony.Messages.STRING_00960);
			return;
		}
		if (Server.server.REVISION_TYPE_TW.equals(Server.server.revision)) {
			try {
		...
		else if(Server.server.REVISION_TYPE_CMCC.equals(Server.server.revision)||Server.server.REVISION_TYPE_TEL.equals(Server.server.revision)){
