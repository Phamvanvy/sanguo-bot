		...
		try {
			...
			if(type==1){
				if(!validPhone(value1)){
					ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ACCOUNTBINDING_CLIENT, peony.Messages.STRING_01314);
					return;
				}
				if(!Server.server.getServiceRegistry().getAccountBindingService().validIdcard(accountId, idcard)){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNTBINDING_CLIENT, peony.Messages.STRING_01315);
					return;
				}
				sms = Server.server.getServiceRegistry().getAccountBindingService().bindPhone(accountId, value1);
				retMessage = peony.Messages.STRING_01316;
			}else if(type==2){
				if(!validIdcard(value1)){
					ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ACCOUNTBINDING_CLIENT, peony.Messages.STRING_01317);
					return;
				}
				Server.server.getServiceRegistry().getAccountBindingService().bindIdCard(accountId, value1);
//				Server.server.getServiceRegistry().getMailService().sendSystemMail(playerId, "系统", "绑定身份证奖励", "", 0, ObjectAccessor.createGameItem(REWARDS[0][0]), REWARDS[0][1], "ACCBIND");
//				Server.server.getServiceRegistry().getMailService().sendSystemMail(playerId, "系统", "绑定身份证奖励", "", 0, ObjectAccessor.createGameItem(REWARDS[1][0]), REWARDS[1][1], "ACCBIND");
//				retMessage = "恭喜您绑定成功，您的奖励已通过飞鸽发送。请稍候查收。";
				retMessage = peony.Messages.STRING_01318;
			}else if(type==3){
				if(!validQna(value1, value2)){
					ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ACCOUNTBINDING_CLIENT, peony.Messages.STRING_01319);
					return;
				}
				if(!Server.server.getServiceRegistry().getAccountBindingService().validIdcard(accountId, idcard)){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNTBINDING_CLIENT, peony.Messages.STRING_01315);
				...
				retMessage = peony.Messages.STRING_01318;
//				retMessage = "恭喜您绑定成功，您的奖励已通过飞鸽发送。请稍候查收。";
//				Server.server.getServiceRegistry().getMailService().sendSystemMail(playerId, "系统", "绑定问题奖励", "", 0, ObjectAccessor.createGameItem(REWARDS[2][0]), REWARDS[2][1], "ACCBIND");
//				Server.server.getServiceRegistry().getMailService().sendSystemMail(playerId, "系统", "绑定问题奖励", "", 0, ObjectAccessor.createGameItem(REWARDS[3][0]), REWARDS[3][1], "ACCBIND");

			}else if(type==4){
				if(!validMail(value1)){
					ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ACCOUNTBINDING_CLIENT, peony.Messages.STRING_01320);
					return;
				}
				if(!Server.server.getServiceRegistry().getAccountBindingService().validIdcard(accountId, idcard)){
					ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNTBINDING_CLIENT, peony.Messages.STRING_01315);
				...
				retMessage = peony.Messages.STRING_01321;
			}
			if (type == 1) {
