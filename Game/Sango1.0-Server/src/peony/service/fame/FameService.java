...
public class FameService implements Service, ServiceEventListener {
	...
	public Map<Integer,Integer> topFame = new HashMap<Integer,Integer>();
	...
	public static int rewardBuff = 3500;//国公buff
						...
						synchronized (statuePlayer) {
							try {
								...
								for(int faction=GameObject.FACTION_WEI; faction<=GameObject.FACTION_WU; faction++){
									...
									nation.pool.setInt(Nation.PROPERTY_GET_KINGBUFF, 0);
								}
	...
	protected void freshStatue(int playerId, int mapId, int x, int y) {
		synchronized (statuePlayer) {
			...
			statue.x = x;
			statue.y = y;
			statue.name = MessageFormat.format(peony.Messages.STRING_00539,p.name);
		...
		if (fame != null && fame.size() != 0) {
			...
			for (Fame fa : fame) {
			    ...
			    if(fa.faction==1){
			    	...
			    	if(weiIndex == 0){
			    		topFame.put(fa.faction, -statue.id);
			    	}
			    	weiIndex++;
				...
				statue.systemState = Player.SYSTEMSTATE_READY;
				statue.cards.cardInfos=fa.cards.cardInfos;
				statue.cards.equipCards=fa.cards.equipCards;
				statue.cards.exp=fa.cards.exp;
				statue.cards.horseEquipCards=fa.cards.horseEquipCards;
				statuePlayer.put(statue.id, statue);
			}
		} else {
			...
			if (p!=null) {
				if (p.id!=playerId) {
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.FAME_ADDINFO_CLIENT, peony.Messages.STRING_00540);
					return;
				}
				...
				statue.refreshProperties(false);
				statue.setCredit(p.getCredit(),"FAME"); 
				statue.setWeekCredit(p.getWeekCredit());
				statue.setRank(p.getRank());
				statue.relations = p.relations;
				statue.cards=p.cards;
	...
	public void saveFame(Player statue){
		...
		fame.cards.cardInfos=statue.cards.cardInfos;
		fame.cards.equipCards=statue.cards.equipCards;
		fame.cards.exp=statue.cards.exp;
		fame.cards.horseEquipCards=statue.cards.horseEquipCards;
		PvpInfo pvpInfo = service.getPvpInfo(-statue.id, statue.faction);
		fame.pool=pvpInfo.pool.clone();
		//卡片收藏信息复制
		fame.pool.deleteByPrefix(CardService.CARD_NEW_PROPERTY);
		fame.pool.deleteByPrefix("PROPERTY_HAVECARD");
...
//				fame.pool.setInt(CardService.getPropertyOfPlayerCard(cd.id), statue.pool.getInt(CardService.getPropertyOfPlayerCard(cd.id),0));
				CardService.addCard_Fame(fame, cd.id, CardService.getCardCount(statue, cd.id));
			}
		}
...
//	}
	...
	protected void rewardTitleItems(int playerId){
		String message = peony.Messages.STRING_00541;
		...
		service.sendSystemMail(playerId, peony.Messages.STRING_00004, message, peony.Messages.STRING_00542, 0, item, 1, "FAMEREWARD");
	}
