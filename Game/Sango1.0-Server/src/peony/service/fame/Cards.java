...
//			throw new CardException("此装备位已经镶嵌");
	...
	public static Cards getFromDBBytes(DataInputStream dis, Fame fame){
		try{
			dis.readByte(); //version
			Cards cards = new Cards(fame);
			int exp = dis.readInt();
			cards.exp = exp;
	...
	public static Cards getFromDBBytes(byte[] bytes, Fame fame){
		try{
