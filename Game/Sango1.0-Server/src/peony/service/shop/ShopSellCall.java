...
public class ShopSellCall extends ClientSessionAsyncCall {
	...
	public ShopSellCall(ClientSession session, Packet packet) {
		...
		if(gridIdByte!=-1 && gridIdByte<0){
			this.gridId = gridIdByte & 0xFF;
		}else{
			this.gridId = gridIdByte;
		}
	...
	public void run() {
		...
		if (player != null) {
			...
			if (count > 0) {
				...
				if (template != null) {
					if (!template.canSale) {
						error(null, peony.Messages.STRING_00998);
					} else {
						...
						if(gameItem != null && gameItem.template.isEquipment()){
							...
							if(itemEnhance != null && itemEnhance.getJewelCount() > 0 && itemEnhance.findJewelByLevel(ItemEnhance.CANT_REMOVE_LEVEL)){
								error(null, peony.Messages.STRING_00999);
								addToClientSession();
								return;
							}
						}
						...
						if (grid != null) {
							...
							if(tong!=null && tong.taxRate>0 && tongService.getPlayerTong(player.id,true)!=tong){
							...
							tx.rollback();
							error(null, peony.Messages.STRING_01000);
						}
					}
				} else {
					error(null, peony.Messages.STRING_01000);
				...
				error(null, peony.Messages.STRING_01001);
			}
			addToClientSession();
		}
	}
