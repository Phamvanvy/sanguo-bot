			...
			if (Server.server.REVISION_TYPE_CMCC.equals(Server.server.revision)
				...
				if (Server.server.REVISION_TYPE_PIP.equals(Server.server.revision)
					...
					LegacyBuy1Message message = new LegacyBuy1Message(
							a.getId(), a.getKey(), imoney, !allowUntrustIMoney);
					service.sendAndRegister(message, this);
				}
			}
			try {
		...
		if (message != null && message instanceof LegacyBuyResultMessage) {
			...
			if (msg.isSuccess()) {
				LogUtil.logIMoneyBuyOK(request.player.id, accountId,
						request.buyObject, true, msg.getLongBalance(), msg
								.getCost(), level);
				IBuy ibuy;
				...
				LogUtil.logIMoneyBuyOK(request.player.id, accountId,
						request.buyObject, false, msg.getLongBalance(), msg
								.getCost(), level);
			}
			Player p = ObjectAccessor.getPlayer(request.player.id);
			if (p != null && p.session != null) {
				if (msg.isSuccess()||(Server.server.REVISION_TYPE_PIP.equals(Server.server.revision))) { //如果连移动的认证，那么购买失败以后可能传回0的余额
					if (!(Server.server.REVISION_TYPE_PIP.equals(Server.server.revision) && this.consumeCode != null)) { //如果是pip版本，并且consumeCode不为空，那么就是话费专区
						Account a = (Account) p.session.getIdentity();
						long oldIMoney = a.getLongIMoney();
						if (oldIMoney != msg.getLongBalance()) {
							a.setLongIMoney(msg.getLongBalance());
