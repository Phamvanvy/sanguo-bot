...
public class TowerDefendService implements Service, VMapManager, ServiceEventListener {
	...
	protected static String[] endPiroid = {"16:00","22:00","00:00"};
	...
	protected Map<Integer, Integer> enterCount = new HashMap<Integer, Integer>();//记录玩家每日已进入副本次数
	protected static int COUNTLIMIT = 5;//每日进入副本上限
...
//					sendWorldMessage(peony.Messages.STRING_01111);
				}
			}
		...
		String p = endPiroid[endPiroid.length-1];
		int hour = Integer.parseInt(p.split(":")[0]);
		int min = Integer.parseInt(p.split(":")[1]);
		Server.server.scheduExec.scheduleAtFixedRate(new Runnable(){
			public void run() {
	...
	public void signUp(Player player) throws TowerDefendException {
		synchronized (this) {
			...
			if(currentHour!=15 && currentHour!=21 && currentHour!=23)
				throw new TowerDefendException("此时间不能进行斗阵");
			if (player != null) {
				if (player.party == null || player.party.leader.getId() != player.id)
					throw new TowerDefendException(peony.Messages.STRING_01112);
				if(isInQueue(player))
					throw new TowerDefendException(peony.Messages.STRING_01113);
				List<String> countLimit = canSign(player);
				if(countLimit!=null && countLimit.size()>0){
					throw new TowerDefendException(MessageFormat.format("{0}今天已经进入5次了", countLimit.get(0)));
				}
				if(getInstanceByPartyId(player.party.id)!=null)
					throw new TowerDefendException(peony.Messages.STRING_01114);
				...
				if(!canSignUp)
					throw new TowerDefendException(MessageFormat.format(peony.Messages.STRING_01115, config.MINLEVEL));
...
//					sendWorldMessage(peony.Messages.STRING_01111);
	...
	public void removeInstance(TowerDefendInstance instance){
		synchronized (this) {
			//清理副本中所有GameObject
			for (GameObject o : instance.map.instanceid2objects.values()) {
				if(o==null)
					continue;
	...
	protected void processPlayerLoad(Player p){
		try {
			if(p!=null && p.getVMap()!=null && p.getVMap().getId()!=config.mapId){
				...
				int count = p.bag.getGameItemCount(921);
				if(count>0){
					PlayerTransaction tx = p.newTransaction("TOWERDEFEND");
					GameItem item = p.bag.removeGameItemIngoreInstanceId(921, count, tx, false);
					if(item!=null)
						tx.commit();
					else
						tx.rollback();
	...
	/** 报名时监测队员已经进入副本次数*/
	public List<String> canSign(Player p){
		List<String> ret = new ArrayList<String>();
		if(p.party!=null){
			for(PartyMember pm : p.party.members){
				if(enterCount.containsKey(pm.player.id)){
					int count = enterCount.get(pm.player.id);
					if(count>=COUNTLIMIT){
	...
	/** 记录队员进入副本次数*/
	public void enterCount(int playerId){
		if(enterCount.containsKey(playerId)){
			int count = enterCount.get(playerId);
			enterCount.put(playerId, count+1);
		}else{
			enterCount.put(playerId, 1);
		}	
	}
