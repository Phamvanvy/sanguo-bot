...
public class TowerDefendInstance implements Instance {
	...
	public void update(int diff) {
		...
		if(state==STATE_LOAD && !playerTransed){
			checkEnd();
		}
	}
...
//					MessageFormat.format(peony.Messages.STRING_01175, td.leaderName));
//		}else if(td.type==1){
//			Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(td.faction, 
//					MessageFormat.format(peony.Messages.STRING_01176, td.leaderName));
//		}
		Server.server.getEventManager().addEvent(new ServiceEvent(ServiceEvent.EVENT_DOUZHEN_WIN,td.partyId));
					...
					if(re.itemIds!=null && re.count!=null){
						for(int i=0;i<re.itemIds.size();i++){
							try {
								...
								mailService.sendSystemMailAsync(p.id, peony.Messages.STRING_00004, "斗阵奖励", "", 0, 
										ObjectAccessor.createGameItem(re.itemIds.get(i)), re.count.get(i), "TDWIN");
							}
						}
					}
					p.setHonor(p.honor+re.rank, true, "TD");
					p.goMap(out[0], out[1], out[2]);
					if(p.pool.getInt(Player.WELFARE_JOIN_TOWER,0) == 0){
						p.pool.setInt(Player.WELFARE_JOIN_TOWER, 1);
