...
public class CleanBossActivity implements IActivityImpl, VMapManager, Runnable {
	...
	protected static int[] expQuotiety = {10, 10, 20}; // 经验系数
	
	protected static int[][] position = {{150,250}, {370,300}, {240,420}, {480,560}}; // 真天狗位置
					...
					public void run() {
						for(int i=0;i<maps.length;i++){
							...
							if(map.getCreatureById(BOSS[i])!=null && map.getCreatureById(BOSS[i]).isAlive())
								continue;
							RefreshNpcCall call = new RefreshNpcCall(RefreshNpcCall.CLEANBOSSACTIVITY);
							call.npcId = BOSS[i];
							call.map = map;
...
//							VMapUtil.addCreature(map, ps[0], ps[1],(GameMapNPC) gmo, true, 0, null);
	...
	public static void refreshBoss(int bossId,VMap map){
		ProjectData proj = Server.server.getServiceRegistry().getDataService().data;
		GameMapObject gmo = GameMapObject.findByID(proj, bossId);
		int index = random.nextInt(position.length);
		int[] ps = position[index];
		VMapUtil.addCreature(map, ps[0], ps[1],(GameMapNPC) gmo, true, 0, null);
	}
			...
			if(getMapIndex(mapId)!=-1){
				...
				if(player.level<maxLevel){
					...
					ErrorHandler.sendErrorMessage(player.session, -1,
							OpCode.TELEPORT_CLIENT, peony.Messages.STRING_00807);
					return null;
					
				}
			}
			ErrorHandler.sendErrorMessage(player.session, -1,
					OpCode.TELEPORT_CLIENT, peony.Messages.STRING_00808);
			return null;
		}
	}
	...
	public void update(int diff) {
		...
		for(int i=0;i<maps.length;i++){
			...
			for(GameObject o : map.instanceid2objects){
				if(o instanceof Creature && (o.name.equals(peony.Messages.STRING_00809) || o.name.equals(peony.Messages.STRING_00810))){
					if(o.isAlive())
						f = false;
				}
			}
