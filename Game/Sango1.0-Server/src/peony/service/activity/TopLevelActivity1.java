...
public class TopLevelActivity1 implements IActivityImpl {
	...
	protected void sendReward(Actor actor, int rewardLevel, int itemID) {
		...
		switch (rewardLevel) {
		case 0:
			title = peony.Messages.STRING_00533;
			content = peony.Messages.STRING_00534;
			break;
		case 1:
			title = peony.Messages.STRING_00535;
			content = peony.Messages.STRING_00536;
			break;
		case 2:
			title = peony.Messages.STRING_00537;
			content = peony.Messages.STRING_00538;
			break;
		}
		content = MessageFormat.format(content, actor.name);
		Server.server.getServiceRegistry().getMailService().sendSystemMail(
			actor.id, peony.Messages.STRING_00004, title, content, 0, item, 1, "ACTV");
