...
public class DayOnlineActivity1 implements IActivityImpl, ServiceEventListener {
	...
	protected void sendReward(Player p, int days) {
		...
		try {
			...
			tx.commit();
			String content = MessageFormat.format(peony.Messages.STRING_00630, p.name);
			p.message(-1, content, -1, -1);
		} catch (Exception e) {
			tx.rollback();
			
			// 如果背包满则发送邮件
			String content = MessageFormat.format(peony.Messages.STRING_00631, p.name);
			Server.server.getServiceRegistry().getMailService().sendSystemMail(
				p.id, peony.Messages.STRING_00004, peony.Messages.STRING_00632, content, 0, item, itemCount, "ACTV");
			content = MessageFormat.format(peony.Messages.STRING_00633, p.name);
			p.message(-1, content, -1, -1);
		}
