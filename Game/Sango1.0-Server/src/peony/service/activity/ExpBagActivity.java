...
//				ServiceEvent.EVENT_CHARGE_SUCCESS,
	...
	protected void processCharge(Player p, int type, int money){
		if(p!=null){
			addGain(p, peony.Messages.STRING_00869, "CHARGE");
		}
	}
	
	protected void processRemoveJewel(Player p){
		if(p!=null){
			addGain(p, peony.Messages.STRING_01721, "REMOVEJEWEL");
		}
	}
	
	protected void processMergeJewel(Player p){
		if(p!=null){
			addGain(p, peony.Messages.STRING_01802, "MERGEJEWEL");
		}
	}
	...
	protected void processEnhance(Player p, int type){
		if(p!=null){
			if(type==0){
				addGain(p, peony.Messages.STRING_01803, "NATUALENHANCE");
			}else if(type==1){
				addGain(p, peony.Messages.STRING_01804, "STARENHANCE");
			}
		}
	}
	...
	protected void processIbuy(int playerId, int imoney){
		...
		if(p!=null){
			addGain(p, peony.Messages.STRING_01805, "IBUY");
		}
	}
	
	protected void processProduce(Player p){
		if(p!=null && p.level>=50){
			addGain(p, peony.Messages.STRING_01806, "PRODUECE");
		}
	}
	...
	protected void processUseItem(Player p, int itemId){
		if(p!=null){
			if(itemId==1183){
				addGain(p, peony.Messages.STRING_01807, "YIHESU");
			}else if(itemId==499){
				addGain(p, peony.Messages.STRING_01808, "SHIZIHOU");
			}
		}
	}
	...
	protected void processSkillRefresh(Player p){
		if(p!=null){
			addGain(p, peony.Messages.STRING_01809, "SKILLREFRESH");
		}
	}
	
	protected void processDig(Player p){
		if(p!=null){
			addGain(p, peony.Messages.STRING_01810, "DIG");
		}
	}
		...
		if(p!=null){
			if(channel==ChatOption.FACTION){
				...
				if(nationService.getForbidByTargetId(p.id)==null && (System.currentTimeMillis()-lastTime)>=60*60*1000L){
					addGain(p, peony.Messages.STRING_01811, "FACTIONCHAT");
					factionChatTime.put(p.id, System.currentTimeMillis());
				}
			}else if(channel==ChatOption.WORLD){
				addGain(p, peony.Messages.STRING_01812, "WORLDCHAT");
			}
		}
	}
	...
	protected void processBattleWin(int playerId, int type){
		...
		if(p!=null){
			if(type==0)
				addGain(p, peony.Messages.STRING_01813, "NATIONBATTLEWIN");
			else if(type==1)
				addGain(p, peony.Messages.STRING_01814, "FLAGBATTLEWIN");
			else if(type==2)
				addGain(p, peony.Messages.STRING_01815, "TONGBATTLEWIN");
		}
	}
	
	protected void processAddJewel(Player p, GameItem item){
		if(p!=null){
			addGain(p, peony.Messages.STRING_01816, "ADDJEWEL");
		}
	}
		...
		if(kill instanceof Player && died instanceof Player){
			...
			if(Server.server.getServiceRegistry().getFlagBattleFieldVMapManager().isInFlagBattle(killPlayer)==1){
				...
				if(record.itemCount>=5)
					return;
				if(addGain(killPlayer, peony.Messages.STRING_01817, "FLAGBATTLEKILLPLAYER")){
					record.itemCount++;
				}
			}
		}
