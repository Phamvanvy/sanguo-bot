...
public class FestivalConvoyAI implements CreatureAI {
		...
		public static int convoyTimes = 60 * 60 * 1000;	//押镖时间1个小时
		
		public boolean isLeave;	//是否离镖车30码

		public FestivalConvoyAI(PlayerConvoy convoy, Creature creature,int index) {
			this.convoy = convoy;
		...
		public void success() {
			FestivalConvoyActivity.success(convoy);
		}
		...
		class InitState implements State {
			...
			public void update() {
				if ((Time.currTime - createTime) >= creatureWaitTime) {
					FestivalConvoyAI.this.state = new RunningState(Time.currTime, indexInit);
				}
			}
		}
		...
		class RunningState implements State {
			...
			public void update() {
				...
				if(Time.currTime - convoy.beginTime > convoyTimes){	//押镖1个小时没有完成
