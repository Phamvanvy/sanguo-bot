...
public class ActivitySchedule {
	...
	public void parse(String data) throws Exception {
		...
		for (String line : lines) {
		    ...
		    if ("starttime".equals(varName)) {
		    	...
		    	timePeriods = parseTimePeriodString(varValue);
		    }
		}
	}
	
	private int[] parseTimePeriodString(String str) {
		String[] secs = Utils.splitString(str, ',');
		int[] ret = new int[secs.length];
		for (int i = 0; i < ret.length; i++) {
			if (secs[i].contains(":")) {
				int pos = secs[i].indexOf(':');
				int hour = Integer.parseInt(secs[i].substring(0, pos));
				int min = Integer.parseInt(secs[i].substring(pos + 1));
				ret[i] = hour * 60 + min;
			} else {
				int hour = Integer.parseInt(secs[i]);
				ret[i] = hour * 60;
			}
		}
		return ret;
	...
	public String toString() {
		...
		if (timePeriods != null) {
			...
			buf.append(getTimePeriodString());
			buf.append(";");
		}
	...
	private String getTimePeriodString() {
		...
		for (int i = 0; i < timePeriods.length; i++) {
			...
			int hour = timePeriods[i] / 60;
			int min = timePeriods[i] % 60;
			if (min == 0) {
				sb.append(hour);
			} else {
				sb.append(hour + ":" + min);
	...
	public boolean in() {
		...
		if (timePeriods != null) {
			...
			for (int i = 0; i < timePeriods.length; i += 2) {
				if (min >= timePeriods[i] && min < timePeriods[i + 1]) {
					match = true;
					break;
				}
			}
