...
public class IBuyActivity1 implements IActivityImpl, ServiceEventListener {
	...
	protected void sendReward(int playerID, int type) {
		...
		try {
			...
			String content;
			if (type == 0) {
				content = MessageFormat.format(peony.Messages.STRING_01325, p.name);
			} else {
				content = MessageFormat.format(peony.Messages.STRING_01326, p.name);
			}
			...
			String content;
			if (type == 0) {
				content = MessageFormat.format(peony.Messages.STRING_01327, p.name);
			} else {
				content = MessageFormat.format(peony.Messages.STRING_01328, p.name);
			}
			Server.server.getServiceRegistry().getMailService().sendSystemMail(
				p.id, peony.Messages.STRING_00004, peony.Messages.STRING_01329, content, 0, item, 1, "ACTV");
			if (type == 0) {
				content = MessageFormat.format(peony.Messages.STRING_01330, p.name);
			} else {
				content = MessageFormat.format(peony.Messages.STRING_01331, p.name);
			}
			p.message(-1, content, -1, -1);
		}
