	...
	protected void sendGift(Player p, int itemId, int count){
		if(p!=null){
			...
			try {
				...
				tx.commit();
				String content = MessageFormat.format(peony.Messages.STRING_01676, p.name, GIFT_MONEY);
				p.message(-1, content, -1, -1);
			} catch (Exception e) {
				tx.rollback();
				String content = MessageFormat.format(peony.Messages.STRING_01677, p.name, GIFT_MONEY);
				...
				content = MessageFormat.format(peony.Messages.STRING_01678, p.name, GIFT_MONEY);
				p.message(-1, content, -1, -1);
			}
		}
	}
