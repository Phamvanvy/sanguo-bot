...
public class TongInviteCall extends ClientSessionAsyncCall {
	...
	public static int INVITEMINLEVEL = 20;
	...
	public void run() {
		...
		if(actor.id==player.id){
			error(null, peony.Messages.STRING_00683);
			addToClientSession();
			return;			
		}
		if(actor.faction!=player.faction){
			error(null, peony.Messages.STRING_01642);
			addToClientSession();
			return;
		}
		if(actor.level<INVITEMINLEVEL){
			error(null, MessageFormat.format(peony.Messages.STRING_01643, INVITEMINLEVEL));
			addToClientSession();
			return;
		}
		...
		if(rel!=null&&rel.blackList.exists(player.id)){
			error(null, peony.Messages.STRING_01644);
			addToClientSession();
			return;
		}
		targetID = actor.id;
		...
		if(targetPlayer != null){
			if(!targetPlayer.isAllowGuild()){
				error(null, peony.Messages.STRING_00261);
				addToClientSession();
				return;
			}
		}
		...
		try {
			...
			tong = tongService.getPlayerTong(player.id,true);
