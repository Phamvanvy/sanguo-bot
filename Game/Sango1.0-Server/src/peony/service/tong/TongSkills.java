...
public class TongSkills {
	protected Map<Integer,TongSkill> skills = new TreeMap<Integer,TongSkill>();
	
	/**
	 * 在载入一个国家的技能以后需要进行init操作，传入的参数是当前初始就应该存在的国家技能，用在添加新的国家技能以后可以即时的更新
	 * @param initSkills
	 */
	public void init(Collection<TongSkill> initSkills){
		for(TongSkill skill:initSkills){
	...
	public byte[] toDBBytes() {
		...
		try {
			...
			for (TongSkill skill : skills.values()) {
	...
	public static TongSkills fromDBBytes(byte[] bytes) {
		if(bytes==null){
			TongSkills ret = new TongSkills();
			return ret;
		}
		TongSkills ret = null;
		...
		try {
			...
			if (version == 1) {
				...
				for (int i = 0; i < size; i++) {
					...
					TongSkill skill = TongService.newTongSkill(id, level);
...
//			throw new IllegalArgumentException();
	...
	public void skillLevelUp(){
		Iterator<TongSkill> s = skills.values().iterator();
		while(s.hasNext()){
			TongSkill ts = s.next();
			ts.level++;
		}
