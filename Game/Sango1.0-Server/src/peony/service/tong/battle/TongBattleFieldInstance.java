...
public class TongBattleFieldInstance implements Instance {
	...
	public void addPlayer(Player player) throws VMapException {
		if(state==STATE_END)
			throw new VMapException(peony.Messages.STRING_01513);
	}
	...
	public void update(int diff) {
		...
		if (state == STATE_PREPARE && Time.currTime >= startTime) {
			state = STATE_STARTED;
			map.shout(peony.Messages.STRING_01514, 0xff5555, 5000);
			List<Creature> flagCreature = Server.server.getServiceRegistry().getTongBattleVMapManager().getFlagCreature();
			if(flagCreature!=null && flagCreature.size()>0){
				for(Creature c : flagCreature){
					if(c!=null && c.buffs!=null)
					    c.buffs.removeBuff(TongBattleVMapManager.FLAG_IMMUNE);
				}
			}
		}
	}
	...
	public TongBattleSide getSide(Tong tong){
		if(defend!=null && defend.tong!=null && defend.tong.id == tong.id)
			return defend;
		if(attack1!=null && attack1.tong!=null && attack1.tong.id == tong.id)
			return attack1;
		if(attack2!=null && attack2.tong!=null && attack2.tong.id == tong.id)
			return attack2;
		return null;
	}
	...
	public void winNotify(TongBattleSide side, TongBattleSide fail, int money){
		...
		if(fail==null){
			chatService.sendGuildSystemMessage(MessageFormat.format(
					peony.Messages.STRING_00660, side.tong.name,side.def.battleDef.name), side.tong.id);
			chatService.sendFactionSystemMessage(side.faction, MessageFormat.format(
					peony.Messages.STRING_00660, side.tong.name,side.def.battleDef.name));
		}else{
			chatService.sendGuildSystemMessage(MessageFormat.format(
					peony.Messages.STRING_01515, 
	...
	public void failNotify(TongBattleSide side, TongBattleSide win, int money){
		...
		ChatService chatService = Server.server.getServiceRegistry().getChatService();
		if(win==null){
			chatService.sendGuildSystemMessage(MessageFormat.format(peony.Messages.STRING_01516,
					side.tong.name,side.def.battleDef.name), side.tong.id);
			chatService.sendFactionSystemMessage(side.faction, MessageFormat.format(
					peony.Messages.STRING_01516,side.tong.name,side.def.battleDef.name));
		}else{
			chatService.sendGuildSystemMessage(MessageFormat.format(peony.Messages.STRING_01517,
					side.tong.name,side.def.battleDef.name,money), side.tong.id);
			chatService.sendFactionSystemMessage(side.faction, MessageFormat.format(
					peony.Messages.STRING_01518,side.tong.name,side.def.battleDef.name,money));
		}
	}
	...
	protected void transPlayers(){
		if(!hasTrans){
			Iterator<GameObject> itor = map.instanceid2objects.values().iterator();
			while (itor.hasNext()) {
