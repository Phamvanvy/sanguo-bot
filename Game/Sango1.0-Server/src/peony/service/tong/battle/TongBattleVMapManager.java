...
public class TongBattleVMapManager implements VMapManager,Service {
	...
	public static int FLAG_IMMUNE = 645; //城战开始前两分钟战旗免疫buff
	
	public static List<Creature> flagCreature = new ArrayList<Creature>();//加了免疫buff的战旗集合
		...
		Server.server.scheduExec.scheduleAtFixedRate(new Runnable(){
			public void run() {
				Server.server.syncRunner.add(new Runnable(){
					public void run(){
//						openInstance();
						RefreshNpcCall call = new RefreshNpcCall(RefreshNpcCall.TONGBATTLE);
				...
				if(def.signMapId==currentMapId){
					...
					if(applys!=null && applys.length>0){
						if(applys.length==2){
							...
							((Creature)npc1).buffs.addBuff(BuffUtil.createBuff(FLAG_IMMUNE, 1, 
									(Creature)npc1, (Creature)npc1, 0));
							flagCreature.add((Creature)npc1);
							...
							((Creature)npc2).buffs.addBuff(BuffUtil.createBuff(FLAG_IMMUNE, 1, 
									(Creature)npc2, (Creature)npc2, 0));
							flagCreature.add((Creature)npc2);
					...
					((Creature)npc3).buffs.addBuff(BuffUtil.createBuff(FLAG_IMMUNE, 1, 
							(Creature)npc3, (Creature)npc3, 0));
					flagCreature.add((Creature)npc3);
					if(apply3!=null){
						// 存在防守方C
						if(applys==null || applys.length==0){
		...
		if(check){
			...
			if(instance == null){
				if(def!=null){
					...
					//测试用秘籍进战场，特除情况特殊处理
					return Server.server.getWorld().addPlayerToMap(player, 848, 800, 700, true);
				}
			}else{
			...
			if(tmpInstance!=null && tmpInstance instanceof TongBattleFieldInstance){
				...
				if(instanceTran){
					if(instance.state==TongBattleFieldInstance.STATE_PREPARE){
						...
						String s = null;
						if(min!=0){
							s = MessageFormat.format(peony.Messages.STRING_01254, min);
						}else{
							int sec = t/(1000);
							s = MessageFormat.format(peony.Messages.STRING_01255, sec);
						}
				...
				if(instance.state == TongBattleFieldInstance.STATE_STARTED){
					...
					throw new VMapException(peony.Messages.STRING_01256);
				}
			}else{
				TongBattleFieldInstance instance = getInstanceByMapId(mapId);
				if(instance == null){
					throw new VMapException(peony.Messages.STRING_01257);
				}else{
					Tong tong = Server.server.getServiceRegistry().getTongService().getPlayerTong(player.id,false);
					if(tong == null)
						throw new VMapException(peony.Messages.STRING_01258);
					else{
	...
	protected VMap in(Player p, TongBattleFieldInstance instance,TongBattleSide side) throws VMapException{
		if(!side.containsPlayer(p.id)&&side.isFull())
			throw new VMapException(peony.Messages.STRING_01259);
	...
	public void tran(Player p) throws VMapException {
		Tong tong = Server.server.getServiceRegistry().getTongService()
				.getPlayerTong(p.id,false);
		if (tong != null) {
			TongBattleDef def = getDefBySignMapId(p.map.getId());
			if (def == null)
				throw new VMapException(peony.Messages.STRING_01260);
			...
			if (instance == null || (instance.state!=TongBattleFieldInstance.STATE_STARTED && 
				...
				throw new VMapException(peony.Messages.STRING_01261);
			TongBattleSide side = instance.getSide(tong);
			if (side == null) 
				throw new VMapException(peony.Messages.STRING_01262);
			TongService tongService = Server.server.getServiceRegistry().getTongService();
			if(tongService.getPlayerInfo(p.id).battleTag==0){
				throw new VMapException(peony.Messages.STRING_01263);
			}
			if(p.level<MINLEVEL){
				throw new VMapException(MessageFormat.format(peony.Messages.STRING_01264, MINLEVEL));
			}
	...
	public void buyWarCarriage(Player p) throws TongBattleException {
		if (p != null) {
			...
			if (tong == null)
				throw new TongBattleException(peony.Messages.STRING_01265);
			if (tongService.getPlayerInfo(p.id).duty != TongService.CHAIRMAN)
				throw new TongBattleException(peony.Messages.STRING_01266);
			TongBattleApplyService applyService = Server.server.getServiceRegistry().getTongBattleApplyService();
			if (!applyService.isWinner(tong.id) && applyService.hasApplyed(tong.id)) {
				if (in(Calendar.getInstance(), BATTLESTARTTIME - 1, 30,BATTLESTARTTIME, 0)) {
					...
					try {
						...
						tx.rollback();
						throw new TongBattleException(peony.Messages.STRING_01267);
					} catch (NoEnoughSpaceException e) {
						tx.rollback();
						throw new TongBattleException(peony.Messages.STRING_00555);
					}
				} else {
					throw new TongBattleException(peony.Messages.STRING_01268);
				}
			} else if(applyService.isWinner(tong.id)){
				throw new TongBattleException(peony.Messages.STRING_01269);
			} else {
				throw new TongBattleException(peony.Messages.STRING_01268);
			}
		}
	...
	public void buyTower(Player p) throws TongBattleException {
		if (p != null) {
			...
			if (applyService.isWinner(tong.id)
				...
				if (in(Calendar.getInstance(), BATTLESTARTTIME - 1, 30,BATTLESTARTTIME, 0)) {
					...
					try {
						if(defend.hasBuyTower)
							throw new TongBattleException(peony.Messages.STRING_01270);
				...
				throw new TongBattleException(peony.Messages.STRING_01271);
			}
		}
	}
    ...
    public List<Creature> getFlagCreature(){
    	return flagCreature;
    }
