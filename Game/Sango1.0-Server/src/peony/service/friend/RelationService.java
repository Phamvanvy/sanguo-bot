...
import peony.game.changed.PlayerEnemyChangeToken;
...
public class RelationService implements Service, ServiceEventListener {
	...
	public void shutdown() {
		...
		for (PlayerRelation r : relations.values()) {
			if(r.apprenticeList == null){
				r.apprenticeList = new RelationList();
			}
			dao.updateEntity(r);
		}
	...
	public void handleEvent(ServiceEvent event) {
		...
		case ServiceEvent.EVENT_MAP_PLAYER_LOADED:
			playerChangeMap((VMap)event.param1,(Player)event.param2);
			break;
		}
	}
	
	public void playerChangeMap(VMap map,Player p){
		if(p!=null){
			...
			if(map.getFaction() == 5){
				...
				while (it.hasNext()) {
					...
					if (o.type == GameObject.TYPE_PLAYER) {
						if(o.id == p.id)
							continue;
						Player enemy = ObjectAccessor.getPlayer(o.id);
						if(enemy!=null && p.changed!=null){
							int check = checkEnemy(p,enemy,relation);
							if(check == 0){
								PlayerEnemyChangeToken changedItem = new PlayerEnemyChangeToken(o.instanceId,(byte)0);
								p.changed.addChangedItem(changedItem);
							}else if(check == 1 || check == 3){
								PlayerEnemyChangeToken changedItem = new PlayerEnemyChangeToken(o.instanceId,(byte)1);
								p.changed.addChangedItem(changedItem);
								if(check == 3 && enemy.changed!=null){
									PlayerEnemyChangeToken changedItem2 = new PlayerEnemyChangeToken(p.instanceId,(byte)1);
									enemy.changed.addChangedItem(changedItem2);
								}
							}else if(check == 2 && enemy.changed!=null){
								PlayerEnemyChangeToken changedItem2 = new PlayerEnemyChangeToken(p.instanceId,(byte)1);
								enemy.changed.addChangedItem(changedItem2);
							}
						}
	...
	/** 删除仇人时改变状态*/
	public void delEnemy(Player p,Player targetPlayer){
		if(p.map.map.getFaction() == 5 && p.map.map.getId() == targetPlayer.map.map.getId()){
			if(p.changed!=null){
				PlayerEnemyChangeToken changedItem = new PlayerEnemyChangeToken(targetPlayer.instanceId,(byte)0);
				p.changed.addChangedItem(changedItem);
			}
		}
	}
	...
	/** 添加仇人时改变状态*/
	public void addEnemy(Player p,Player targetPlayer){
		if(p.map.map.getFaction() == 5 && p.map.map.getId() == targetPlayer.map.map.getId()){
			if(p.changed!=null){
				PlayerEnemyChangeToken changedItem = new PlayerEnemyChangeToken(targetPlayer.instanceId,(byte)1);
				p.changed.addChangedItem(changedItem);
			}
		}
	}
	...
	/** 进入地图时检测仇人关系*/
	public int checkEnemy(Player self,Player target,PlayerRelation relation){
		Actor actor = Server.server.getServiceRegistry().getActorCacheService().find(target.id);
		Actor aP = Server.server.getServiceRegistry().getActorCacheService().find(self.id);
		if(relation!=null && relation.enemies!=null && relation.enemies.players.contains(actor)){
			if(target.relations!=null && target.relations.enemies!=null && target.relations.enemies.players.contains(aP)){
				return 3; //互为仇人
			} else {
				return 1;//对方是玩家的仇人
			}
		}else if(target.relations!=null && target.relations.enemies!=null && target.relations.enemies.players.contains(aP)){
			return 2; //玩家是对方的仇人
		}
		return 0;
	}
	...
	private void unitDie(Unit u1, Unit u2){
		if(u1.type==GameObject.TYPE_PLAYER&&u2.type==GameObject.TYPE_PLAYER){
			if(u1.faction!=u2.faction){
				Player u11=(Player)u1;
				Player u22=(Player)u2;
				if(u11.asyncTargetId>0){
					return;
				}
...
//					r.addEnemy(a2,(Player)u1);
					if(r.addEnemy(a2, (Player)u1)){
						if(u1.map.map.getFaction()==5 && u1.map.map.getId() == u2.map.map.getId()){//如果在中立地图遇见显示仇人标示
							if(u1.changed!=null){
								PlayerEnemyChangeToken changedItem = new PlayerEnemyChangeToken(u2.id,(byte)1);
								u1.changed.addChangedItem(changedItem);
							}
						}
					}
				}
				...
				if(rel.friends.exists(p.id)){
					if(p.lastRemindFriendTime.containsKey(p.id)){
						if((Time.currTime -p.lastRemindFriendTime.get(p.id)) > FRIEND_ONLINE_REMAIN_TIME){
							 ...
							 chatService.sendPrivateMessage(p.id, MessageFormat.format(peony.Messages.STRING_01862, player.name));
						 }
					}else{
						p.lastRemindFriendTime.put(p.id, Time.currTime);
						chatService.sendPrivateMessage(p.id, MessageFormat.format(peony.Messages.STRING_01862, player.name));
	...
	public void removeAllRelation(Player player){
		...
		if(relation.apprenticeList != null){
			Iterator<Actor> it4 = relation.apprenticeList.players.iterator();
			while(it.hasNext()){
				Actor actor = it4.next();
				if(dao.getActor(actor.id)==null || service.find(actor.id).exist==0){
					it4.remove();
					relation.apprenticeList.degrees.remove(actor.id);
				}
			}
		}
		...
		if(relation.teacherId!=-1 && dao.getActor(relation.teacherId)==null){
			relation.removeTeacher();
		}
	}
			...
			if(target!=null){
				...
				try{
					...
					ErrorHandler.sendErrorMessage(session, serial,
								OpCode.ENEMY_POSITION_CLIENT, peony.Messages.STRING_00129);
					return;
				}
				...
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.ENEMY_POSITION_CLIENT, peony.Messages.STRING_01863);
			}
		}
	}
}
