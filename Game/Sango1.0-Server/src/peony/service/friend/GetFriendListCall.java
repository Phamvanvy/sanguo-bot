		...
		if (success) {
			...
			if (relation == null) {
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.GET_FRIENDLIST_CLIENT, peony.Messages.STRING_00436);
				return;
			}
			...
			if(type==0){
				...
				for(int i=0;i<count;i++){
					...
					pt.put(0);
					Actor actor = relation.friends.getPlayerAt(i);
					Player p = ObjectAccessor.getPlayer(actor.id);
					if(p!=null && p.party!=null)
						pt.put(1);
					else
						pt.put(0);
				}
...
//			}
	...
	private void putPlayer(Packet pt, RelationList list, int index, int type) {
		...
		pt.put(actor.sex);
		pt.put(actor.clazz);
		Tong tong = ts.getPlayerTong(actor.id,true);
