...
public class MailService implements Service, Runnable {
	...
	protected void deleteOrBack(MailDAO dao, Mail mail) {
		if (mail.getSourceId() > 0 && mail.getPrice() > 0
			...
			Mail rMail = new Mail();
			rMail.setSourceId(-1);
			rMail.setSourceName(peony.Messages.STRING_00004);
			rMail.setTitle(MessageFormat.format(peony.Messages.STRING_00005, mail.getTitle()));
	...
	public int sendSystemMail(int target, String sourceName, String title,
		...
		try {
			...
			if (attMoney > 0) {
				...
				if(attItem.template!=null&&attItem.template.itemValid!=null&&attItem.template.itemValid.time>0){
					//装备本身属性
					attItem.validTime=(int)(System.currentTimeMillis()/60000+attItem.template.itemValid.time);
				}
