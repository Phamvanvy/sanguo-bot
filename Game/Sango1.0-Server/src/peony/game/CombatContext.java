...
public class CombatContext {
	...
	/** 判断这个攻击是否是由于其他技能或Buff触发的附加效果。附加效果不受治疗量增加效果的影响。*/
	public boolean isAdditive = false;
	
	/** 是否免伤害 */
	public boolean isNoDamage = false;
	/** 免伤最大概率 */
	public int ratio = 0;
	/** 免伤时间间隔 */
	public int timeDis = 0;
		...
		if (critical) {
			...
			try{
				if(target!=null && target.type==GameObject.TYPE_PLAYER)
					log.info("[CRITICAL]SOURCEID["+source.id+"]TARGETID["+target.id+"]");
			}catch(Exception e){}
		}
	}
	...
	public void calculateDamage() {
		if (damageType == DAMAGE_PHYSICAL) {
			...
			if(target.type==GameObject.TYPE_ATTENDANT)
				df = PropertyCalculator.calcAttendantDefensePercent(actArmor, source.level);
			damage = (int)(ap * (1 - df));
			damage *= target.getDamageScale();
			...
			if(target.type==GameObject.TYPE_ATTENDANT)
				df = PropertyCalculator.calcAttendantSpellDefensePercent(actDef, source.level);
			damage = (int)(ap * (1 - df));
			damage *= target.getDamageScale();
	...
	public void sendResult() {
		if (parent == null) {  //只有没有parent的CombatContext需要发送Attack包，比如AOE时，只需要发送一次AOE成功
			...
			if ((skill.getTargetType() & Skill.TARGET_FLAG_SELF) != 0) {
				...
				pt.putInt(target.getInstanceId());
			}
...
//			source.broadcast(pt, null, true,false);
		...
		pt.put(attackResult);
		pt.put(damageType);
//		if((damageType==DAMAGE_PHYSICAL || damageType==DAMAGE_MAGIC) && target!=null && target.type==GameObject.TYPE_PLAYER){
//			Player t = (Player)target;
//			if(t.attendant!=null){
//				int value = (int) (damage * (1f - t.attendant.distance/100f));
//				pt.putInt(value);
//			}else{
//				pt.putInt(damage);
//			}
...
//			target.broadcast(pt, null, true, false);
			if(source!=null && source.type==GameObject.TYPE_ATTENDANT)
				broadcastAttendantAttack();
			else
		...
		try{
			if((source.type == GameObject.TYPE_PLAYER && StatService.trackLogPlayer.contains(source.getInstanceId()))
				||(target.type == GameObject.TYPE_PLAYER && StatService.trackLogPlayer.contains(target.getInstanceId()))){
			      log.info("[TRACKATTACKRESULT]SOURCE["+source.id+"]SOURCEINSTANCEID["+source.getInstanceId()+"]TARGET["+target.id+"]TARGETINSTANCEID["+target.getInstanceId()+"]SKILLID["+skill.getId()+"]AUTOATTACK["+(source.attack.autoAttack?1:0)+"]DAMAGETYPE["+damageType+"]DAMAGE["+damage+"]");
			}
		}catch(Exception e){
	...
	private void broadcastAttendantAttack(){
		if(source.type==GameObject.TYPE_ATTENDANT){
			Attendant att = (Attendant)source;
			if(att.owner!=null){
				Packet pt1 = new Packet(OpCode.SKILL_ATTACKED_SERVER);
				pt1.putInt(target.instanceId);
				pt1.putInt(CommonUtil.currentMillis());
				pt1.putInt(att.owner.instanceId);
				pt1.put(attackResult);
				pt1.put(damageType);
				pt1.putInt(damage);
				pt1.putInt(skill.getHitAnimation(source));
				target.broadcast(pt1,att.owner,target.type==GameObject.TYPE_PLAYER?(Player)target:null, true, false,true);
			}
		}
	}
