...
//		player.removePvpFlag();
	...
	protected void processPvpDie(Player player,Unit source){
		...
		for (int pid : player.enemyPlayers.keySet()) {
			...
			if (p == null || !p.inRange(player, 200) || !p.isAlive()) {
				continue;
			}
...
//					Tong tong = ts.getPlayerTong(p.id,false);
//					if(tong!=null&&tong.ismaintain == TongService.MAINTAIN&&tong.skills!=null&&tong.skills.get(3)!=null
//							&&ts.isContainOneTong(p)){
//						TongSkill3 skill = (TongSkill3)tong.skills.get(3);
//						num = addValue * skill.getRatio() / 100;
//					}
					p.setCredit(p.credit + addValue + num, true, "PVP");
				}
			}
		}
		// 杀人掉装备处理
