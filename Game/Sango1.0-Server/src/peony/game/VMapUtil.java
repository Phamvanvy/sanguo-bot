...
public class VMapUtil implements DataChangeHandler {
	...
	public static Map<Integer, Set<String>> map2NpcFileNames = new HashMap<Integer, Set<String>>();
	public static Map<Integer, Set<String>> map2NpcFileNamesLarge = new HashMap<Integer, Set<String>>();
	...
	public static void load() throws Exception{
		...
		while (ite.hasNext()) {
			...
			for(GameMapInfo mi : area.getAreaInfo().maps){
				if(mi!=null){
					Set<String> npcs = new HashSet<String>();
					Set<String> npcsLarge = new HashSet<String>();
					for (GameMapObject gmo : mi.objects) {
		            	if(gmo instanceof GameMapNPC){
		            		GameMapNPC n = (GameMapNPC)gmo;
		            		PipAnimateSet as = new PipAnimateSet();
		            		try {
								as.load(n.template.image.source);
							} catch (IOException e) {
								e.printStackTrace();
							}
		            		for (int i = 0; i < as.getFileCount(); i++) {
		            			File f = as.getSourceFile(i);
		            			if(f!=null){
		            				npcs.add(f.getName());
		            			}
		            		}
		            		try {
		            			as.load(n.template.image.largeSource);
		            		} catch (IOException e) {
		            			e.printStackTrace();
		            		}
		            		for (int i = 0; i < as.getFileCount(); i++) {
		            			File f = as.getSourceFile(i);
		            			if(f!=null){
		            				npcsLarge.add(f.getName());
		            			}
		            		}
		            		npcs.add(n.template.image.getID()+".ctn");
		            		npcsLarge.add(n.template.image.getID()+".ctn");
		            	}
		            }
					int mapId = mi.getGlobalID();
					map2NpcFileNames.put(mapId, npcs);
					map2NpcFileNamesLarge.put(mapId, npcsLarge);
				}
			}
		}
	}
...
//				creature.speed = 25;
	...
	public static void createFall(Fall fall, NPCTemplate template,boolean worldDrop) {
		...
		if (worldDrop && template.type.id==0){ //只有怪物才加世界掉落,世界掉落率是5%
