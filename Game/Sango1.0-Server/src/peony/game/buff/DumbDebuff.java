 ...
 * @author lighthu
 */
public class DumbDebuff implements Updatable, Buff, UnitEffect {
	/*
	 * 过期时间
	 */
	protected int time;
	...
	public DumbDebuff(Unit src, int time) {
	    if (src != null) {
	        source = src.ref();
	    }
	...
	// 用于从数据库load时创建buff对象
	public DumbDebuff(int lvl, Unit src, Unit tgt, int dmg) {
	    instanceID = BuffUtil.getNextID();
	}
	...
	public String getName() {
	    return peony.Messages.STRING_01931;
	}
	
	public String getDesc() {
		return peony.Messages.STRING_01932;
	}
	...
	public boolean merge(Buff buff) {
		if (buff instanceof DumbDebuff) {
			DumbDebuff other = (DumbDebuff) buff;
			if (other.time > time) {
				time = other.time;
			}
			return true;
