 ...
 * @author lighthu
 */
public class StayDebuff implements Updatable, Buff, UnitEffect {
	/*
	 * 过期时间
	 */
	protected int time;
	...
	public StayDebuff(Unit src, int time) {
        if (src != null) {
            source = src.ref();
        }
	...
	// 用于从数据库load时创建buff对象
    public StayDebuff(int lvl, Unit src, Unit tgt, int dmg) {
        instanceID = BuffUtil.getNextID();
    }
    ...
    public String getName() {
        return peony.Messages.STRING_01956;
    }
	
	public String getDesc() {
		return peony.Messages.STRING_01957;
	}
	...
	public boolean merge(Buff buff) {
		if (buff instanceof StayDebuff) {
			StayDebuff other = (StayDebuff) buff;
			if (other.time > time) {
				time = other.time;
			}
			return true;
