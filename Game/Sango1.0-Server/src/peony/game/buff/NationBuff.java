...
//		本周内免暴率提升了XX%
		...
		if(defense!=0.0f){
			sb.append(String.format(peony.Messages.STRING_01772,defense*100));
		}
		if(antiCrti!=0.0f){
			sb.append(String.format(peony.Messages.STRING_01773,antiCrti*100));
		}
		if(expratio!=0.0f){
			sb.append(String.format(peony.Messages.STRING_01774,expratio*100));
		}
		return sb.toString();
	}
	...
	public String getName() {
		return peony.Messages.STRING_01775;
	}
	...
	public void enhance(PropertyCalculator pc) {
	    ...
	    if (pc.unit instanceof Player) {
	        if (pc.unit.map!=null && pc.unit.map.map != null && pc.unit.map.map.instance != null) {
