 ...
 * @author lighthu
 */
public class FearDebuff implements Updatable, Buff, UnitEffect {
	...
	public FearDebuff(Unit src, int time) {
        if (src != null) {
            source = src.ref();
        }
    ...
    // 用于从数据库load时创建buff对象
    public FearDebuff(int lvl, Unit src, Unit tgt, int dmg) {
        instanceID = BuffUtil.getNextID();
    }
    ...
    public String getName() {
        return peony.Messages.STRING_01860;
    }

	public String getDesc() {
		return peony.Messages.STRING_01861;
	}
	...
	public boolean merge(Buff buff) {
		if (buff instanceof FearDebuff) {
			FearDebuff other = (FearDebuff) buff;
			if (other.time > time) {
				time = other.time;
			}
			return true;
