...
public class AccountLoginDuokuCall extends AccountAsyncCall{

	private static final Logger log = Logger.getLogger(AccountLoginDuokuCall.class);
	...
	protected Version v;
	public AccountLoginDuokuCall(ClientSession session, int serial, String uId, 
			String sessionId, String model,String uiModel, String clientVersion,String clientPhone, int playerId) {
		super(session);
		this.serial = serial;
		this.uId = uId;
		...
		log.info("[DUOKUACCLOGIN]UID["+uId+"]SESSIONID["+sessionId+"]UIMODEL["+uiModel+"]VERSION["+clientVersion+"]TRY");
	}
	...
	public void callFinish() throws Exception {
		if(success){
			if (message instanceof AccountRegOkMessage) {
				// 自动注册成功返回。再发起一个登录请求。
				LegacyLoginMessage loginMsg = new LegacyLoginMessage(accountName, PUBLIC_PASSWORD, clientPhone);
			...
			if (em.getCode() == ErrorMessages.UNKNOW_ACCOUNT || em.getCode() == ErrorMessages.ERROR_NAMEORPASSWORD) {
				...
				AccountRegMessage regMsg = new AccountRegMessage(accountName,"","",-1,sessionId,Server.server.gameCode,version2,clientPhone,accountKey);
				Server.server.getServiceRegistry().getAccountService().sendAndRegister(regMsg, this);
				return;
			}
			
			// 其他错误通知客户端
			ErrorHandler.sendErrorMessage(getSession(), serial, OpCode.ACCOUNT_LOGIN_DUOKU_CLIENT, ErrorMessages.getErrorMesssage(em));
			log.info("[360ACCLOGINEEOR]AUTH["+uId+"]CAUSE["+ErrorMessages.getErrorMesssage(em)+"]");
	...
	public void run() {
		try {
			...
			log.info("[DUOKUACCLOGINEEOR]AUTH["+uId+"]CAUSE[VERSION]");
			ErrorHandler.sendErrorMessage(getSession(), serial, OpCode.ACCOUNT_LOGIN_DUOKU_CLIENT, e.getMessage());
			return;
		}
		
		String[] userInfo = new String[]{uId,sessionId};
		try {
			String[] accountInfo = PartnerAPI.verifyUser("BAIDU2", userInfo, model, clientVersion);
			...
			log.info("[DUOKUACCLOGINEEOR]AUTH["+uId+"]CAUSE["+e.getMessage()+"]");
			ErrorHandler.sendErrorMessage(getSession(), serial, OpCode.ACCOUNT_LOGIN_DUOKU_CLIENT, e.getMessage());
			return;
		}
	}
