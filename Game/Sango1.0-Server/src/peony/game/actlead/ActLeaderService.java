...
//	}
	...
	protected void loadXml(Document doc){
		...
		for(Element el : mapL){
			int mapId = Integer.parseInt(el.attributeValue("mapId").trim());
			String mapName = el.attributeValue("mapName");
			int x = Integer.parseInt(el.attributeValue("x").trim());
			int y = Integer.parseInt(el.attributeValue("y").trim());
			int minLevel = Integer.parseInt(el.attributeValue("minLevel").trim());
			int maxLevel = Integer.parseInt(el.attributeValue("maxLevel").trim());
	...
	public synchronized void addAct(String type, String name, String createTime, String mapName, 
		...
		ActLeader actLeader = new ActLeader(type, name, mapName, createTime, mapId, x, y, 
				rewardType, minLevel, maxLevel, instruction, faction);
		if(isActiveAct(name))
			throw new ActLeaderException(peony.Messages.STRING_01793);
		else{
			acts.add(actLeader);
		}
		flag = 1;
	...
	public synchronized void removeAct(String name) throws ActLeaderException{
		...
		throw new ActLeaderException(peony.Messages.STRING_01794);
	}
