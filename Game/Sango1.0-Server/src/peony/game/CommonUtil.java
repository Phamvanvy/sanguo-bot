...
public class CommonUtil {
    ...
    public static String formatSecond(int sec) {
        if (sec < 60) {
        	return MessageFormat.format(peony.Messages.STRING_00332, sec);
        } else if (sec < 3600) {
        	return MessageFormat.format(peony.Messages.STRING_00863, (sec / 60), (sec % 60));
        } else {
        	return MessageFormat.format(peony.Messages.STRING_00864, (sec / 3600), ((sec % 3600) / 60));
        }
    }
    ...
    public static String formatMillSecond(int ms) {
        ...
        if (sec < 60) {
            return MessageFormat.format(peony.Messages.STRING_00332, formatFloat(sec2));
        } else if (sec < 3600) {
        	return MessageFormat.format(peony.Messages.STRING_00863, (sec / 60), (sec % 60));
        } else {
        	return MessageFormat.format(peony.Messages.STRING_00864, (sec / 3600), ((sec % 3600) / 60));
...
//		int j= 0;
    ...
    /** Light删除：这个方法较危险，应使用XML文档自带的编码解释。
    ...
    public static String getRemainTimeString(Date date){
    	...
    	if (remain <= 0) {
    		return peony.Messages.STRING_00865;
    	} else if (remain < 60) {
    		return MessageFormat.format(peony.Messages.STRING_00866, remain);
    	} else if (remain < 60 * 24) {
    		return MessageFormat.format(peony.Messages.STRING_00867, (remain / 60));
    	} else {
    		return MessageFormat.format(peony.Messages.STRING_00868, (remain / 1440));
    	}
    }
