...
import peony.game.itemeffect.AttendantAddLoyalEffect;
...
import peony.game.itemeffect.CardDropItemEffect;
import peony.game.itemeffect.ComboItemEffect;
import peony.game.itemeffect.DivorceEffect;
import peony.game.itemeffect.DropGroupItemEffect;
...
import peony.game.itemeffect.ReadBookEffect;
...
import peony.game.itemeffect.TongBoxEffect;
...
import peony.game.itemeffect.ValentineEffect;
import peony.game.itemeffect.WordPowerItemEffect;
...
import peony.service.tong.TongSkill11;
...
public class ItemUtil implements DataChangeHandler {
	...
	public static final int ITEM_HORSEFOODS = 4113; //新手马粮
	...
	public static final int ITEM_STAR_ENHANCE_LEVEL3 = 4047; //顶级星级鉴定符
	public static final int ITEM_STAR_ENHANCE_LEVEL4 = 3761; //资质鉴定符
...
//	顶级星级鉴定符	0%	0%	0%	0%	0%	0%	0%	0%	100%
	...
	public static List<Integer> worldMessageItems = new ArrayList<Integer>();
	public static List<Integer> bindItems = new ArrayList<Integer>();
	...
	/**
	 * 判断杀死世界Boss物品掉落时是否需要发送世界聊
	 * @param itemID
	 * @return 0 不需要  1 世界聊
	 */
	public static int getWorldMessageType(int itemID) {
		if(worldMessageItems!=null && worldMessageItems.size()>0){
			if(worldMessageItems.contains(itemID)){
				return 1;
			}
		}
		return 0;
	 ...
	 * 获得物品发送世界聊
	 */
	public static void sendWoldMessage(Gain gain,Player p,String creatureName){
		GainItem[] items = gain.getGainItems();
		for(GainItem item : items){
			if(getWorldMessageType(item.getItem().template.id)==1){
				//世界喊话
				if(item.getItem().template.id == 3970){
					ItemChatAttachment attItem = new ItemChatAttachment(item.getItem());
					...
					ItemChatAttachment attItem = new ItemChatAttachment(item.getItem());
					String message = MessageFormat.format("恭喜{0}的{1}带领小队击杀{2}获得了/-1!", p.getFactionName(),p.name, creatureName);
					ChatMessage cm = new ChatMessage(ChatOption.WORLD, p.id, -1, peony.Messages.STRING_00004, message, attItem);
					Server.server.getServiceRegistry().getChatService().addChatMessage(cm);
				}
			}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static void loadWorldMessageItems() throws Exception {
		worldMessageItems.clear();
		byte[] bytes = Server.server.getServiceRegistry().getDataService().data.findFile("Items/worldmessageitems.xml");
		Document doc = CommonUtil.getDocument(new ByteArrayInputStream(bytes));
		Element root = doc.getRootElement();
		Iterator ite = root.elementIterator("item");
		while (ite.hasNext()) {
	 ...
	 * @throws Exception
	 */
	public static void loadneedBindItems() {
		bindItems.clear();
//		byte[] bytes = Server.server.getServiceRegistry().getDataService().data.findFile("Items/bindItems.xml");
//		Document doc = CommonUtil.getDocument(new ByteArrayInputStream(bytes));
//		Element root = doc.getRootElement();
//		Iterator ite = root.elementIterator("item");
//		while (ite.hasNext()) {
...
//		}
		try {
			...
			File file = new File(dir, "bindItems.xml");
			SAXReader reader = new SAXReader();
			Document doc = reader.read(file);
			Iterator itor = doc.getRootElement().elementIterator("item");
			while (itor.hasNext()) {
				Element elem = (Element) itor.next();
				int itemId = Integer.parseInt(elem.attributeValue("id"));
				bindItems.add(itemId);
			}
			log.info("load binditems ok !");
		} catch (Exception e) {
		}
	}
	 ...
	 * @param item
	 */
	public static void naturalEnhance(GameItem item,Player p){
//		34%	24%	23%	15%	4%
...
//				}
		...
		if(tm!=null && tm.skills != null && tm.skills.get(11)!=null){
			TongSkill11 tskill = (TongSkill11)tm.skills.get(11);
			if(tskill != null && tskill.level>0){
				Iterator<NaturalEnhance> it = l.iterator();
				int liangnum = 0;
				int younum = 0;
				if(it.hasNext()){
					NaturalEnhance ne = it.next();
					if(ne.level > 0)
						liangnum++;
					if(ne.level > 1)
						younum ++;
				...
				if(tskill.level == 1){
					if(liangnum < 1){
						l.get(0).setLevel(1);
						l.get(0).setPercent(8);
						l.get(0).setValue(item.getNatureEnhanceAttribute(l.get(0).attType,8));
					}
				}else if(tskill.level == 2){
					if(liangnum < 2){
						if(l.get(0).getLevel() < 1){
							l.get(0).setLevel(1);
							l.get(0).setPercent(8);
							l.get(0).setValue(item.getNatureEnhanceAttribute(l.get(0).attType,8));
						}
						if(l.get(1).getLevel() < 1){
							l.get(1).setLevel(1);
							l.get(1).setPercent(8);
							l.get(1).setValue(item.getNatureEnhanceAttribute(l.get(1).attType,8));
						}
						
					}
				}else if(tskill.level == 3){
					if(younum < 1){
						l.get(0).setLevel(2);
						l.get(0).setPercent(14);
						l.get(0).setValue(item.getNatureEnhanceAttribute(l.get(0).attType,14));
					}
				}
			}
		}
...
//				sb.append("消耗行动力:").append(f.movePoint).append("\n");
				sb.append(MessageFormat.format(peony.Messages.STRING_01440, ProduceService.getPracticeByLevel(f.level)
						,ProduceService.getLevelToString(f.level),f.movePoint));
				for(BuyRequirement br:f.requirements){
					sb.append(br.toString()).append("\n");
				}
				if(f.productType==Formula.PRODUCT_DROPGROUP){
//					sb.append("产出:").append(ObjectAccessor.getItemTemplate(f.itemID).name).append("(随机属性)");
					sb.append(MessageFormat.format(peony.Messages.STRING_01441, ObjectAccessor.getItemTemplate(f.itemID).name));
				}
				else if(f.productType==Formula.PRODUCT_ITEM){
//					sb.append("产出:").append(ObjectAccessor.getItemTemplate(f.itemID).name).append("");
					sb.append(MessageFormat.format(peony.Messages.STRING_01442, ObjectAccessor.getItemTemplate(f.itemID).name));
				}
				if (f.minAmount != 1 || f.maxAmount != 1) {
//					sb.append("\n数量:").append(f.minAmount).append("~").append(f.maxAmount);
					sb.append(MessageFormat.format(peony.Messages.STRING_01443, f.minAmount,f.maxAmount));
				}
...
//					  数量范围:fml.minCnt+"~"+fml.maxCnt
	...
	public static void loadEquipmentTemplates(ProjectData data) throws Exception{
		...
		List suiteConfigs_New = data.getDataListByType(SuiteConfig_New.class);
		for(Object o:suiteConfigs_New) {
			SuiteConfig_New config_New = (SuiteConfig_New)o;
			SuiteEffects effects = translateSuiteConfig(config_New);
			ObjectAccessor.newSuites.put(effects.getID(), effects);
		}
...
//	}
	...
	public static ItemTemplate translateItemTemplate(Equipment equ){
		...
		equTemplate.initCardCount = equ.cardCount;
		equTemplate.maxCardCount = equ.maxCardCount;
		...
		equTemplate.equipCategory = equ.equipCategory;
		...
		if(Server.isBindItem && bindItems.contains(template.id)){
			template.bindType = ItemTemplate.BIND_REWARD;
		}
		equTemplate.clazz = equ.job;
		equTemplate.limitType=equ.limitType;
		equTemplate.mask();
	...
	public static ItemTemplate translateItemTemplate(Item item){
		...
		template.extraItemType = item.extraType;
		template.maxCount = item.addition;
		template.showImage = item.iconImage;
		...
		template.canLevelUp=item.canLevelUp;
		if(item.canLevelUp){
			template.nextLevelEquipID=item.nextEquipId;
		}else{
			template.nextLevelEquipID=-1;
		}
		template.equipLevelUpPriceType=item.equipLevelUpPriceType;
		template.equipLevelUpPriceNum=item.equipLevelUpPriceNum;
...
//			} 
			    ...
			    ret.add(e);
			}else if (effect.effectType == com.pip.sanguo.data.item.ItemEffect.TYPE_MULTI_DROPGROUP) {
			    String dg = effect.param.get("dropgroupsvalue");
			    int ui = effect.getIntParam("useitem");
			    int uic = effect.getIntParam("useitemcount");
			    int mail = effect.getIntParam("mail");
			    DropGroupItemEffect e = new DropGroupItemEffect(dg, ui, uic, mail==1);
			    ret.add(e);
				...
				ret.add(new RiddleItemEffect(effect.getIntParam("volume")));
			} else if (effect.effectType == com.pip.sanguo.data.item.ItemEffect.TYPE_UNSIGNUP){
			    UnSignupItemEffect e = new UnSignupItemEffect(effect.getIntParam("itemid"));
				ret.add(new UnSignupItemEffect(effect.getIntParam("itemid")));
            ...
            } else if(effect.effectType==com.pip.sanguo.data.item.ItemEffect.TYPE_CARD_DROP){
                ret.add(new CardDropItemEffect(effect.getIntParam("ratio"),effect.getIntParam("activepower")));
            } else if(effect.effectType==com.pip.sanguo.data.item.ItemEffect.TYPE_ATTENDANT_ADDLOYAL){
                ret.add(new AttendantAddLoyalEffect(effect.getIntParam("loyal")));
            } else if(effect.effectType==com.pip.sanguo.data.item.ItemEffect.TYPE_JEWEL_LIST){
            	ret.add(new JewelsBagItemEffect(effect.getIntParam("jewels")));
            } else if(effect.effectType==com.pip.sanguo.data.item.ItemEffect.TYPE_JUNTUAN_BOX){
                ret.add(new TongBoxEffect());
            } else if(effect.effectType==com.pip.sanguo.data.item.ItemEffect.TYPE_VALENTINE){
                ret.add(new ValentineEffect(effect.getIntParam("count")));
            } else if(effect.effectType==com.pip.sanguo.data.item.ItemEffect.TYPE_READBOOK){
                ret.add(new ReadBookEffect(effect.getIntParam("bookid")));
            } else if(effect.effectType==com.pip.sanguo.data.item.ItemEffect.TYPE_ADDCARDEXP){
            	ret.add(new AddCardExpItemEffect(effect.getIntParam("exp")));
            } else if(effect.effectType==com.pip.sanguo.data.item.ItemEffect.TYPE_KINGITEM){
                ret.add(new KingItemEffect());
            } else if(effect.effectType == com.pip.sanguo.data.item.ItemEffect.TYPE_ITEM_ACTIVITY){
            	ActivityItemEffect e = new ActivityItemEffect(effect.getIntParam("jewels"),effect.getIntParam("value"),effect.getFloatParam("addValue"));
                ret.add(e);
            }else if (effect.effectType == com.pip.sanguo.data.item.ItemEffect.TYPE_ITEM_WORDPOWER) {
                int dg1 = effect.getIntParam("dropgroup1");
                int mapId = effect.getIntParam("mapId");
                int mapX = effect.getIntParam("mapX");
                int mapY = effect.getIntParam("mapY");
                int dis = effect.getIntParam("distance");
                WordPowerItemEffect e = new WordPowerItemEffect(dg1, mapId, mapX, mapY, dis);
                ret.add(e);
            } else if(effect.effectType == com.pip.sanguo.data.item.ItemEffect.TYPE_ATTENDANTEXP){
                AddAttendantExpEffect e = new AddAttendantExpEffect(effect.getIntParam("attendantexp"));
                ret.add(e);
            }else if(effect.effectType == com.pip.sanguo.data.item.ItemEffect.TYPE_VIPEXP){
            	AddVipExpEffect e = new AddVipExpEffect(effect.getIntParam("vipexp"));
                ret.add(e);
            }else if(effect.effectType == com.pip.sanguo.data.item.ItemEffect.TYPE_ALCHEMYEXP){
            	AddAlchemyExpEffect e=new AddAlchemyExpEffect(effect.getIntParam("alchemyexp"));
            	ret.add(e);
            }
		}
		if(!items.isEmpty()){
	...
	private static SuiteEffects translateSuiteConfig(SuiteConfig_New config) {
		SuiteEffect[] se = new SuiteEffect[config.effects.size()];
		int i = 0;
		for (com.pip.sanguo.data.equipment.SuiteConfig_New.SuiteEffect_New effect : config.effects) {
			SuiteEffect effect2 = new SuiteEffect();
			effect2.type = SuiteEffect.TYPE_WEIGHT_CALC;
			effect2.setBuffId(effect.buffID);
			effect2.setBuffLevel(1);
			se[i] = effect2;
			i++;
		}
		SuiteEffects effects = new SuiteEffects(config.id, config.title, se);
		for (com.pip.sanguo.data.equipment.Equipment equ : config.equipments) {
			ItemTemplate template = ObjectAccessor.getItemTemplate(equ.id);
			template.equipment.suiteEffects = effects;
			template.equipment.suiteEffects.type=SuiteEffects.TYPE_WEIGHT;
			template.equipment.suiteEffects.weights = new HashMap<Integer, Integer>(config.weights);
			effects.addEquip(equ.id);
		}
		return effects;
	}
    ...
    public void dataObjectAdded(DataObject obj) {
        if (obj instanceof Equipment) {
            ...
            SuiteEffects effects = translateSuiteConfig(config);
            ObjectAccessor.suites.put(effects.getID(), effects);
        }else if (obj instanceof SuiteConfig_New) {
        	// 添加新套装
            SuiteConfig_New config = (SuiteConfig_New)obj;
            SuiteEffects effects = translateSuiteConfig(config);
            ObjectAccessor.newSuites.put(effects.getID(), effects);
        }
    }
    ...
    public void dataObjectRemoved(DataObject obj) {
        if (obj instanceof Item) {
            ...
            // 删除新套装
            SuiteEffects eff = ObjectAccessor.newSuites.remove(obj.id);
            if (eff != null) {
                for (int equID : eff.getEquips()) {
                    ItemTemplate it = ObjectAccessor.getItemTemplate(equID);
                    if (it != null && it.equipment != null) {
    ...
    public void dataObjectChanged(DataObject newobj) {
        ...
        }else if (newobj instanceof SuiteConfig_New) {
            // 套装属性修改
            SuiteEffects se = ObjectAccessor.newSuites.get(newobj.id);
            if (se != null) {
                // 删除旧的
                for (int equID : se.getEquips()) {
                    ItemTemplate it = ObjectAccessor.getItemTemplate(equID);
                ...
                // 替换新的
                SuiteEffects effects = translateSuiteConfig((SuiteConfig_New)newobj);
                ObjectAccessor.newSuites.put(effects.getID(), effects);
            }
        }
    }
    ...
    public static String parseUseConfirm(String useConfirm){
		try {
			if(useConfirm.contains("$(")){
				...
				String showPrice = formater.format(price);
				String value = useConfirm.substring(0, beginIndex)+showPrice+useConfirm.substring(endIndex+1);
				if(value.contains("&[")){
					...
					String showV = formater.format(v);
					String value0 = value.substring(0, beginIndex0)+showV+value.substring(endIndex1+1);
					return value0;
				}
				return value;
			}
