...
//	初始魔法暴击率	0	0	3%	2.6%
	...
	public static final int LIMITTYPE_PLAYER=0;//只限玩家使用
	public static final int LIMITTYPE_ATTENDANT=1;//只限随从
	
	public static final String[] CLASS_NAME = {peony.Messages.STRING_01308,peony.Messages.STRING_01309,peony.Messages.STRING_01310,peony.Messages.STRING_01311};
	...
	private static final String SEX_MALE = peony.Messages.STRING_00081;
	private static final String SEX_FEMALE = peony.Messages.STRING_00082;
	private static final String SEX_SHEMALE = peony.Messages.STRING_01312;
	...
	@Column(name = "books")
	@Type(type="peony.service.read.BookUserType")
	public Books books;
	...
	@Transient
	public int minorFaction = 0;
	
	@Transient
	public int lastEffectTime = 0;
	
	@Transient
//	protected int[] preBuff = {387, 388, 394, 420, 579, 580, 582};
	
	//使用时要小心，需要优化
	protected static int[] preBuffHorse = {387, 388, 394, 420, 579, 580,703,714,715,716,712,717,};
	
	//使用时要小心，需要优化
	protected static int[] preBuff = {387, 388, 394, 420, 579, 580,650,654,655,656,
			662,659,660,661,662,663,664,665,666,667,668,669,670,
			671,672,673,674,675,676,677,678,679,680,681,682,701
			,695,696,697,698,703,714,715,716,712,717,
	};
	...
	public void setAttack(Attack attack) {
		...
		if (attack != null) {
			...
			if(attack.targetRef!=null&&attack.targetRef.type==GameObject.TYPE_PLAYER){
				if(attack.attackType!=Attack.ATTACK_TYPE_ASYNC_TARGET){
					target = ObjectAccessor.getPlayer(attack.targetRef.id);
				}else{
					target =(Player)ObjectAccessor.getAsyncGameObject(attack.targetRef.id, attack.mapInstanceId);
				}
			}
	...
	public void processDie(){
		if(dieInfo!=null){
		    if (dieInfo.source == null) {
    			...
    			if(type == GameObject.TYPE_PLAYER){
    				player=(Player)this;
    			}else if(type == GameObject.TYPE_ATTENDANT){
    				player=((Attendant)this).owner;
    			}
    			if(player!=null&&dieInfo.source.type==GameObject.TYPE_PLAYER && player.asyncEnterMapId>0)
    				source = (Unit) ObjectAccessor.getAsyncGameObject(dieInfo.source.id, player.asyncMapInstanceId);
    			if(dieInfo.source.battleType==Player.TYPE_ASYNC_PLAYER && dieInfo.source.type==GameObject.TYPE_ATTENDANT){
    				source = ObjectAccessor.asyncAttendants.get(AsyncPlayer.getSearchKey(dieInfo.source.instanceId, dieInfo.source.mapInstanceId));
    			}
    			realDie(source);
		    }
		}
	...
	protected void realDie(Unit source){
		...
		if (source != null) {
			if(this instanceof Creature){
				...
				ServiceEvent evt = new ServiceEvent(ServiceEvent.EVENT_UNIT_DIE, this, source, c.battleContribList.getMaxContributionFaction());
	    		Server.server.getEventManager().fireEvent(evt);
			}else{
	    		ServiceEvent evt = new ServiceEvent(ServiceEvent.EVENT_UNIT_DIE, this, source);
	    		Server.server.getEventManager().fireEvent(evt);
...
//	}
	 ...
	 * i币转化为元宝
	 */
	public String ibToYuanbao(long ib){
		double yaunbao = (double)ib / 36 / 100;
...
//		}
		...
		}else if(this.credit < oldCredit){
			Server.server.getEventManager().fireEvent(
					new ServiceEvent(ServiceEvent.EVENT_CREDIT_DEC, this, oldCredit - this.credit));
		}
	}
...
//	}
	...
	public int setHp(int hp,boolean notify) {
		...
		float changedValue = hp - this.hp;
		if(this.hp!=hp){
//			if(changedValue<0)
//				changedValue = processAttendantHp(changedValue);
			addIntPropertyChangedItem(ChangedItem.HP,oldHp,(int) (this.hp+changedValue),notify);
			this.hp += (int) changedValue;
			if(this.hp<0)
				this.hp = 0;
			moveType |= MOVE_HPMP;
			if(notify){
				if(changedValue!=0){
					broadcastHPMP(0,(int) changedValue);
				}
			}
			if(Server.isStepServer && type==GameObject.TYPE_PLAYER&&((Player)this).isInStep){
				Packet pt = new Packet(OpCode.STEPSERVER_CLIENTSERVER_INFO_SERVER);
				pt.putInt(StepServer.TYPE_BATTLE_HP);
				pt.putString(Integer.toString(this.hp));
				((Player)this).send(pt);
			}
		}
		return (int) changedValue;
	}
	
	private float processAttendantHp(float changeValue){
		if(type==GameObject.TYPE_PLAYER && getThreatCount()>0){
			Player player = (Player)this;
			if(player.attendant!=null){
				int value = (int) (player.attendant.hp + changeValue * player.attendant.distance / 100f);
				if(value<=0) value = 0;
				player.attendant.setHp(value, false);
				return changeValue * (1 - player.attendant.distance/100f);
			}
		}
		return changeValue;
	}
...
//	}
	...
	public void setMp(int mp,boolean notify) {
		...
		if(this.mp!=mp){
			...
			if(Server.isStepServer && type==GameObject.TYPE_PLAYER&&((Player)this).isInStep){
				Packet pt = new Packet(OpCode.STEPSERVER_CLIENTSERVER_INFO_SERVER);
				pt.putInt(StepServer.TYPE_BATTLE_MP);
				pt.putString(Integer.toString(this.mp));
				((Player)this).send(pt);
			}
		}
	}
...
//	}
	...
	public int prepareSkillAttack(Unit target,Skill skill,int offsetTime){
		...
		if ((skill.getTargetType() & Skill.TARGET_FLAG_SELF) == 0) {
			...
			if ((skill.getType() & Skill.TYPE_RELIVE) == 0) {
				...
				if(skill.getClazz() == 5){
					if(this.type == GameObject.TYPE_PLAYER && target.type == GameObject.TYPE_PLAYER ){
						Player p = (Player)this;
						if(p.relations.mateId != target.id){
							return 8;
						}
					}else{
						return 8;
		...
		setAttack(new Attack(skill,this,target,offsetTime,map.map.asyncbattleInstanceId));
		return 0;
	}
	...
	public int prepareSkillAttack(Unit[] targets, Skill skill) {
        ...
        if (targets == null) {
            setAttack(new Attack(skill,this,null,0,map.map.asyncbattleInstanceId));
        } else if (targets.length == 1) {
            setAttack(new Attack(skill,this,targets[0],0,map.map.asyncbattleInstanceId));
        } else {
...
//			log.debug(String.format("send hp:%d mp:%d id:%d", hp*200/maxhp,mp*200/maxmp,id));
	...
	public Packet getStepBattleMovePacket(Player target, short moveType){
		Packet pt = new Packet(OpCode.UNIT_MOVE_SERVER);
		pt.put(type|moveType);
		pt.putInt(getInstanceId());
		if((moveType&MOVE_POINT)!=0){
...
//	}
	...
	public int canAttack(GameObject unit) {
	    ...
	    if(Server.isStepServer && unit.type == GameObject.TYPE_PLAYER && this.type==GameObject.TYPE_PLAYER){
	    	return 0;
	    }
...
//	    }
		...
		if(type==GameObject.TYPE_ATTENDANT)
			return 0;
	...
	public void refreshProperties(boolean levelUp){
		...
		processPreBuff(calc);
		if(type==GameObject.TYPE_PLAYER){
			((Player)this).enhanceAlchemyData(calc);
		}
	...
	protected void processPreBuff(PropertyCalculator calc){
		if(buffs==null)
			return;
		for(int id : preBuff){
			if(id == Horse.jewelBuffId){
				for(Buff buff : buffs.getBuffs()){
					if(buff!=null && buff.getId() == id && buff instanceof PropertyEnhancer){
						((PropertyEnhancer)buff).enhance(calc);
					}
				}
			}else{
				Buff buff = buffs.getBuffByID(id);
				if(buff!=null && buff instanceof PropertyEnhancer){
					((PropertyEnhancer)buff).enhance(calc);
				}
			}
		}
	...
	protected void processHorsePreBuff(PropertyCalculator calc){
		if(buffs==null)
			return;
		for(int id : preBuffHorse){
			if(id == Horse.jewelBuffId){
				for(Buff buff : buffs.getBuffs()){
					if(buff!=null && buff.getId() == id && buff instanceof PropertyEnhancer){
						((PropertyEnhancer)buff).enhance(calc);
	...
	public static boolean isPreBuff(int buffId){
		for(int id : preBuff){
			if(buffId==id)
				return true;
		}
	...
	public void addThreatUnit(Unit u, float initThreat, boolean direct) {
		if(u.type==GameObject.TYPE_ATTENDANT && ((Attendant)u).owner!=null && ((Attendant)u).owner.id==id){
			setAttackState(true);
			return;
		}
	...
	public float getDamageScale(){
		if(type==GameObject.TYPE_PLAYER){
			Player player = (Player)this;
			if(player.attendant!=null)
				return 1 - player.attendant.distance / 100f;
		}else if(type==GameObject.TYPE_ATTENDANT){
			return ((Attendant)this).distance / 100f;
		}
		return 1f;
	}
