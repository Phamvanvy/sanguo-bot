...
public class Skills implements Cloneable{
	...
	public static final int DEFAULT_BOOKSKILL_SIZE = 4;
...
//	}
	...
	public Skill getSkillByGroup(int groupId){
		for(Skill s:skills.values()){
			if(s.getLevel()==0&&s.getGroupId()==groupId){
				return s;
			}
	...
	public void addSkillSlient(Skill skill){
		skills.put(skill.getId(), skill);
	}
...
//	}
	...
	public Skill getSkill(){
		for(Skill s: this.skills.values()){
			if(s!=null && (s.getType() & Skill.TYPE_ATTACK)!=0)
				return s;
		}
		return null;
	}
...
//					}
	...
	public static Skills getSkillsFromDB(byte[] bytes, Fame fame) {
		ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
		DataInputStream dis = new DataInputStream(bais);
		Skills skills = null;
		try {
