...
//		synchronized (p) {
			if (p != null) {
				...
				if (grid == null || grid.item == null
					...
					p.sendUseItemFail(itemUse.itemId, peony.Messages.STRING_00588);
					tx.rollback();
					return;
				}
				if (grid.item.template.useType == null
						|| grid.item.template.useType.effect == null) {
					p.sendUseItemFail(itemUse.itemId, peony.Messages.STRING_00589);
					tx.rollback();
					return;
				}
				if (grid.item.template.useType.occasion == UseType.OCCASION_BATTLE) {
					if (p.getThreatCount() == 0) {
						p.sendUseItemFail(itemUse.itemId, peony.Messages.STRING_00590);
						tx.rollback();
						return;
					}
				} else if (grid.item.template.useType.occasion == UseType.OCCASION_NOBATTLE) {
					if (p.getThreatCount() > 0) {
						p.sendUseItemFail(itemUse.itemId, peony.Messages.STRING_00591);
						tx.rollback();
						return;
					}
				}
				...
				if (itemUse.target != null) {
					...
					if (target == null) {
						p.sendUseItemFail(itemUse.itemId, peony.Messages.STRING_00592);
						tx.rollback();
						return;
					} else {
						if (!target.isAlive()) {
						...
						try {
							...
							if (mitems != null) {
						        ...
						        p.message(-1, peony.Messages.STRING_00593, -1, -1);
						        for (GainItem gitem : mitems) {
						            ...
						            Server.server.getServiceRegistry().getMailService().sendSystemMailAsync(p.id, peony.Messages.STRING_00004, itemTitle, "", 0,
