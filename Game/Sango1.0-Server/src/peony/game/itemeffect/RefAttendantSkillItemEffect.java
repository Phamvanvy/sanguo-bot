...
public class RefAttendantSkillItemEffect implements ItemEffect {
	...
	public void use(Unit source, GameItem item, Unit target, PlayerTransaction tx) throws UseItemException {
		...
		if(target instanceof Player){
			...
			if(item.template.id==2273 || item.template.id==2274 || item.template.id==2275 || item.template.id==4065 || item.template.id == 4066
		...
		if(source instanceof Player){
			...
			if(src!=null){
				if(src.attendant!=null){
					...
					Attack attack = new Attack(skill,source,target,0,src.map.map.getInstanceId(),useBuffs,true);
					attack.trigger();
					Skill attendantSkill = ObjectAccessor.getSkill(Skills.getSkillId(groupId2, level2));
					Attack attendantAttack = new Attack(attendantSkill,src.attendant,src.attendant,0,src.map.map.getInstanceId(),useBuffs,true);
