...
public class AccountDepotService implements Service{
	...
	private static final Logger log = Logger.getLogger(AccountDepotService.class);
	...
	public void turnOn(Player p, int serial,short id) throws Exception {
		if (p != null) {
			if(Server.shopItemBind)
				throw new Exception("您的等级不足，此功能暂未开放");
			if(id == 0){
				AccountDepot accountDepot = accountDepots.get(p.accountId);
				if (accountDepot!=null && accountDepot.depot != null && accountDepot.depot.getGrids().size() > 0) {
					throw new Exception(peony.Messages.STRING_01446);
				}
			    ...
			    if(item == null){
					tx.rollback();
					throw new Exception(peony.Messages.STRING_01447);
			    }
				tx.commit();
	...
	public void getItemFromBagToDepot(Player p, ClientSession session, int serial, 
		...
		if (p != null) {
			...
			if(accountDepot.depot==null || accountDepot.depot.getSize()==0){
				throw new Exception(peony.Messages.STRING_01448);
			}
			int currentCount = p.bag.getGameItemCount(itemId);
			if(currentCount <= 0){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.DEPOT_GETFROMBAG_CLIENT, peony.Messages.STRING_01449);
				return;
			}
			if (count > currentCount) {
				ErrorHandler.sendErrorMessage(session, serial, OpCode.DEPOT_GETFROMBAG_CLIENT, peony.Messages.STRING_01450);
				return;
			}
			...
			if(item == null || item.template.itemType != Item.TYPE_JEWEL || item.template.useLevel < 4){
				throw new Exception(peony.Messages.STRING_01451);
			}
			...
			if(bagGrid==null){
				tx.rollback();
				throw new Exception(peony.Messages.STRING_01452);
			}
			// 记录日志
			...
			try {
				...
				tx.rollback();
				throw new Exception(peony.Messages.STRING_01453);
			}
			// 记录日志
	...
	public void getItemFromDepotToBag(Player p, ClientSession session, int serial,  
		...
		if (p != null) {
			...
			if(accountDepot == null || accountDepot.depot==null || accountDepot.depot.getSize()==0){
				throw new DepotException(peony.Messages.STRING_01454);
			}
			...
			if (count > currentCount) {
				ErrorHandler.sendErrorMessage(session, serial, OpCode.DEPOT_GETFROMDEPOT_CIENT, peony.Messages.STRING_01450);
				return;
			}
			...
			if(depootGrid==null){
				tx.rollback();
				throw new Exception(peony.Messages.STRING_01455);
			}
			// 记录日志
			...
			try {
				...
				tx.rollback();
				throw new Exception(peony.Messages.STRING_01456);
			}
			// 记录日志
			LogUtil.logAccountDepotGetOK(p, depootGrid.getItem(), count);
			tx.commit();
			GameItem item = ObjectAccessor.createGameItem(itemId);
			Server.server.getServiceRegistry().getCardService().processHistoryCards(p, item, count);
			Packet pt = new Packet(OpCode.ACCOUNTDEPOT_GETFROMBAG_SERVER);
			pt.putInt(serial);
			pt.put(1);
			...
			if(accDepot != null){
				if (accDepot.depot.arrange()) {
					...
					ErrorHandler.sendErrorMessage(session, serial,
							OpCode.ACCOUNTDEPOT_ARRANGE_SERVER, peony.Messages.STRING_01457);
				}
			}
		}
	}
...
//		}
	...
	public void loadAccountDepot(Player player){
		AccountDepotDAO accountDepotDAO= Server.server.getServiceRegistry().getDbService().accountDepotDAO;
		AccountDepot depot = accountDepots.get(player.accountId);
		if(depot==null){
			List<AccountDepot> list = accountDepotDAO.getAccountDepots(player.accountId);
			if(list!=null && list.size()>0){
				depot = list.get(0);
				if(list.size()>1){
					for(int i=1;i<list.size();i++){
						try {
							AccountDepot otherDepot = list.get(i);
							for(int gridId=0;gridId<otherDepot.depot.getGrids().size();gridId++){
								if(otherDepot.depot.getGrid(gridId)!=null && otherDepot.depot.getGrid(gridId).item!=null 
										&& otherDepot.depot.getGridItemCount(gridId)>0){
									GameItem item = otherDepot.depot.getGrid(gridId).item;
									int count = otherDepot.depot.getGridItemCount(gridId);
									boolean b = depot.depot.addGameItem0(item, count);
									if(!b){
										MailService mailService = Server.server.getServiceRegistry().getMailService();
										mailService.sendSystemMailAsync(player.id, "系统", "珍宝阁", "珍宝阁物品补发", 0, item, count, "MERGEDEPOT");
										log.info("[ACCDEPOTMISSTOMAIL]"+LogUtil.getGameItemString(item, count)+"ACC["+player.accountId+"]");
									}else{
										log.info("[ACCDEPOT]"+LogUtil.getGameItemString(item, count)+"ACC["+player.accountId+"]");
									}
								}
							}
							accountDepotDAO.delete("delete from AccountDepot a where a.id=?", otherDepot.id);
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
...
//				try {
//					depot = accountDepotDAO.getAccountDepot(accountId);
//				} catch (Exception e) {
//					//曾经出现一个bug,玩家在极短时间内重复登录可能导致开启多个账号仓库
//					List<AccountDepot> list = accountDepotDAO.list("from AccountDepot a where a.accountId=?", accountId);
//					if(list!=null){
//						int maxId = 0;
//						for(AccountDepot ac : list){
//							if(ac.id>maxId)
//								maxId = ac.id;
//						}
//						Iterator<AccountDepot> it = list.iterator();
//						while(it.hasNext()){
//							AccountDepot a = it.next();
//							if(a.id!=maxId)
//								accountDepotDAO.delete("delete from AccountDepot a where a.id=?", a.id);
//							else
//								depot = a;
//						}
//					}
//				}
		...
		if(depot!=null){
			accountDepots.put(player.accountId, depot);
		}
	}
