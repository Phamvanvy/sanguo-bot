...
import peony.game.buff.ParalyzeDebuff;

/**
 * 辅助技能：麻痹。
 * @author lighthu
 */
public class ParalyzeSkill extends AbstractSkill {
	/*
	 * 持续时间(毫秒)
	 */
	protected int time;
	
	public ParalyzeSkill(int time) {
		super(0, "", 0);
		this.time = time;
		type = TYPE_ATTACK;
            ...
            public void finished(CombatContext context, boolean isActive) {
				...
				if (context.hited()) {
					if(context.target instanceof Player){
				    	...
				    	if(p.timeRatio[2]!=1.0f){
				    		time = Math.round(time*p.timeRatio[2]);
				    	}
					}
					context.target.buffs.addBuff(new ParalyzeDebuff(context.source, time));
				}
			}
		};
	}
