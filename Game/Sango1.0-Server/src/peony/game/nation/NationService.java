...
//	protected Map<Long,NationBattleFieldDef> defs = new HashMap<Long,NationBattleFieldDef>();
	...
	public List<Forbid> forbids = new ArrayList<Forbid>();
	...
	public static int[] BATTLE_FIELD_MINLEVEL = {0,70,70,70}; //进入战场的最低等级
...
//	public static final int BATTLE_FIELD_MAXPLAYER = 6; //每方的最大人数
	public static final long BATTLE_FIELD_TIME = 1800  * 1000L; //国战的持续时间
...
//		skills.put(1, new NationSkill1(0));
		...
		skills.put(6, new NationSkill6(0));
		skills.put(7, new NationSkill7(0));
		skills.put(8, new NationSkill8(0));
		skills.put(9, new NationSkill9(0));
	}
	...
	protected void playerFirstLoad(Player p){
		if(isKing(p)){
			...
			if(rels[p.faction][othersFaction[0]].type==NationRel.TYPE_WAR_REQUESTED){
				p.message(-1, 
						MessageFormat.format(peony.Messages.STRING_01868, GameObject.getFactionName(othersFaction[0])), -1, -1);
			}
			if(rels[p.faction][othersFaction[1]].type==NationRel.TYPE_WAR_REQUESTED){
				p.message(-1, 
						MessageFormat.format(peony.Messages.STRING_01868, GameObject.getFactionName(othersFaction[1])), -1, -1);
			}
		}
	}
	...
	public NationBattleFieldInstance battleFieldSignup(Player player,boolean attack) throws NationBattleFieldSignupException{
		if(!isKing(player) && player.level<BATTLE_FIELD_MINLEVEL[player.faction]){
			throw new NationBattleFieldSignupException(MessageFormat.format(peony.Messages.STRING_01869, BATTLE_FIELD_MINLEVEL[player.faction]));
		}
		for(NationBattleFieldInstance instance:instances){ //找到sourceFaction相同的战场
			if((instance.def.sourceFaction==player.faction)||(instance.def.destFaction==player.faction)){
				if(!instance.contains(player)){
					if(isKing(player)){ //如果是国王，那么无条件进入战场
						...
						if(instance.getSignupPlayerCount(player.faction)<BATTLE_FIELD_MAXPLAYER){
							...
							throw new NationBattleFieldSignupException(peony.Messages.STRING_01870);
						}
					}
				}else{
					return instance;
		...
		throw new NationBattleFieldSignupException(peony.Messages.STRING_01871);
	}
	
	public NationSneakBattleFieldInstance sneakSignup(Player player,boolean attack) throws NationBattleFieldSignupException{
		if(!isKing(player) && player.level<BATTLE_FIELD_MINLEVEL[player.faction]){
			throw new NationBattleFieldSignupException(MessageFormat.format(peony.Messages.STRING_01872, BATTLE_FIELD_MINLEVEL[player.faction]));
		}
	...
	public void declareSneak(int sourceFaction,int destFaction) throws NationDeclareException{
		...
		if(requests[sourceFaction]!=null)
			throw new NationDeclareException(peony.Messages.STRING_01873);
		if(rel.type!=NationRel.TYPE_FAIL)
			throw new NationDeclareException(peony.Messages.STRING_01874);
		Date now = new Date();
		if((now.getTime()-rel.createTime.getTime())>10*60*1000)
			throw new NationDeclareException(peony.Messages.STRING_01875);
...
//		cal.add(Calendar.MINUTE, 2);
	...
	public void declareWar(int sourceFaction,int destFaction,Date begin,Date end) throws NationDeclareException{
		...
		if(Time.betweenHour(Time.currDate, 18, 24))
			throw new  NationDeclareException(peony.Messages.STRING_01876);
		Nation destNation = getNationByFaction(destFaction);
		if(destNation.guardTime!=null&&destNation.guardTime.after(Time.currDate))
			throw new NationDeclareException(peony.Messages.STRING_01877);
		NationRel rel = rels[sourceFaction][destFaction];
		if(rel.type==NationRel.TYPE_WAR_REQUEST)
			throw new NationDeclareException(peony.Messages.STRING_01878);
		if(rel.type==NationRel.TYPE_WAR_REQUESTED)
			throw new NationDeclareException(peony.Messages.STRING_01879);
		if(rel.type==NationRel.TYPE_WAR_PREPARE||rel.type==NationRel.TYPE_WARED_PREPARE)
			throw new NationDeclareException(peony.Messages.STRING_01880);
		if(rel.type==NationRel.TYPE_ATTACK||rel.type==NationRel.TYPE_DEFENSE)
			throw new NationDeclareException(peony.Messages.STRING_01881);
		...
		if(rels[sourceFaction][otherFaction].type==NationRel.TYPE_WAR_PREPARE||rels[sourceFaction][otherFaction].type==NationRel.TYPE_WAR_REQUEST)
			throw new NationDeclareException(peony.Messages.STRING_01882);
...
//			throw new NationDeclareException("不能对目标国家宣战");
		if(rels[destFaction][otherFaction].type==NationRel.TYPE_WAR_REQUESTED)
			throw new NationDeclareException(peony.Messages.STRING_01883);
		if(rels[destFaction][otherFaction].type==NationRel.TYPE_WARED_PREPARE)
			throw new NationDeclareException(peony.Messages.STRING_01884);
//		if(rels[destFaction][otherFaction].type==NationRel.TYPE_WAR_REQUESTED)
//			throw new NationDeclareException("目标国家已经处于战前准备状态");
		if(rels[destFaction][otherFaction].type==NationRel.TYPE_DEFENSE)
			throw new NationDeclareException(peony.Messages.STRING_01885);
		...
		Nation sourceNation = getNationByFaction(sourceFaction);
		Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(destFaction,
				MessageFormat.format(peony.Messages.STRING_01886, 
						GameObject.getFactionName(sourceFaction),sourceNation.getKingName()));
		Server.server.getServiceRegistry().getChatService().sendWorldMessage(MessageFormat.format("{0}正式向{1}宣战。", GameObject.getFactionName(sourceFaction),GameObject.getFactionName(destFaction)));
	}
	...
	public void acceptWar(int sourceFaction,int destFaction) throws NationDeclareException{
		...
		if(rel1.type!=NationRel.TYPE_WAR_REQUESTED)
			throw new NationDeclareException(peony.Messages.STRING_01887);
...
//		cal.add(Calendar.MINUTE,5);
		...
		count = 1;
		Nation sourceNation = getNationByFaction(sourceFaction);
		Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(sourceFaction,
				MessageFormat.format(peony.Messages.STRING_01888, 
						sourceNation.getKingName(),GameObject.getFactionName(destFaction)));
		Nation destNation = getNationByFaction(destFaction);
		Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(destFaction,
				MessageFormat.format(peony.Messages.STRING_01889, 
						GameObject.getFactionName(sourceFaction),destNation.getKingName()));
	...
	public void refuseWar(int sourceFaction,int destFaction) throws NationDeclareException{
		...
		.sendFactionSystemMessage(destFaction, 
				MessageFormat.format(peony.Messages.STRING_01890,nation.getKingName(),String.valueOf(v)));
		Server.server.getServiceRegistry().getChatService()
		.sendFactionSystemMessage(sourceFaction, 
				MessageFormat.format("国公{0}拒绝了{1}的挑战，国库损失{2}资金。",nation.getKingName(),GameObject.getFactionName(destFaction),String.valueOf(v)));
	}
	...
	protected void playerDie(Unit p, Unit p1){
		if(p instanceof Player && p1 instanceof Player && isKing((Player)p) && p.faction!=p1.faction){
			GameMapDefinition def = VMapUtil.getDefinition(p.map.id);
			String name = def.mapInfo.name;
			char[] c = name.toCharArray();
			String mapName = name.substring(0, getIndex(c));
			if(((Player)p).battleType!=Player.TYPE_ASYNC_PLAYER){
				Server.server.getServiceRegistry().getChatService()
				.sendFactionSystemMessage(p.faction, 
						MessageFormat.format(peony.Messages.STRING_01891, p.name,mapName,GameObject.getFactionName(p1.faction),p1.name));
				Server.server.getServiceRegistry().getChatService()
					.sendFactionSystemMessage(p1.faction, MessageFormat.format(peony.Messages.STRING_01892, p1.name,mapName,GameObject.getFactionName(p.faction),p.name));
			}
		}
	}
	...
	protected void playerLogOut(Player p){
		if(isKing(p)){
			...
			if(lastLogoutTime==0 || (System.currentTimeMillis()-lastLogoutTime)>=5*60*1000L){
				Server.server.getServiceRegistry().getChatService()
				.sendFactionSystemMessage(p.faction, MessageFormat.format(peony.Messages.STRING_01893, p.name));
	...
	protected void playerLoaded(Player player, int type) {
		...
		if(isKing(player)){
			...
			if(lastLoginTime==0 || (System.currentTimeMillis()-lastLoginTime)>=5*60*1000L){
				Server.server.getServiceRegistry().getChatService()
				.sendFactionSystemMessage(player.faction, MessageFormat.format(peony.Messages.STRING_01894, player.name));
		...
		synchronized (forbids) {
			for(Forbid forbid : forbids){
				if(forbid.targetId==id){
					throw new NationVoteException(peony.Messages.STRING_01895);
				}
			}
...
//		nations[2].buff = new NationBuff(0.05f,0.06f,0.07f);
	...
	public void collect(Player p, int money) throws NationVoteException{
		...
		if(p==null)
			return;
		if(money<=0)
			throw new NationVoteException(peony.Messages.STRING_01896);
		...
		try {
			...
			tx.rollback();
			throw new NationVoteException(peony.Messages.STRING_00129);
		}
		...
		if(money>=20000){
			Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(p.faction,
					MessageFormat.format(peony.Messages.STRING_01897, p.name,money));
		}
		...
		log.info("[COLLECTSUCCESS]"+LogUtil.getPlayerLogString(p)+"MONEY["+money+"]BALANCE["+p.money+"]");
	}
	...
	public void calculateCollection(Nation nation){
		...
		Server.server.getServiceRegistry().getChatService().addChatMessage(
				new ChatMessage(ChatOption.FACTION,-1,nation.faction ,peony.Messages.STRING_00004,nation.faction
						, MessageFormat.format(peony.Messages.STRING_01898, collection,nation.getName()),null));
	}
	...
	protected void updateRequests(){
		for(int i = 0;i<requests.length;i++){
			if(requests[i] != null && Time.currDate.after(requests[i].endTime)){
				...
				chatService.sendFactionSystemMessage(req.destFaction, MessageFormat.format(peony.Messages.STRING_01899, GameObject.getFactionName(req.sourceFaction)));
				chatService.sendFactionSystemMessage(req.sourceFaction,
						MessageFormat.format(peony.Messages.STRING_01900,
						GameObject.getFactionName(req.destFaction)));
				chatService.sendFactionShout(req.sourceFaction, peony.Messages.STRING_01901, 0x0000ff, 6000);
				chatService.sendFactionShout(req.destFaction, peony.Messages.STRING_01901, 0x0000ff, 6000);
			}
		}
	}
	...
	protected void updateRel(NationRel rel){
		if(rel.endTime!=null&&rel.endTime.before(Time.currDate)){
			if(rel.type==NationRel.TYPE_WAR_REQUEST){//如果宣战时间结束还没有确认，那么就进入战前准备
				...
				ChatService chatService = Server.server.getServiceRegistry().getChatService();
				chatService.sendFactionSystemMessage(rel.sourceFaction, peony.Messages.STRING_01902);
				chatService.sendFactionSystemMessage(rel.destFaction, 
						MessageFormat.format(peony.Messages.STRING_01903, GameObject.getFactionName(rel.sourceFaction)));
				chatService.sendFactionShout(rel.sourceFaction, peony.Messages.STRING_01904, 0x0000ff, 6000);
				chatService.sendFactionShout(rel.destFaction, peony.Messages.STRING_01904, 0x0000ff, 6000);
			}
		...
		else if(rel.endTime!=null){
			if(rel.endTime.getTime() - System.currentTimeMillis() <= 60*60*1000l && count == 1){
				ChatService chatService = Server.server.getServiceRegistry().getChatService();
				chatService.sendFactionSystemMessage(rel.sourceFaction, MessageFormat.format(peony.Messages.STRING_01905, GameObject.getFactionName(rel.destFaction)));
				chatService.sendFactionSystemMessage(rel.destFaction, 
						MessageFormat.format(peony.Messages.STRING_01906,GameObject.getFactionName(rel.sourceFaction)));
				count = 0;
			}
		}
	}
...
//		return defs.get(l);
	...
	public void setBattleFieldMinLevel(Player p, int setLevel) throws NationBattleFieldSignupException{
		if(p!=null){
			if(!isKing(p))
				throw new NationBattleFieldSignupException(peony.Messages.STRING_01907);
			if(setLevel<50 || setLevel>65)
				throw new NationBattleFieldSignupException(peony.Messages.STRING_01908);
