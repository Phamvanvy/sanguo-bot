		...
		if (p != null) {
			...
			synchronized (nation) {
				...
				if (officer == null || officer.level != Officer.KING) {
					...
					error(null, peony.Messages.STRING_01846);
					addToClientSession();
					return;
				}
				...
				if (actor == null) {
					log.info("[OFFICERFAILED]"+LogUtil.getPlayerLogString(p)+"LEVEL["+level+"]TARGET["+targetName+"]");
					error(null, peony.Messages.STRING_00497);
					addToClientSession();
					return;
				}
				if (slogan.length() == 0) {
					error(null, peony.Messages.STRING_01847);
					addToClientSession();
					return;
				}
				if (actor.faction != p.faction) {
					log.info("[OFFICERFAILED]"+LogUtil.getPlayerLogString(p)+"LEVEL["+level+"]TARGET["+targetName+"]");
					error(null, peony.Messages.STRING_01848);
					addToClientSession();
					return;
				}
				if (actor.level < 50) {
					log.info("[OFFICERFAILED]"+LogUtil.getPlayerLogString(p)+"LEVEL["+level+"]TARGET["+targetName+"]");
					error(null, peony.Messages.STRING_01849);
					addToClientSession();
					return;
				}
				...
				if (targetOfficer != null) {
					log.info("[OFFICERFAILED]"+LogUtil.getPlayerLogString(p)+"LEVEL["+level+"]TARGET["+targetName+"]");
					error(null, peony.Messages.STRING_01850);
					addToClientSession();
					return;
				}
				...
				Server.server.getServiceRegistry().getChatService()
						.sendFactionSystemMessage(
								actor.faction,
								MessageFormat.format(peony.Messages.STRING_01851, actor.name,slogan,targetOfficer.getName()));
//				Server.server.getServiceRegistry().getChatService()
//				.sendFactionSystemMessage(
//						actor.faction,slogan);
				Player targetPlayer = ObjectAccessor.getPlayer(actor.id);
				if(targetPlayer!=null){
				    targetPlayer.addIntPropertyChangedItem(ChangedItem.NATION_DUTY,targetOfficer.level,false,true);targetPlayer.addIntPropertyChangedItem(ChangedItem.NATION_DUTY,targetOfficer.level,false,true);
				}
