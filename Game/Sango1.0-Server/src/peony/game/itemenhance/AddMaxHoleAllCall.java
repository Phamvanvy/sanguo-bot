...
public class AddMaxHoleAllCall  extends ClientSessionAsyncCall{
	protected final Logger log = Logger.getLogger(AddMaxHoleAllCall.class);
	protected int serial;
	protected Player player;
    protected int equItemID;
	protected int equInstanceID;
	protected ItemEnhance itemEnh;
	public static final int ADD_MAX_HOLE_ALLITEM = 994; //极限打孔符
	public static final int MAX_HOLE_COUNT = 6;

	public AddMaxHoleAllCall(ClientSession session, Packet packet) {
		super(session);
	...
	protected void go() throws Exception {
	    ...
	    if (player.bag.removeGameItem(ADD_MAX_HOLE_ALLITEM, GameItem.GENERAL_INSTANCEID, 1, tx, true) == null) {
	        tx.rollback();
	        ItemTemplate it = ObjectAccessor.getItemTemplate(ADD_MAX_HOLE_ALLITEM);
	...
	public void callFinish() {
	    try {
	        ...
	        // 回送打孔成功的包
	        Packet pt = new Packet(OpCode.DECORATE_ADD_MAXHOLE_ALL_SERVER);
            pt.putInt(serial);
            pt.putInt(equItemID);
	        ...
	        ErrorHandler.sendErrorMessage(session, serial,
                    OpCode.DECORATE_ADD_MAXHOLE_ALL_CLIENT, e.getMessage());
	    }
	}
