...
public class RemoveJewelCall extends ClientSessionAsyncCall {
	...
	protected void go() throws Exception {
	    ...
	    if (jewelID == -1) {
	        throw new Exception(peony.Messages.STRING_01083);
	    }
	    ...
	    if(lvl > baseLvl){
		    ...
		    if(removeSuccess==0){
		    	tx.rollback();
		        throw new Exception(MessageFormat.format(peony.Messages.STRING_01084, js.removeItems[lvl - 1].name));
		    }
	    }
	    
	    // 把摘除的宝石加入背包
	    GameItem jewel = ObjectAccessor.createGameItem(jewelTemp, -1);
	    int count = itemEnh.jewelUpgrades[itemEnh.getUpgradesIndexByHole(hole)+1] + 1;
