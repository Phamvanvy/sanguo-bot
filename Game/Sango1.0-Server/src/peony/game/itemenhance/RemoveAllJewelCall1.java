...
public class RemoveAllJewelCall1 extends SyncIbuyCall {
	...
	public RemoveAllJewelCall1(ClientSession session, Packet packet) {
	...
	public void callFinish() throws Exception {
		if(success){
			...
			ErrorHandler.sendErrorMessage(session, serial, OpCode.DECORATE_REMOVE_ALLJEWEL_CLIENT, errorMessage);
		}
	}
	...
	public static Packet getRequest(ItemEnhance enhance, ClientSession session ,int serial) throws Exception{
		...
		for(int i = 0; i < jewelNum; i ++){
			...
			int baseLvl = VipPrivilegeService.VIP_REMOVEJEWEL[player.vipLevel];
			if(jewelTemp.useLevel > baseLvl){	//3级以下的宝石，不需要摘除符(包括3级)
		...
		if(success){
			...
			if(player != null){
				...
				if (gi.template.equipment == null) {
					error(peony.Messages.STRING_00016);
			        addToClientSession();
			        return;
			    }
			    if (gi.object == null) {
			    ...
			    if (!(gi.object instanceof ItemEnhance)) {
			    	error(peony.Messages.STRING_00017);
			    	addToClientSession();
			    	return;
			    }
				...
				for(int j = 0; j < jewelNum; j ++){
				    ...
				    if(removeSuccess==0){
					    ...
					    try {
					    	if(lvl<baseLvl)
					    		waitBuy(player, 0, service.getShopByItemId(js.removeItemIDs[jewelTemp.useLevel - 1]).id, js.removeItemIDs[jewelTemp.useLevel - 1], 1, this);
					    	else
					    		waitBuy(player, 0, service.getShopByItemId(js.removeItemIDs[lvl - 1<2?2:lvl-1]).id, js.removeItemIDs[jewelTemp.useLevel - 1], 1, this);
					    } catch (Exception e) {
					    	tx.rollback();
					    	addToClientSession();
					    	return;
				 ...
				 // 把摘除的宝石加入背包
					GameItem jewel = ObjectAccessor.createGameItem(jewelsId[j]);
					int count = enhance.jewelUpgrades[1] + 1;
				    if (!player.bag.addGameItem(jewel, count, tx, true)) {
				    	//背包已满发飞鸽
				    	MailService ms = Server.server.getServiceRegistry().getMailService();
				    	if(ms.sendSystemMail(player.id, player.name, peony.Messages.STRING_01721, peony.Messages.STRING_01721, 0, jewel, count, peony.Messages.STRING_00555) == -1){
				    		tx.rollback();
				    		error(peony.Messages.STRING_01722);
				    		addToClientSession();
				    		return;
				    	}
