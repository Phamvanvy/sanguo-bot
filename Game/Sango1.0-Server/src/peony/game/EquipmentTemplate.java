...
public class EquipmentTemplate {
	...
	public int limitType;//限制类型（人，随从...）
	...
	public int initCardCount, maxCardCount;
	...
	public int equipCategory; //装备新图标套系
	...
	public int levelPromoteRate = 6;  //星级装备物品等级比例
	public int basePromoteValue = 7;  //最低物品等级
	...
	public byte[] toClientBytes(ItemEnhance ie) {
		...
		try {
			...
			dos.writeByte(initCardCount); //初始卡槽数
			dos.writeByte(maxCardCount); //最大卡槽数
			dos.write(markCharCount);
			dos.write(mask1);
			dos.write(mask2);
			dos.write(mask3);
			dos.write(equipCategory);
			if (getMaxHP(ie) > 0) {
			...
			if (v != 0) {
				if(ie.getPrimaryEnhance() != 0){
					if(ie.getStar()==0)
						...
						float sv = f * (level + ie.getStar()*levelPromoteValue(level))/level;
					...
					if(ie.getStar() == 0){
						...
						return Math.round(f * (level + ie.getStar()*levelPromoteValue(level))/level);
					}
				}
			} else {
				return 0;
			...
			if (v != 0) {
				if(ie.getPrimaryEnhance() != 0){
					if(ie.getStar()==0){
						...
						float fs = f * (level + ie.getStar()*levelPromoteValue(level))/level;
			...
			if (v != 0) {
				if(ie.getPrimaryEnhance() != 0){
					if(ie.getStar()==0){
						...
						float fs = f * (level + ie.getStar()*levelPromoteValue(level) + 9)/(level+9);
						float fv = f*(1f+ie.getPrimaryEnhance()/1000f)+(fs-f)*(1f+ie.getStarEnhance()/1000f);
						return Math.round(fv);
					}
				} else {
					if(ie.getStar() == 0)
						...
						return Math.round(f * (level + ie.getStar()*levelPromoteValue(level) + 9)/(level+9));
			...
			if (v != 0) {
				if(ie.getPrimaryEnhance() != 0){
					if(ie.getStar()==0){
						...
						float sv = f * (level + ie.getStar()*levelPromoteValue(level) + 9)/(level+9);
	 ...
	 * @return
	 */
	public int levelPromoteValue(int level){
		int ret = basePromoteValue;
		double tempRet = (double)(level * levelPromoteRate)/100;
		int r = (int)Math.ceil(tempRet);
		if(r>ret){
			ret = r;
		}
		return ret;
