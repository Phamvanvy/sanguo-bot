...
public class TransactionBag {
	...
	protected static int sizeLimit = 200;
	
	// 上次添加物品失败时，剩余未加入成功的物品数量
	public transient int remainCountOnLastFail;
...
//				if (grids.get(i).id > 127 || grids.get(i).id < 0) {
//					if (grids.get(i).getItem() != null) {
//						System.out.println(grids.get(i).getItem().template.id + "x" + grids.get(i).getCount());
//					}
//					grids.remove(i);
//					i--;
//				}
//			}
//			for (Set<Integer> sset : indexes.values()) {
...
//	}
	...
	public TransactionBag(int size, int addedSize){
		this.size = size;
		this.addedSize = addedSize;
		grids = new ArrayList<TransactionBagGrid>(getSize());
		for (int i = 0; i < getSize(); i++) {
		...
		if (sourceId >= 0 && sourceId <= grids.size() && targetId >= 0
			...
			try {
				...
				if (targetGrid.isEmpty()) {
					...
					if (isBag) {
						owner.changed.addChangedItem(new BagChangedItem(sourceGrid,0));
						owner.changed.addChangedItem(new BagChangedItem(targetGrid,0));
					}
					return true;
				}
	...
	public GameItem removeDepotGameItemIngoreInstanceId(int itemId,int count,PlayerTransaction tx, boolean notify){
		lock.lock();
		try {
			Set<Integer> set = getIndexSet(itemId);
			if (set == null) {
			...
			for (int gridId : set) {
				TransactionBagGrid grid = grids.get(gridId);
				int v = grid
						.removeDepotGameItem(itemId, grid.item.instanceId, all, tx, notify);
				all -= v;
				if (all == 0)
					return grid.item;
			}
	...
	public void addGameItemComplete(GameItem item, int count,
		...
		try {
			isSpecialItem = filterSpecialGameItem(item, count);
		} catch (Exception e) {
		} finally{
			if(isSpecialItem){
				lock.unlock();
	...
	/** 处理一些特殊物品的添加,如新手第一次获得坐骑物品直接生成坐骑并上骑 */
	protected boolean filterSpecialGameItem(GameItem item, int count) throws Exception{
		//特殊处理坐骑物品
		if(count==1 
				&& item.template.itemType==Item.TYPE_HORSE ){
			if(Horse.getFreeTime(item.template.id)>0){
				ItemEffect effect = item.template.useType.effect;
				if(effect!=null && effect instanceof HorseItemEffect){
					((HorseItemEffect)effect).getHorseAndRide(owner,item,
							Time.currentTimeMillis(Time.currTime)+Horse.getFreeTime(item.template.id));
					return true;
				}
			}else{
				if(Horse.indexOfAutoRide(item.template.id)!=-1){
					ItemEffect effect = item.template.useType.effect;
					if(effect!=null && effect instanceof HorseItemEffect){
						((HorseItemEffect)effect).getHorseAndRide(owner,item,
								0);
		...
		}else if(item.template.itemType==Item.TYPE_CARD){
			//处理卡片:新的卡片处理方式
			...
			if(card == null){
				throw new Exception("卡片类型错误");
			}
//			boolean hasMatch = owner.pool.getInt(CardService.getPropertyOfPlayerCard(card.id),0) >0;
			boolean hasMatch = CardService.getCardCount(owner, card.id)>0;
		    if(!hasMatch){
				int groupId = card.suiteId;
				if(groupId != -1){
					owner.pool.setInt(CardService.getPropertyOfPlayerSuit(groupId), owner.pool.getInt(
							CardService.getPropertyOfPlayerSuit(groupId), 0) + 1);
				}
				owner.pool.setInt(CardService.PROPERTY_HAVECARD, owner.pool.getInt(CardService.PROPERTY_HAVECARD, 0) + count);		
//				owner.pool.setInt(CardService.getPropertyOfPlayerCard(card.id), count);
				CardService.addCard(owner, card.id, count);
				CardInfo cardInfo = new CardInfo(card.id);
				cardInfo.level=1;
				owner.cards.cardInfos.put(card.id, cardInfo);
		    }else if(card.prorertyType==8){
		    	CardService.addCard(owner, card.id, count);
//		    	owner.pool.setInt(CardService.getPropertyOfPlayerCard(card.id), owner.pool.getInt(CardService.getPropertyOfPlayerCard(card.id))+count);
		    }
		    return true;
		}
		return false;
...
//					true);
	...
	public ChangedItem[] splitGridGameItem(int gridId, int itemId, int count) throws NoEnoughSpaceException {
		...
		try {
			...
			ret[0] = new BagChangedItem(grid,0);
			ret[1] = new BagChangedItem(newGrid,0);
			return ret;
		} finally {
			lock.unlock();
		}
