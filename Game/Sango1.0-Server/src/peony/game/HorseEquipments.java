...
//	}
	...
	protected void decDuration(GameItem item,int value,Player p){
		...
		if(p!=null)
			notifyRepairEquipments(oldDuration, item.duration, maxDuration, p);
		if(item.duration==0){
	...
	protected void notifyRepairEquipments(int oldDuration, int currentDuration, int maxDuration, Player player){
		if(!player.notifyReparHorseEquipment_1 && maxDuration>0 && oldDuration>0 && currentDuration==0){
			Server.server.getServiceRegistry().getChatService().sendPrivateMessage(player.id, "您有坐骑装备耐久已经变成0了，实力大幅削减，请赶快修理。");
			player.notifyReparHorseEquipment_1 = true;
			return;
		}
		if(!player.notifyReparHorseEquipment && maxDuration>0 && oldDuration>=maxDuration*0.1 && currentDuration>0 && currentDuration<maxDuration*0.1){
			Server.server.getServiceRegistry().getChatService().sendPrivateMessage(player.id, "您有坐骑装备该修理了，还请英雄尽快修理，若耐久为0后您的实力可是会大打折扣的呢。");
			player.notifyReparHorseEquipment = true;
		}
	}
	...
	public boolean hasEquipments(){
		if(equs==null)
			return false;
		for(GameItem item : equs){
			if(item!=null && item.template!=null && (item.template.isHorseEquipment() || item.template.isEquipment()))
				return true;
		}
		return false;
	}
