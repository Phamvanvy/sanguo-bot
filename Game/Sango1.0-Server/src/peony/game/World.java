...
public class World implements Service, ServiceEventListener, DataChangeHandler {
	...
	protected ArrayBlockingQueue<AsyncCall> calls = new ArrayBlockingQueue<AsyncCall>(4096);
...
//			iteCalls.remove();
	...
	public VMap addPlayerToMap(Player player,int mapId,int x,int y,boolean check) throws VMapException{
		...
		if(manager!=null){
			VMap map = manager.addToMap(player, mapId,x,y,check);
			//处理随从过地图
			if(player!=null && player.attendant!=null){
				try{player.attendant.removeFromWorld();}catch(Exception e){}
			}
			return map;
		}
		return null;
