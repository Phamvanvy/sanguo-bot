...
public class FlagBattleFieldVMapManager implements VMapManager, Service, ServiceEventListener  {
	...
	public LinkedHashMap<Integer, SignUp> signups = new LinkedHashMap<Integer, SignUp>();
...
//	protected IntRange[] levelRanges;
	...
	public VMap addToMap(Player player, int mapId, int x, int y, boolean check)
		...
		if(instance!=null){
			...
			if(instanceTran){
				if(instance.state==FlagBattleFieldInstance.STATE_INIT){
					...
					if(min!=0){
						...
						int sec = t/(1000);
						s = MessageFormat.format(peony.Messages.STRING_01604, sec);
					}
			...
			throw new VMapException(peony.Messages.STRING_01605);
		}else{
			int[] out = outs[player.faction-1];
			return world.addPlayerToMap(player, out[0], out[1],
					out[2], false);
	...
	public void signup(Player player,int serial){
		...
		if(!canSignup()){
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, peony.Messages.STRING_01606);
			return;
		}
		if(duelService.signUps.contains(player.id)){
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, peony.Messages.STRING_01607);
			return;
		}
		if (signups.containsKey(player.id)){
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, peony.Messages.STRING_01608);
			return;
		}
		...
		if  (playerid2forbid.containsKey(player.id)){
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, peony.Messages.STRING_01609);
			return;
		}
		if (player.getVMap().instance!=null){
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, peony.Messages.STRING_01610);
			return;
		}
		int levelIndex = getLevelIndex(player.level);
		if(levelIndex==-1){
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, peony.Messages.STRING_01587);
			return;
		}
		...
		try {
			...
			tx.rollback();
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, MessageFormat.format(peony.Messages.STRING_01611,money));
			return;
		}
	...
	protected FlagBattleFieldInstance createInstance(FlagBattleFieldDefinition def,LevelDef levelDef){
		...
		VMap map = VMapUtil.create(this, world, def.mapId,1024,1024,Server.server.revision);
			...
			for (int i = 0; i < triggers.length; i++) {
				if (triggers[i].in(hour, min)) {
					...
					Server.server.getServiceRegistry().getChatService()
							.sendSystemMessage(ChatOption.SYSTEM, peony.Messages.STRING_00004,
									peony.Messages.STRING_01612);
					break;
				}
			}
		}
    ...
    public boolean hasStarted(Player p){
    	if(p!=null){
    		FlagBattleFieldInstance instance = playerid2instance.get(p.id);
    		if(instance!=null){
    			return instance.state==FlagBattleFieldInstance.STATE_INIT;
    		}
    	}
    	return false;
    }
...
class StartNotify implements Runnable{
	...
	public void run(){
		Server.server.getServiceRegistry().getChatService().sendSystemMessage(ChatOption.SYSTEM, peony.Messages.STRING_00004, message);
	}
}
