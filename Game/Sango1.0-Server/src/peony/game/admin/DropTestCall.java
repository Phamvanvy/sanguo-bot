...
public class DropTestCall extends ClientSessionAsyncCall {
	...
	public void callFinish() throws Exception {
		if (map.size() > Short.MAX_VALUE) {
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_GM_DROP_TEST, MessageFormat.format(peony.Messages.STRING_01098, map.size()));
			return;
		}
	...
	public static String qualityName[] = {peony.Messages.STRING_01099,peony.Messages.STRING_01100,peony.Messages.STRING_01101,peony.Messages.STRING_01102,peony.Messages.STRING_01103};
	public int qualityCount[] = new int[qualityName.length];
	public void run() {
		GroupDrop d = ObjectAccessor.getGroupDrop(dropGroupId);
		if (d == null) {
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_GM_DROP_TEST, MessageFormat.format(peony.Messages.STRING_01104, dropGroupId));
			return;
		}
