		...
		if (actor != null) {
			if(count<=0){
				ErrorHandler.sendAdminErrorMessage(session, serial,
						OpCode.ADMIN_SENDMAIL_CLIENT, MessageFormat.format(peony.Messages.STRING_01439, count));
				return;
			}
				
			if (itemId == -1) {
				Server.server.getServiceRegistry().getMailService()
						.sendSystemMail(playerId, peony.Messages.STRING_00004, title, content, count,
								null, 0, "GM");
			} else {
				...
				if (item != null) {
					...
					if (enhanceOk) {
						...
						ErrorHandler.sendAdminErrorMessage(session, serial,
								OpCode.ADMIN_SENDMAIL_CLIENT, peony.Messages.STRING_00611);
					}
				} else {
					ErrorHandler.sendAdminErrorMessage(session, serial,
							OpCode.ADMIN_SENDMAIL_CLIENT, peony.Messages.STRING_00280);
				}
			}
		} else {
			ErrorHandler.sendAdminErrorMessage(session, serial,
					OpCode.ADMIN_SENDMAIL_CLIENT, peony.Messages.STRING_00497);
		}
	}
