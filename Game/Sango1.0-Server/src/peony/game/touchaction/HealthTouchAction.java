		...
		if (player.pkInfo == null
			...
			if(attendant==null){
				if (player.maxhp == player.hp && player.maxmp == player.mp) {
					ErrorHandler.sendErrorMessage(player.session, -1, OpCode.TOUCHNPC_CLIENT, peony.Messages.STRING_00621);
					return;
				} else {
...
//			ErrorHandler.sendErrorMessage(player.session, -1, OpCode.TOUCHNPC_CLIENT, peony.Messages.STRING_00622);
			player.message(-1, peony.Messages.STRING_00622, -1, -1);
		}
	}
