...
//	public static int MAX_TIMES = 5;
   ...
   public List<NormalInstance> getNormalInstance(int playerId){
	   return playerid2instances.get(playerId);
   }
   
   public void setNormalInstance(int playerId,List<NormalInstance> instances){
	   playerid2instances.put(playerId, instances);
   }
			...
			if(oldMap!=null){
				if(oldMap.instance!=null){ //如果原来的地图有副本
					...
					if(newMap!=null){//如果就在当前副本，并且从一张地图到另外一张地图
						...
						throw new VMapException(peony.Messages.STRING_01958);
					}
				}else{
					...
					if(player.level<definition.minLevel)
						throw new VMapException(MessageFormat.format(peony.Messages.STRING_00007, definition.minLevel));
...
//					return map;
				...
				throw new VMapException(peony.Messages.STRING_01959);
			}
		}
	}
	...
	public void update(int diff) {
		...
		while(ite.hasNext()){
			...
			if(instance.isTimeOut()||instance.refs.size()==0){
				...
				service.removeFiveElement(instance);
				ite.remove();
