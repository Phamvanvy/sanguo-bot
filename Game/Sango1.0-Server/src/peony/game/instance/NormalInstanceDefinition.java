...
public class NormalInstanceDefinition {
	...
	public void checkEnter(Player p, int oldTimes) throws VMapException {
		if (oldTimes >= maxTimes) {
			throw new VMapException(MessageFormat.format(peony.Messages.STRING_01704, maxTimes));
		}
		if (usePower > 0) {
			try {
				p.decActivePower(usePower);
			} catch (NoEnoughValueException e) {
				throw new VMapException(MessageFormat.format(peony.Messages.STRING_01705, usePower));
			}
		}
	}
}
