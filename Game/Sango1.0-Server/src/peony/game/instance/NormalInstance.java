...
public class NormalInstance implements Instance{
	...
	public void addPlayer(Player player) throws VMapException{
		if(playerCount>=definition.maxPlayer){
			throw new VMapException(peony.Messages.STRING_00006);
		}
	...
	public static void out(VMap map){
	...
	class LeavePartyTransferScheduleAction extends ScheduleAction{
		...
		public String[] timeMessage = {peony.Messages.STRING_00008,peony.Messages.STRING_00009,peony.Messages.STRING_00010,peony.Messages.STRING_00011};
			...
			if(index>=0&&index<=3){
				...
				if(p!=null){
					Server.server.getServiceRegistry().getChatService().sendPrivateMessage(p.id, 
							MessageFormat.format(peony.Messages.STRING_00012, timeMessage[index]));
				}
				return false;
			}
			else if(index==4){
