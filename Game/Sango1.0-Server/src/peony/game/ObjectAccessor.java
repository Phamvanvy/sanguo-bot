...
public class ObjectAccessor {
	 ...
	 * 新套装效果索引表。
	 */
	public static final IntHashMap<SuiteEffects> newSuites = new IntHashMap<SuiteEffects>();
	...
	public static final Map<String, AsyncPlayer> asyncPlayers = new HashMap<String, AsyncPlayer>();
	
	public static final Map<String, Attendant> asyncAttendants = new HashMap<String, Attendant>();
	...
	public static void addGameObject(GameObject unit) {
		...
		instanceid2objects.put(unit.getInstanceId(), unit);
	}
	...
	public static GameObject getAsyncGameObject(int instanceId, int mapInstanceId){
		AsyncPlayer asyncPlayer = asyncPlayers.get(AsyncPlayer.getSearchKey(instanceId, mapInstanceId));
		if(asyncPlayer!=null)
			return asyncPlayer.asyncPlayer;
		return null;
	}
...
//			gm = players.get(ref.id);
		if(ref.battleType==Player.TYPE_ASYNC_PLAYER)
			gm = getAsyncGameObject(ref.instanceId, ref.mapInstanceId);
		return gm;
	}
	...
	public static void removeGameObject(GameObject unit) {
		instanceid2objects.remove(unit.getInstanceId());
		if (unit.type == GameObject.TYPE_PLAYER) {
			...
			public void run(){
				while (Server.server.running) {
					...
					for (Player p:players.values()) {
						try {
							if (p.id % 40 == round) {
								log.info("[AUTOSAVE-Player]ID["+p.id+"]");
								p.recordLogOutHpMp();
	...
	public static Horse createHorse(int itemId, int typeId, int imageId,
			int initLevel, String name, int iconImage, int iconId) {
	 ...
	 * @throws Exception
	 */
	public static ItemTemplate getHorseTemplate(int horseType,int itemId) throws Exception{
	 ...
	 * @return
	 */
	public static GameItem createGameItem2(int id) {
		ItemTemplate template = getItemTemplate(id);
		ItemTemplate newTemplate = new ItemTemplate();
		newTemplate.copyFrom(template);
		return createGameItem(newTemplate, -1);
	}
