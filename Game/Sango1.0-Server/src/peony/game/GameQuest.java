...
import peony.game.quest.SalaryRewardEntry;
...
//			log.debug(String.format("quest:%d triggerAction:%s", quest.getID(),trigger.action));
		...
		for(QuestRewardSet rewardSet:ls){
			...
			for(QuestRewardItem item:items){
				...
				}else if(item.rewardType==QuestRewardItem.REWARD_SALARY){
					branch.addQuestRewardEntry(new SalaryRewardEntry(item.rewardValue));
				}
			}
