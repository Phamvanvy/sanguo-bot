...
public class Attack {
	...
	public static int[] JAPAN_CANATTACK_MAP = {848, 816, 896, 2032}; //日本服可以打架的地图ID
	
	public int attackType; //异步战场用,攻击类型
	public static int ATTACK_TYPE_ASYNC_TARGET = 1; //异步战场攻击类型-目标为机器人
	public static int ATTACK_TYPE_ASYNC_SOURCE = 2; //异步战场攻击类型-攻击者为机器人
	
	public int mapInstanceId; //战斗的地图instanceId
	...
	public Attack(Skill skill, GameObject source, GameObject target,
			int timeOffset, int mapInstanceId) {
		this(skill, source, target, timeOffset, mapInstanceId, true, false);
	}

	public Attack(Skill skill, GameObject source, GameObject target,
			int timeOffset, int mapInstanceId, boolean useBuffs, boolean autoAttack) {
		this.skill = skill;
		...
		if(target instanceof Player && ((Player)target).battleType==Player.TYPE_ASYNC_PLAYER){
			this.attackType = ATTACK_TYPE_ASYNC_TARGET;
		}
	...
	public int update(int diff) {
	    if (targets == null) {
    		...
    		if ((skill.getTargetType() & Skill.TARGET_FLAG_SELF) == 0) {
    			...
    			if(attackType==ATTACK_TYPE_ASYNC_TARGET)
    				o = ObjectAccessor.getAsyncGameObject(targetRef.instanceId, mapInstanceId);
    			if (o == null) {
    				return 3;
    			}
    			...
    			if (Time.tick % 5 == 0) {
        			...
        			if(sourceRef.battleType==Player.TYPE_ASYNC_PLAYER){
        				if(sourceRef.type==GameObject.TYPE_PLAYER)
        					src = ObjectAccessor.getAsyncGameObject(sourceRef.instanceId, sourceRef.mapInstanceId);
        				if(sourceRef.type==GameObject.TYPE_ATTENDANT)
        					src = ObjectAccessor.asyncAttendants.get(AsyncPlayer.getSearchKey(sourceRef.instanceId, sourceRef.mapInstanceId));
        			}
        			if (src==null || !o.inRange(src, skill.getDistance((Unit)src) * 2 + 40)) {
        			    // 目标跑出过远
        			    return 1;
        			}
    			}
	...
	protected void combat(CombatContext context, boolean triggerActiveSkill, boolean triggerPassiveSkill) {
		...
		if(target.type==GameObject.TYPE_PLAYER && (context.damageType==CombatContext.DAMAGE_PHYSICAL 
				|| context.damageType==CombatContext.DAMAGE_MAGIC || context.damageType==CombatContext.DAMAGE_DEBUFF)){
			Player targetPlayer = (Player)target;
			Attendant attendant = targetPlayer.attendant;
			if(attendant!=null){
				CombatContext contextAttendnt = new CombatContext(source, attendant, context.skill, null, 0);
				combat(contextAttendnt, true, true);
			}
		}
		...
		if (context.hited()) {
			...
			if (context.hited()) {
				...
				if(CommonUtil.hit(RND, context.ratio*100, 10000)){
                    if(context.target !=null && context.target.lastEffectTime==0 || Time.currTime-context.target.lastEffectTime>=context.timeDis*1000){
                    	context.isNoDamage = true;
                        context.target.lastEffectTime = Time.currTime;
                    }
                }
				if(context.isNoDamage)
					context.damage = 0;
				
				if(target!=null && context.needMp>0 && !context.isNoDamage){
					int decMp = (target.mp<context.needMp?target.mp : context.needMp);
					target.setMp(target.mp - decMp, true);
				}
...
//                if(context.isNoDamage)
//                	againstContext.isNoDamage = true;
                if(this.attackType==ATTACK_TYPE_ASYNC_TARGET)
                	this.attackType = ATTACK_TYPE_ASYNC_SOURCE;
                againstContext.isAdditive = true;
	...
	public int trigger() {
		...
		if(sourceRef.type==GameObject.TYPE_ATTENDANT && sourceRef.battleType==Player.TYPE_ASYNC_PLAYER)
			source = ObjectAccessor.asyncAttendants.get(AsyncPlayer.getSearchKey(sourceRef.instanceId, sourceRef.mapInstanceId));
		if(attackType==ATTACK_TYPE_ASYNC_SOURCE)
			source = (Unit) ObjectAccessor.getAsyncGameObject(sourceRef.instanceId, mapInstanceId);
		if (source == null || source.isChaosState()) {
		    return 11;
		}
		int checkPrison = checkPrison(source.map.map, source.x, source.y);
		if(checkPrison!=-1)
			return checkPrison;
		...
		if (this.targets == null) {
            ...
            if ((skill.getTargetType() & Skill.TARGET_FLAG_SELF) != 0) {
    			...
    			if(this.attackType==ATTACK_TYPE_ASYNC_TARGET)
    				target = (Unit) ObjectAccessor.getAsyncGameObject(targetRef.instanceId, mapInstanceId);
    		...
    		if ((skill.getTargetType() & Skill.TARGET_FLAG_SINGLE) == 0) {
    			if ((skill.getTargetType() & Skill.TARGET_FLAG_ATTACK) != 0) {
    				...
    				if((skill.getTargetType() & Skill.TARGET_FLAG_SELF) != 0){
    					//以自己为目标
    					targets = bubbleEnemies(source, targets);
    				}else{
    					//以选中目标为目标
    					if(target!=null && target.type!=GameObject.TYPE_PLAYER )
    						targets = bubbleEnemies(source, targets);
    				}
    				target = null;
    			} else {
    		...
    		if (target == null && (targets == null || targets.length == 0)) {
    			...
    			pt.putInt(source.getInstanceId());
    			pt.putInt(source.getInstanceId());
		...
		if (!autoAttack && (source.type == GameObject.TYPE_PLAYER || source.type == GameObject.TYPE_ATTENDANT)) { // 处理CD,如果是自动攻击，那么不应该加上CD
			int cdGroup = skill.getCDGroup();
			int t = skill.getCDTime(source);
			source.setCoolDown(cdGroup, Time.currTime, Time.currTime + t);
		}
		if(source.type==GameObject.TYPE_PLAYER && ((Player)source).battleType==Player.TYPE_ASYNC_PLAYER){
			((PlayerBodyAi)((Player)source).ai).skill = null;
			((PlayerBodyAi)((Player)source).ai).nextSkill = null;
			((PlayerBodyAi)((Player)source).ai).lastAttackTime = Time.currTime;
		}
		return 0;
	}
	
	private Unit[] bubbleEnemies(Unit source, Unit[] targets){
		if(source!=null && source.type==GameObject.TYPE_PLAYER && !source.isPvpFaction()){
			List<Unit> list = new ArrayList<Unit>();
			for(Unit unit : targets){
				if(unit!=null && unit.type==GameObject.TYPE_PLAYER && unit.isPvpFaction())
					continue;
				list.add(unit);
			}
			if(list.size()!=targets.length)
				return list.toArray(targets);
		}
		return targets;
	}
	
	private int checkPrison(VMap map, int x, int y) {
		if(!map.mapDef.mapInfo.getPathFinder().canReach(x, y))
			return 11;
		else 
			return -1;
	}
	...
	public static boolean canAttack(int mapId, String revision){
		if(revision.equalsIgnoreCase(Server.REVISION_TYPE_JAPAN)){
			boolean can = false;
			for(int id : JAPAN_CANATTACK_MAP){
				if(id==mapId)
					can = true;
			}
			return can;
