				...
				for(final Clock clock : clocks){
					...
					Server.server.scheduExec.scheduleAtFixedRate(new Runnable(){
						public void run() {
							...
							.sendWorldMessage(peony.Messages.STRING_01824);
						}
...
//							refreshNpc(mapId, npcs, positions);
							RefreshNpcCall call = new RefreshNpcCall(RefreshNpcCall.NPCSERVICE);
							call.mapId = mapId;
							call.npcIds = new ArrayList<Integer>();
							call.npcIds = npcs;
							call.position = new ArrayList<Position>();
							call.position = positions;
							Server.server.getWorld().schedule(call);
						}
					});
					if(cal.get(Calendar.HOUR_OF_DAY)!=11 && cal.get(Calendar.HOUR_OF_DAY)!=16){
	...
	public void refreshNpc(int mapId, List<Integer> npcs, List<Position> positions) {
		...
		chatService.sendWorldMessage(peony.Messages.STRING_01825);
	}
