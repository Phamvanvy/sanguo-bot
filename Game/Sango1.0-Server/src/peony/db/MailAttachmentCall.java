						...
						if (attachment instanceof ItemMailAttachment) {
							...
							if (m.getPrice() > 0 && p.getMoney() < m.getPrice()) {
								...
								if((att.getGameItem().template.id==CandidateService.KING_WEAL_BAG || att.getGameItem().template.id==CandidateService.KING_TOKEN
										...
										&& p.isKing()!=1){
									error(null, "您不是国公，不能提取此信件！");
//									p.message(-1, "您不是国公，不能提取此邮件！", -1, -1);
									addToClientSession();
									return;
								}
								...
								try {
									...
									for(GainItem gi : gain.getGainItems()){
										if(gi!=null){
											...
											Server.server.getServiceRegistry().getCardService().processHistoryCards(p, item, gi.getCount());
										}
									}
									if (m.getPrice() > 0) {
						...
						error(null, peony.Messages.STRING_01362);
					}
				} else {
					error(null, peony.Messages.STRING_01273);
				}
			}
			addToClientSession();
		}
