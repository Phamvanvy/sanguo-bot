...
public class DeletePlayerCall extends ClientSessionAsyncCall {
	...
	public static int DELTE_LIMIT_LEVEL = 40; //允许删号的最高级别
			...
			if (player == null) {
				...
				if (player.exist != 1) {
					...
					if(player.level>=DELTE_LIMIT_LEVEL){
						error(null, MessageFormat.format(peony.Messages.STRING_01348, DELTE_LIMIT_LEVEL));
						addToClientSession();
						return;
					}
					//如果是国王不能删除账号
					...
					if(nationService.isKing(player)){
						error(null, peony.Messages.STRING_01349);
						addToClientSession();
						return;
					}
					...
					for(Officer officer : nation.getOfficers()){
						if(officer!=null && officer.actor.id==player.id){
							error(null, MessageFormat.format(peony.Messages.STRING_01350, officer.getName()));
							addToClientSession();
							return;
						}
					}
					...
					if(association!=null){
						error(null, peony.Messages.STRING_01351);
						addToClientSession();
						return;
					}
					...
					if (tong == null) {
						...
						//解除师徒关系
						Server.server.getServiceRegistry().getApprenticeService().playerDeleteRemove(player);
					} else {
