...
public class IBuyDAO extends GenericHibernateDAO<IBuy, Integer> {
	 ...
	 * @return 单位为1/100分，1/1000元宝
	 */
	public long getConsumeCount(int accountId, Date endTime) {
		try {
			String hql = "select sum(imoney) from IBuy where accountid = ? and buytime <= ?";
			return (Long) uniqueResult(hql, accountId, endTime);
		} catch (Exception e) {
			return 0;
		}
	}
		...
		@SuppressWarnings("unchecked")
		public List<IBuy> getIbuyAfter(int accountId,Date date){
			return list("from IBuy b where b.accountid=? and b.buytime<=?", accountId,date);
		}
}
