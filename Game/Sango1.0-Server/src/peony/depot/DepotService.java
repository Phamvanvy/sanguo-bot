...
public class DepotService implements Service {
	...
	public void turnOn(Player p, int serial) throws DepotException {
		if (p != null) {
			if (p.depot != null && p.depot.getGrids().size() > 0) {
				throw new DepotException(peony.Messages.STRING_01833);
			}
			int playerLevel = p.level;
			if (playerLevel < 30) {
				throw new DepotException(peony.Messages.STRING_01834);
			}
	...
	public void getItemFromDepotToBag(Player p, ClientSession session, int serial,  
		...
		if (p != null) {
			if(p.depot==null || p.depot.getSize()==0){
				throw new DepotException(peony.Messages.STRING_01835);
			}
			...
			try {
				...
				tx.rollback();
				throw new DepotException(peony.Messages.STRING_01456);
			} catch (NoEnoughValueException e){
				tx.rollback();
				throw new DepotException(peony.Messages.STRING_01836);
			}
			// 记录日志
			try {
	...
	public void getItemFromBagToDepot(Player p, ClientSession session, int serial, 
			int gridId, int itemId, int instanceId, int count) throws DepotException {
		if (p != null) {
			CandidateService service = Server.server.getServiceRegistry().getCandidateService();
			if(service.isKingEquipment(itemId) || itemId==CandidateService.KING_TOKEN || itemId==CandidateService.KING_WEAL_BAG)
				throw new DepotException(peony.Messages.STRING_01837);
			if(p.depot==null || p.depot.getSize()==0){
				throw new DepotException(peony.Messages.STRING_01835);
			}
			...
			if(bagGrid!=null){
				...
				try {
					...
					tx.rollback();
					throw new DepotException(peony.Messages.STRING_01838);
				}
				// 记录日志
				...
				if(map.get(p.id) != null && !map.get(p.id)){
					...
					pt.putInt(0);
					pt.putString(peony.Messages.STRING_01839);
					pt.putInt(0);
					pt.putInt(0);
					p.send(pt);
					map.remove(p.id);
	...
	public void exchangeGrid(Player p, ClientSession session, int serial, int grid1, int grid2) throws DepotException {
		if (p != null) {
			...
			if (ret) {
				...
				throw new DepotException(peony.Messages.STRING_01598);
			}
		}
	}
	...
	public void splitItem(Player p, ClientSession session, int serial, int gridId, int itemId, int count) throws DepotException {
		...
		try {
			...
			throw new DepotException(peony.Messages.STRING_01840);
		} catch (Exception e) {
			throw new DepotException(peony.Messages.STRING_00314);
		}
	}
	...
	public void getExtendDepotPrice(Player player, ClientSession session, int serial) {
		if (player.depot == null || player.depot.getGrids().size() == 0) {
			ErrorHandler.sendErrorMessage(session, serial, OpCode.EXTEND_DEPOT_PRICE_CLIENT, peony.Messages.STRING_01835);
			return;
		}
	...
	public void extendDepot(Player player, ClientSession session, int serial) {
		if (player.depot == null || player.depot.getGrids().size() == 0) {
			ErrorHandler.sendErrorMessage(session, serial, OpCode.EXTEND_DEPOT_CLIENT, peony.Messages.STRING_01835);
			return;
		}
		if (player.depot.isSizeLocked()) {
			ErrorHandler.sendErrorMessage(session, serial, OpCode.EXTEND_DEPOT_CLIENT, peony.Messages.STRING_00136);
			return;
		}
	...
	public void extendTurnOn(Player p, int count) throws DepotException {
		if (p != null) {
			int playerLevel = p.level;
			if (playerLevel < 30) {
				throw new DepotException(peony.Messages.STRING_01834);
			}
			p.depot = new TransactionBag(p, count, 0);
			Packet pt = new Packet(OpCode.DEPOT_REQUEST_SERVER);
			pt.putInt(-1);
			pt.put(p.depot.getSize());
			for (TransactionBagGrid grid : p.depot.getGrids()) {
			...
			p.send(pt);
			Server.server.getServiceRegistry().getDepotService().setDepotMap(p, false);
			// 记录玩家动作
			p.addAction(Action.OPEN_DEPOT);
		}
	}
	
	public void setDepotMap(Player p,boolean state){
		map.put(p.id, state);
	}
