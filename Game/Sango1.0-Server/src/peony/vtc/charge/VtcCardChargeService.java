...
public class VtcCardChargeService implements Service {
	...
	public void vtccard(String CARDID, String CARDCODE, Player player, AccountAsyncCall call) throws VtcCardChargeException {
		if(player!=null){
			try{
				...
				if(status>0){
					addBalance(player.accountId, getimoney(vtc_price), vtc_price, call, CARDCODE);
	...
	protected void addBalance(int accountId, int imoney, int money, AccountAsyncCall call, String cardNum) {
		...
		Server.server.getServiceRegistry().getAccountService().sendAndRegister(new AddBalanceMessage(accountId,imoney,"vtccard_"+ cardNum, "", money),call);
	}
