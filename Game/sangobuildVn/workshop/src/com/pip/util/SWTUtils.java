...
public class SWTUtils {
    ...
    public static Rectangle[] getBestLayout(Rectangle[] frames) {
        ...
        // 全部分在一个图片里面试一次
        Rectangle bounds = getBestLayoutImpl(frames, 0, frames.length);
        if (bounds.width * bounds.height <= totalArea * 102 / 100) {
            // 如果差别在2%之内，直接返回
        ...
        for (int splitType = 0; splitType <= 4; splitType++) {
            ...
            for (int i = 1; i < sortFrames.length; i++) {
                Rectangle b1 = getBestLayoutImpl(sortFrames, 0, i);
                Rectangle b2 = getBestLayoutImpl(sortFrames, i, sortFrames.length - i);
        ...
        if (bestType == -1) {
            // 还是最原始的方式好
            retBounds = new Rectangle[] { getBestLayoutImpl(frames, 0, frames.length) };
        } else {
            // 按找出来的最佳方式重算
            Rectangle[] sortFrames = new Rectangle[frames.length];
            System.arraycopy(frames, 0, sortFrames, 0, frames.length);
            sortAreas(sortFrames, bestType);
            Rectangle b1 = getBestLayoutImpl(sortFrames, 0, bestSplit);
            Rectangle b2 = getBestLayoutImpl(sortFrames, bestSplit, sortFrames.length - bestSplit);
     ...
     * @return 返回结果的宽度和高度。
     */
    public static Rectangle getBestLayoutImpl(Rectangle[] frames, int start, int count) {
        // 计算最小图片宽度，最大图片宽度
        ...
        for (int w = maxw; w <= totalw; w += minw) {
