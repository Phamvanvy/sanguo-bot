	...
	protected void paintInput(GC gc) {
		...
		if (input != null) {
			...
			if(areaCnt != 1){
				...
				PipAnimateFrame fr = animate.getParent().getFrame(0);
				Rectangle frBds = fr.getBounds();
				...
				if(focusIndex >= 0){
					...
					int diffX = frBds.x + drawX - frBds.width - 4;
					int diffY = drawY+frBds.y;
					Rectangle newClip = new Rectangle(diffX, diffY, focusRect.width, focusRect.height);
					gc.setBackground(SWTResourceManager.getColor(SWT.COLOR_BLACK));
					
					Rectangle clip = gc.getClipping();
					gc.setClipping(newClip);
					fr.draw(gc, drawX - frBds.width - 4 - focusRect.x, drawY - focusRect.y, ratio);
