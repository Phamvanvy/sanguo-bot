...
public class WeddingService implements VMapManager, Service {
	...
	public String[] slang = { "白头偕老，永结同心。", "早生贵子，百年好合。", "天作之合，鸾凤和鸣。","比翼齐飞，连理共生。" };
...
//				int money = (2 * level - 1) * 1000000;
		...
		if (p != null) {
			if (p.map.map.instance != null && p.map.map.instance instanceof WeddingInstance) {
				...
				if (p.id == instance.man.id || p.id == instance.woman.id) {
					ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_SIGNIN_CLIENT, "新郎和新娘无需签到");
					return;
				}
				if (instance.signIns.contains(actor)) {
					ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_SIGNIN_CLIENT, "您已经签过到了");
					return;
				}
				...
				String message = "祝你们" + slang[index];
	...
	public void sendGift(Player p, int targetId, int itemId, int count)
		...
		if (p != null) {
			...
			if (target != null) {
				...
				if (item == null) {
					tx.rollback();
					throw new MarriageException("没有物品或者物品数量不足");
				}
				...
				try {
					...
					tx.commit();
					tx1.commit();
					target.message(-1, MessageFormat.format("你收到了{0}的赠送", p.name), -1, -1);
				} catch (NoEnoughSpaceException e) {
					tx.rollback();
					tx1.rollback();
					throw new MarriageException("对方背包已满");
				}
			}
		}
	}
	...
	public void kickGuest(Packet packet, ClientSession session) {
		...
		if (p != null) {
			...
			if (p.id != instance.man.id) {
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_KICK_CLINT, "只有新郎才可以踢除宾客");
				return;
			}
			Player targetPlayer = ObjectAccessor.getPlayer(targetId);
			if(targetPlayer==null){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_KICK_CLINT, "Đối phương không trên mạng");
				return;
			}
			if (targetPlayer.map.map.instance==null || (targetPlayer.map.map.instance!=null && targetPlayer.map.map.instance.getId()!=instance.id)) {
				ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_KICK_CLINT, "对方不在同个地图上");
				return;
			}
			if (targetPlayer != null) {
				try {
	...
	public void getWeddingExp(Packet packet, ClientSession session) {
		...
		if (p != null) {
			...
			if (instance != null) {
				if (p.id == instance.man.id || p.id == instance.woman.id) {
					ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_GETEXP_CLIENT, "新郎和新娘不能领取经验");
					return;
				}
				Actor actor = Server.server.getServiceRegistry().getActorCacheService().find(p.id);
				if (!instance.signIns.contains(actor)) {
					ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_GETEXP_CLIENT, "请您先到婚礼司仪处签到");
					return;
				}
				if (instance.deskgift.contains(p.id)) {
					ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_GETEXP_CLIENT, "您已经吃过酒席，别太贪心哦");
					return;
				}
				if (instance.fetchCount >= instance.MAXCOUNT[instance.level]) {
					ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_GETEXP_CLIENT, "对不起，酒席已空");
					return;
				}
				if (instance.playerentertime.get(p.id) > (System.currentTimeMillis()-STAYTIME)) {
					ErrorHandler.sendErrorMessage(session, serial, OpCode.WEDDING_GETEXP_CLIENT, "签到后五分钟才可以领取经验");
					return;
				}
				PlayerTransaction tx = p.newTransaction("WEDDING");
	...
	public VMap addToMap(Player player, int mapId, int x, int y, boolean check)
		...
		if (check) {
			...
			WeddingInstance instance = getInstance(player.id);
			if (instance == null) {
				throw new VMapException("婚礼已经结束");
			} else {
				return in(player, instance);
			}
		}
