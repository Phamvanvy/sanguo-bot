	...
	public void callFinish() throws Exception {
		if(success){
			...
			if(player != null){
				...
				packet.putString(MessageFormat.format("{0}thân mến: bạn đã gửi đơn xin lí hôn với vợ/chồng {1} theo phán quyết, xin lí hôn đã được phê chuẩn.", service.find(player.id).name,service.find(annotherPersonId).name));
				player.send(packet);
				player.send(packet0);
				MailService mailService = Server.server.getServiceRegistry().getMailService();
				mailService.sendSystemMail(player.id, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "您已离婚", MessageFormat.format("Lời tuyên bố hôn nhân của bạn và {0}rạn nứt", service.find(annotherPersonId).name), 0, null, 0, "");
				if(ObjectAccessor.getPlayer(annotherPersonId) != null){
					Packet packet1 = new Packet(OpCode.MARRIAGE_DIVORCE_SERVER);
					packet1.putString(MessageFormat.format("{0} đáng kính, bạn tình của bạn {1} đã xin gỡ bỏ quan hệ hôn nhân, phán định cuối cùng, đơn xin li hôn được phê chuẩn.", service.find(annotherPersonId).name,player.name));
...
//					mailService.sendSystemMail(annotherPersonId, "系统", "您已离婚", "您的伴侣"+service.find(player.id).name+"提出申请和您解除婚姻关系，经判定，离婚申请获得批准", 0, null, 0);
					mailService.sendSystemMail(annotherPersonId, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "您已离婚", MessageFormat.format("Lời tuyên bố hôn nhân của bạn và {0}rạn nứt", service.find(player.id).name), 0, null, 0, "");
				}else if(type == 0){
					mailService.sendSystemMail(annotherPersonId, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "您已离婚", MessageFormat.format("Lời tuyên bố hôn nhân của bạn và {0}rạn nứt", service.find(player.id).name), 0, null, 0, "");
				}
				chatService.addChatMessage(new ChatMessage(ChatOption.FACTION,-1,player.faction,"<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>",player.faction,MessageFormat.format("{0} và{1} đã li hôn", player.name,service.find(annotherPersonId).name),null));
