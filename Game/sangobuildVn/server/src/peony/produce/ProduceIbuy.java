				...
				try {
					...
					if(movePoint > 0){
						try {
							...
							ErrorHandler.sendErrorMessage(session, serial, OpCode.PRODUCE_CLIENT,"Lực hành động không đủ");
							throw new ShopException("Lực hành động không đủ"); 
						}
					}
				} catch (NoEnoughSpaceException e) {
//					tx.rollback();
					ErrorHandler.sendErrorMessage(session, serial, OpCode.PRODUCE_CLIENT, "Hành trang của bạn đã đầy");
					throw new ShopException("Túi đồ đã đầy");
				}
