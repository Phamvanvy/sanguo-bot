...
//	顶级星级鉴定符	0%	0%	0%	0%	0%	0%	0%	0%	100%
	 ...
	 * @return
	 */
	public static int startEnhance(GameItem item,int type){
	...
	protected static void modifyDescriptions(ProjectData data) {
		for (ItemTemplate template : ObjectAccessor.itemTemplates.values()) {
			if (template.useType.effect instanceof AddFormulaEffect) {
				...
				sb.append("\n");
				sb.append("<c0>{x}所需熟练度:\n<c0>{x}Cần luyện tập đến độ :").append(ProduceService.getPracticeByLevel(f.level)).append("\n");
				sb.append("<c0>{x}级别:\n<c0>{x}Cấp biệt:").append(ProduceService.getLevelToString(f.level)).append("\n");
				sb.append("<c0>{x}消耗行动力:</c><c0000ff>{x}\n<c0>{x}tiêu hao lực hành động:</c><c0000ff>{x}").append(f.movePoint).append("\n");
				for(BuyRequirement br:f.requirements){
					sb.append(br.toString()).append("\n");
				}
				if(f.productType==Formula.PRODUCT_DROPGROUP){
					sb.append("产出:").append(ObjectAccessor.getItemTemplate(f.itemID).name).append("<c008242>随机属性</c>\n<c008242> Thuộc tính ngẫu nhiên</c>");
				}
				...
				if (f.minAmount != 1 || f.maxAmount != 1) {
					sb.append("\n数量: (1~\nSố lượng: (1~").append(f.minAmount).append("~").append(f.maxAmount);
				}
...
//			} 
				...
				DecImoneyEffect e = new DecImoneyEffect(effect.getIntParam("value"));
				ret.add(e);
