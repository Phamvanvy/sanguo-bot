...
public class NationConvoyService implements Service {
	...
	public void success(NationConvoy convoy) {
		...
		for(Player p:l){
			...
			mailService.sendSystemMailAsync(p.id, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "国家押送奖励", "", 0, item1, 2, "COV");
			mailService.sendSystemMailAsync(p.id, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "国家押送奖励", "", 0, item2, 1, "COV");
		}
		if (l.size() > 0) {
	...
	public void fail(NationConvoy convoy){
		...
		for(Player p:l){
			...
			GameItem item1 = ObjectAccessor.createGameItem(1311);
			GameItem item2 = ObjectAccessor.createGameItem(1683);
			mailService.sendSystemMailAsync(p.id, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "劫取国家押送奖励", "", 0, item1, 1, "COV");
			mailService.sendSystemMailAsync(p.id, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "劫取国家押送奖励", "", 0, item2, 1, "COV");
		}
		if (l.size() > 0) {
			int v = DEPOSITE / l.size();
			for (Player p : l) {
