...
public class Association {
	...
	public Association(Player p, String name) throws AssociationException{
		if(p!=null){
			if(StringUtil.isValidText(name)!=IStringValidator.OK || StringUtil.hasBadWord(name))
				throw new AssociationException("Danh hiệu của huyết minh rất quan trọng, sử dụng từ ngữ khiếm nhã có thể ảnh hưởng đến uy danh của tráng sĩ, xin chọn một danh hiệu đẹp đẽ khác");
			this.name = name;
	...
	public void addMember(int playerId, int duty, int state) throws AssociationException{
		if(isFull(playerId))
			throw new AssociationException("Huynh đệ quý ở nghĩa tình không phải số lượng, hiện nay số lượng huynh đệ trong huyết minh đã đạt đến giới hạn");
		...
		if(getMember(playerId)!=null && getMember(playerId).state==AssociationMember.STAT_WORK)
			throw new AssociationException("Bạn hiện tại đang ở trong huyết minh, không thể tham gia lần nữa.");
			...
			if(state==AssociationMember.STAT_WAIT && inviteTime+OUTTIME<System.currentTimeMillis()){
				...
				try {
					...
					if(player!=null){
						...
						player.message(-1, "Tráng sĩ, bạn trong 24h không hoàn thành nhiệm vụ kết nghĩa, không thể cắt máu ăn thề cùng các huynh đệ.", -1, -1);
					}
