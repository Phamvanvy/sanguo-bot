...
//			}
	...
	public static void getPlayerSnapshot(StringBuilder out, Player player) {
		...
		if (acc == null) {
			...
			out.append(")IME(").append(acc.getIMoney());
		}
	...
	/** 记录元宝支付购买结果 */
	public static void logIMoneyBuyOK(int pid, int accid, IBuyObject buyObject, boolean succ, int balance, int cost, int level) {
		StringBuilder sb = new StringBuilder(100);
	...
	/** 记录玩家发送聊天信息 */
	public static void logChat(Player p, int channel, int destId, String message) {
		StringBuilder sb = new StringBuilder(100);
		sb.append("[CHAT]");
		LogUtil.getPlayerLogString(sb, p);
	...
	public static void logUseItem(Player p, GameItem item, boolean consume) {
		...
		if(Server.server.revision.equals("CMCC") && p.getAccount()!=null && p.getAccount().getCmccUserId()!=null && !p.getAccount().getCmccUserId().equals("") && item.template.id == 2261){
			sb.append("CMCCUSERID["+p.getAccount().getCmccUserId()+"]");
