		...
		if(u1.type==GameObject.TYPE_CREATURE){
			...
			if(faction!=0){
				...
				if(ref.type==GameObject.TYPE_PLAYER){
					...
					String msg = MessageFormat.format("{0}国王{1}被{2}卑鄙的杀害，举国陷入悲痛", c.getFactionName(),c.name.substring(0,3),GameObject.getFactionName(actor.faction));
					...
					try {
						...
						if(pvpInfo.pool.getString(statService.getPropertyOfKillKing(index)) == ""){
							...
							String msg2 = MessageFormat.format("Chúc mừng anh hùng <cff0000>{0}</c> đạt được thành tựu <cff0000>{1}</c>", actor.name,achName[faction]);
