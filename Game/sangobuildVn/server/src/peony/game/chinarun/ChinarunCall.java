...
public class ChinarunCall extends ClientSessionAsyncCall {
	...
	public void run() {
		if (playerName.length() != 0) {
			...
			if (actor == null) {
				message = "Không tìm thấy mục tiêu nhân vật ";
				addToClientSession();
				return;
			}
	...
	protected void chinarun(){
		...
		if (code == 200) {
			...
			if ("200".equals(header.getValue())) {
				message = "Lệnh xin nạp thẻ của bạn đã giử, vui lòng chờ.";
			} else {
				message = "Giao dịch nạp thẻ của bạn thất bại, xin vui lòng đối chiếu dãy số và mật mã thực Hiện đăng nhập lần nữa";
			}
		} else {
			message = "Giao dịch nạp thẻ của bạn thất bại, xin vui lòng đối chiếu dãy số và mật mã thực Hiện đăng nhập lần nữa";
		}
