...
public class AdminRemoveItemCall extends ClientSessionAsyncCall {
	...
	public void callFinish() throws Exception {
		if (p != null) {
			...
			ErrorHandler.sendAdminErrorMessage(session, serial,
					OpCode.ADMIN_REMOVE_ITEM_CLIENT, "Không tìm thấy vật phẩm chỉ định");
		} else {
			ErrorHandler.sendAdminErrorMessage(session, serial,
					OpCode.ADMIN_REMOVE_ITEM_CLIENT, "Không tìm thấy nhân vật chỉ định");
		}
	}
	...
	public void run() {
		...
		if(actor==null){
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_REMOVE_ITEM_CLIENT, "Không tìm thấy nhân vật chỉ định");
			return;
		}
