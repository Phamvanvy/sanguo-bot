		...
		if (actor != null) {
			if(count<=0){
				ErrorHandler.sendAdminErrorMessage(session, serial,
						OpCode.ADMIN_SENDMAIL_CLIENT, "Số lượng vật phẩm sai, không thể là:"+count);
				return;
			}
				
			if (itemId == -1) {
				Server.server.getServiceRegistry().getMailService()
						.sendSystemMail(playerId, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", title, content, count,
								null, 0, "GM");
			} else {
				...
				if (item != null) {
					...
					if (enhanceOk) {
						...
						ErrorHandler.sendAdminErrorMessage(session, serial,
								OpCode.ADMIN_SENDMAIL_CLIENT, "Dữ liệu sai");
					}
				} else {
					ErrorHandler.sendAdminErrorMessage(session, serial,
							OpCode.ADMIN_SENDMAIL_CLIENT, "Không tìm được vật phẩm chỉ định");
				}
			}
		} else {
			ErrorHandler.sendAdminErrorMessage(session, serial,
					OpCode.ADMIN_SENDMAIL_CLIENT, "Người sử dụng đã rời mạng");
		}
	}
