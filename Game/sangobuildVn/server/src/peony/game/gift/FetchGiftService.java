...
public class FetchGiftService implements Service {
	...
	public void checkRuleAndSendGift(Player p, int type, int subtype) throws Exception{
		if(p!=null){
			...
			if(f!=null){
				if(f.enable == 0){
					throw new Exception("Tạm thời chưa mở, xin vui lòng đợi");
				}
				if(p.level > f.levelLimit){
					sendGift(p,f,subtype);
				} else {
					String msg = MessageFormat.format("Đạt đến cấp {0} mới có thể nhận",f.levelLimit);
					throw new Exception(msg);
				}
			} else {
				throw new Exception("Dữ liệu sai");
			}
		}
	}
	...
	public void sendGift(Player p,FetchGiftRule f,int subtype) throws Exception{
		if(p!=null){
			if(f.fetchLimit == TYPE_ONCE_INACTIVITY && p.pool.getInt(getProperty(f.giftType,subtype), 0)!=0){
				throw new Exception("Chỉ có thể nhận một lần, bạn đã nhận rồi");
			} else if(f.fetchLimit == TYPE_EVERYDAY_INACTIVITY && p.pool.getInt(getProperty(f.giftType,subtype), 0)==Time.day){
				throw new Exception("Mỗi ngày chỉ có thể nhận một lần, hôm nay bạn đã nhận rồi");
			} else {
				if(checkRule(p,f,subtype)){
					...
					try {
						...
						tx.rollback();
						String content = MessageFormat.format("Nhận {0} thành công",gift.typeName);
						Server.server.getServiceRegistry().getMailService()
								.sendSystemMail(p.id, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "Nhận phần thưởng", content, 0,
										rewardItem, 1, "FETCHGIFT");
					}
					p.pool.setInt(getProperty(f.giftType,subtype), Time.day);
				} else {
					GiftType gift = f.getGifById(subtype);
					String msg = MessageFormat.format("Bạn chưa đạt được điều kiện nhận {0}, tạm thời không thể nhận",gift.typeName);
					throw new Exception(msg);
				}
			}
		}
