...
//			addedBuffs.add(buff);
	 ...
	 */
	public List<Buff> addBuffs(Buffs buffs) {
	...
	public void removeUpdatableBuffs(){
		Iterator<Buff> ite = buffs.iterator();
		while(ite.hasNext()){
			Buff buff = ite.next();
	 ...
	 * @param ref
	 */
	public void removeDebuffs(GameObjectRef ref){
		Iterator<Buff> ite = buffs.iterator();
		while(ite.hasNext()){
			Buff buff = ite.next();
	 ...
	 * @param removeAll
	 */
	public void dispelGoodBuff(boolean removeAll) {
	    int count = buffs.size();
        for (int i = count - 1; i >= 0; i--) {
     ...
     * @param removeAll
     */
    public void dispelBadBuff(boolean removeAll) {
        int count = buffs.size();
        for (int i = count - 1; i >= 0; i--) {
            Buff buff = buffs.get(i);
            if (buff instanceof Updatable && !buff.isGood() && buff.dispelable()) {
...
//		int logoutTime = unit.lastLogoutTime==null?(Time.currTime-3600*1000):Time.elapseTime(unit.lastLogoutTime.getTime());
		...
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				dis.read(bs);
				Buff buff = BuffUtil.createBuff(buffId, 1, unit, unit, 0);
				if (buff != null) {
