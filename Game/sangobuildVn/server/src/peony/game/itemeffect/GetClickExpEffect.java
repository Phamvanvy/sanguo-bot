...
//		}
		if(System.currentTimeMillis()-lastUseTime>=CD && totalOnlineTime>=ONLINE_TIME){
			...
			if(lastUseTime==0 || (System.currentTimeMillis()-lastUseTime>=CD && totalOnlineTime<ONLINE_TIME)){
				...
				min = min%60;
				if(hour>0 && min==0)
					p.message(-1, MessageFormat.format("还需要{0}小时才能继续使用", hour), -1, -1);
				else if(hour>0)
					p.message(-1, MessageFormat.format("还需要{0}小时{1}分钟才能继续使用", hour, min), -1, -1);
				else
					p.message(-1, MessageFormat.format("还需要{0}分钟才能继续使用", min), -1, -1);
			}
		}
	}
