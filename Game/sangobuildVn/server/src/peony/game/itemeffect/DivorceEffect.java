...
public class DivorceEffect implements ItemEffect {
	public void use(Unit source, GameItem item, Unit target, PlayerTransaction tx) throws UseItemException {
		...
		if(annotherPersonId == -1){
			throw new UseItemException("Bạn vẫn là thân đơn bóng chiếc, tại sao lại có thể hủy bỏ hôn ước ");
		}
		...
		mailService.sendSystemMail(p.id, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "您已离婚", MessageFormat.format("Lời tuyên bố hôn nhân của bạn và {0}rạn nứt", service.find(annotherPersonId).name), 0, null, 0, "");
		mailService.sendSystemMail(annotherPersonId, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "您已离婚", MessageFormat.format("Lời tuyên bố hôn nhân của bạn và {0}rạn nứt", service.find(p.id).name), 0, null, 0, "");
		ObjectAccessor.getPlayer(p.id).send(packet0);
		chatService.addChatMessage(new ChatMessage(ChatOption.FACTION,-1,p.faction,"<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>",p.faction,
				MessageFormat.format("{0} và{1} đã li hôn", p.name,service.find(annotherPersonId).name),null));
		if(ObjectAccessor.getPlayer(annotherPersonId) != null){
			Packet packet1 = new Packet(OpCode.MARRIAGE_DIVORCE_SERVER);
			packet1.putString(MessageFormat.format("{0} đáng kính, bạn tình của bạn {1} đã xin gỡ bỏ quan hệ hôn nhân, phán định cuối cùng, đơn xin li hôn được phê chuẩn.", service.find(annotherPersonId).name,p.name));
