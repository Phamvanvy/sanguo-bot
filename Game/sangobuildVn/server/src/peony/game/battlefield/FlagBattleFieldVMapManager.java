...
//	protected IntRange[] levelRanges;
		...
		if(instance!=null){
			...
			if(instanceTran){
				if(instance.state==FlagBattleFieldInstance.STATE_INIT){
					...
					String s = null;
					if(min!=0){
						s = MessageFormat.format("战场将在{0}分钟以后开启",min);
					}else{
						int sec = t/(1000);
						s = MessageFormat.format("战场将在{0}秒后开启", sec);
					}
			...
			throw new VMapException("战场已经不存在");
		}else{
			int[] out = outs[player.faction-1];
			return world.addPlayerToMap(player, out[0], out[1],
					out[2], false);
	...
	public void signup(Player player,int serial){
		...
		if(!canSignup()){
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, "还没到报名时间");
			return;
		}
		if(duelService.signUps.contains(player.id)){
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, "您已报名比武");
			return;
		}
		if (signups.containsKey(player.id)){
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, "已经报名");
			return;
		}
		...
		if  (playerid2forbid.containsKey(player.id)){
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, "你稍后才可以再次报名");
			return;
		}
		if (player.getVMap().instance!=null){
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, "状态错误，不能报名");
			return;
		}
		int levelIndex = getLevelIndex(player.level);
		if(levelIndex==-1){
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, "等级不够");
			return;
		}
		...
		try {
			...
			tx.rollback();
			ErrorHandler.sendErrorMessage(player.session, serial, OpCode.BATTLEFIELD_SIGNUP_CLIENT, MessageFormat.format("需要{0}钱才能报名",money));
			return;
		}
			...
			for (int i = 0; i < triggers.length; i++) {
				if (triggers[i].in(hour, min)) {
					...
					Server.server.getServiceRegistry().getChatService()
							.sendSystemMessage(ChatOption.SYSTEM, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>",
									"南郡之战开放,可以到国都战场管理员报名参加");
					break;
				}
			}
		}
...
class StartNotify implements Runnable{
	...
	public void run(){
		Server.server.getServiceRegistry().getChatService().sendSystemMessage(ChatOption.SYSTEM, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", message);
	}
}
