...
public class AddJewelCall extends ClientSessionAsyncCall {
	...
	protected void go() throws Exception {
	    ...
	    if (itemEnh.getJewel(hole) != -1) {
	        throw new Exception("Trên chiếc khảm lỗ này đã có bảo thạch rồi, xin vui lòng lấy xuống rồi khảm nạm");
	    }
	    if (itemEnh.findJewelByType(jewelTemp.jewelType)) {
	        throw new Exception("Trên một bộ trang bị đồng một loại bảo thạch chỉ có thể khảm nạm một cái");
	    }
	    if (jewelTemp.isHorseJewel && !gi.template.isHorseEquipment()) {
	        throw new Exception("Bảo thạch này chỉ có thể khảm nạm lên trang bị của thú cưỡi");
	    }
	    ...
	    if ((jewel = player.bag.removeGameItem(jewelID, GameItem.GENERAL_INSTANCEID, 1, tx, true)) == null) {
	        tx.rollback();
	        throw new Exception("bạn không có bảo thạch này");
	    }
	    this.jewel = jewel;
	    ...
	    if (method == 1) {
	        ...
	        if (player.bag.removeGameItem(js.decorateItemID1, GameItem.GENERAL_INSTANCEID, 1, tx, true) == null) {
	            tx.rollback();
	            throw new Exception(MessageFormat.format("<cff0000>缺少</c>\nMột phong thư dán có vết máu ", js.deocrateItem1.name));
	        }
	    } else if (method == 2) {
            ...
            if (player.bag.removeGameItem(js.decorateItemID2, GameItem.GENERAL_INSTANCEID, 1, tx, true) == null) {
                tx.rollback();
                throw new Exception(MessageFormat.format("<cff0000>缺少</c>\nMột phong thư dán có vết máu ", js.deocrateItem2.name));
            }
	    }
	    tx.commit();
	    ...
	    if (rand.nextInt(100) > rate) {
	        ...
	        throw new Exception("Thật đáng tiếc, khảm nạm thất bại ");
	    }
        ...
        if(jewel.template.useLevel==5){
        	...
        	String s = MessageFormat.format("{0} đem 1 viên {1} hoàn mỹ vô khuyết khảm nạm thành công lên {2}!", player.name,jewel.template.name,gi.template.name);
        	Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(player.faction, s);
        }
        else if(jewel.template.useLevel==7){
        	String s = MessageFormat.format("{1} của {0} đã khảm nạm thành công 1 viên tuyệt thế thần thạch {2} lên {3}!", player.getFactionName(),player.name,jewel.template.name,gi.template.name);
