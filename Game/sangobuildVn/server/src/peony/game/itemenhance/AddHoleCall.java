...
public class AddHoleCall extends ClientSessionAsyncCall {
	...
	protected void go() throws Exception {
		...
		if (l.size() == 0) {
			tx.rollback();
			throw new Exception("没有找到对应的打孔符");
		}
		boolean ok = false;
		...
		if (ok) {
			...
			if (rand.nextInt(10000) > rate) {
				...
				throw new Exception("Rất lấy làm tiếc, đục lỗ thất bại rồi");
			}
			...
			throw new Exception(MessageFormat.format("缺少{0}以上等级的打孔符", l.get(0).name));
		}
	}
