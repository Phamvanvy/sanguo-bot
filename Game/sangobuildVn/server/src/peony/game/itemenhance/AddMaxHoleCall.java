...
public class AddMaxHoleCall extends ClientSessionAsyncCall {
	...
	protected void go() throws Exception {
	    ...
	    if(gi.template.equipment.equ.place!=Equipment.PROTECTOR_SHIELD){
	    	throw new Exception("Trang bị không phải là loại tay phụ, không thể tiến hành đục lỗ cấp cao nhất");
	    }
	    
	    // 检查是否已达到最大孔位
	    itemEnh = (ItemEnhance)gi.object;
	    if (itemEnh.addMaxHole + gi.template.equipment.maxHole >= 20) {
	        throw new Exception("Đã đạt đến số lỗ lớn nhất");
	    }
	    ...
	    if (player.bag.removeGameItem(ADD_MAX_HOLE_ITEM, GameItem.GENERAL_INSTANCEID, 1, tx, true) == null) {
            ...
            throw new Exception(MessageFormat.format("Bắt buộc phải có {0} mới có thể đục lỗ", it.name));
	    }
	    tx.commit();
	    itemEnh.addMaxHole++;
	    itemEnh.addHole++;
