...
public class ChatService implements Service, ServiceEventListener, Runnable {
	...
	protected static final String[] NATIVES = { "辽宁", "Cát Lâm", "Hắc long giang", "Bắc Kinh", "Thiên tân",
			"Hà bắc", "Sơn Tây", "Nội mông cổ", "Hồ bắc", "Hồ Nam", "Hà Nam", "Giang Tây", "上海", "Giang Tô", "Chiết Giang", "Sơn đông",
			"An huy", "Quảng Đông", "Quảng Tây", "Hải Nam", "Phúc Kiến", "Tứ xuyên", "Trùng Khánh", "Quý châu", "Vân Nam", "西藏", "Thiểm tây",
			"Cam túc ", "Tân Cương", "Thanh Hải", "宁夏", "Hương cảng", "Ma Cao", "台湾",
	...
	// 特殊地图channel
	protected static int[] specialMap = new int[]{785, 737, 801, 1457};
...
//			cm.message = s.replace('\n', ' ');
					...
					if (cm.sourceId > 0) {
						...
						if (source != null) {
							source.send(new ChatMessage(ChatOption.PRIVATE, -1,
									-1, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "Đối phương không trên mạng", null).getPacket());
						}
					}
				}
			} else {
	...
	public void sendSystemMessage(int ch, String name, String message) {
		ChatMessage cm = new ChatMessage(ch, -1, -1, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", message, null);
		addChatMessage(cm);
	}
	
	public void sendPrivateMessage(int destId,String message){
		ChatMessage cm = new ChatMessage(ChatOption.PRIVATE, -1, -1, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>",
				destId, message, null);
		addChatMessage(cm);
	}
	
	public void sendFactionSystemMessage(int faction, String message) {
		ChatMessage cm = new ChatMessage(ChatOption.FACTION, -1, faction, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", message, null);
		cm.destId = faction;
		addChatMessage(cm);
	}
	
	public void sendAreaSystemMessage(String message,int destId){
		ChatMessage cm = new ChatMessage(ChatOption.AREA,-1,-1,"<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>",message,null);
		cm.destId = destId;
		addChatMessage(cm);
	}
	
	public void sendFactionShout(int faction,String message,int color,int duration){
		ChatMessage cm = new ChatMessage(ChatOption.FACTION_SHOUT, -1, faction, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", message, null);
		cm.destId = faction;
		cm.shoutColor = color;
		cm.shoutDuration = duration;
		addChatMessage(cm);
	...
	public void sendGuildSystemMessage(String message, int destId){
		ChatMessage cm = new ChatMessage(ChatOption.GUILD,-1,-1,"<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>",message,null);
		cm.destId = destId;
		addChatMessage(cm);
	}
