...
public class AdminPacketHandler implements PacketHandler{
	...
	protected void addActLeader(Packet packet, ClientSession session){
		...
		try {
			if(mapId<=0 || x<=0 || y<=0 || minLevel<=0 || minLevel>=100 || maxLevel>100 || maxLevel<=0){
				ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_NEWACTLEADER_CLIENT, "Sắp xếp tham số sai");
				return;
			}
	...
	protected void deleteActivity(Packet packet, ClientSession session) {
		...
		if (act == null) {
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_DELETE_ACTIVITY_CLIENT, "Hoạt động không tồn tại");
			return;
		}
	...
	protected void enableActivity(Packet packet, ClientSession session) {
		...
		Activity act = service.getActivityByName(name);
		if (act == null) {
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_ENABLE_ACTIVITY_CLIENT, "Hoạt động không tồn tại");
			return;
		}
	...
	protected void newActivity(Packet packet, ClientSession session) {
		...
		try {
			if (service.getActivityByName(name) != null) {
				throw new Exception("Đã có một hoạt động cùng tên rồi, mỗi hoạt động nhất định phải có một tên nội bộ riêng biệt");
			}
			...
			if (act.getImpl() == null) {
				throw new Exception("Loại thực hiện hoạt động sai");
			}
			...
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_ACTIVITY_DETAIL_CLIENT, "Thành lập hoạt động thất bại:" + e);
		}
	}
	...
	protected void activityDetail(Packet packet, ClientSession session) {
		...
		if (act == null) {
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_ACTIVITY_DETAIL_CLIENT, "ID Hoạt động ID không tồn tại ");
			return;
		}
	...
	protected void reload(Packet packet, ClientSession session){
		...
		try {
			...
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_RELOAD_CLIENT, "Tải lại thông tin máy chủ thất bại");
		}
	}
	...
	protected void gotoMap(Packet packet, ClientSession session){
		...
		if(p==null){
			ErrorHandler.sendAdminErrorMessage(session, serial,
					OpCode.ADMIN_GOTO_CLIENT, "Người chơi chỉ định không có trên mạng");
			return;
		}
		int[] point = {mapId,x,y};
		if (mapId == -1) {
		...
		try {
			...
			ErrorHandler.sendAdminErrorMessage(session, serial,
					OpCode.ADMIN_GOTO_CLIENT, MessageFormat.format("Không có cách nào chuyển đi{0}", e.getMessage()));
			return;
		}
	}
	...
	protected void chat(Packet packet, ClientSession session) {
		...
		Admin admin = (Admin) session.getClient();
		if (admin != null) {
			ChatMessage cm = new ChatMessage(channel, -admin.id, -1 , "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", message, null);
	...
	protected void adminLogin(Packet packet,ClientSession session){
		...
		if(session.getClient()==null){
			...
			if(!service.getPassword().equals(password)){
				ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_LOGIN_CLIENT, "Mật khẩu đăng nhập sai");
			...
			try {
				...
				ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_LOGIN_CLIENT, "Không có ID có thể đăng nhập");
			}

		}else{
			ErrorHandler.sendAdminErrorMessage(session, serial, OpCode.ADMIN_LOGIN_CLIENT, "已经登录");
		}
	}
