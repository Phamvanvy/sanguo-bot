...
public class ActivationCodeCall extends ClientSessionAsyncCall {
	...
	protected static final int[][] TYPES2ITEMID = {
		...
		{1078,2329}, //明珠赐福梦想礼包
		{1079,2364}, //一剪没(30点增体符、资质鉴定符、特级饲料、还魂香包、传送符、一合酥)
	};
						...
						try {
							...
							Server.server.getServiceRegistry()
									.getMailService().sendSystemMail(
											player.id, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "Phần thưởng mã kích hoạt", "", 0,
											item, 1, "ACTCODE");
						}
					}else{
						Server.server.getServiceRegistry().getMailService()
							.sendSystemMail(player.id, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "Phần thưởng mã kích hoạt",
								"", 0, item, 1, "ACTCODE");
					}
				} else if (retCode == 1) {
					error(null, "Bạn đăng nhập mã kích hoạt không chính xác, xin vui lòng kiểm tra sau đó thử lại");
				} else if (retCode == 2) {
					error(null, "Bạn nhập mã kích hoạt không phải mã kích hoạt của“ MCTQ”, xin vui lòng kiểm tra sau đó thử lại");
				} else if (retCode == 3) {
					error(null, "Mã kích hoạt bạn nhập không thể đổi ở đây, mời tìm nhân viên đổi khác");
				} else if (retCode == 4) {
					error(null, "您输入的激活码已经兑换过了。");
				} else if (retCode == 5) {
					error(null, "Mã kích hoạt bạn nhập đã quá hạn");
				} else if (retCode == 6) {
				    error(null, "Hoạt động đổi đã dừng");
				} else if (retCode == 7) {
				    error(null, "Một tài khoản chỉ có thể đổi 1 lần");
				}
			}
		} catch (Exception ex) {
			log.error(ex, ex);
			error(null, "您的兑换申请提交失败，请稍后再试。");
		} finally {
