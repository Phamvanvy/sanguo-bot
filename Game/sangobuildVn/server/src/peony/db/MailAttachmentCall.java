		...
		if (p != null) {
			...
			synchronized (mailService) {
				...
				if (m != null && m.getDestId() == p.id) {
					...
					if (attachment != null) {
						try {
							Mail cacheMail = mailService.cache.get(p.id);
							if(cacheMail!=null && cacheMail.getId()==m.getId()){
								error("已经提取过该附件了");
								return;
							}
						} catch (Exception e2) {}
						if (attachment instanceof ItemMailAttachment) {
							...
							if (m.getPrice() > 0 && p.getMoney() < m.getPrice()) {
								...
								try {
									...
									if (m.getContent().length() == 0 || m.getSourceName().equals("Bán đấu giá nhà")) {
									...
									if (m.getPrice() > 0) {
										...
										returnMail.setSourceName("<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>");
										returnMail.setStatus(Mail.UNREADED);
										returnMail.setTitle(MessageFormat.format("Trả lời :{0}", m.getTitle()));
