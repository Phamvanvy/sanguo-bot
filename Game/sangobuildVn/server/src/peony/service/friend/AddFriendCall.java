		...
		if (success) {
			...
			if (relation == null) {
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.ADD_FRIEND_CLIENT, "Hội thoại lỗi, xin vui lòng thoát ra đăng nhập lại");
				return;
			}
			if( player.getName().equalsIgnoreCase(name)){
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.ADD_FRIEND_CLIENT, "Ko thể thêm riêng của bạn");
				return;
			}
			boolean addOK = false;
			int degree = 0;
			...
			case 0:
                ...
                if (relation.friends.exists(actor.id)) {
                    ErrorHandler.sendErrorMessage(session, serial,
                            OpCode.ADD_FRIEND_CLIENT, "已经在好友列表里了");
                    return;
                }
                if(actor.faction!=player.faction){
                    ErrorHandler.sendErrorMessage(session, serial,
                            OpCode.ADD_FRIEND_CLIENT, "Không thể thêm người chơi nước địch làm bạn bè");
                    return;               	
                }
			...
			case 1:
                ...
                if (relation.blackList.exists(actor.id)) {
                    ErrorHandler.sendErrorMessage(session, serial,
                            OpCode.ADD_FRIEND_CLIENT, "已经在黑名单里了");
                    return;
                }
			...
			case 2:
                ...
                if (relation.enemies.exists(actor.id)) {
                    ErrorHandler.sendErrorMessage(session, serial,
                            OpCode.ADD_FRIEND_CLIENT, "已经在仇人列表里了");
                    return;
                }
			...
			if (!addOK) {
				ErrorHandler.sendErrorMessage(session, serial,
						OpCode.ADD_FRIEND_CLIENT, "已到达数量上限");
				return;
			}
