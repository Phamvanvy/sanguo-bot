...
public class DayOnlineActivity1 implements IActivityImpl, ServiceEventListener {
	...
	protected void sendReward(Player p, int days) {
		...
		try {
			...
			tx.commit();
			String content = MessageFormat.format("Chúc mừng ngươi, {0}, trường kỳ vì nước nhà chinh chiến, quốc quân vô cùng vui mừng nhân đây tán thưởng.(Phần thưởng đã cho vào túi đeo, xin vui lòng kiểm tra và nhận)", p.name);
			p.message(-1, content, -1, -1);
		} catch (Exception e) {
			tx.rollback();
			
			// 如果背包满则发送邮件
			String content = MessageFormat.format("Chúc mừng bạn,  {0}, vì nước chinh chiến trường kì, quốc công rất vui mừng, đặc biệt tuyên dương.", p.name);
			Server.server.getServiceRegistry().getMailService().sendSystemMail(
				p.id, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "三国君主齐点名，每天登陆有惊喜！", content, 0, item, itemCount, "ACTV");
			content = MessageFormat.format("Chúc mừng bạn, {0}, Chinh chiến trường kì cho quốc gia, quốc vương rất vui vẻ và đặt biệt khen ngợi. (Hành trang của bạn đã đầy, vật phẩm gửi theo bồ câu đưa thư đến, xin kiểm tra)", p.name);
			p.message(-1, content, -1, -1);
		}
