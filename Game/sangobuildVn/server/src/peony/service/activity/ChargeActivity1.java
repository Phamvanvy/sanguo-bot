	...
	protected void sendGift(Player p, int itemId, int count){
		if(p!=null){
			...
			try {
				...
				tx.commit();
				String content = MessageFormat.format("Chúc mừng bạn, {0}, lần nạp đầu tiên nhận được {1} (Phần thưởng đã cho vào hành trang, xin kiểm tra)", p.name, GIFT_MONEY);
				p.message(-1, content, -1, -1);
			} catch (Exception e) {
				tx.rollback();
				String content = MessageFormat.format("} xin chúc mừng bạn, {0}, nạp thẻ lần đầu đạt đến {1}", p.name, GIFT_MONEY);
				...
				content = MessageFormat.format("Chúc mừng bạn,  {0},  lần nạp đầu tiền đạt được {1} (Hành trang của bạn đã đầy, vật phẩm gửi theo bồ câu đưa thư đến, xin kiểm tra)", p.name, GIFT_MONEY);
				p.message(-1, content, -1, -1);
			}
		}
	}
