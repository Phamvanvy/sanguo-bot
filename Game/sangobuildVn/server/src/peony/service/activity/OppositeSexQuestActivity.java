...
public class OppositeSexQuestActivity implements IActivityImpl, VMapManager {
	...
	public void startup() throws Exception {
		...
		for(int i=0;i<num;i+=2){
			if(between(activity.schedule.timePeriods[i], activity.schedule.timePeriods[i+1], hour)){
				currentCycle = cycle;
			}
			cycle++;
		}
		...
		Server.server.getServiceRegistry().getChatService().sendSystemMessage(
				ChatOption.SYSTEM, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>","Vườn đào tiên đã mở, mời người chơi đến dịch tram viên Giang Lăng chuẩn bị vào");
	}
		...
		if (check) {
			...
			if (activity.schedule.in()) {
				...
				ErrorHandler.sendErrorMessage(player.session, -1,
						OpCode.TELEPORT_CLIENT, "Trong thời gian này không thể vào bản đồ");
				return null;
			}
		}
	}
