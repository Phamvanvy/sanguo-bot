...
//				ServiceEvent.EVENT_CHARGE_SUCCESS,
	...
	protected void processRemoveJewel(Player p){
		if(p!=null){
			addGain(p, "Gỡ bỏ bảo ngọc", "REMOVEJEWEL");
		}
	}
	
	protected void processMergeJewel(Player p){
		if(p!=null){
			addGain(p, "Bảo Thạch Hợp Thành ", "MERGEJEWEL");
		}
	}
	...
	protected void processEnhance(Player p, int type){
		if(p!=null){
			if(type==0){
				addGain(p, "Giám định tư chất", "NATUALENHANCE");
			}else if(type==1){
				addGain(p, "Giám định cấp sao", "STARENHANCE");
			}
		}
	}
	...
	protected void processUseItem(Player p, int itemId){
		if(p!=null){
			if(itemId==1183){
				addGain(p, "Một hộp bơ", "YIHESU");
			}else if(itemId==499){
				addGain(p, "Sư tử gầm", "SHIZIHOU");
			}
		}
	}
	...
	protected void processSkillRefresh(Player p){
		if(p!=null){
			addGain(p, "Tẩy điểm", "SKILLREFRESH");
		}
	}
	...
	protected void processBattleWin(int playerId, int type){
		...
		if(p!=null){
			if(type==0)
				addGain(p, "国战胜利", "NATIONBATTLEWIN");
			else if(type==1)
				addGain(p, "战场胜利", "FLAGBATTLEWIN");
			else if(type==2)
				addGain(p, "城战胜利", "TONGBATTLEWIN");
		}
	}
	
	protected void processAddJewel(Player p, GameItem item){
		if(p!=null){
			addGain(p, "镶嵌", "ADDJEWEL");
		}
	}
		...
		if(kill instanceof Player && died instanceof Player){
			...
			if(Server.server.getServiceRegistry().getFlagBattleFieldVMapManager().isInFlagBattle(killPlayer)==1){
				...
				if(record.itemCount>=5)
					return;
				if(addGain(killPlayer, "战场杀人", "FLAGBATTLEKILLPLAYER")){
					record.itemCount++;
				}
			}
		}
