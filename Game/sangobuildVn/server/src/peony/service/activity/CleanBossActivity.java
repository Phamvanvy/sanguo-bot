			...
			if(getMapIndex(mapId)!=-1){
				...
				if(player.level<maxLevel){
					...
					ErrorHandler.sendErrorMessage(player.session, -1,
							OpCode.TELEPORT_CLIENT, "Nơi này rất không còn tác dụng chiến đấu với bạn, kiến nghị bạn nên đến bản đồ cấp độ cao hơn.");
					return null;
					
				}
			}
			ErrorHandler.sendErrorMessage(player.session, -1,
					OpCode.TELEPORT_CLIENT, "Bạn không thể vào bản đồ này");
			return null;
		}
	}
