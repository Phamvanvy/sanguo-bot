 ...
 * @author dchen
 */
public class ChargeActivity2 implements IActivityImpl, ServiceEventListener, DayListener {

	protected Activity activity;
	
	protected IntHashMap<Integer> playerHasGifted = new IntHashMap<Integer>();
	
	protected static int GIFT_MONEY = 50;
	...
	public ChargeActivity2(Activity owner){
		this.activity = owner;
	}
	...
	private String getDBName() {
		return "ChargeActivity2" + activity.getId();
	}
	...
	protected void processPlayerCharge(Player p, int money){
		if(p!=null){
			if(p!=null){
				...
				int chargeMoney = service.getChargeByDay(accountId, new Date());
				chargeMoney += money;
	...
	protected void sendGift(Player p, int itemId, int count){
		if(p!=null){
			...
			try {
				...
				tx.commit();
				String content = MessageFormat.format("Chúc mừng bạn,  {0}, hôm nay nạp đến {1} (Phần thưởng đã đặt trong túi đeo, xin kiểm tra và nhận", p.name, GIFT_MONEY);
				p.message(-1, content, -1, -1);
			} catch (Exception e) {
				tx.rollback();
				String content = MessageFormat.format("Chúc mừng bạn,  {0},  hôm nay nạp thẻ đã nhận được {1}", p.name, GIFT_MONEY);
				Server.server.getServiceRegistry().getMailService().sendSystemMail(
					p.id, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "Phần thưởng nạp thẻ!", content, 0, item, 1, "ACTV");
				content = MessageFormat.format("Chúc mừng bạn, {0},  hôm nay nạp thẻ đạt được {1} (Hành trang của bạn đã đầy, vật phẩm gửi theo bồ câu đưa thư đến, xin kiểm tra)", p.name, GIFT_MONEY);
				p.message(-1, content, -1, -1);
			}
		}
	}
