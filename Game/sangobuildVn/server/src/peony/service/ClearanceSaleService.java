...
public class ClearanceSaleService implements Service {
	...
	public void signUp(Player player) throws Exception{
		if(player!=null){
			if(!onSale)
				throw new Exception("Người chơi thân mến hiện tại không phải thời gian đấu giá, xin đợi đến thời gian đấu giá rồi quay lại mua");
			if(signUps.contains(player.id))
				throw new Exception("Bạn đã tham gia lần đấu giá này, để cơ hội cho người chơi khác đi, làm người cần nhân đạo !");
			if(signUps.size()>=maxPlayers)
				throw new Exception("Rất tiếc, số người đấu giá đã đủ, lần sau bạn phải nhanh tay hơn");
			...
			if(player.bag.getGameItemCount(decItem)>0){
				...
				try {
					...
					if(item==null){
						tx.rollback();
						throw new Exception(MessageFormat.format("Cần {0}{1} cái", template.name, decItemCount));
					}
					tx.commit();
	...
	public void begin(int decItem, int decItemCount, int minPlayers, int maxPlayers, 
		...
		chatService.sendSystemMessage(ChatOption.SYSTEM, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "Đấu giá bắt đầu, mọi người nhanh đến quốc đô chỗ đại thần đấu giá báo danh đi ");
		refreshNpc();
	}
	...
	public void end(){
		...
		if(signUps.size()>=minPlayers){
			...
			for(int playerId : signUps){
				mailService.sendSystemMailAsync(playerId, "", "Thương phẩm đấu giá", "", 0, attItem, 1, "CLEARSALE");
			}
			chatService.sendSystemMessage(ChatOption.SYSTEM, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "Đấu giá kết thúc, thương phẩm đấu giá đã gửi vào hòm thư.");
		}else{
			chatService.sendSystemMessage(ChatOption.SYSTEM, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "Đấu giá kết thúc, đấu giá tuần này thất bại");
		}
		signUps.clear();
		clearNpc();
	}
	...
	public void toPlayer(Player player, int serial){
		if(player!=null){
			if(!onSale){
				ErrorHandler.sendErrorMessage(player.session, serial, OpCode.CLEARSALE_LIST_CLIENT, "Không thể đấu giá");
				return;
			}
			...
			for(int i=0;i<signUps.size();i++){
				...
				if(!online){
					...
					pt.putString(actor==null ? "Không biết" : actor.name);
	...
	public String getLeavingTime(){
		...
		sb.append(arr[0]).append("Thời").append(arr[1]).append("Phút").append(arr[2]).append("Giây");
		return sb.toString();
	}
