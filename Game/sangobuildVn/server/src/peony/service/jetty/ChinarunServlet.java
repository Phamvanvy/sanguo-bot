...
public class ChinarunServlet extends HttpServlet{
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
        ...
        if(session == null){
            sendError(response, "Người sử dụng không tồn tại");
            return;
        }else{
            ...
            if(p != null){
                if("true".equals(successinfo)){
                    // 充值成功
                    p.message(-1, "Bạn đã nạp thẻ thành công", -1, -1);
                }else if("false".equals(successinfo)){
                    // 充值失败
                    p.message(-1, "Bạn nạp không thành công, xin vui lòng đối chiếu dãy số và mật mã thực hiện nạp Lại lần nữa", -1, -1);
                }
            }
        }
