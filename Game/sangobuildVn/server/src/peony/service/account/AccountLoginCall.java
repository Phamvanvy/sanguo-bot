...
public class AccountLoginCall extends AccountAsyncCall{
    ...
    public void run(){
    	if(ObjectAccessor.players.size()>=Server.server.maxPlayer){
            ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNT_LOGIN_CLIENT, "Đã đạt đến số lượng đăng nhập nhiều nhất");
            return;
    	}
        ...
        if(v == null || v.status == Version.STATUS_OBSOLETE){
            ErrorHandler.sendErrorMessage(session, serial, OpCode.ACCOUNT_LOGIN_CLIENT, v == null? "Số phiên bản lỗi ": v.message);
