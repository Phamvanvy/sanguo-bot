...
public class QmeAdapter {
	...
	public static QmeAccount register(String name, String password, String email) throws QmeException {
		...
		try {
            ...
            if (code == 200) {
                ...
                if (secs.length < 3) {
                    throw new QmeException("Khi đăng ký hệ thống nội bộ  phát sinh lỗi ");
                }
                ...
                if (retID > 0) {
                	...
                	case -2:
                		throw new QmeException("Bao hàm từ ngữ đưng ký không cho phép");
                	case -3:
                		throw new QmeException("Tên nhân vật đã tồn tại");
                	case -4:
                		throw new QmeException("quy cách Email có lỗi");
                	case -5:
                		throw new QmeException("Email không cho phép đăng kí");
                	case -6:
                		throw new QmeException("Email này đã bị xóa");
                	case -7:
                		throw new QmeException("Thiếu tài khoản/ mật khẩu/ email nhất định phải điền");
            		default:
	...
	public static QmeAccount login(String name, String password) throws QmeException {
		...
		try {
            ...
            if (code == 200) {
                ...
                String[] secs = result.trim().split(",");
                if (secs.length < 4) {
                    throw new QmeException("Khi đăng nhập phát sinh lỗi nội bộ hệ thống");
                }
                ...
                if (lastSec.startsWith("sms://")) {
                	...
                	try {
                		...
                		throw new QmeException(MessageFormat.format("Thời gian đăng nhập xảy ra sai sót: {0}", lastSec));
                	}
                	QmeAccount acc = new QmeAccount();
                	acc.name = name;
                	acc.tsi = secs[2];
	...
	public static void pay(int userID, int amount) throws QmeException {
		if (!contentIDMap.containsKey(amount)) {
			throw new QmeException("Số vàng mua bán phi pháp");
		}
		...
		try {
            ...
            if (code == 200) {
                ...
                if ("100000015".equals(retCode)) {
                	...
                	throw new QmeException(MessageFormat.format("Khi thanh toán xuất hiện lỗi:{0}", retMsg));
                }
            } else {
                log.info("[QME_PAY]code[" + code + "]ERROR");
                throw new QmeException("Khi thanh toán xuất hiện lỗi nội bộ hệ thống");
            }
		} catch (QmeException qe) {
			throw qe;
        } catch (Exception ex1) {
