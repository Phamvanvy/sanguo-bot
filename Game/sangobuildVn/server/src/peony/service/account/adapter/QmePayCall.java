...
public class QmePayCall extends AccountAsyncCall {
	...
	public void callFinish() throws Exception {
		if (success) {
			...
			if (playerName.length() != 0) {
				report(MessageFormat.format("Bạn đã  mua thành công cho {0} {1} xu", playerName,money));
			} else {
				report(MessageFormat.format("Bạn đa mua thành công {0} xu", money));
			...
			report("Khi mua xuất hiện lỗi hệ thống, xin liên hệ với GM để giải quyết");
		}
	}
	...
	public void run() {
		if (playerName.length() != 0) {
			...
			if (actor == null) {
				report("Không tìm thấy mục tiêu nhân vật ");
				return;
			}
