...
public class TongBattleApplyService implements Service, ServiceEventListener {
	...
	public void tagTongBattle(Player p, int playerId) throws TongBattleException{
		...
		if(tong!=null && tong.getChairmanName().equals(p.name)){
			...
			if(tm.actor!=null && tm.actor.level<TongBattleVMapManager.MINLEVEL)
				throw new TongBattleException(MessageFormat.format("不能标记{0}级以下的军团成员", TongBattleVMapManager.MINLEVEL));
			tm.battleTag = 1;
		}else{
			throw new TongBattleException("您不是都督不能行使此权利");
		}
	}
	...
	public void abandon(Player p, int tongId) throws TongBattleException{
		if(isWinner(tongId)){
			if(p.map.getId()!=getWinnerMapId(tongId)){
				throw new TongBattleException("没有权利操作此项");
			}
			int mapId = 0;
			...
			if(isPreBattleSide(tongId)){
				throw new TongBattleException("此时间不能放弃占领");
			}
			owners.remove(mapId);
			TongService tongService = Server.server.getServiceRegistry().getTongService();
			Tong tong = tongService.getTong(tongId);
			Server.server.getServiceRegistry().getChatService().sendWorldMessage(
					MessageFormat.format("{0}军团都督放弃了原属于他们的领地{1}", tong.name, getMapName(mapId)));
			Server.server.getServiceRegistry().getChatService().sendGuildSystemMessage(
					MessageFormat.format("都督{0}放弃了原属于你们的领地{1}", p.name, getMapName(mapId)), tongId);
		}else{
			throw new TongBattleException("您没有占领该城市");
		}
	}
	...
	public String getCanApplyTime(int mapId){
		...
		if(controller==null)
			return "";
		return MessageFormat.format("{0}申请争夺的时间:{1}\n{2}城战开始时间:{3}", controller.name,
	...
	public void apply(Player p, int mapId, int money) throws TongBattleApplyException {
		if (p != null) {
			...
			Tong tong = tongService.getPlayerTong(p.id);
			if (tong == null) {
				throw new TongBattleApplyException("尚未加入军团");
			} else if (tongService.getPlayerInfo(p.id).duty != TongService.CHAIRMAN) {
				throw new TongBattleApplyException("对不起,您不是都督不能行使此权利");
			}
			if (hasApplyed(tong.id)) {
				throw new TongBattleApplyException("尊敬的都督,您已经报名成功,不能再次报名.");
			}
			if(isWinner(tong.id)){
				throw new TongBattleApplyException("尊敬的都督,您已经占领一个城池,不可报名.");
			}
			if (canApply.get(mapId)==null || !canApply.get(mapId)) {
				throw new TongBattleApplyException("尊敬的都督,当前时间不可以发起对该城市的争夺");
			}
			if(money<MIN_BATTLEAPPLY_MONEY){
				throw new TongBattleApplyException("参加军团竞价最少需要交纳30万军团资金");
			}
			...
			try {
				...
				throw new TongBattleApplyException("尊敬的都督,您的军团资金不足");
			}
	...
	public void tongBattleBid(Player p, int mapId, int money) throws TongBattleApplyException{
		if(p!=null){
			if(money<=0){
				throw new TongBattleApplyException("请输入正确的金额");
			}
			TongService tongService = Server.server.getServiceRegistry().getTongService();
			Tong tong = tongService.getPlayerTong(p.id);
			if (tong == null) {
			...
			if(battleApply==null){
				throw new TongBattleApplyException("尊敬的都督,您尚未申请城战");
			}
			boolean can = canApply.get(mapId);
			if (!can) {
				throw new TongBattleApplyException("尊敬的都督,当前时间不可以竞价");
			}
			try {
				tong.decMoney(money);
				battleApply.addMoney(money);
	...
	protected void notifyBattles(TongBattleApply[] apps, TongBattleApply owner){
		...
		for(TongBattleApply apply : apps){
			...
			service.sendGuildSystemMessage(MessageFormat.format(
					"{0}军团在今天的攻城竞价中获得最终胜利,将于晚上19点参加{1}争夺战", apply.tongName, getMapName(apply.mapId)), tongId);
			service.sendFactionSystemMessage(apply.faction
					, MessageFormat.format("{0}军团在今天的攻城竞价中获得最终胜利,将于晚上19点参加{1}争夺战", apply.tongName, getMapName(apply.mapId)));
		}
		if(owner!=null){
			if(apps.length==2){
				service.sendGuildSystemMessage(MessageFormat.format(
						"{0}军团及{1}军团将于19：00对你们的城池发起突袭，请注意防守", apps[0].tongName, apps[1].tongName), owner.tongId);
			}else if(apps.length==1){
				service.sendGuildSystemMessage(MessageFormat.format(
						"{0}军团将于19：00对你们的城池发起突袭，请注意防守", apps[0].tongName), owner.tongId);
			}
		}
	}
	...
	public void preWin(int mapId, TongBattleApply apply, int type){
		...
		if(type==0){
			chatService.sendGuildSystemMessage(MessageFormat.format(
					"{0}军团进攻{1}胜利,获得该城池的占领权", apply.tongName,getMapName(mapId)), apply.tongId);
			chatService.sendFactionSystemMessage(apply.faction, MessageFormat.format(
					"{0}军团进攻{1}胜利,获得该城池的占领权", apply.tongName,getMapName(mapId)));
		}else if(type==1){
			chatService.sendGuildSystemMessage(MessageFormat.format(
					"{0}军团防守{1}胜利,获得该城池的占领权", apply.tongName,getMapName(mapId)), apply.tongId);
			chatService.sendFactionSystemMessage(apply.faction, MessageFormat.format(
					"{0}军团防守{1}胜利,获得该城池的占领权", apply.tongName,getMapName(mapId)));
		}
		if(applys.get(mapId)!=null){
	...
	private void notifyBattle(final TongBattleApply apply){
		Server.server.scheduExec.schedule(new Runnable(){
			public void run() {
				Server.server.getServiceRegistry().getChatService().sendGuildSystemMessage(MessageFormat.format(
						"军团\"{0}\"将于19:00参加{1}的争夺战", apply.tongName,getMapName(apply.mapId)), apply.tongId);
			}
		...
		timer.schedule(new TimerTask(){
			public void run() {
				Server.server.getServiceRegistry().getChatService().sendGuildSystemMessage(
						MessageFormat.format("军团全体勇士,请大家速去{0}的都尉处参加城市争夺战", getMapName(apply.mapId)), apply.tongId);
			}
		...
		for(int mapId : applys.keySet()){
			...
			while(it.hasNext()){
				...
				if(!has(battles, a)){
					...
					tong.addMoney(money);
					ChatService chatService = Server.server.getServiceRegistry().getChatService();
					chatService.sendGuildSystemMessage("城战竞标失败，竞标资金已返还", a.tongId);
	...
	protected void notifyEnterTongOccupy(VMap map, Player player){
		...
		if(winner!=null){
			...
			chatService.sendPrivateShout(player.id, 0xff4700, 11000, player.faction, MessageFormat.format(
					"你已经进入{0}军团{1}的领土。", winner.name, factionName=="" ? "" : ("("+factionName+")")));
		}
	}
