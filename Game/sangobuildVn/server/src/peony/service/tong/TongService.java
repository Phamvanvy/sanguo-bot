...
public class TongService implements Service, ServiceEventListener {
	 ...
	 * 帮派职务名称。
	 */
	public static final String[] DUTY_NAMES = { "Quân sĩ", "Tham quan", "Phó tướng", "Đô đốc " }; 
	...
	public Tong createTong(int owner, String name) throws TongException {
		...
		if (tm != null) {
			throw new TongException("您已经在一个军团里了");
		}
		
		// 判断名称是否合法
		int code = StringUtil.isValidTongName(name);
		if (code != IStringValidator.OK) {
			throw new TongException(MessageFormat.format("军团名称{0}", StringUtil.getValidatorMessage(code)));
		}
		
		// 判断军团名称是否已存在
		DBService dbs = Server.server.getServiceRegistry().getDbService();
		if (dbs.tongDAO.findByName(name) != null) {
			throw new TongException("这个名称已经被使用了");
		}
	...
	public int createInvitation(int oper, String target) throws TongException {
		...
		if (actor1 == null || actor2 == null ) {
			throw new TongException("Mục tiêu không tồn tại");
		}
		if( actor1.faction != actor2.faction){
			throw new TongException("对方是敌对国家玩家");
		}
		if (!actor2.online) {
			throw new TongException("目标不在线");
		}
		...
		if(t!=null && (t.pool.getLong(Player.PROPERTY_LEAVETONG_TIME, 0)+REFUSEADDTONGTIME)>new Date().getTime()){
			throw new TongException("退出军团后24小时内不能加入军团");
		}
		...
		if (tm2 != null) {
			if (tm2.id == tm1.id) {
				throw new TongException("已经在军团中了");
			} else {
				throw new TongException("已经加入别的军团了");
			}
		}
	...
	public void join(int inviteID, int oper) throws TongException {
		...
		if (tm != null) {
			throw new TongException("你已经在一个军团中了");
		}
		...
		if(p!=null){
			...
			.sendGuildSystemMessage(MessageFormat.format("加入军团:<c0000ff>\nTham gia quân đoàn:<c0000ff>", p.name),invite.tong.id);
	...
	public void removeMember(int oper, int target) throws TongException {
		...
		if (tm2 == null || tm2.tongID != tm1.tongID) {
			throw new TongException("目标不在军团中");
		}
		...
		synchronized (tong) {
			...
			if (nextDuty != CHAIRMAN) {
				...
				if (existCount >= maxCount) {
					throw new TongException(MessageFormat.format("{0}的名额已经满了", getDutyName(nextDuty)));
				}
			}
			...
			if (nextDuty == CHAIRMAN) {
				...
				if(service.isPreBattleSide(tm1.tongID)){
					throw new TongException("夺城战时，军团都督不得转让");
				}
				tm1.duty = tm2.duty;
			}
	...
	public void demote(int oper, int target) throws TongException {
		...
		if (nextDuty == -1) {
			throw new TongException("已经不能再降职了");
		}
		tm2.duty = nextDuty;
	}
	...
	public void setSlogon(int oper, String newStr) throws TongException {
		...
		if (code != IStringValidator.OK) {
			throw new TongException(MessageFormat.format("公告内容{0}", StringUtil.getValidatorMessage(code)));
		}
	...
	public void exitTong(int oper) throws TongException {
		...
		if (tm1.duty == CHAIRMAN) {
			throw new TongException("必须转让都督职位后才能退出军团");
		}
	...
	public List<TongMember> listMember(Tong tong, int pageStart, int pageSize) {
		synchronized (tong) {
			...
			List<TongMember> online = new ArrayList<TongMember>(); // 在线成员
			List<TongMember> notOnline = new ArrayList<TongMember>(); // 非在线成员
	...
	public void rename(int oper, String name) throws TongException {
		...
		if (tm1.duty != CHAIRMAN) {
			throw new TongException("您不是军团长");
		}
