...
public class DuelService implements Service, VMapManager, DayListener, ServiceEventListener {
	...
	public int[] npcs = {5963937, 5963938, 5963936, 5963939};
	public String[] npcNames = {"昭阳公主驸马", "午阳公主驸马", "平阳公主驸马", "未阳公主驸马"};
	...
	protected static int itemId = 916; // 参加比武获得奖励
			...
			timer.schedule(new TimerTask(){
				public void run() {
					...
					Server.server.getServiceRegistry().getChatService().sendWorldMessage(
							"Đại Hán công chúa điện hạ sắp tổ chức đấu võ kén phò mã rồi, hiện nay mọi người đang nô nức báo danh ở sai nha giữa thành, anh hùng ngươi còn không mau đi!");
				}
...
//		removeStatue();
			...
			if(player.getVMap().instance instanceof DuelInstance){
				...
				if(instance!=null){
					...
					try {
						...
						tx.rollback();
						Server.server.getServiceRegistry().getMailService().sendSystemMailAsync(player.id, "<cFF0000>[系统]</c>\n<cFF0000>[hệ thống]</c>", "Phần thưởng đấu võ", "", 0, item, 1, "DUEL");
					}
				}
			}
			if(instance!=null){
	...
	public void signUp(Player p) throws DuelException{
		if(p!=null){
			if(!canSignUp)
				throw new DuelException("Bây giờ không phải là thời gian ghi danh");
			if(Server.server.getServiceRegistry().getFlagBattleFieldVMapManager().signups.containsKey(p.id))
				throw new DuelException("Dũng sĩ, bạn đã có trong danh sách báo danh chiến trường, nên không thể báo danh đấu võ kén rể");
			if(Server.server.getServiceRegistry().getTongBattleVMapManager().isInTongBattle(p)==1)
				throw new DuelException("Dũng sĩ, bạn đã có trong danh sách báo danh thành chiến, nên không thể báo danh đấu võ kén rể");
			if(signUps.contains(p.id))
				throw new DuelException("您已经报过名了");
			...
			try {
				...
				tx.rollback();
				throw new DuelException("Tiền của dũng sỹ ngươi không đủ, không có cách nào ghi danh tham gia cuộc tỉ thí võ thuật chọn rể");
			}
		}
	}
	...
	protected void processWinner(int playerId){
		...
		if(p!=null){
			Server.server.getServiceRegistry().getChatService().sendPrivateMessage(playerId, "Dũng sỹ, chúc mừng ngươi đã giành được vị trí đứng đầu trong cuộc tỉ thí võ thuật kén rể, công chúa đang đợi ngươi");
			p.pool.setInt("DUELTILE", 0);
			Server.server.getServiceRegistry().getChatService().sendWorldMessage(MessageFormat.format("恭喜{0}经过了一番殊死拼杀力克群雄，最终获得了驸马称号！", p.name));
		}
		// 刷新公主
