...
public class CardListDetailCall extends ClientSessionAsyncCall {
	...
	public void run() {
		...
		if (p != null) {
			...
			if(person==null){
				ErrorHandler.sendErrorMessage(session, serial, OpCode.CARD_DETAILLIST_CLIENT, "Người chơi đã rời mạng");
				return;
			}
			// 判断是否显示卡片名称
			...
			for (Card cd : cardGroup.cards) {
				...
				if (fd == null || "".equals(fd)) {
					pt.putUTF("Thẻ này là thẻ cơ bản, không cần ghép lại");
				} else {
					pt.putUTF(fd);
				}
				//卡片id
