...
public class ExpansionNation {
	...
	public void update(int diff){
		if(flag_die==0){
			if(getNpcCount(ExpansionNpcTemplate.NPC_FLAG, true)==0){
				Server.server.getServiceRegistry().getChatService().
					sendAreaSystemMessage(MessageFormat.format("悲呼！{0}军阵营的大旗被砍断了，{1}军无法取得最终的胜利。他们发誓在下一场战斗中奋起反击！",
		...
		if(!getExpansionNpc(ExpansionNpcTemplate.NPC_DOOR).npc.isAlive() && 
			...
			chatService.sendWorldMessage(MessageFormat.format("勇猛无敌的{0}军战士砍倒了董卓军的大旗，取得了本场战斗的胜利，让我们一起欢呼吧！10分钟后华雄来袭！", GameObject.getFactionName(faction)));
