 ...
 * NPC¹¦ÄÜ±à¼­Æ÷¡£
 */
public class NPCFunctionCellEditor extends CellEditor {
    protected Composite container;
    protected Text text;
    protected Button button;
    protected String[] funcs;

    public NPCFunctionCellEditor(Composite parent) {
    ...
    protected Control createControl(Composite parent) {
        ...
        text.addFocusListener(new FocusAdapter() {
            public void focusLost(FocusEvent e) {
                NPCFunctionCellEditor.this.focusLost();
            }
        });
    ...
    protected boolean dependsOnExternalFocusListener() {
        return getClass() != NPCFunctionCellEditor.class;
    }
    ...
    protected void editText() {
        NPCFunctionDialog dlg = new NPCFunctionDialog(text.getShell());
        dlg.setFunctions(funcs);
        if (dlg.open() == Dialog.OK) {
            funcs = dlg.getFunctions();
            text.setText(funcs[0]);
            fireApplyEditorValue();
            deactivate();
        }
