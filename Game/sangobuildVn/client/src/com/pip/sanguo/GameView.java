...
public class GameView {
	...
	private void processNearSprite() {
		int[] roleBox = GameWorld.player.sprite.getCollisionBox(false);
		...
		for (int i = 0; i < autoSelectOrderCount; i += 4) {
			...
			case Tool.DRAW_ITEMS_EXIT: {
				...
				if (gameExit != null) {
					int[] exitBox = gameExit.sprite.getCollisionBox(false);
			...
			case Tool.DRAW_ITEMS_PLAYER: {
				...
				if (gameSprite != null && GameWorld.netplayerNameNearShow && checkTarget(gameSprite) != null) {
					int[] spriteBox = gameSprite.sprite.getCollisionBox(false);
					spriteBox[3] *= 2;
	...
	public int collisionMap(int x, int y, int w, int h, int direct, int step, int oldX, int oldY, int currentResult) {
		...
		if (direct == Tool.DIR_DOWN) {
			startX = x;
			endX = x + w;
			startY = oldY + h;
			endY = y + h;
		} else if (direct == Tool.DIR_RIGHT) {
			startX = oldX + w;
			endX = x + w;
			startY = y;
			endY = y + h;
		} else if (direct == Tool.DIR_LEFT) {
			startX = x;
			endX = oldX;
			startY = y;
			endY = y + h;
		} else if (direct == Tool.DIR_UP) {
			startX = x;
			endX = x + w;
			startY = y;
			endY = oldY;
...
//				}
		...
		for (int i = 0; i < yOrderCount; i += 4) {
			...
			case Tool.DRAW_ITEMS_NPC: {
				...
				if (npcSprite.needCollision) {
					int[] box = npcSprite.sprite.getCollisionBox(true);
	...
	public void drawMiniMap(Graphics g, int x, int y) {
    	...
    	//# {
		//#  drawMiniMapW = Math.min(miniMapMaxWidth, miniMapWidth);
		//#  drawMiniMapH = Math.min(miniMapMaxHeight, miniMapHeight);
		//#  if(drawMiniMapW < miniMapWidth) {
		//#  	x += miniMapWidth - drawMiniMapW;
		//#  }
		//#
	...
	public void draw(Graphics g, int viewX, int viewY) {
		int mx = 0;
		int my = 0;
		int mw = GameMain.viewWidth;
		int mh = GameMain.viewHeight;

		if (GameMain.viewWidth > map.width) {
			mx = -viewX;
			mw = map.width;
		}

		if (GameMain.viewHeight > map.height) {
			my = -viewY;
			mh = map.height;
		}
	...
	private void addMapNpcDirtyData(int[] box, int viewX, int viewY) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!(tmp[0] + tmp[2] <= box[0] || tmp[0] >= box[0] + box[2] || tmp[1] + tmp[3] <= box[1] || tmp[1] >= box[1]
					+ box[3])) {
		...
		mapNpcDirtyData.addElement(box);
	}
	...
	private Vector getMapNpcDirtyList(int[] box, int viewX, int viewY) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!(box[0] + box[2] <= dirty[0] || box[0] >= dirty[0] + dirty[2] || box[1] + box[3] <= dirty[1] || box[1] >= dirty[1]
					+ dirty[3])) {
	...
	private void drawYOrder(Graphics g, int viewX, int viewY) {
		...
		for (int i = 0; i < yOrderCount; i += 4) {
			...
			case Tool.DRAW_ITEMS_SKY_MAPNPC:
				...
				int[] npcBox = mapNpcAnimateSet.getAnimateBox(animateId);
	...
	private void createYOrder(int viewX, int viewY) {
		...
		for (int i = 0; i < GameWorld.gameExits.size(); i++) {
			...
			box = gameExit.sprite.getAnimateBox();
		...
		for (int j = 0; j < 3; j++) {
			...
			for (int i = 0; i < npcData[0].length; i++) {
				...
				box = mapNpcAnimateSet.getAnimateBox(animateId);
				box[0] += npcData[1][i];
				box[1] += npcData[2][i];

				if (!(box[0] + box[2] <= viewX || box[0] >= viewX + viewWidth || box[1] + box[3] <= viewY || box[1] >= viewY
						+ viewHeight)) {
					count++;
		...
		for (int i = 0; i < GameWorld.gameSprites.size(); i++) {
			...
			case Tool.SPRITE_TYPE_GATHER_NPC:
			case Tool.SPRITE_TYPE_ATTENDANT:
				box = gameSprite.sprite.getCollisionBox(false);
	...
	private void drawCellMap(int startX, int startY, int endX, int endY) {
		...
		for (int i = 0; i < mapNpcDrawData.size(); i++) {
			...
			drawStillMapNpc(gg, map.groundNPCs, data[4], data[5], data[6], data[7], data[0] - data[4], data[1]
					- data[5]);
			drawStillMapNpc(gg, map.roleNPCs, data[4], data[5], data[6], data[7], data[0] - data[4], data[1] - data[5]);
			drawStillMapNpc(gg, map.skyNPCs, data[4], data[5], data[6], data[7], data[0] - data[4], data[1] - data[5]);
		}
	}

	private void drawStillMapNpc(Graphics g, short[][] npcData, int mapBoxX, int mapBoxY, int mapBoxWidth,
			int mapBoxHeight, int offsetX, int offsetY) {
		...
		for (int i = 0; i < npcData[0].length; i++) {
			...
			int[] box = mapNpcAnimateSet.getAnimateBox(animateId);
			box[0] += npcData[1][i];
			box[1] += npcData[2][i];

			if (!(box[0] + box[2] <= mapBoxX || box[0] >= mapBoxX + mapBoxWidth || box[1] + box[3] <= mapBoxY || box[1] >= mapBoxY
					+ mapBoxHeight)) {
