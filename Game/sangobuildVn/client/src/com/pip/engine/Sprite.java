...
public class Sprite{
	...
	public void drawBubble(Graphics g, int screenX, int screenY, boolean isTop){
        ...
        GameWorld.gameView.addPendingBubble(bubbleBuffer, screenX, screenY - anmiateBox[3], isTop);
    }
    ...
    private static int isMyAttendant = -1;
    /**
     * 绘制精灵
     * @param g
     */
    ...
    private void drawAttendantHpMp(Graphics g,int screenX,int screenY){
    	...
    	int hpY = screenY - anmiateBox[3] - Utilities.font.getHeight() - 4;
    	//血条
    	g.setColor(0xffffff);
...
//        }
    ...
    //#  }
    //# }
    //#elif ModelID == Android || ModelID == AndroidLarge
    //# private void handleMove_Android(){
    //#    int xx, yy, realStep;
    //#    int spritex = x;
    //#    int spritey = y;
	...
	//# 	double distance = Math.sqrt((GameMain.viewHeight - 98 - nY)*(GameMain.viewHeight - 98 - nY) + (nX - 98)*(nX - 98));
	//# 	double sinA = (nY - (GameMain.viewHeight - 98)) / distance;
	//# 	double cosA = (nX - (98)) / distance;
    //# 	//#else
    //#    distance = Math.sqrt((GameMain.viewHeight - 61 - nY)*(GameMain.viewHeight - 61 - nY) + (nX - 61)*(nX - 61));
    //#    sinA = (nY - (GameMain.viewHeight - 61)) / distance;
    //#    cosA = (nX - (61)) / distance;
