...
public class VMCounter {
	...
	public static int createVMCounter(int counterTime) {
		...
		if(counterTime > 0){
			...
			vc.startTime = System.currentTimeMillis();
		}
	...
	public static void cycle() {
		...
		while(enum2.hasMoreElements()) {
			...
			if(counter.type == COUNTER_TYPE_SERVER_TIME_BASED){
				...
				if((System.currentTimeMillis() - counter.startTime) <  counter.counterTime) {
					newCounters.put(key, counter);
				}
			}
	...
	public static int getProcess(int key) {
		...
		if(vc != null) {
			if(vc.type == COUNTER_TYPE_SERVER_TIME_BASED){
				...
				return (int)(System.currentTimeMillis() - vc.startTime) * 100 / vc.counterTime;	
			}
		} else {
			return -1;
		}			
	...
	public static int getElapseTime(int key) {
		...
		if(vc != null) {
			if(vc.type == COUNTER_TYPE_SERVER_TIME_BASED){
				...
				return (int)(System.currentTimeMillis() - vc.startTime);	
			}
		} else {
			return -1;
		}	
	...
	public static int getSaveTimeSec(int key) {
		...
		if(vc != null) {
			if(vc.type == COUNTER_TYPE_SERVER_TIME_BASED){
				...
				return (vc.counterTime - (int)(System.currentTimeMillis() - vc.startTime)) / 1000;
			}
		} else {
			return -1;
		}	
	...
	public static int getSaveTimeMillis(int key) {
		...
		if(vc != null) {
			if(vc.type == COUNTER_TYPE_SERVER_TIME_BASED){
				...
				return vc.counterTime - (int)(System.currentTimeMillis() - vc.startTime);	
			}
		} else {
			return -1;
		}	
