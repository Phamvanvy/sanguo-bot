...
public class ProjectData {
    ...
    public static final Class[] supportDataClasses = { 
        ...
        EquipmentPrefix.class, Sound.class
    };
    // 不同类型对象对应的XML根标签
    private static final String[] dataRootTags = { 
        ...
        "buffs", "horsetypes", "prefixes", "sounds"
    };
    // 不同类型对象对应的XML标签
    ...
    private static final String[] dataFiles = { 
        ...
        "Skill/buffs.xml", "Horse/horsetype.xml", "Items/prefix.xml", "Sounds/index.xml"
    };
...
//        }
    ...
    public String translateFileName(String name, String model) {
        ...
        if (ret == null) {
            ...
            if (name.endsWith(".etf")) {
                ...
                // 非内置PNG文件，搜索顺序是client_res/model, client_res
                ret = findFile("client_res/" + model, name);
                if (!new File(baseDir, ret).exists()) {
                    if(!new File(baseDir, ret).exists()) {
                        ret = findFile("client_res", name);
    ...
    public boolean isUseLarge(String model) {
        return "Lenovo".equals(model) || "AndroidLarge".equals(model) || "Flash".equals(model) || "Nokia5800".equals(model);
    }
    ...
    public String getCharacterDir(String model) {
        if ("Lenovo".equals(model) || "AndroidLarge".equals(model) || "Flash".equals(model) || "Nokia5800".equals(model)) {
            // 640以上的机型用2倍大小
            return "2x";
