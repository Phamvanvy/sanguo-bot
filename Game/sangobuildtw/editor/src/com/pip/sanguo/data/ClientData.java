...
public class ClientData {
    ...
    private void makePkgFileName(PackageDefine pdef, String pkgFiles) {
        if(pkgFiles.equals("all")) {
            ...
            for(int i=0; i<dataLength; i++) {
                if (pd.isUseLarge(pdef.uimodel)) {
                    pdef.pkgFileName[0][i] = baseDir + "\\Areas\\" + fileNames[i] + "\\client_l.pkg";
                } else {
                    pdef.pkgFileName[0][i] = baseDir + "\\Areas\\" + fileNames[i] + "\\client.pkg";
                }
        ...
        for(int i=0; i<fileCount; i++) {
            ...
            if(pkgFile.exists()) {
                try {
                    ...
                    System.out.println("copy " + pkgSrcFileName + " -> " + pkgTargetFileName);                    
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }    
