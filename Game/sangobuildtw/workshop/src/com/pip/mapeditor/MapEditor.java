...
public class MapEditor extends EditorPart implements ImageViewerListener, SelectionListener, Runnable {
	...
	private TileConfigTool cellConfiger;
...
//			realPlayItem.setImage(ResourceManager.getPluginImage(WorkshopPlugin.getDefault(), "icons/mapeditor/joy.gif"));
		...
		newItemToolItem.setDisabledImage(ResourceManager.getPluginImage(WorkshopPlugin.getDefault(), "icons/mapeditor/sep.gif"));
		newItemToolItem.setImage(ResourceManager.getPluginImage(WorkshopPlugin.getDefault(), "icons/mapeditor/sep.gif"));
		...
		final ToolItem importItem = new ToolItem(toolBar, SWT.PUSH);
		...
		final ToolItem exportItem = new ToolItem(toolBar, SWT.PUSH);
		...
		final ToolItem mergeItem = new ToolItem(toolBar, SWT.PUSH);
		...
		final ToolItem smallerItem = new ToolItem(toolBar, SWT.PUSH);
		...
		final ToolItem dupItem = new ToolItem(toolBar, SWT.PUSH);
		...
		if(mapFile.isLibMode){
			final ToolItem propertyItem = new ToolItem(toolBar, SWT.NONE);
		...
		cellConfiger = new TileConfigTool(mapView, TileConfigTool.CONFIG_MASK_GROUND);
...
//    	    // 把指定动画序列中所有用到的图片帧加入到地图素材中
    	    ...
    	    HashMap<Point, int[]> pieceIDMap = reflectNewPieces(animates, animateSet, usedFrames, false);
...
//	    	mapFile.addAnimate(animateSet, animateIndex);
	 ...
	 * @throws Exception
	 */
	private HashMap<Point, int[]> reflectNewPieces(PipAnimateSet animates, PipAnimateSet animateSet, HashSet<Point> usedFrames, boolean silent) throws Exception {
		if(silent){
			return animates.reflectNewPieces(animateSet, usedFrames);
		}
	...
	private void onAddPage() {
		...
		NewMapDialog dlg = new NewMapDialog(getSite().getShell());
		if(isLibMode()){
	...
	private void updateTool() {
	    if (pickupItem.getSelection()) {
            ...
            if(this.mapFile.isLibMode){
            	cellConfiger.setConfigMask(TileConfigTool.CONFIG_MASK_GROUND);
            	mapView.setTool(cellConfiger);
            }else{
	    	...
	    	cellConfiger.setConfigMask(TileConfigTool.CONFIG_MASK_SAFE_AREA);
	    	mapView.setTool(cellConfiger);
	    }else if( tileConfigTool!=null && tileConfigTool.getSelection() ){
	    	cellConfiger.setConfigMask(TileConfigTool.CONFIG_MASK_SKY);
	    	mapView.setTool(cellConfiger);
	    	...
	    	if(this.mapFile.isLibMode){
	    		cellConfiger.setConfigMask(TileConfigTool.CONFIG_MASK_SIGHT);
	    		mapView.setTool(cellConfiger);
	    	}else{
