...
public class ImageEditor extends EditorPart implements ImageViewerListener {
	...
	public void createPartControl(Composite parent) {
        ...
        if (allImage.isTrueColor()) {
		    ...
		    comboMode.select(2);
		} else if (allImage.isSupportColorOp()) {
		    if (allImage.isSupportMoreColors()) {
		        comboMode.select(4);
		    } else {
		        comboMode.select(1);
		    }
	...
	private void onOptimize() {
        ...
        if (dlg.open() == IntegerInputDialog.OK /*&& count > dlg.getValue()*/) {
            ...
            // 第一步，把所有图片拼成一个数组
            int totalSize = 0;
            for (PipImageData id : allImage.getImageDatas()) {
                totalSize += id.getWidth() * id.getHeight();
            }
            int pos = 0;
            int[] mergeData = new int[totalSize];
            PipImagePalette pal = allImage.getImagePalettes().get(0);
            for (PipImageData id : allImage.getImageDatas()) {
                int[] fdata = id.make(allImage.isTrueColor() ? null : pal);
                System.arraycopy(fdata, 0, mergeData, pos, fdata.length);
                pos += fdata.length;
            }
            
            // 第二步，用优化算法构建新的颜色表
            ColorQuantization cq = new ColorQuantization(mergeData, dlg.getValue());
            cq.process();
