...
//                    }
                    if (index == 0) {
        				...
        				if(p!=null){
        					...
        					Server.server.getServiceRegistry().getMailService().sendSystemMailAsync
    							(info.ref.id, "系統", "獲取物品成功", "", 0, item, count, "ROLL");
        					break;
        				}
        				//pvp装备
        				if(rollService.pvpRollIds.contains(id)){
            				...
            				if(quality>=2){
                				...
                				chatService.sendAreaSystemMessage(MessageFormat
                						.format("{0}擊殺敵將,繳獲{1}一件", p.name, gain.getGainItems()[0].getItem().template.name), p.map.id);
            				}
        				}
        				tx.commit();
        				break;
			...
			if (max != null) {
				...
				if (p != null) {
					...
					sendResult(allDiscard, max.ref.id);
					Server.server.getServiceRegistry().getMailService().sendSystemMailAsync
						(max.ref.id, "系統", "獲取物品成功", "", 0, item, count, "ROLL");
				}
			}
			this.state = STATE_END;
		}
