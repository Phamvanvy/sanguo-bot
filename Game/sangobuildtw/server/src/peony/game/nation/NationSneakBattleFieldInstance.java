...
public class NationSneakBattleFieldInstance implements Instance{
	...
	protected int state;
	protected NationSneakBattleFieldDef def;
...
//		}
	...
	protected String getDefenseMessage(){
		...
		sb.append("錢糧").append(WIN_SCORE-score).append('/').append(WIN_SCORE);
		return sb.toString();
	}
	
	protected String getAttackMessage(){
		StringBuilder sb = new StringBuilder(50);
		sb.append("戰利品").append(score).append('/').append(WIN_SCORE);
		return sb.toString();
	}
	...
	protected void win(){
		...
		chatService.sendFactionSystemMessage(def.destFaction, 
				MessageFormat.format("{0}陣腳已亂,我軍偷襲成功,搶回之前損失大量輜重和錢糧.", GameObject.getFactionName(def.sourceFaction)));
		chatService.sendFactionSystemMessage(def.sourceFaction, 
				MessageFormat.format("{0}進兵突然,我軍猝不及防,損失慘重.\t", GameObject.getFactionName(def.destFaction)));
		transPlayers();
	}
	...
	protected void fail(){
		...
		ChatService chatService = Server.server.getServiceRegistry().getChatService();
		chatService.sendFactionSystemMessage(def.destFaction, 
				MessageFormat.format("{0}防守堅強,敵將臨陣不亂事不可為速速撤軍.", GameObject.getFactionName(def.sourceFaction)));
		chatService.sendFactionSystemMessage(def.sourceFaction, 
				MessageFormat.format("無恥{0}妄圖偷襲我軍營寨已被擊潰,國庫得以保全.", GameObject.getFactionName(def.destFaction)));
		transPlayers();
	}
	...
	protected void checkShout(){ //每降低100喊一次
		...
		if(x>=0&&x<shouted.length){
			if(!shouted[x]){
				shouted[x] = true;
				map.shout(
						MessageFormat.format("糧草都尉和軍械都尉被殺,大量糧草輜重被劫,{0}速速前去支援", GameObject.getFactionName(def.sourceFaction)), 
						0xff5555, 5000);
			}
		}
	}
