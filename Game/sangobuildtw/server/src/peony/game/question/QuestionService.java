	...
	public void anwser(Player p, int questionId, String answer) throws QuestionException{
		if(p!=null){
			if(questionMap.get(p.id)==null || questionMap.get(p.id)!=questionId)
				throw new QuestionException("答題請求不合法");
			Question question = questions.get(questionId);
			if(question==null)
				throw new QuestionException("沒有找到此題目");
			String a = question.getAnswer();
			if(!a.equals(answer))
				throw new QuestionException("抱歉,答題錯誤");
			//答题成功之后进行奖励
			...
			mailService.sendSystemMailAsync(p.id, "系統", "答題成功獎勵", "", 0, item, 1, "QSA");
