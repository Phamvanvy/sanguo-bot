...
public class ExpService implements Service, ServiceEventListener {
	...
	public void addAgentHorse(Player p, int horseId) throws ExpException{
		if(p!=null){
			...
			if(horse == null){
				throw new ExpException("沒有對應的馬.");
			}
			if(horse.agentHorse==1){
				throw new ExpException("不可重复代理飼養");
			}
			if(p.horseBag.getAgentHorses().size()>=1){
				throw new ExpException("不可代理飼養更多的馬");
			}
			if(horse.level>p.level){
				throw new ExpException("不能代理飼養超過人等級的馬");
			}
			horse.agentHorse = 1;
			...
			PlayerTransaction tx = p.newTransaction("BUY");
				...
				if(item!=null){
					...
					Server.server.getServiceRegistry().getChatService().addChatMessage(
							new ChatMessage(ChatOption.PRIVATE, -1, -1, "系統",
									p.id, MessageFormat.format("閣下獲{0}經驗,消耗1盒{1}", getExp,ObjectAccessor.getItemTemplate(1183).name), null));
					tx.commit();
					...
					Server.server.getServiceRegistry().getChatService().addChatMessage(
							new ChatMessage(ChatOption.PRIVATE, -1, -1, "系統",
									p.id, MessageFormat.format("您的{0}不足,請進入充值商店購買.", ObjectAccessor.getItemTemplate(1183).name), null));
				}
		}
	}
...
//		}
	...
	public int getExp(int level, long distance, int type) {
		...
		if(type==ExpService.TYPE_PLAYER){
			return onlineExps[level] * onlinedis;
		}else if(type==ExpService.TYPE_HORSE){
			return horseOnlineExps[level] * notOnlineDis;
		}
		return 0;
	}
	...
	private void handleNotOnlineExp(Player p) {
		if(p!=null){
			...
			if(count > 0){
				...
				leavingExp += count * onlineExps[p.level];
...
//				}
	...
	public void exchaneExp(Player p, Packet packet, ClientSession session, int serial){
		if(p!=null){
			...
			if(leavingExp==0){
				p.message(-1, "暫時沒有可兌換的离線經驗", -1, -1);
				return;
			}
			int count = p.bag.getGameItemCount(1183);
			if(count==0){
				p.message(-1, MessageFormat.format("您的{0}不足,請進入充值商店購買", ObjectAccessor.getItemTemplate(1183).name),
						-1, -1);
				return;
			}else{
				int exp = 0;
				...
				if(exp > 0){
					...
					String msg;
					if(leavingExp < getExp){
						msg = MessageFormat.format("恭喜您獲得經驗{0},消耗{1}盒{2},剩余{3}經驗值不夠兌換",
								exp,count1,ObjectAccessor.getItemTemplate(1183).name,leavingExp);
					} else {
						msg = MessageFormat.format("恭喜您獲得經驗{0},消耗{1}盒{2},還剩余{3}經驗值可兌換,快去充值商店購買吧",
								exp,count1,ObjectAccessor.getItemTemplate(1183).name,leavingExp);
					}
					Server.server.getServiceRegistry().getChatService()
					.sendPrivateMessage(p.id, msg);
					Server.server.getEventManager().fireEvent(new ServiceEvent(ServiceEvent.EVENT_USEITEM,p,1183));
	...
	public void handleHorseExp(Player p){
		...
		if(p!=null){
			...
			if(signCount==0 && p.horseBag.getAgentHorses().size()>0){
				Server.server.getServiceRegistry().getChatService()
				.sendPrivateMessage(p.id, MessageFormat.format("您的{0}不足,請進入充值商店購買.", ObjectAccessor.getItemTemplate(1184).name));
			}
			if(signCount>0){
				...
				if(horses.size()>0){
					...
					while(leavingExp>0){
						...
						if(leavingExp<getExp){
							if(exp==0){
								Server.server.getServiceRegistry().getChatService()
								.sendPrivateMessage(p.id, MessageFormat.format("剩余經驗{0}不足兌換", leavingExp));
							}
							break;
						}
						...
						if(horseUpLevel>Player.MAX_LEVEL||horseUpLevel>p.level){
							Server.server.getServiceRegistry().getChatService()
							.sendPrivateMessage(p.id, "坐騎等級不能超過人物等級,离線代理喂養停止," +
									"加油練級才能駕馭更高級的坐騎.");
							break;
						}
						PlayerTransaction tx = p.newTransaction("BUY");
					...
					if(exp > 0){
						...
						if(leavingExp>0){
							Server.server.getServiceRegistry().getChatService()
							.sendPrivateMessage(p.id, MessageFormat.format("代理飼養的馬獲取經驗值{0},消耗{1}{2}剩余{3}經驗,快去充值商店購買{4}吧", 
									exp,ObjectAccessor.getItemTemplate(1184).name,count,leavingExp,ObjectAccessor.getItemTemplate(1184).name));
						}else{
							Server.server.getServiceRegistry().getChatService()
							.sendPrivateMessage(p.id, MessageFormat.format("代理飼養的馬獲取經驗值{0},消耗{1}{2}剩余{3}經驗", 
