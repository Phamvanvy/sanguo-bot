...
//	方士   	2	1	2	3
	...
	public float defensePercent;//减伤
...
//		this.spelldodge = INIT_SPELL_DODGE[clazz];
	...
	protected void initPlayer(){
		...
		this.basicstrength = this.strength = pros[0] + unit.strengthAdded;
		this.basicagility = this.agility = pros[1] + unit.agilityAdded;
		this.basicstamina = this.stamina = pros[2] + unit.staminaAdded;
		this.basicintellect = this.intellect = pros[3] + unit.intellectAdded;
...
//		基础物理防御力 = 初始物理防御力+装备加成+修正 
    ...
    public static float calcSpellDefensePercent(float armor, int level) {
        float def = 1.0f - 1.0f / (1 + armor / ((level + 9) * 4.75f));
        if (def >= 0.99f) {
            def = 0.99f;
        }
        if (def < 0) {
	...
	public float getSpeed() {
		return 1.0f + horserating + fastrating - slowrating; 
	}
}
