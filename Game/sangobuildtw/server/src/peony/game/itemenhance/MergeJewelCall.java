	...
	protected void go() throws Exception {
	  ...
	  //检查宝石是否有瑕疵
	    if(gi.template.isFlaw){
	    	throw new Exception("有瑕疵的寶石沒有合成功能");
	    }
	    
	    if (gi == null) {
	        tx.rollback();
	        throw new Exception("沒有足夠的寶石");
	    }
	    if (gi.template.itemType != Item.TYPE_JEWEL) {
	        tx.rollback();
	        throw new Exception("選擇物品錯誤");
	    }
	    if (gi.template.useLevel >= JewelService.JEWEL_LEVELS) {
	        tx.rollback();
	        throw new Exception("你選擇的寶石已經是最高級的了");
	    }
	    ...
	    if (player.bag.removeGameItem(js.mergeItemIDs[lvl - 1], GameItem.GENERAL_INSTANCEID, 1, tx, true) == null) {
	        tx.rollback();
	        throw new Exception(MessageFormat.format("缺少{0}",js.mergeItems[lvl - 1].name));
	    }
        ...
        if(gi3.template.useLevel==5){
        	String s = MessageFormat.format("{0}成功合成出了一顆閃閃發光的{1}", player.name,gi3.template.name);
        	Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(player.faction, s);
        }
        else if(gi3.template.useLevel==6){
        	String s = MessageFormat.format("{0}竟然成功的合成出了一顆完美無瑕的{1}", player.name,gi3.template.name);
        	Server.server.getServiceRegistry().getChatService().sendFactionSystemMessage(player.faction, s);
        }
        else if(gi3.template.useLevel==7){
        	String s = MessageFormat.format("{0}的{1}讓絕世神石{2}奇跡般的誕生在這個世界上!", player.getFactionName(),player.name,gi3.template.name);
