	...
	protected static void modifyDescriptions(ProjectData data) {
		for (ItemTemplate template : ObjectAccessor.itemTemplates.values()) {
			if (template.useType.effect instanceof AddFormulaEffect) {
				...
				sb.append("\n");
				sb.append("所需熟練度:").append(ProduceService.getPracticeByLevel(f.level)).append("\n");
				sb.append("級別:").append(ProduceService.getLevelToString(f.level)).append("\n");
				sb.append("消耗行動力:").append(f.movePoint).append("\n");
				for(BuyRequirement br:f.requirements){
					sb.append(br.toString()).append("\n");
				}
				if(f.productType==Formula.PRODUCT_DROPGROUP){
					sb.append("產出:").append(ObjectAccessor.getItemTemplate(f.itemID).name).append("(隨机屬性)");
				}
				else if(f.productType==Formula.PRODUCT_ITEM){
					sb.append("產出:").append(ObjectAccessor.getItemTemplate(f.itemID).name).append("");
				}
				if (f.minAmount != 1 || f.maxAmount != 1) {
					sb.append("\n數量:").append(f.minAmount).append("~").append(f.maxAmount);
				}
