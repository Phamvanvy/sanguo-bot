 ...
 * @author lighthu
 */
public class SlowSkill extends AbstractSkill {
	/*
	 * 减速级别
	 */
	protected int level;
	...
	public SlowSkill(int level, int time) {
		super(0, "", 0);
		this.level = level;
		this.time = time;
		type = TYPE_ATTACK;
	...
	protected CombatEffect createActEffect() {
		return new CombatEffect() {
			...
			public void finished(CombatContext context, boolean isActive) {
				// 命中后给对方加对应级别的减速DEBUFF
				if (context.hited()) {
					context.target.buffs.addBuff(new SlowDebuff(context.source, level, time));
				}
			}
		};
	}
