...
public class NpcService implements Service {
			...
			for(int mapId : keys){
				...
				for(final Clock clock : clocks){
					...
					if(in(cal, beginTime, endTime)){
						new Timer().schedule(new RefreshNpcTask(clock,mapId,timeDis,npcs,positions,true), 0);
					}
					new Timer().schedule(new RefreshNpcTask(clock,mapId,timeDis,npcs,positions,true), getScheduleTime(cal, beginTime), ONEDAY);
					...
					}, getScheduleTime(cal, endTime), ONEDAY);
					new Timer().schedule(new TimerTask(){
						public void run() {
							Server.server.getServiceRegistry().getChatService()
							.sendWorldMessage("天降橫財,2分鐘后在西域的西北方向將會出現寶箱,大家快前往挖寶吧.");
						}
					}, getScheduleTime(cal, (beginTime-1), 58), ONEDAY);
				}
			}
		}
	}
	...
	public void refreshNpc(int mapId, List<Integer> npcs, List<Position> positions) {
		...
		chatService.sendWorldMessage("西域的西北方發出耀眼的紅光,有人在那撿到寶箱,大家都爭相前往挖寶.");
	}
