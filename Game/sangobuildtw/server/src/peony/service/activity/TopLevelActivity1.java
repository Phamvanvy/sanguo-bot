...
public class TopLevelActivity1 implements IActivityImpl {
	...
	protected void sendReward(Actor actor, int rewardLevel, int itemID) {
		...
		case 0:
			title = "等級排名狀元獎勵";
			content = "{0},你好,因為驍勇無匹,沖級迅速,在本次等級大排行中成為狀元,特此獎勵.";
			break;
		case 1:
			title = "等級排名榜眼獎勵";
			content = "{0},你好,因為驍勇無匹,沖級迅速,在本次等級大排行中成為榜眼,特此獎勵.";
			break;
		case 2:
			title = "等級排名探花獎勵";
			content = "{0},你好,因為驍勇無匹,沖級迅速,在本次等級大排行中成為探花,特此獎勵.";
			break;
		}
		content = MessageFormat.format(content, actor.name);
		Server.server.getServiceRegistry().getMailService().sendSystemMail(
			actor.id, "系統", title, content, 0, item, 1, "ACTV");
