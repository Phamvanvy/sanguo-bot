...
//				}
	...
	protected void sendReward(Player p, int days) {
		...
		try {
			...
			tx.commit();
			String content = MessageFormat.format("恭喜你,{0},獲得了周年登錄獎勵,活動期間每日登錄將有惊喜(獎勵已放入背包,請查收)", p.name);
			p.message(-1, content, -1, -1);
		} catch (Exception e) {
			tx.rollback();
			
			// 如果背包满则发送邮件
			String content = MessageFormat.format("恭喜你,{0},獲得了周年登錄獎勵,活動期間每日登錄將有惊喜", p.name);
			Server.server.getServiceRegistry().getMailService().sendSystemMail(
				p.id, "系統", "周年登錄獎勵!", content, 0, item, itemCount, "ACTV");
			content = MessageFormat.format("恭喜你,{0},獲得了周年登錄獎勵,活動期間每日登錄將有惊喜（您的背包已滿,獎勵已經以飛鴿附件的方式發送到你的飛鴿里了,請及時查收.）", p.name);
			p.message(-1, content, -1, -1);
		}
