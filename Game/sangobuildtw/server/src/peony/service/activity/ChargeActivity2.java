	...
	protected void sendGift(Player p, int itemId, int count){
		if(p!=null){
			...
			try {
				...
				tx.commit();
				String content = MessageFormat.format("恭喜你,{0},今天充值達到{1}（獎勵已放入背包,請查收", p.name, GIFT_MONEY);
				p.message(-1, content, -1, -1);
			} catch (Exception e) {
				tx.rollback();
				String content = MessageFormat.format("恭喜你,{0},今天充值達到{1}", p.name, GIFT_MONEY);
				Server.server.getServiceRegistry().getMailService().sendSystemMail(
					p.id, "系統", "充值獎勵!", content, 0, item, 1, "ACTV");
				content = MessageFormat.format("恭喜你,{0},今天充值達到{1}（您的背包已滿,獎勵已經以飛鴿附件的方式發送到你的飛鴿里了,請及時查收.）", p.name, GIFT_MONEY);
				p.message(-1, content, -1, -1);
			}
		}
	}
