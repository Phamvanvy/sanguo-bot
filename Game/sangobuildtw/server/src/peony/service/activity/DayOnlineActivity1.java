...
public class DayOnlineActivity1 implements IActivityImpl, ServiceEventListener {
	...
	protected void sendReward(Player p, int days) {
		...
		try {
			...
			tx.commit();
			String content = MessageFormat.format("恭喜你,{0},長期為國家征戰,國君十分欣慰特此嘉獎.（獎勵已放入背包,請查收.）", p.name);
			p.message(-1, content, -1, -1);
		} catch (Exception e) {
			tx.rollback();
			
			// 如果背包满则发送邮件
			String content = MessageFormat.format("恭喜你,{0},長期為國家征戰,國君十分欣慰特此嘉獎.", p.name);
			Server.server.getServiceRegistry().getMailService().sendSystemMail(
				p.id, "系統", "三國君主齊點名,每天登陸有惊喜!", content, 0, item, itemCount, "ACTV");
			content = MessageFormat.format("恭喜你,{0},長期為國家征戰,國君十分欣慰特此嘉獎.（您的背包已滿,獎勵已經以飛鴿附件的方式發送到你的飛鴿里了,請及時查收.）", p.name);
			p.message(-1, content, -1, -1);
		}
