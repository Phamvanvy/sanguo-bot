...
public class BuyRequest {
	...
	public void lock() throws ShopException {
		...
		if (p == null) {
			throw new ShopException("角色狀態异常");
		}
		...
		while (reqItor.hasNext()) {
			...
			case Shop.TYPE_HONOR:
				if (p.getCredit() < req.amount) {
					throw new ShopException(MessageFormat.format("要求擁有{0}戰功", req.amount));
				}
				break;
			case Shop.TYPE_IMONEY:
			{
				...
				if (account == null || account.getIMoney() / 100 < req.amount) {
					throw new ShopException(MessageFormat.format("要求賬戶里至少擁有{0}{1}", req.amount,Server.iMoneyString));
				}
				break;
			}
			case Shop.TYPE_ITEM:
			{
				...
				if (it == null) {
					throw new ShopException("商品配置錯誤");
				}
				PlayerTransaction tx1 = p.newTransaction(buyObject.getCause());
				if (p.bag.removeGameItem(req.item.id, -1, req.amount, tx1, true) == null) {
					tx1.rollback();
					if (req.amount == 1) {
						throw new ShopException(MessageFormat.format("要求擁有{0}", it.name));
					} else {
						throw new ShopException(MessageFormat.format("要求擁有{0}個{1}", req.amount,it.name));
					}
				}
				tx1.rollback();
				break;
			...
			case Shop.TYPE_MONEY:
			{
			    if (p.getMoney() < req.amount) {
			        throw new ShopException(MessageFormat.format("要求擁有{0}金錢", req.amount));
			    }
				break;
			}
			case Shop.TYPE_RANK:
			{
				...
				if (p.getRank() < rank.id) {
					throw new ShopException(MessageFormat.format("需要擁有{0}或以上軍銜才能購買", rank.title));	
				}
				break;
			}
			case Shop.TYPE_VARIABLE:
			{
			    ...
			    if (value < req.amount) {
			        throw new ShopException(MessageFormat.format("條件不足：{0}", req.varDesc));
			    }
			    break;
			}
			case Shop.TYPE_LEVEL:
			{
			    if (p.level < req.amount) {
			        throw new ShopException(MessageFormat.format("需要達到{0}級", req.amount));
			    }
			    break;
			}
	...
	public boolean pay() throws ShopException {
				...
				if(it.isEquipment()){
					if (p.bag.removeGameItemIngoreInstanceId(req.item.id, req.amount*count, tx, true) == null) {
						if (req.amount * count == 1) {
							...
							throw new ShopException(MessageFormat.format("需要{0}個{1}", (req.amount * count),it.name));
						}
					}
				}else{
		...
		if (imoney > 0) {
			if("CMCC".equals(Server.server.revision)){
				if(count>1){
					throw new ShopException("不能批量購買此商品");
				}
			}
			if ("CMCC".equals(Server.server.revision)||"CHINATEL".equals(Server.server.revision)) {
				if (consumeCode == null) {
					throw new ShopException("此商品暫不支持購買");
				}
			}
			// 向认证服务器发起扣费请求，并把自己添加到ShopService的回调请求中
			shopService.asyncPay(p.accountId, imoney, consumeCode, this, p.level);
			return false;
		}
