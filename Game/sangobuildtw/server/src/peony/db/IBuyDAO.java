...
public class IBuyDAO extends GenericHibernateDAO<IBuy, Integer> {
	...
	public List<IBuy> getTopConsume(Date startTime, Date endTime, int max) {
		...
		try {
			...
			list = getSession()
					.createSQLQuery(
							"select id,playerid,sum(imoney)imoney,accountid,itemname,itemid,type,buytime,otherplayerid,otherplayername,count,giftflag,level from ibuy where buytime>\'"
									+ time
									+ "\' and buytime<\'"
									+ time2
