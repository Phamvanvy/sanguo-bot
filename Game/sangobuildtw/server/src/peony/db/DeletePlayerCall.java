			...
			if (player == null) {
				...
				if (player.exist != 1) {
					...
					if(player.level>=DELTE_LIMIT_LEVEL){
						error(null, MessageFormat.format("{0}級以上的玩家不允許刪除角色", DELTE_LIMIT_LEVEL));
						addToClientSession();
						return;
					}
					//如果是国王不能删除账号
					...
					for(Officer officer : nation.getOfficers()){
						if(officer!=null && officer.actor.id==player.id){
							error(null, MessageFormat.format("您是{0},不可刪除賬號", officer.getName()));
							addToClientSession();
							return;
						}
					}
