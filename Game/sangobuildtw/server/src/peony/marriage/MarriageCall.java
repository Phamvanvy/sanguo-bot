...
public class MarriageCall extends ClientSessionAsyncCall {
	...
	public void callFinish() throws Exception {
		...
		if(success){
			...
			p1.putString(MessageFormat.format("恭喜您和{0}結為夫妻", annotherPerson.name));
			person.send(p1);
			Packet p2 = new Packet(OpCode.MARRIAGE_SERVER);
			p2.putString(MessageFormat.format("恭喜您和{0}結為夫妻", person.name));
			annotherPerson.send(p2);
			chatService.addChatMessage(new ChatMessage(ChatOption.FACTION,-1,person.faction,"系統",person.faction,MessageFormat.format("{0}和{1}已結婚,大家一起祝福他們.", person.name,annotherPerson.name),null));
		}else{
			Packet p1 = new Packet(OpCode.MARRIAGE_SERVER);
